...
| Date | Achievement |
|------|-------------|
| 2026-03-17 | 📚 **v0.29.0 — Lexmark ISV + Full Doc Audit:** Added Lexmark Cloud Print Management preset (3 schemas: Core User, EnterpriseUser required, CustomUser with writeOnly/never badgeCode+pin. User-only, no Groups). 46 E2E tests + 112 live tests + 9 unit tests. 6 built-in presets total. Full documentation recreation: README.md (architecture diagrams, compliance matrix, quick start), COMPLETE_API_REFERENCE.md (every endpoint with examples), INDEX.md, PROJECT_HEALTH_AND_STATS.md (fresh LoC: 58,891 src / 12,711 test), 8 example JSONs (user, group, SPC, patch, search, bulk, errors, endpoints). **72 unit suites (2,884), 34 E2E suites (743+6 skip), ~621 live, 112 Lexmark live — ALL PASSING.** |
...
## Current Focus
All 27 migration gaps (G1–G20) fully resolved. v0.29.0: Fixed 19 test failures, removed legacy config field, added multi-endpoint isolation E2E tests. Endpoint profile configuration — unified profile JSONB, 6 built-in presets, Prisma 7→5 models, endpoint-profile module. README, docs, and all JSONs recreated from source on 2026-03-16. FP-1 fix: empty extension objects removed from responses when all attrs are returned:never (schemas[] cleanup). Schema-driven uniqueness enforcement for custom extension attributes. **Remaining P0 gaps:** legacy token backdoor in scim-auth.guard.ts, console.log statements, CORS origin:true, /scim/v2 rewrite middleware. Current version v0.29.0 — all unit (2,887 / 72 suites), E2E (763 / 35 suites) passing. Live: 621 assertions. 📊 See [PROJECT_HEALTH_AND_STATS.md](docs/PROJECT_HEALTH_AND_STATS.md#test-suite-summary) for current counts.

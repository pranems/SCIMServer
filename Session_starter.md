...
## Current Focus
All 27 migration gaps (G1–G20) fully resolved. v0.29.0+: Precomputed schema characteristics cache with Parent→Children maps for zero per-request schema tree walks and name-collision precision. Schema-driven uniqueness enforcement for custom extension attributes. Endpoint profile configuration — unified profile JSONB, 6 built-in presets, Prisma 7→5 models. **Remaining P0 gaps:** legacy token backdoor in scim-auth.guard.ts, console.log statements, CORS origin:true, /scim/v2 rewrite middleware. Current version v0.29.0 — all unit (2,923 / 73 suites), E2E (763 / 35 suites) passing. Live: 602 + 112 Lexmark assertions. Deployed: ghcr.io/pranems/scimserver:0.29.2.

...
Current Version: v0.22.0 (Prisma 7 + PostgreSQL 17 + ESLint 10 + Jest 30 + React 19 + Vite 7 + Node 24 Docker)

---

## Status
Production Ready (v0.22.0) — **All 27 migration gaps (G1–G20) fully resolved. SCIM RFC 7643/7644 compliance: 100%.** v0.22.0: RFC 7643 §2.2 readOnly attribute stripping & warning URN system for Users/Groups/Generic resource types. `EndpointContextStorage` AsyncLocalStorage middleware (`createMiddleware()` + `storage.run()`) for request-scoped warning accumulation. 2 new config flags: `IncludeWarningAboutIgnoredReadOnlyAttribute`, `IgnoreReadOnlyAttributesInPatch` (14th, 15th). Generic service dynamic schema resolution via `getSchemaDefinitions()`. Phase 11 per-endpoint credentials (G11, bcrypt 3-tier auth, `PerEndpointCredentialsEnabled` 12th boolean flag), Phase 10 `/Me` endpoint (RFC 7644 §3.11), Phase 12 sorting (RFC 7644 §3.4.2.3, `sort.supported: true`), G17 service deduplication (`scim-service-helpers.ts`, `ScimSchemaHelpers`), Multi-tenant discovery + D1–D6 resolved (v0.19.3), Phase 9 Bulk Operations (RFC 7644 §3.7, `BulkProcessorService`), G8g write-response attribute projection (RFC 7644 §3.9), G8f group PUT/PATCH uniqueness, G8b custom resource type registration, G8e returned characteristic filtering (RFC 7643 §2.4), G8c PATCH readOnly pre-validation, externalId CITEXT→TEXT, AllowAndCoerceBooleanStrings, ReprovisionOnConflict, Adversarial validation hardening (V2-V31), SchemaValidator domain class (950 lines), ETag + Conditional Requests, soft delete, strict schema validation, H-1 PATCH validation, H-2 immutable enforcement (Feb 2026). Per-endpoint config flags (15 total: 12 boolean + logLevel + 2 readOnly): `SoftDeleteEnabled`, `StrictSchemaValidation`, `RequireIfMatch`, `AllowAndCoerceBooleanStrings`, `ReprovisionOnConflictForSoftDeletedResource`, `CustomResourceTypesEnabled`, `BulkOperationsEnabled`, `PerEndpointCredentialsEnabled`, `IncludeWarningAboutIgnoredReadOnlyAttribute`, `IgnoreReadOnlyAttributesInPatch`, multi-member PATCH, verbose PATCH, remove-all-members, + logLevel. SPC: `bulk.supported: true, sort.supported: true`. 3-tier auth: per-endpoint bcrypt → OAuth JWT → global secret. `scim-service-helpers.ts` (386 lines): 13 extracted shared functions; Users −29%, Groups −28%. `ScimMeController` JWT sub→userName. `scim-sort.util.ts` sortBy push-down. `EndpointCredential` bcrypt-hashed storage. Parallel E2E (maxWorkers=4, ~22s). **2,532 unit tests (73 suites), 539 E2E tests (26 suites), 485 live integration tests** — **ALL PASSING (0 failures)**. Previously 24 unit + 41 E2E pre-existing failures — all resolved via SchemaValidator id catch-22 fix + G8f mock drift fix. 25 Microsoft SCIM Validator tests + 7 preview (0 false positives). Stack: Prisma 7, PostgreSQL 17, ESLint 10, Jest 30, React 19, Vite 7, Node 24. (Feb 2026).

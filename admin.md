...
## TL;DR Fast Path
1. Dev & test locally.
2. Bump `api/package.json` version (e.g. 0.23.0 -> 0.24.0) – keep semver.
3. Commit & push.
...
6. Update Container App: `az containerapp update -n scimserver-prod -g scimserver-rg --image ghcr.io/pranems/scimserver:0.24.0`.
...
2. Tag: `v0.24.0` (create if not existing).
3. Title: `v0.24.0`.
...
- Add `-Tag 0.24.0` to override auto-detected version (rarely needed).
...
```
scimserverpublic.azurecr.io/scimserver:0.24.0
...
```powershell
az containerapp update -n scimserver-prod -g scimserver-rg --image scimserverpublic.azurecr.io/scimserver:0.23.0
```
...
| Hotfix | Increment PATCH (0.24.1) |
| Feature batch | Increment MINOR (0.25.0) |
...
```powershell
docker run --rm -p 8080:80 ghcr.io/pranems/scimserver:0.24.0
...
# Tag
git tag -a v0.24.0 -m "v0.24.0"; git push origin v0.24.0

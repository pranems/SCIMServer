...
```powershell
docker run --rm -p 8080:80 ghcr.io/pranems/scimserver:latest
...
- Database persistence uses PostgreSQL (or in-memory for dev/test). Multi-instance scaling is supported with a shared PostgreSQL backend.

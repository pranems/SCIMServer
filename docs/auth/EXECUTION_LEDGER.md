...
| 8 | **A2** computed authenticationSchemes + RFC 8414 metadata + JWKS publication | DONE | _pending_ | computeAuthenticationSchemes wired into getSpcFromProfile: baseline always + enabled methods add schemes + primary on defaultMethodId. (RFC 8414 + JWKS shipped Pre-Q.B/Q0.) 8 unit + 4 E2E + 5 live (9z-AR). Backend-agnostic. Local live 1081/0. Docker/dev-Azure -> next checkpoint |

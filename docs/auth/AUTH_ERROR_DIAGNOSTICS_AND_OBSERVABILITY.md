...
**Planned next (2026-07-21): make the auth decision a first-class part of the request-log detail (not a disjointed sibling panel).** Today the `AuthDiagnosticsPanel` sits beside the request-log list and the log detail only exposes a "View auth decision" focus-jump button. [CONNECT_AND_LOGS_UX_OVERHAUL_PLAN.md](CONNECT_AND_LOGS_UX_OVERHAUL_PLAN.md) (U11 + U12) proposes rendering the auth-decision trace for a request **inside its `DetailDrawer`** (an "Authentication" section, joined by `requestId === correlationId`), adding an **auth-outcome chip to each request-log row**, and re-scoping the standalone panel to the endpoint-wide "recent auth decisions" view on **Connect -> Health**. This operationalizes the principle that authentication is a first-class step of every request's lifecycle - see the plan doc's Q2/Q3 for the options weighed and the usability rationale.

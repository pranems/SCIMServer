...
| **D1** | `admin-credential.controller.ts` is **1,221+ lines** and carries credential CRUD, rotate, reveal, activate/deactivate/purge, the WIF resolve/verify/debug trio, and P2 caps - well past the ~400-line / ~5-responsibility god-class threshold | credential admin plane | Med | - . **Raised by the 2026-08-27 Stage 3c.1 / DA-gate pass; reconfirmed by v0.55.30. Disposition (b) SCHEDULED, auth platform owner, before the next credential-admin feature adds another route.** Splitting it inside a deploy-bound change carries real regression risk in the exact file that produced the orphaned-decorator defect (I-42). The natural seam is to lift the three `wif/*` diagnostic routes into their own controller, which shares no state with credential CRUD. |

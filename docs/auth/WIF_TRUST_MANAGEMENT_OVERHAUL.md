...
> **Follow-up (IMPLEMENTED 2026-07-21, v0.54.32-0.54.37).** The second operator batch in [CONNECT_AND_LOGS_UX_OVERHAUL_PLAN.md](CONNECT_AND_LOGS_UX_OVERHAUL_PLAN.md) shipped and refined this WIF surface further: the add-trust form now opens behind an "Add trust" button (U3); Edit opens **in-card below the trust** (`wif-trust-edit-form-{id}`, U4); each saved trust shows **per-field validity** badges (`wif-credential-{id}-{field}-status`, U5) + a Verified/Unverified line (U7); a per-trust **Connect** view expands in-card (`wif-credential-connect-{id}`, U6); `allowedTenantId` is **gleaned** from the issuer/JWKS URI when omitted, with the source indicated (U8); the form inputs are reordered to **Token Issuer -> JWKS URI -> Subject -> Audience** (U9); and the assertion debugger + JWKS notice moved behind an **"Advanced / troubleshooting" accordion** (`wif-advanced-accordion`, U1 - the deferred single accordion replaced by targeted progressive disclosure). See the plan doc for the full design + acceptance criteria.

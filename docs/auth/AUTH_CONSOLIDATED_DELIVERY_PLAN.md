...
3. **SyncFabric roadmap** - the canonical guide, **revision 7 (2026-08-19)** - RFC 8693, RFC 7523 binding corrections, persona model, security + metadata truthfulness, migration (its Phases -1..6 and file-by-file Section 25). The in-repo mirror [SCIMSERVER_SYNCFABRIC_WIF_ARCHITECTURE_AND_IMPLEMENTATION_GUIDE.md](SCIMSERVER_SYNCFABRIC_WIF_ARCHITECTURE_AND_IMPLEMENTATION_GUIDE.md) was refreshed to revision 7 on 2026-08-19 and is byte-identical to the canonical copy at `OneDrive - Microsoft\Documents\SCIMServer\auth\`. (It had drifted twice before: a revision-2 mirror of 4,318 lines against a 5,470-line canonical, and again at revision 6 when the canonical was regenerated to revision 7 the same morning this was corrected. A mirror drifts silently, so the assertion above is only worth making if it is re-checked.)
...
> **Re-verified against `origin/master` `09b4b78d` (v0.55.7) on 2026-08-19.** Rows marked
> **[was stale]** were wrong before a verification pass - waves ship without this table being
> updated, so the plan understates its own progress and would cause re-work.
>
> **This has now happened twice** (2026-08-04 and 2026-08-19), to different rows, for the same
> structural reason: an item's status line is updated when it ships, but this summary table is not,
> because nothing binds the two. Two sightings is the promotion threshold, so the fix is tracked as
> **N4** in [REMAINING_WORK_REGISTER.md](REMAINING_WORK_REGISTER.md) - either this table stops
> duplicating the status lines, or a gate asserts the two agree. A third manual correction is not a fix.
...
| Mint `client_secret` path | **[was stale] W2.3 DELIVERED** - extracted to its own provider class with its own spec; no longer inlined in the controller | [client-secret-token-provider.ts](../../api/src/modules/scim/controllers/client-secret-token-provider.ts) |
| JWKS pre-warm / background refresh | **[was stale] W1.4 DELIVERED (v0.55.5)** - background refresh sweep (`refreshTimer` / `setInterval`, `unref`ed, never rejects), TTL raised 10 min -> **24 h**, rate-limited unknown-`kid` refetch, and a hard stale ceiling (`staleIfErrorMs`, default 48 h) that closes the "no maximum age" gap. W1.3 (canonical `jwks_uri` redirect memo) also shipped. **Only the startup prefetch (W1.2) remains** | [external-jwks-validator.service.ts#L126-L174](../../api/src/oauth/external-jwks-validator.service.ts) |
...
- **Status: DELIVERED - api v0.55.5 (2026-08-05).** Feature doc: [W1_4_JWKS_CACHE_CADENCE.md](W1_4_JWKS_CACHE_CADENCE.md); issues + RCA: [EXECUTION_ISSUES_AND_RCA.md](EXECUTION_ISSUES_AND_RCA.md) section 10C (I-35 to I-37). The hard constraint was honored: the TTL raise (600,000 -> **86,400,000 ms**) ships in the same commit as the background refresher, the rate-limited unknown-`kid` path, and the **overlap-window rotation test** (`W1.4-T3`, which asserts the RETIRING key still verifies from cache after the new one forces a refetch - the assertion a replace-rather-than-merge cache fails). Three knobs added to [egress-policy.ts](../../api/src/oauth/egress-policy.ts) with defaults, bounds, env resolution and endpoint overrides, plus three matching per-endpoint flags (`JwksRefreshIntervalMs`, `JwksUnknownKidMinIntervalMs`, `JwksStaleIfErrorMs`). Implementation notes: the cache entry now carries a **kid index** and an `expiresAt` that folds `Cache-Control` in **downward only** (an IdP may ask for a shorter cache, never a longer one); the sweep runs at `max(60s, refreshInterval/4)`, is `unref()`ed and **never rejects**; and the entry is replaced wholesale so a concurrent reader never sees a half-updated set. **The open question in section 10.2 is RESOLVED**: an allowlist rejection is now a distinct `JwksHostNotPermittedError` that is **not stale-eligible**, while a network failure still is - closing the post-revocation window without the availability cost of a purge. The **stale-age ceiling** W1.5 left open is now implemented (`staleIfErrorMs`, default 48 h). Coverage: **+12 unit** (new [external-jwks-cache.w14.spec.ts](../../api/src/oauth/external-jwks-cache.w14.spec.ts)), **+8 E2E** ([config-flags.e2e-spec.ts](../../api/test/e2e/config-flags.e2e-spec.ts)) and **+14 live** (`9z-CE`). Startup prefetch remains W1.2.
...
- SyncFabric guide (Phases -1..6, file-by-file Section 25, test strategy Section 23): [SCIMSERVER_SYNCFABRIC_WIF_ARCHITECTURE_AND_IMPLEMENTATION_GUIDE.md](SCIMSERVER_SYNCFABRIC_WIF_ARCHITECTURE_AND_IMPLEMENTATION_GUIDE.md) - **revision 7**, refreshed 2026-08-19, byte-identical to the canonical copy.
...
| 0.54.63 | Operator-decision revision: **W0.1 secret redaction DECLINED** (`PERSIST_REQUEST_SECRETS` stays default true; runtime opt-out only), so Wave 0 is now just the 200/no-store + capability-truthful-metadata correctness items (15 -> 7 points, ~116 core). Added **W2.5** (consolidate the overlapping `retired combined credential setting` legacy umbrella into the per-method flags + co-locate enablement with each method via the `AuthenticationMethod` model, so each `ResourceAuthenticator` owns `isEnabled()` and metadata derives from the same source) with a design answer on why co-locating enablement is good design. Cross-refs the delivered X14 copy/download-JSON drawer UI. |
...
| 0.55.6 | **Cross-source stock-take (2026-08-19).** Re-verified against `origin/master` `09b4b78d`. Section 1 had gone stale **again**, in the same understating direction as 2026-08-04 and for the same structural reason: two rows described delivered work as outstanding (`client_secret` mint "inlined in the controller" - W2.3 shipped it as its own provider class; JWKS background refresh "still none, 10-min TTL" - W1.4 shipped the sweep and a 24 h TTL in v0.55.5). Both corrected. Because this is the **second** sighting of one pattern, the fix is no longer another manual correction: it is tracked as **N4** (bind the table to the item status, or delete the duplication). **Correction (same day):** this entry originally also claimed the canonical guide cited a "v0.55.7" that did not exist. That claim was **wrong and is retracted** - v0.55.7 is real (commit `07db4088`, CHANGELOG line 11); the stock-take had read a working tree that was 3 commits behind `origin/master` and lacked that commit. See [REMAINING_WORK_REGISTER.md](REMAINING_WORK_REGISTER.md) section 1.2. New consolidated register: [REMAINING_WORK_REGISTER.md](REMAINING_WORK_REGISTER.md). Documentation only - no runtime change. |

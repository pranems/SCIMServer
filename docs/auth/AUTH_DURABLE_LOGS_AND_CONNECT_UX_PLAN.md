...
> **Status.** ALL W1-W12 IMPLEMENTED. Export track (W1-W6) DEPLOYED + validated on dev (v0.54.45). Reorg track (W7-W12) implemented (v0.54.46 W8/W9/W10, v0.54.47 W11/W12, v0.54.48 W7); deploy + Playwright validation on dev in progress. **W1** durable `authDecision` trace on the `RequestLog` row (v0.54.42). **W2** JWT decode everywhere + admin `POST /scim/admin/decode-jwt` (v0.54.43). **W3+W4** endpoint + per-method Connect export (v0.54.44). **W5+W6** per-card + per-subpanel Connect export (v0.54.45). **W8+W9+W10** bearer Connect subpanel + rename to "Connect this endpoint to IdP like Entra ID" + per-parameter `InfoLabel` help (v0.54.46). **W11+W12** removed the redundant "All" method tab (default to the first configured per-endpoint method) + removed the redundant endpoint-level Connect card except on the shared-secret tab (v0.54.47). **W7** organized the crowded credential-card actions into a primary set + a "More" overflow menu (v0.54.48). Each item shipped with TDD (web vitest + Playwright), CHANGELOG, version bump. Prod is never auto-promoted.
>

...
> **Status.** IN PROGRESS - export track (W1-W6) DONE + DEPLOYED + validated on dev; reorg track (W7-W12) pending. **W1 IMPLEMENTED + DEPLOYED (v0.54.42)** (durable `authDecision` trace on the `RequestLog` row; U11 renders from the row). **W2 IMPLEMENTED + DEPLOYED (v0.54.43)** (JWT decode everywhere: `JwtDecodeButton` + `CopyableJsonBlock` inline decode + admin `POST /scim/admin/decode-jwt`; live-test 9z-BP 5/5 green). **W3+W4 IMPLEMENTED + DEPLOYED (v0.54.44)** (endpoint-level + per-method Connect export via `SettingsJsonExport`). **W5+W6 IMPLEMENTED + DEPLOYED (v0.54.45)** (per-card + per-subpanel Connect export). W3-W6 validated on dev: `connect-tab` + `wif-credentials` Playwright green. **W7-W12 (Connect-tab UX reorganization) pending.** Each item follows the standard feature checklist (TDD; web vitest + Playwright; docs; version bump; measured dev deploy per track). Prod is never auto-promoted.
>

...
|---|---|---|---|---|---|---|
| PIPE-01 | Tooling / environment | Medium | `dev-deployment-pipeline.ps1` Stage 4.1 `docker compose build` failed at the web-build `npm ci` (node:25-alpine) with `npm error Exit handler never called!` | **CORRECTED 2026-07-29 - the original root cause below was wrong.** Actual mechanism: every request to `registry.npmjs.org` fails with `ERR_SSL_SSL/TLS_ALERT_HANDSHAKE_FAILURE`. **SECOND CORRECTION 2026-07-30 - the "machine-wide block" framing below was ALSO wrong.** `registry.npmjs.org` is egress-blocked **by design** as a Microsoft corporate secure-supply-chain control; npm on the host is redirected to an internal feed proxy and **works fine** (`npm config get registry`). The original diagnosis tested with `--registry https://registry.npmjs.org`, explicitly overriding the corporate registry, and so concluded the machine was broken. Only the **container** path is blocked, because a container does not inherit the host `~/.npmrc`. That makes this the second wrong root cause recorded for the same symptom, and the lesson is now a standing rule: **check which endpoint was actually contacted before concluding the network is blocked.** See [../strategy/NPM_SUPPLY_CHAIN_QUARANTINE_POLICY.md](../strategy/NPM_SUPPLY_CHAIN_QUARANTINE_POLICY.md). The block is machine-wide (the Windows host fails the same handshake) and specific to npmjs - `github.com`, `ghcr.io`, `login.microsoftonline.com` and `nodejs.org` all handshake cleanly, and plain HTTP to the registry returns a proxy `403`. `Exit handler never called!` is just npm's poor surfacing of a mass TLS failure. It is **deterministic, not a flake** (3/3 `npm ping` failures; identical ~73s death in two unrelated dependency trees). ~~Original (wrong): "Known npm-in-Docker environmental flake"~~ | Bypass the blocked local build: `az acr import --name acrscimserver20622 --source ghcr.io/pranems/scimserver:latest --image scimserver:<sha> --force`, then `az containerapp update --image acrscimserver20622.azurecr.io/scimserver:<sha> --revision-suffix <fresh>` | **Do NOT "re-run once" - it will never pass while the registry is blocked.** Confirm the mechanism first with `docker run --rm node:24-alpine npm ping`; if it is the TLS block, go straight to the CI/GHCR image as the deploy source. The misclassification as a "flake" is why this recurred on 2026-07-29 and cost a second investigation |

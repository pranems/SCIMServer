...
>
> **Status.** IMPLEMENTED (0.54.0-alpha.28). All 17 work items of the [section 11A backlog](#11a-work-items-delivery-backlog) shipped: the `connection-info` API ([Part 6](#6-proposed-connection-info-api-single-source-of-truth), WI-2) + BFF embed (WI-3), the `ConnectionPanel` UI + Connect tab ([Part 8](#8-ui-mockups), WI-4/WI-5), the re-viewable-secret feature ([Part 6A](#6a-secret-visibility-credentialsecretvisibility--envelope-encryption-kek): envelope encryption WI-6, `CredentialSecretVisibility` WI-7, reveal + server security settings WI-8, rotate WI-9), the auth-method hygiene (per-method flag split WI-11, per-endpoint OAuth metadata WI-12, trust aliases WI-13, discovery resolver WI-14, JWKS host allowlist WI-15), and multi-IdP (WI-16/WI-17). Execution tracked in [CONNECTION_INFO_EXECUTION_LEDGER.md](CONNECTION_INFO_EXECUTION_LEDGER.md); issues in [CONNECTION_INFO_EXECUTION_RCA.md](CONNECTION_INFO_EXECUTION_RCA.md). The auth flows, routes, models, and URL behavior in [Part 2](#2-scimservers-actual-url-shapes-authoritative) through [Part 5](#5-the-authentication-combination-matrix) are CURRENT and verified against the sources cited inline.
>
...
> **Status.** DONE (2026-07-06, [endpoint-oauth-metadata.controller.ts](../../api/src/modules/scim/controllers/endpoint-oauth-metadata.controller.ts)). Today the GLOBAL RFC 8414 metadata also exists (at the deployment root, [oauth-metadata.controller.ts](../../api/src/oauth/oauth-metadata.controller.ts)); WI-12 added the PER-ENDPOINT document in the append form below.
...
> **Status.** DONE (2026-07-06, [wif-discovery-resolver.service.ts](../../api/src/oauth/wif-discovery-resolver.service.ts)). [WI-14](#11a-work-items-delivery-backlog). Nothing here changes the runtime validation path - it is all config-time ergonomics that fills the SAME stored `expected*` / `jwksUri` fields.
...
> **Status.** DONE (local), shipped 0.54.0-alpha.20. [WI-15](#11a-work-items-delivery-backlog). Implemented in [jwks-host-allowlist.service.ts](../../api/src/oauth/jwks-host-allowlist.service.ts) (effective = seed union env union persisted; O(1); hot-reload) + admin API [admin-jwks-host.controller.ts](../../api/src/modules/scim/controllers/admin-jwks-host.controller.ts) + Settings-page JWKS host card. The runtime SSRF enforcement point stays; this changes only how the allowlist is populated and edited.
...
> **Status.** PARTIALLY IMPLEMENTED. The envelope-encryption foundation (6A.4 KEK -> DEK, 6A.5 KEK env + default) shipped as WI-6 (0.54.0-alpha.25): [credential-envelope.ts](../../api/src/security/credential-envelope.ts) + [credential-kek.ts](../../api/src/security/credential-kek.ts) + [credential-encryption.service.ts](../../api/src/security/credential-encryption.service.ts) + the `CredentialDek` table. The `CredentialSecretVisibility` setting (6A.2/6A.3), the retain-on-create wiring, and the reveal (6A.7) + rotate admin surfaces are WI-7/8/9 (in progress). The KEK env instructions are live in [DEPLOYMENT.md](../../DEPLOYMENT.md), `docker-compose.yml`, and the README env table.
...
> **Status.** IMPLEMENTED as WI-8 (0.54.0-alpha.27): the `GET/PUT /admin/settings/security` server surface ([admin-security-settings.controller.ts](../../api/src/modules/scim/controllers/admin-security-settings.controller.ts)) and the `POST .../reveal` endpoint ([admin-credential.controller.ts](../../api/src/modules/scim/controllers/admin-credential.controller.ts)) are live and audit-logged. The shapes below match what ships.
...
| WI-15 | JWKS host allowlist: prepopulate + persist + admin-editable at runtime | M | none | **DONE (2026-07-06).** Three-layer effective allowlist (seed of well-known IdP hosts + env + persisted admin-editable layer, hot-reloaded) in [jwks-host-allowlist.service.ts](../../api/src/oauth/jwks-host-allowlist.service.ts); admin API `GET/POST/DELETE /scim/admin/settings/jwks-hosts` + server-level Settings-page JWKS host card. A convenience/runtime-flexibility choice - no deny-list, no lock flag; the existing https + exact-host-match validation is retained. See [5D](#5d-jwks-host-allowlist-prepopulated-persisted-hot-editable). |

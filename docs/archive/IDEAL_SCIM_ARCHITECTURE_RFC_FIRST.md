...
2. **Endpoint Isolation by Design:** Multi-tenancy is baked into the lowest persistence layer using Row-Level Security (RLS) and endpoint-aware routing.
3. **Configuration-Driven Behavior:** Every endpoint has an independent `ServiceProviderConfig` (RFC 7644 §4) and custom `Schemas` (RFC 7643 §2). The API behavior dynamically adapts to these flags.
4. **Hybrid Persistence (Relational + Document):** Uses a robust relational database (PostgreSQL) for strict endpoint/metadata relationships, combined with `JSONB` for dynamic SCIM resource payloads and GIN indexing for fast RFC 7644 filtering.
5. **Stateless & Horizontally Scalable:** The API layer maintains no state. All endpoint configurations are cached via distributed caches (e.g., Redis).
...
graph TD
    ...
    API_Gateway --> AuthZ[Auth, Endpoint & Subject Resolution]
    
    subgraph SCIM Application Layer
        AuthZ --> Router[SCIM Protocol Router]
        Router --> ConfigEngine[Endpoint Config & Schema Engine]
...
### 3.1. Endpoint & Subject Resolution
* **Endpoint Resolution:** The `Authorization: Bearer <token>` is decoded to extract the `endpoint_id`.
...
Each endpoint possesses a configuration profile mapping to `/ServiceProviderConfig`.
...
-- Row Level Security (RLS) for strict Endpoint Isolation
ALTER TABLE scim_resources ENABLE ROW LEVEL SECURITY;
CREATE POLICY endpoint_isolation_policy ON scim_resources 
    USING (endpoint_id = current_setting('app.current_endpoint')::UUID);
...
CREATE UNIQUE INDEX idx_scim_username_endpoint ON scim_resources(endpoint_id, user_name) WHERE resource_type = 'User';
CREATE INDEX idx_scim_external_id ON scim_resources(endpoint_id, external_id);
```
...
* `GET /ServiceProviderConfig`: Returns endpoint-specific feature flags.
* `GET /Schemas`: Returns all schemas (Core User, Enterprise User, Group, plus endpoint custom extensions).
...
* **`DELETE /Users/{id}` (Delete):**
  * Hard deletes the row from `scim_resources` or sets an `active=false` flag depending on endpoint compliance requirements.
...
* Supports GET, PUT, PATCH, and DELETE (if self-deletion is allowed by endpoint config).
...
3. **Uniqueness:**
   * `none`: No checks.
   * `server`: Unique within the endpoint (e.g., `userName`). Enforced via DB Unique Indexes.
   * `global`: Unique across all endpoints (rare, but supported).
...
* **Rate Limiting:** Applied per-endpoint and per-IP at the API Gateway to prevent noisy-neighbor problems in the multi-endpoint environment.
* **CORS:** Configurable per-endpoint if browser-based SCIM clients are utilized.
...
Instead of hardcoding the Core User schema, the system boots with a "Base Schema". When a endpoint is created, they receive a copy of the Base Schema in `endpoint_schemas`. 
If a endpoint wants to add a custom attribute (e.g., `urn:ietf:params:scim:schemas:extension:custom:2.0:User:badgeNumber`), they simply add a new row to `endpoint_schemas`. The Schema Validator dynamically loads this at runtime, instantly allowing validation, storage, and filtering of `badgeNumber` without a single line of code change or DB migration.

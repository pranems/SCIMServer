...
> **Status:** Living reference - **Created:** 2026-07-29 - **Last verified:** 2026-08-12 - **Repo version at capture:** `api/package.json` = `0.55.5`
...
| Running app version | **0.55.6** | **0.55.6** | **0.55.1** |
...
| **G14** | Info | All three reachable estates that this repo deploys are at **0.55.6**; customer prod (calmsand) remains at **0.55.1** pending an explicit operator go-ahead in its separate tenant | `/scim/admin/version` on all four estates | Expected. Dev and the canary are in lockstep, which is the invariant that matters before a customer-prod promotion | Promote 0.55.6 to calmsand when the operator approves. It needs its own `az login` into the AnandSa tenant |

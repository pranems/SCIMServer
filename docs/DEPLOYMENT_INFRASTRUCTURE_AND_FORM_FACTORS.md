...
> **Status:** Living reference - **Created:** 2026-07-29 - **Last verified:** 2026-08-26 - **Repo version at capture:** `api/package.json` = `0.55.14`
...
| Retention selection | image-aware (2026-09-03, D3) - the second slot goes to a revision running a **different** image | image-aware (2026-09-03, D3) | keep=1: budget spent on the serving revision, selection unchanged |
...
| 4 | 4.1 full-validation pipeline, 4.2 optional ACR mirror resolved from the Compose `api` service's immutable image ID (never its container name), 4.3 dispatch `publish-ghcr.yml` from merged `master`, normalize ANSI/BOM from GitHub CLI JSON, and select the run by exact head SHA + dispatch time, 4.4 prove anonymous version/latest pulls resolve to identical image content, 4.5 `az acr import`, 4.5b tag-visible check, 4.6 `az containerapp update`, 4.6b version-echo poll, 4.7 `live-test.ps1` **gated on 4.6b** |
...
| Date | Change |
|---|---|
| 2026-09-03 | **The prune kept N revisions, which is not the same as keeping a rollback target - and it silently spent the canary's.** Retention selected the newest `revisionKeep` revisions by **creation time**, which only equals "serving + something to roll back to" while those revisions run **different images**. On the v0.55.18 promote an interrupted first attempt left an orphan revision carrying the **same** image as the one being deployed, so the two newest were both v0.55.18; the prune deactivated `green-0902-2136`, the only v0.55.17 revision, and the estate was left with **no way back while every check reported success** - the policy satisfied to the letter and defeated in purpose. Selection moved to [revision-selection.ps1](../scripts/revision-selection.ps1), shared by the prune and its self-test so the two cannot disagree (the same reason `node-lts.ps1` is shared by the base-image gate and the doctor); remaining budget now prefers a revision whose **image differs** from the serving one and only backfills with a duplicate when there is no alternative, and the prune **names the rollback target** or says plainly there is none. **A second defect sat in the same output:** the promote computed `blueRetained` as `revisionKeep -ge 2`, which was *true* while the prune had just deactivated blue, so it printed an instant-rollback command naming a revision that could not serve - **a rollback command that cannot work is worse than none, because it is read under pressure and looks authoritative.** It now reads the state back from Azure after the prune and no longer asserts a cause it did not verify. **The customer-prod cost policy is protected structurally rather than special-cased:** at `keep=1` the budget is spent once the serving revision is retained, so every image-aware branch is skipped and behaviour there is byte-identical to before - pinned by `PR-T3` and verified against a real estate with `-Keep 1 -WhatIf`. Eight selector cases run in pre-push as `deploy: revision retention selector`, because deploy tooling gets no feedback from the app's test suites and an ungated selector is one nobody would notice breaking. |
| 2026-08-26 | **Stage 3b.3 now runs BOTH flag gates, because only one direction was ever checked.** The pipeline ran `endpoint-config-conformance` alone, which asks *does the UI offer a key the server does not know?* Nothing asked the reverse, so a flag could be registered, bounded, validated, persisted and **enforced** while being unreachable in the UI - and **eleven had drifted into exactly that state**. The pattern is now `endpoint-config-(conformance/ui-coverage)`. The two are separate specs on purpose: F-T4 failing means the UI **misleads** the operator about what they control, U-T1 failing means a control **exists that the operator cannot find**, and merging them would blur which fired. **The new gate took three attempts and the negative control caught both misses** - `includes()` matched `MaxActiveWifTrustsREMOVED` while looking for `MaxActiveWifTrusts` so a removal was undetectable, then a word-boundary scan across all of `web/src` matched the flag name inside its own `description:` prose so documentation counted as coverage. It now matches a **declared control** (`key: '<flag>'`), with genuinely bespoke controls declared in `BESPOKE_UI` alongside their location. Proven falsifiable: green as-is, fails naming the flag when its control is removed, green again on restore. |
| 2026-08-25 (later) | **The estates are now watched between deploys, not only during them (register item N2).** Every scheduled workflow this repo owned was a **static gate** - CodeQL, Dependabot, the `.trivyignore` review, doc freshness, RFC currency - and each reads the *repository*, never the running estates. Nothing in CI could distinguish "prod is serving" from "prod has been down for a week"; `audit-deployment-doc.ps1 -Live` check C4 does notice, but only runs **post-deploy**, which is exactly when an estate is already known to be alive. That is why customer prod being unreachable was found by **inspection rather than by alert**. New [probe-estates.ps1](../scripts/probe-estates.ps1) + [estate-liveness.yml](../.github/workflows/estate-liveness.yml) probe `/scim/health` on every live estate every 6 hours. **Target resolution was the whole design problem:** [scim-estates.json](../scripts/scim-estates.json) forbids storing an FQDN (Azure assigns the environment domain and it changes at each tenant rollover, so a committed FQDN is a value guaranteed to become false), resolving one properly requires ARM, and **no workflow here holds Azure credentials** - granting CI standing access to two production subscriptions to run a health check would be a bad trade. Targets therefore arrive as **derived** input: `-Target`, then the `SCIM_ESTATE_URLS` repository variable (set from ARM, outside the repo, changeable without a commit), then ARM directly where auth exists. **Exit code 2 is the point of the whole thing:** resolving zero targets is reported as its own outcome and never as success, because a probe that checked nothing and a probe that found everything healthy must not look identical - that equivalence is precisely the blind spot being removed. Seventeen controls, weighted toward "can this be green without checking anything?" over HTTP mechanics; the zero-target path is additionally proven by running the script where the registry helper cannot be found, and the down path by probing a non-existent FQDN. |
| 2026-08-25 (latest) | **Two deployment gates that could not fail were closed, and three mandated static gates were wired in for the first time.** (1) [verify-deployment.ps1](../scripts/verify-deployment.ps1) excludes the pixel-baseline specs with `--grep-invert`, which is correct because those baselines are coupled to fixture data that differs per estate - but **an excluded test appears in no pass/skip/fail count**, so every customer-prod verification reported `196 passed` while carrying **zero** visual-regression coverage and saying nothing about it. It now prints `223 collected, 23 excluded, 200 will run`, fails when the filter leaves nothing to run, and fails when the three counts do not balance, so an over-broad pattern cannot silently delete coverage. Negative control: adding `|Settings` to the pattern excludes **57** instead of 23. **A skipped test is visible; an excluded one is not, and that difference is the whole defect.** (2) [dev-deployment-pipeline.ps1](../scripts/dev-deployment-pipeline.ps1) recorded all **11** Stage 3 audits as `PENDING` at 0 s via an unconditional loop with no execution path - they could never return PASS and were explicitly non-blocking for the canary, so 11 of 39 gates were decorative. `endpointConfigFlagAudit` and `dependencyCveSweep` (production-only audit, CRITICAL blocks, plus `.trivyignore` staleness) now execute; the remaining eight are genuine reviewer-judgement gates and are labelled as such rather than pretending to be automatable. (3) Stages **1.9** (`prismaMigrationAudit` - a `schema.prisma` edit with no migration deploys code expecting columns the database lacks, and that surfaces at container start rather than in CI), **1.10** (base-image LTS) and **1.11** (deployment-doc currency) were mandated but had **never been wired into the pipeline** - only into pre-push. All three now run there. **(4) A shipping tag is now always built from `master`.** Stage 4.3 published from the **current branch**, so every 0.55.12 / 0.55.13 image - including the one promoted to customer prod - was built from `feat/wif` rather than from merged `master`, and `:latest` pointed at a feature-branch build. That ref was chosen in 2026-05-29 to fix the **mirror-image** bug: the workflow then defaulted to `master` while the work sat on a feature branch and shipped a **stale** image. Both failures share one root cause - **the built ref and the shipped ref disagreed** - so flipping the ref back would only trade one direction of the bug for the other. Stage **4.3a** now asserts `git merge-base --is-ancestor HEAD origin/master` and **refuses to publish** when HEAD is not contained in master; 4.3 then dispatches `publish-ghcr.yml --ref master`. Order is **merge first, build second**. Because this repo fast-forwards, master and the feature branch point at the same commit after the merge and the artifact is identical - but the invariant now holds even if a merge is ever non-fast-forward. |

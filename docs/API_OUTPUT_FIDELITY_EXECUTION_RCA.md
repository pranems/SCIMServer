...
| F29 | Security gate | High | Second CodeQL run increased to 21 high remote-property alerts despite explicit sink guards | The analyzer still modeled user-controlled property names at dynamic assignments/deletes; a validated string was not a provable barrier | Replaced canonicalized writes/removals with immutable `Object.fromEntries` reconstruction helpers, eliminating the sink category; PATCH suites 330/330, lint 0 errors, build green; exact-tip CodeQL rerun required before merge |
| F30 | Test correctness | High | Independent review found that failed endpoint fixtures became skips, missing enum options were silently ignored, and the editable-flag assertion was only a lower bound | Convenience fallbacks converted missing setup and coverage into non-failures, allowing a green matrix to prove less than its name claimed | Fixture creation now throws with the HTTP response, the flag count is exactly 19, and every rendered enum option must be found and persisted; complete dev matrix 16/16 |
| F31 | Test correctness | Medium | The first strengthened coercion assertion expected the `rfc-standard` preset to start disabled but dev accepted `active:"True"` | The effective preset default is enabled but is absent from the endpoint's stored override object, so stored-profile inspection cannot determine initial polarity | Read the effective starting value from the rendered Switch, prove the toggle inverts it, then require one 201 with native boolean `true` and one 400 rejection; RED reproduced and complete dev matrix GREEN 16/16 |
| F32 | Test correctness | Medium | Second review found that exact counts and DOM-derived option lists still could not detect identity substitution or an omitted option | A cardinality check proves quantity, while a DOM-only baseline merely restates whatever the UI rendered | Compare rendered boolean keys, enum keys, and enum labels to `endpoint-settings-definitions.ts`; missing, duplicated, renamed, or omitted controls now fail; complete dev matrix 16/16 |

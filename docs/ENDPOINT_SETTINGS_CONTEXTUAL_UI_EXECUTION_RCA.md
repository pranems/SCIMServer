...
| Authentication methods presentation | **Four real methods, collapsed, setup order** | OAuth2, WIF, global shared secret and per-endpoint bearer are methods. `retired combined credential setting` is a legacy fallback, not a fifth method. |
...
## 9. v0.55.25 Authentication Cleanup Follow-Up

| ID | Type | Severity | Symptom | Root cause | Resolution and prevention |
|---|---|---|---|---|---|
| UX-26 | Test contract | Medium | The focused resolver suite remained red after production stopped reading the combined credential setting | Eight tests still asserted validation, defaults, and runtime fallback for the contract being removed | Removed obsolete contract tests, retained negative runtime/rejection tests, and rewrote precedence cases around dedicated settings. Focused endpoint tests pass 445/445. |
| UX-27 | Tooling / worktree | Medium | Workspace search reported zero retired-setting references even though 216 remained | VS Code workspace search indexed the primary checkout, not the linked feature worktree | Scoped every inventory to the linked worktree with PowerShell `Select-String` and treated the false-empty result as non-evidence. Future linked-worktree audits must use an absolute worktree path. |
| UX-28 | Authoring / syntax | Low | The first web GREEN attempt failed to parse `endpoint-settings-definitions.ts` | Removing the legacy category with a broad patch left a malformed array boundary | Repaired the array and reran the same focused Vitest slice before continuing. Small per-region edits are the standing prevention for registry arrays. |
| UX-29 | User-facing copy | Medium | Internal labels remained visible in a WIF field label, credential-visibility guidance, and an MSW-backed deployment value | The prior check covered one method-description array rather than all production UI string literals | Added an AST-based source gate over production TypeScript/TSX literals. It went RED on all three values, then passed 142/142 after product-language replacements. The permanent rule is recorded in `.github/copilot-instructions.md`. |
| UX-30 | Documentation truth | Medium | The docs content gate failed after the setting was removed | The documentation index retained the previous registry totals of 38 controls and 21 booleans | Updated the measured totals to 37 controls and 20 booleans. The existing semantic docs gate caught the drift, so no new gate is needed. |
| UX-31 | Migration correctness | High | Read-time normalization alone would preserve runtime behavior but leave PostgreSQL profiles carrying the removed key indefinitely | The endpoint cache boundary mutates the loaded profile but does not persist that normalization back to the database | Added a one-way JSONB migration that fills only missing dedicated values, preserves explicit overrides, and removes the old key. A disposable PostgreSQL 17 replay passed legacy true/false, explicit override, and unrelated-row cases. |
| UX-32 | User-facing API copy | Medium | Internal labels remained in auth diagnostics, endpoint-setting descriptions, a shadow-enforcement log, and the keyed-hash storage placeholder | The first permanent copy gate covered only web production literals | Added a matching AST gate over API production literals, replaced the remaining values with product language, and aligned affected assertions/fixtures. The API gate and focused auth/token suites pass 270/270. |
| UX-33 | Live-test correctness | Medium | The new live assertion received the correct 400 body but failed to find the two replacement setting names | PowerShell exposed `application/scim+json` response content as `byte[]`, and piping that directly to `ConvertFrom-Json` did not decode the body | Decode byte-array content as UTF-8 before parsing. The complete local live suite then passed 1,493/1,493 with clean orphan reconciliation. |
| UX-34 | Browser harness | Medium | A zero-endpoint local estate caused 53 browser failures behind onboarding; after seeding one fixture, a TanStack Router Devtools button intercepted the collapsed-sidebar control and polluted visuals | Local Vite includes first-run onboarding and dev-only router tooling that the deployed production surface does not | Seeded and later deleted one disposable endpoint, hid the dev-only overlay in smoke/visual tests, and kept production layout unchanged after DOM inspection falsified the chart-overlap hypothesis. Sidebar and endpoint-tab smoke pass in isolation. |
| UX-35 | Visual-test correctness | Low | Command Palette and shortcut-help visuals never opened their dialogs | The visual spec used `Control+KeyK` and `Shift+Slash` without focusing the app shell, while passing behavioral specs already documented `Control+k`, literal `?`, and shell focus | Reused the verified trigger/focus sequence. Both visual cases pass with existing baselines; no snapshot was regenerated. |
| UX-36 | Review / migration and cache | High | Review found that invalid dedicated values could suppress a valid legacy value, Prisma updates could re-persist the retired key, and Connect's dedicated query caches were not invalidated by direct mutations or cross-tab SSE events | Migration precedence checked key presence rather than boolean validity; read-time normalization ran after the Prisma write; cache invalidation covered only overview/detail | Exported and reused the canonical boolean parser, normalized before Prisma persistence, hardened SQL validity/idempotence, and centralized invalidation for overview, connection-info, and retained-secret caches in direct mutation and SSE paths. RED checks became API 447/447 and web 213/213 GREEN; PostgreSQL replay covers null/malformed/idempotent cases. |
| UX-37 | Test compilation | Medium | The new web AST gate added four TypeScript diagnostics to the accepted baseline | The web tsconfig intentionally omits Node globals, while the source-scanning test imports `node:fs`, `node:path`, and `node:url` | Added a file-local Node type reference and explicit `Dirent` type. The workspace returned to its 74-diagnostic baseline with zero errors in the new file. |
| UX-38 | Browser test correctness | Medium | The route-mocked credential form test could not find the create button | It assumed bearer was the initially selected method even though the shipped setup order puts WIF before bearer | Select the bearer tab explicitly before opening the create dialog. The exact browser test passes. |
| UX-39 | Tooling / shell | Low | Two API-label inventory commands stopped at PowerShell's `>>` continuation prompt | Inline regular expressions mixed single quotes, double quotes, and backticks in one shell command | Killed the incomplete commands and used AST-based gates/read-only code analysis. Future scans use scripts or quote-safe patterns rather than nested inline regex. |
| UX-40 | Test harness | Low | The first full API unit command finished test work but never returned its captured summary | Jest retained an open handle and the package's unit command does not force exit | Re-ran with `--forceExit` to recover authoritative results. Final corpus is 5,140/5,140; open-handle diagnosis remains separate harness work because no production behavior failed. |
| UX-41 | Security / secret retention | High | Endpoint visibility changes did not purge retained envelopes, and server visibility changes left dedicated connection-info/reveal caches stale in other tabs | Endpoint updates had no link to the credential-security purge owner; server settings invalidated only their own query and emitted no cross-tab event | Registered the existing purge service through an EndpointService listener, purged endpoint envelopes on `once`, emitted `scim.security.updated` after server changes, and invalidated all connection-info/reveal cache prefixes directly and over SSE. RED checks became API 135/135 and web 100/100 GREEN; full suites pass 5,140 and 1,453. |
| UX-42 | CI / gate scope | Medium | The exact-tip push validation failed the new API copy gate only after Prisma generation | The scanner recursively included `api/src/generated/prisma/internal/class.ts`, whose embedded schema text contains historical phase labels but is generated code, not user-facing runtime copy; the generated tree was absent in the local linked worktree during the first scan | Excluded the `generated` directory structurally and added a negative control asserting no generated path enters the scan. The gate continues to cover all hand-authored production API literals. |

### 9.1 Dispositions

- **Test/gate disposition: applied.** New API boundary, migration, HTTP rejection, optimistic Connect, browser pre-response, and AST copy assertions cover the removed contract and its user-visible consequence.
- **Design/architecture disposition: applied.** One setting now owns each authentication method. Existing method-entry precedence remains authoritative, and profile migration is confined to the endpoint boundary plus one persisted data migration.
- **Simplicity counter-check: accepted.** No new resolver or settings abstraction was added. The existing resolver, mutation cache, and profile normalizer were sufficient.

### 9.2 Provenance and completeness

The v0.55.25 entries were reconciled against the full session transcript at `transcripts/c6313bb4-738a-49ae-b8e5-9087e8a14b61.jsonl`, not the compacted conversation summary. The first pass searched error and gate signals including `error TS`, `FAIL`, timeout, blocked, rejected, false positive, malformed, stale, migration, Playwright, and live. The second pass searched diagnosis language including root cause, turned out, culprit, silently, stale, falsified, and false-empty. Each real post-v0.55.24 diagnosis maps to UX-26 through UX-42. Dismissed non-issues were quoted standing instructions; source/test names containing historical phase labels; repeated tool-output echoes; expected RED checks; the local Vite Helmet probe (the HTML shell bypasses Nest middleware); three populated-dev visual baselines run against a one-endpoint local estate; Mermaid's known builtin-version `0.0.0` doctor warning; and the absent web ESLint gate, which correctly remained N/A rather than installing an unpinned tool.

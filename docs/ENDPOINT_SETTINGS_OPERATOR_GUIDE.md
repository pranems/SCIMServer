...
**Custom resource types are not behind a setting.** You will not find a `CustomResourceTypesEnabled` toggle, and you do not need one: an endpoint serves whatever is declared in `profile.resourceTypes[]`, so registering a type on the Resource Types tab is all that is required. That flag existed until settings-v8 and was retired as redundant. Until **0.55.13** the UI still hid Create and Delete when it was absent, which made a working capability look unavailable; if you previously concluded custom types were switched off for an endpoint, re-check it.

**A retired setting disappears from your profile.** Retired keys are removed when the endpoint is read, and the three that were renamed carry their value across (for example `SoftDeleteEnabled` becomes `UserSoftDeleteEnabled`). A `PATCH` containing one is accepted rather than rejected, so old automation keeps working, but the key will not be there when you read the profile back.

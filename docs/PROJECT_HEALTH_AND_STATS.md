...
| **Version** | _sourced from [api/package.json](../api/package.json) at runtime_ |
| **Active Since** | September 2025 |
| **RFC Compliance** | 100% (RFC 7643 / RFC 7644) |
...
| **Feature Flags** | Per-endpoint config flags | 12 persisted boolean settings (`AllowAndCoerceBooleanStrings`, `StrictSchemaValidation`, `SoftDeleteEnabled`, `VerbosePatchSupported`, `MultiOpPatchRequestAddMultipleMembersToGroup`, `MultiOpPatchRequestRemoveMultipleMembersFromGroup`, `PatchOpAllowRemoveAllMembers`, `RequireIfMatch`, `ReprovisionOnConflictForSoftDeletedResource`, `PerEndpointCredentialsEnabled`, `IncludeWarningAboutIgnoredReadOnlyAttribute`, `IgnoreReadOnlyAttributesInPatch`) + `logLevel` + 2 derived capabilities (`CustomResourceTypesEnabled` from profile.resourceTypes, `BulkOperationsEnabled` from profile.serviceProviderConfig.bulk.supported) |
...
│ - name            │
│ - profile (JSONB) │     ┌────────────────────┐
│ - active          │────>│ EndpointCredential  │
│ - createdAt       │     │ - credentialType    │
│ - updatedAt       │     │ - credentialHash    │
└───────┬──────────┘     │ - expiresAt         │
        │                 └────────────────────┘
        ▼
┌──────────────────┐     ┌──────────────────┐
...
> **Detailed load & cost analysis:** For 3-tier load scenario projections (Small/Medium/Large), Azure cost breakdowns by load level, and scaling recommendations, see [DEPLOYMENT_INSTANCES_AND_COSTS.md](DEPLOYMENT_INSTANCES_AND_COSTS.md#load-scenario-definitions).
...
| Phase 13 | 3-Tier Auth + Per-Endpoint Credentials | ✅ Complete | v0.21.0 |
| Phase 14 | ReadOnly Attribute Stripping & Warnings | ✅ Complete | v0.22.0 |
| Phase 15 | P1 Schema/Discovery Attribute Fixes | ✅ Complete | v0.23.0 |
| Phase 16 | P2 Attribute Characteristic Enforcement | ✅ Complete | v0.24.0 |
| Phase 17 | P3 Implementation & Projection Fixes | ✅ Complete | v0.25.0 |
...
| ~~No `/Me` endpoint~~ | ~~LOW~~ | ✅ Implemented (v0.20.0) — RFC 7644 §3.11 with JWT sub → userName resolution |

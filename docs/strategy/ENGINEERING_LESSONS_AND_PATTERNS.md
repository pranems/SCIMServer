...
| **PA-5** | When the artifact is provably fine but the human still cannot see it, the fault is in the VIEWER - and no artifact gate can ever catch that | "I cannot see the diagrams" recurred twice. Round 1 found 20 real mermaid syntax errors and concluded syntax WAS the cause; round 2 had a green 596/596 render gate and provably healthy content, because the actual cause was viewer-side (Workspace Trust silently disables an extension that has not opted in, and `.vscode/extensions.json` was gitignored so no clone was ever prompted to install it) | For any artifact a human VIEWS (rendered diagram, preview, dashboard, report), the gate set needs a **viewer-side doctor** beside the content gate, and triage must run **environment first, content second**. Also: the gate must initialize the renderer with the VIEWER's real config (we used `securityLevel:'loose'` while the extension uses `'strict'`), and must fail a **degenerate** render (renders, but 0x0 or no text), not just a throwing one | [scripts/doctor-mermaid.mjs](../../scripts/doctor-mermaid.mjs); Mermaid Diagram Rule 9 + 10 | 2026-07-28 |
...
| PA-5 (viewer-side blindness) | 2 (mermaid invisibility 2026-07-27 + 2026-07-28) | YES - Mermaid Diagram Rule 9 + 10 + the doctor script |

...
2. **The gate's Mermaid version MUST match the VS Code extension's bundled version.** This is the single highest-value rule here: on 2026-07-27 the gate ran mermaid 11.6 while `bierner.markdown-mermaid` bundled 11.12.2, and two `stateDiagram-v2` blocks that 11.6 accepted were REJECTED by 11.12.2 - broken in the operator's preview while the gate stayed green. The root `package.json` pins mermaid EXACTLY (no `^`), and `scripts/render-mermaid.mjs` prints a loud WARNING when the pinned version drifts from the installed extension's bundle. When the extension updates, re-pin.
...
9. **TRIAGE ORDER: environment FIRST, content SECOND. Run `npm run docs:mermaid:doctor` before touching a single diagram.** (added 2026-07-28 after the second recurrence)
   ...
   **The renderer-side cause taxonomy** (each produces "no diagram, and NO error message", which is why they are so hard to spot):
   ...
   |---|---|---|---|
   | R1 | **Workspace is in Restricted Mode.** `bierner.markdown-mermaid` declares no `capabilities.untrustedWorkspaces`, and per [VS Code's Workspace Trust docs](https://code.visualstudio.com/docs/editing/workspaces/workspace-trust) "extensions that have not explicitly opted into Workspace Trust are **disabled** by default in Restricted Mode". Mermaid fences then fall back to plain code blocks. `security.workspace.trust.startupPrompt` defaults to `never`, so the only signal is a dismissible banner. | `npm run docs:mermaid:doctor` reads VS Code's trusted-folders store directly | Command Palette -> "Workspaces: Manage Workspace Trust" -> Trust |
   | R2 | Extension not installed (a fresh clone / new machine) | doctor | `code --install-extension bierner.markdown-mermaid` |
   | R3 | Mermaid version drift between the gate and the extension | doctor | `npm install --save-exact mermaid@<extension version>` |
   ...
   | R6 | A competing preview extension (Markdown Preview Enhanced bundles its OWN mermaid) | doctor | use the built-in preview |
   ...
   `.vscode/extensions.json` is now committed, and `.gitignore` uses `.vscode/*` + `!.vscode/extensions.json` - it **must** be `.vscode/*`, because git cannot re-include a file whose parent **directory** is excluded, so the negation is silently dead under `.vscode/`.

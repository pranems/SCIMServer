...
1. **Every ```mermaid block MUST parse.** Run `npm run docs:mermaid` (or `node scripts/validate-mermaid.mjs`) before committing any doc that adds or edits a diagram. It parses every ```mermaid block in `docs/` + root `*.md` with the real Mermaid parser and exits non-zero on any failure. It is wired into the pre-push gate as `docs: mermaid diagrams parse`.
2. **Quote every node label that contains a special character.** Mermaid's lexer breaks on `[ ] { } ( ) | @ ;` and `/` (leading) inside an UNQUOTED label. `D{URN in schemas[] array?}` is a parse error; `D{"URN in schemas[] array?"}` is correct. When in doubt, quote the label - quoting is never wrong.
3. **Never use backslash-escaped quotes** (`\"`) inside a label. Mermaid does not support them. Use the entity `#quot;` instead.
4. **In `sequenceDiagram`, a `;` inside message text terminates the statement.** Write it as `#59;` (this bit `application/scim+json; charset=utf-8` in a real doc). Every line inside a `sequenceDiagram` must be a valid statement - a bare line of prose is a parse error; fold it into the preceding message or a `Note over A,B:`.
5. **Only use diagram types the pinned Mermaid version renders.** `radar` is NOT supported and silently fails. If a diagram type is unsupported, keep the DATA (a ```text block plus an equivalent markdown table) rather than deleting it.
6. **VS Code needs the extension.** [`bierner.markdown-mermaid`](https://marketplace.visualstudio.com/items?itemName=bierner.markdown-mermaid) is a workspace recommendation in [.vscode/extensions.json](.vscode/extensions.json). Diagrams render only in the **Markdown Preview** (`Ctrl+Shift+V`, or `Ctrl+K V` for side-by-side) - never in the raw editor.
7. **Do not delete a diagram to make the gate pass.** Fix the syntax and preserve every node, edge, label and subgraph. Losing the diagram is a worse outcome than a broken one.

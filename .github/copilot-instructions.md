...
6.1. **Version bump** - `api/package.json` + `web/package.json` + lockfiles regenerated **inside node:24-alpine** for cross-platform reproducibility (matches the deployed runtime, which is built from the ROOT [Dockerfile](Dockerfile) - NOT [api/Dockerfile](api/Dockerfile), which is not what ships).

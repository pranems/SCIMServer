...
Origin: 2026-07-27. The operator reported "I cannot see the diagrams in the docs in VS Code", and noted it had happened before. Investigation found the real cause: **20 of 587 ```mermaid blocks across `docs/` had syntax errors**. A Mermaid syntax error does NOT fail any build, any test, or any lint - the diagram silently renders as an error box (or nothing at all) in the VS Code preview and on GitHub. So a broken diagram is invisible to every automated gate and is only discovered when a human opens the doc, which is why it kept recurring.
...
7. **VS Code needs the extension AND the preview.** [`bierner.markdown-mermaid`](https://marketplace.visualstudio.com/items?itemName=bierner.markdown-mermaid) is a workspace recommendation in [.vscode/extensions.json](.vscode/extensions.json). Diagrams render only in the **Markdown Preview** (`Ctrl+Shift+V`, or `Ctrl+K V` for side-by-side) - never in the raw editor. If a diagram looks blank after an edit, the preview is stale: close it and reopen, or run `Developer: Reload Window`.

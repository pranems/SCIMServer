...
## Mermaid Diagram Rule (CRITICAL - added 2026-07-27)

Origin: 2026-07-27, **and again 2026-07-28**. The operator reported "I cannot see the diagrams in the docs in VS Code", and noted it had happened before. The 2026-07-27 investigation found **20 of 587 ```mermaid blocks had syntax errors** and fixed them, because a Mermaid syntax error does NOT fail any build, test, or lint - the diagram silently renders as an error box (or nothing) in the VS Code preview and on GitHub. That was a real cause, but it was **not the only one**: the symptom recurred on 2026-07-28 with the render gate green at 596/596 and every diagram provably healthy. Broken content and a disabled *viewer* produce the identical symptom, and the second class is invisible to every gate this repo owns. Rules 1-6 and 8 harden the CONTENT; rules 7, 9 and 10 harden the RENDERER and the gate's fidelity to it. **Read rule 9 first when the symptom appears.**

1. **Every ```mermaid block MUST RENDER, not merely parse.** Run `npm run docs:mermaid:render` (a real headless Chromium, the same thing the VS Code preview webview and GitHub use) before committing any doc that adds or edits a diagram. `npm run docs:mermaid` is the faster grammar-only check. The render gate is wired into pre-push as `docs: mermaid diagrams render` (~17s for 586 blocks). **Parsing is NOT rendering** - a diagram can pass the grammar check and still show as a blank/error box.
2. **The gate's Mermaid version MUST match the version the AUTHORITATIVE renderer bundles - and the authoritative renderer is VS Code's BUILT-IN one.** Since VS Code 1.104 the editor ships `vscode.mermaid-markdown-features` inside its own install directory (`<install>/resources/app/extensions/mermaid-markdown-features`); it contributes `markdown.previewScripts` and bundles its own Mermaid. That is what every reader gets, so that is what the gate pins. This rule has now failed twice for the same underlying reason - the gate compared itself to the wrong thing: on 2026-07-27 it ran 11.6 while `bierner.markdown-mermaid` bundled 11.12.2, and on 2026-07-28 it ran 11.12.2 while the **built-in** bundled **11.15.0**, a renderer the gate did not know existed because it only looked in `~/.vscode/extensions`. Renderer discovery now lives in [scripts/mermaid-renderers.mjs](scripts/mermaid-renderers.mjs), shared by the gate and the doctor so they can never disagree. The root `package.json` pins mermaid EXACTLY (no `^`). When VS Code updates, re-run the doctor and re-pin.
3. **Quote every node label that contains a special character.** Mermaid's lexer breaks on `[ ] { } ( ) | @ ;` and `/` (leading) inside an UNQUOTED label. `D{URN in schemas[] array?}` is a parse error; `D{"URN in schemas[] array?"}` is correct. Quote edge labels too (`-->|"no, multiple"|`). When in doubt, quote - quoting is never wrong.
4. **Never use backslash-escaped quotes** (`\"`) inside a label. Mermaid does not support them. Use the entity `#quot;` instead.
5. **A `:` or `;` inside a label is a statement separator.** In `sequenceDiagram` a `;` in message text terminates the statement (write `#59;`); in `stateDiagram-v2` a SECOND `:` in a transition label breaks it - `Active --> Deleted: DELETE /admin/endpoints/:id` fails, use `#58;id` or reword. Every line inside a `sequenceDiagram` must be a valid statement - a bare line of prose is a parse error; fold it into the preceding message or a `Note over A,B:`.
6. **Only use diagram types the pinned Mermaid version renders.** `radar` is NOT supported and silently fails. If a diagram type is unsupported, keep the DATA (a ```text block plus an equivalent markdown table) rather than deleting it.
7. **VS Code needs the extension, a TRUSTED workspace, AND the preview.** Diagrams render only in the **Markdown Preview** (`Ctrl+Shift+V`, or `Ctrl+K V` for side-by-side) - never in the raw editor. See the Rule 9 triage below before assuming anything is wrong with the diagram itself.
8. **Do not delete a diagram to make the gate pass.** Fix the syntax and preserve every node, edge, label and subgraph. Losing the diagram is a worse outcome than a broken one.
9. **TRIAGE ORDER: environment FIRST, content SECOND. Run `npm run docs:mermaid:doctor` before touching a single diagram.** (added 2026-07-28 after the second recurrence)

   Origin: 2026-07-28. The operator again reported "I cannot see the diagrams". The render gate was green at 596/596, and a re-render under the extension's OWN config in BOTH themes confirmed **every diagram was healthy**. The content was never the problem. The 2026-07-27 investigation had (correctly) found 20 syntax errors and then (incorrectly) concluded syntax was *the* root cause, so the second recurrence was spent re-chasing syntax that did not exist. Two structural defects caused that:

   - **The instructions asserted a fact that was false.** This rule used to say the extension "is a workspace recommendation in `.vscode/extensions.json`". `.gitignore` line 38 was `.vscode/`, so the file was **never committed** (`git ls-files .vscode` was empty) - it existed only on one machine. Every fresh clone got no recommendation, and every investigation started from "the environment is handled, so it must be syntax". A false premise in this file is worse than a missing one, because it terminates the investigation in the wrong place.
   - **Every gate we own validates CONTENT; the failure modes are RENDERER-SIDE.** No content gate can ever catch a disabled extension. That gap is now closed by [scripts/doctor-mermaid.mjs](scripts/doctor-mermaid.mjs).

   **The renderer-side cause taxonomy** (each produces "no diagram, and NO error message", which is why they are so hard to spot):

   | # | Cause | How to confirm | Fix |
   |---|---|---|---|
   | **R0** | **TWO Mermaid renderers race in the same preview webview.** This was the ACTUAL 2026-07-28 root cause. VS Code >= 1.104 ships `vscode.mermaid-markdown-features` **built in**; `bierner.markdown-mermaid` predates it and does the same job with the **same** `markdown-mermaid.*` config namespace and the **same** `markdown.previewScripts` contribution. With both installed, two different Mermaid builds (11.15.0 and 11.12.2 here) are injected into one webview and both try to render the same `<pre class="mermaid">` nodes; the loser replaces or re-parses the winner's output and the diagram comes out **blank, with no error**. | `npm run docs:mermaid:doctor` enumerates built-in AND marketplace renderers | `code --uninstall-extension bierner.markdown-mermaid`, then reload the window. The built-in needs no extension at all. |
   | R1 | Workspace is in Restricted Mode. **Only applies if the ACTIVE renderer does not declare `capabilities.untrustedWorkspaces`.** The built-in DOES declare it, so with the built-in renderer trust is irrelevant; `bierner.markdown-mermaid` does NOT, so it is disabled in Restricted Mode. Per [VS Code's Workspace Trust docs](https://code.visualstudio.com/docs/editing/workspaces/workspace-trust) "extensions that have not explicitly opted into Workspace Trust are **disabled** by default in Restricted Mode". | doctor reads VS Code's trusted-folders store directly | Command Palette -> "Workspaces: Manage Workspace Trust" -> Trust |
   | R2 | No renderer at all (VS Code older than 1.104 and no extension) | doctor | update VS Code, or `code --install-extension bierner.markdown-mermaid` |
   | R3 | Mermaid version drift between the gate and the authoritative renderer | doctor | `npm install --save-exact mermaid@<version>` |
   | R4 | Looking at the raw editor, not the preview | n/a | `Ctrl+Shift+V` |
   | R5 | Stale preview webview | n/a | close+reopen the preview, or `Developer: Reload Window` |
   | R6 | A competing full-preview extension (Markdown Preview Enhanced bundles its OWN mermaid) | doctor | use the built-in preview |
   | R7 | `markdown-mermaid.maxHeight` set small, clipping a tall diagram (this repo has **96 diagrams over 1200px**) | doctor | clear the setting |
   | R8 | Content genuinely broken | `npm run docs:mermaid:render` | fix per rules 3-6 |

   **Check R0 first.** It is the only cause where every other signal - renderer installed, version pinned, workspace trusted, content gate green - reads perfectly healthy.

   `.vscode/extensions.json` is now committed, and `.gitignore` uses `.vscode/*` + `!.vscode/extensions.json` - it **must** be `.vscode/*`, because git cannot re-include a file whose parent **directory** is excluded, so the negation is silently dead under `.vscode/`. The file now lists `bierner.markdown-mermaid` under **`unwantedRecommendations`**, not `recommendations`: on VS Code 1.104+ recommending it actively creates the R0 conflict.

10. **The render gate MUST use the renderer's real config, not a convenient one.** Until 2026-07-28 `render-mermaid.mjs` initialized with `securityLevel: 'loose'` while the extension uses Mermaid's default `'strict'` (verified by reading its `dist-preview` bundle), under which labels pass through DOMPurify. The gate now renders every block under `strict` in **both** the light (`default`) and dark (`dark`) themes the extension selects from, and fails a block that renders but is **degenerate** - smaller than 40x20px, or containing no text at all because every label was sanitized away. This is the same lesson as rule 2: a gate is only trustworthy to the extent it reproduces what actually renders for a human.
11. **There is an editor-independent way to read the diagrams: `npm run docs:html`.** [scripts/export-docs-html.mjs](scripts/export-docs-html.mjs) converts Markdown to standalone HTML with every Mermaid block **pre-rendered as inline SVG**, using the same headless Chromium and the same pinned Mermaid the gate uses. Output lands in `test-results/docs-html/` (gitignored build artifacts, never committed) with an `index.html` that lists every doc and its diagram count. Nothing about it depends on a VS Code extension, workspace trust, the Markdown preview, or version pinning - so it is both the guaranteed-visible fallback when the preview misbehaves and a convenient way to read or share a long design doc. **When an operator reports missing diagrams, run the doctor AND regenerate this export**: it separates "the diagrams are broken" from "your viewer is broken" in one step, with a visible artifact to point at.
...
**R5.1. Prefer PERCENTAGE column widths over px under `table-layout: fixed` (added 2026-07-08).** Origin: the 2026-07-08 `OperationsPage` "no table" bug. The All-users/All-groups tables used `table-layout: auto` with a RAW `<Text>{userName}</Text>` name cell (no truncation at all), so a long email-shaped userName ballooned column 1 to 743px of a 1103px table (67%), shoving the other three columns off-screen - the operator saw a single-column list and reported "no table". The first fix (`table-layout: fixed` + px metadata columns + `width:auto` name column) then swung the OTHER way: at a narrow viewport the fixed-px columns consumed the width and STARVED the `auto` name column to 1px (0%). The durable fix is **percentage widths on every column** (they sum to 100% and scale proportionally at ANY viewport). Use px widths only for columns that must be truly fixed (e.g. a 1-icon action column). Every name/URL/user-string column MUST additionally render through `TruncatedText`/`CopyableField truncate` - `table-layout: fixed` bounds the COLUMN, the primitive clips the TEXT; you need both.

**R5.2. Every truncating table MUST carry an R1 bounded-column Playwright assertion.** A table with truncating cells is not covered until a Playwright spec measures REAL header/column bounds (via `page.evaluate` + `getBoundingClientRect`) and fails if (a) any column's right edge overflows the table's right edge, (b) the name column exceeds a sane share (e.g. > 60%) of the table width, OR (c) the name column collapses below a floor (e.g. < 30%) at a deliberately-narrowed viewport. Assert at BOTH a wide (~1440px) and a narrow (~900px) viewport - the Operations bug had two failure modes (too-wide and collapsed) that only a two-viewport check catches. Testid-presence assertions (`await expect(getByTestId('operations-users-search')).toBeVisible()`) do NOT count - see R10.

**R5.3. Known remaining `table-layout: auto` tables (2026-07-08 audit, lower severity).** [LogsPage.tsx](web/src/pages/LogsPage.tsx), [LogsTab.tsx](web/src/pages/LogsTab.tsx) (URL column) and [WorkbenchPage.tsx](web/src/pages/WorkbenchPage.tsx) (history `path` column) use `table-layout: auto` but their long cell is a `CopyableField truncate maxWidth="<px>"`, so the column is CAPPED (not unbounded like Operations was) - functionally OK today but still an R5 gap that can crowd on a very narrow viewport. [DiscoveryExplorerPage.tsx](web/src/pages/DiscoveryExplorerPage.tsx) `diffTable` renders raw `{r.primaryValue}` with NO truncation - a long discovery value can overflow. All four MUST be migrated to `table-layout: fixed` + percentage widths (+ truncation on the Discovery value cells) when next touched, and covered by an R5.2 assertion. Tracked so the audit gate (below) does not re-flag them as new.
...
- **(a) Improvement identified + applied in-place**: add the rule/test/check that would have caught the issue. Land it in the SAME commit chain. If it generalizes beyond this change, also distill it as a pattern in the central [docs/strategy/ENGINEERING_LESSONS_AND_PATTERNS.md](docs/strategy/ENGINEERING_LESSONS_AND_PATTERNS.md).
...
### R10. Presence is not correctness - a gate only proves what it asserts (CRITICAL - added 2026-07-08 after the Operations "no table" miss)

Origin: 2026-07-08. The operator opened the Operations page and saw the All-users subtab render as a single-column list - an obvious visual regression. The agent had, days earlier, (a) written a Playwright spec for that exact page, (b) run the full suite green, (c) done a "headed/visual pass," and (d) declared the work complete. All four missed the bug. The root cause of the MISS (distinct from the root cause of the bug, which was R5) is a single discipline failure that generalizes to UI, API, and everything else:

**A green gate only proves the specific assertion it makes. An assertion that a thing EXISTS is not an assertion that the thing is CORRECT.**

The concrete failures and their standing corrections:

1. **Presence-only assertions masquerading as coverage.** The Operations spec asserted `getByTestId('operations-users-search').toBeVisible()`, `operations-tab-users` visible, CSV button visible - all PRESENCE checks. None asserted the table actually rendered its columns. A spec that only checks "the controls exist" passes even when the primary content is broken. **Rule: every spec for a data view / list / table / form / response MUST assert the RENDERED OUTCOME - column bounds (R5.2), row cell values, response body fields, computed layout - not merely that elements/testids exist. Applies equally to API: asserting HTTP 200 is not asserting the body is correct (use key-allowlist + value assertions, already Stage 3a.2). Asserting a field EXISTS is not asserting its VALUE.**

2. **"Visual pass" that looked but did not measure.** The agent visited `/operations` in the browser, confirmed it "renders," and moved on - it never scrutinized whether all four columns were visible, and never measured column widths. A glance that stops at "the page loaded" is the R3 failure mode in human form. **Rule: a headed/visual verification of a data surface MUST enumerate the expected elements (every column, every row field, every action) and confirm each is visibly present and correctly laid out - via `page.evaluate` + `getBoundingClientRect` for layout, not eyeballing. If the agent cannot state "I measured X and it was Y," it has not verified.**
...
**The gate (run at the END of EVERY change - feature, fix, refactor, even a one-line spot-fix; NEVER skipped because "the change is small," since small changes are exactly where orchestrators quietly grow):**

1. **SRP** - did this change grow a class/function past a single cohesive responsibility? Flag any guard/controller/service/god-file crossing ~400 lines OR ~5 responsibilities as a god-class candidate (precedent: `SharedSecretGuard` 491 lines / ~7 responsibilities; `SchemaValidator` 1,467 lines from the May 2026 design analysis).
2. **Coupling** - did it add a dependency edge coupling two concerns that should be independent? Could an interface + DI-token seam decouple them (as the `IEndpointCredentialRepository` repository seam and the `IAssertionTokenProvider` strategy seam already do in this codebase)?
3. **Pattern consistency** - does it follow the repo's established patterns (repository + DI token, provider/strategy via seam, guard/controller-as-thin-orchestrator, small single-purpose services) or does it DRIFT (inline something that should be a strategy, duplicate a cross-cutting concern like decision-trace emission)? Pattern drift is a finding even when behavior is correct.
4. **Open/Closed** - will the NEXT similar addition require EDITING this code or EXTENDING it? A growing `switch`/`if` cascade over a type/method is the signal to introduce a strategy seam BEFORE the next addition, not after.
5. **Simplicity counter-check (YAGNI, mandatory - this gate must not become an over-abstraction engine)** - is every proposed abstraction justified by at least TWO real implementations OR one concrete near-term one? Reject speculative generality: a seam with one impl and no second on the horizon, a policy DSL where finite typed strategies suffice, or splitting a small cohesive service is over-engineering and MUST be declined with a one-line reason.
6. **Disposition (mandatory output)** - record the outcome as exactly one of: **(a) applied** in-place in the same commit chain; **(b) scheduled** in a `docs/strategy/SELF_AUDIT_<date>.md` or backlog entry with owner + target; or **(c) accepted** with a one-line justification ("cohesive + small; splitting is speculative"). Silence is not a valid disposition.
7. **Promote** - a design smell that recurs (>= 2 sightings) OR is high-severity (an SRP/coupling defect that would compound with the roadmap) is promoted from a note to a PATTERN in [docs/strategy/ENGINEERING_LESSONS_AND_PATTERNS.md](docs/strategy/ENGINEERING_LESSONS_AND_PATTERNS.md), and when it earns enforcement, to a hard rule/gate here - same `issue -> pattern -> rule` loop as R7 and the Execution Issue RCA Ledger.

Every audit prompt and every "is this change complete?" check MUST now end with BOTH the R7 test/gate self-improvement question AND this design/architecture disposition. A change that ships without a recorded DA-gate disposition is incomplete.
...
**CRITICAL - capture timing (compaction-resilience).** The agent's working context is compacted from time to time during a long build, which summarizes away early detail (exact symptom text, the failing assertion, the root-cause mechanism). Therefore the RCA entry for an issue MUST be written **at the moment the fix is confirmed to resolve it** (the gate that was RED is now GREEN because of this change), into the per-build ledger, while the detail is still in full fidelity. Do NOT defer all RCA writing to build-end - by then the early detail may be compacted and you will reconstruct it imperfectly or miss it entirely. The first auth-build ledger proved this: an in-context-only recollection understated a recurrence count by ~16x (a ~50x lint-ceiling churn was misremembered as "3+").

**Completeness method - reconcile against the FULL session transcript, not compacted memory (CRITICAL).** Even with capture-at-fix-time, a long build crosses compaction boundaries, so before declaring a build complete the ledger MUST be reconciled by systematically scanning the **entire session transcript** (the `transcripts/<sessionId>.jsonl` debug log; `VSCODE_TARGET_SESSION_LOG` points at it when present), NOT from in-context recollection or a compaction summary alone. Long builds get summarized mid-flight, and a summary silently drops the early-step issues - so a ledger written from memory is structurally incomplete for exactly the steps furthest from the end. The required scan is: (1) an **error/signal frequency pass** across the whole transcript (`error TS`, HTTP status codes `415`/`422`/`401`, `ECONNRESET`/`ENOENT`/`Cannot find module`, `false positive`, `RED`, `rebase`, lint-ceiling bumps, `overrideProvider`, `parity`, etc.); (2) a **narration-phrase pass** for sentences where a problem was diagnosed (`root cause`, `no-op`, `silently`, `turned out`, `the culprit`, `stale`, `regressed`); (3) classify each real hit, discard self-referential search echoes, and confirm every diagnosed problem maps to a listed entry. The doc MUST carry a **Provenance / completeness note** stating it was reconciled against the full transcript, by what method, and listing any verified-and-dismissed non-issues. Origin: 2026-06-19 - the first RCA ledger was initially written from the compaction summary + in-context memory; the operator asked "did you go through the entire session history?", and the follow-up full-transcript scan corrected an under-counted recurring issue (lint bumps stated as "3+" were actually ~50). Treat "did I reconcile against the full transcript?" as a mandatory self-check before declaring an RCA ledger complete.
...
**Promote generalizable issues to the central self-improving patterns doc.** A per-build RCA ledger is feature-scoped. When a confirmed fix exposes a pattern that generalizes beyond the feature, distill it into the central living doc [docs/strategy/ENGINEERING_LESSONS_AND_PATTERNS.md](docs/strategy/ENGINEERING_LESSONS_AND_PATTERNS.md) - the cross-cutting, continually-appended, continually-reviewed knowledge base that sits between raw issue ledgers and hard rules (`issue -> pattern -> rule`). That doc MUST be consulted at the START of planning / design / architecture / execution of any non-trivial change (it carries a consult-at-stage matrix), and a pattern that recurs (>= 2 escapes) or has a single high-severity escape MUST be promoted from a pattern there to a hard rule in this file in the same commit chain.
...
**F4 is the mechanized form of Google's ["Update Docs with Code"](https://google.github.io/styleguide/docguide/best_practices.html) rule** - change the documentation in the same change as the code, not later - and it is **self-extending**: binding a doc to a source prefix makes every future change under that prefix demand a documentation review with no edit to the gate. **F5 exists because the checker's own inputs can rot**: the first draft of the manifest had 4 source paths that did not exist, and a prefix that matches nothing makes F4 silently dead - green because it can never fire, the same failure shape as `live-test.ps1` having no exit statement.

**Origin: 2026-07-31.** The operator reported the user-facing docs were "missing a lot of things or not updated as per the latest 0.55.1 deployment." Measured: **12 of 18 user-facing docs still advertised v0.53.0** (one said 0.40.0), only 2 carried any provenance date, and 3 links were dead - with every test, gate and deployment green throughout. Five distinct causes, each now closed:

| # | Cause | Why nothing caught it | Closed by |
|---|---|---|---|
| 1 | **No gate covered the user-facing set.** The repo owned exactly one documentation gate (1.11) and it watched a single file. | Everything else relied on the Stage 3c.2 `auditAndUpdateDocs` prompt, which is operator-invoked and therefore skipped under delivery pressure. | F1-F3, manifest |
| 2 | **Version bumps had no doc coupling.** Stage 6.1 bumps `package.json`; nothing obliged doc headers to follow. | Each release silently widened the gap. 0.53.0 -> 0.55.1 is two minors of accumulated drift. | F1 |
| 3 | **Docs were verified once and trusted forever.** "Source-verified against v0.53.0" was true when written. | **Provenance with no expiry is indistinguishable from a stale claim.** Nothing re-checked it. | F2 + `maxAgeDays` |
| 4 | **Docs are not executable, so a wrong doc produces no red.** | Every other artifact in the repo has a test; prose has none. A doc can be entirely wrong and every gate stays green. | the gate as a whole |
| 5 | **Feature work documented itself in NEW internal docs, never in the EXISTING user-facing ones.** The whole auth-diagnostics capability (decision traces, the assertion debugger, per-trust verify, the 26-code reason catalog) shipped with thorough design docs under `docs/auth/` while `AUTHENTICATION_GUIDE.md` still said "four methods" and stopped. | Adding a doc feels like documenting. Nothing asked "which USER-FACING doc must also change?" | F4 coupling |

**Cause 5 is the one that produces "missing a lot of things"** and it is the one to watch: a feature can be exhaustively documented for its authors and invisible to its users at the same time. When shipping a user-visible capability, name the user-facing doc it belongs in, or state explicitly that none applies.

**Generated tables beat hand-written ones.** The reason-code table in [docs/AUTHENTICATION_GUIDE.md](docs/AUTHENTICATION_GUIDE.md) is derived from [auth-reason-catalog.ts](api/src/oauth/auth-reason-catalog.ts) rather than transcribed, so it cannot drift. Prefer deriving a doc table from its source of truth over copying it; a copy is a fact with a half-life.

**Scope extension (2026-09-02): the AGENT-facing context doc is gated too.** [docs/CONTEXT_INSTRUCTIONS.md](docs/CONTEXT_INSTRUCTIONS.md) advertised **v0.55.8 for nine patch releases** (through 0.55.17) and had not been touched in over a month, even though Stage 6.3 mandates updating it on every release. It escaped for the same structural reason as cause 1 above: the mandate lived only in a **prompt nobody is obliged to run**, and the manifest deliberately scoped itself to *user-facing* docs. It is now the one intentional exception in [docs/.doc-manifest.json](docs/.doc-manifest.json), with `sources: []` so it is gated for currency (F1/F2/F3) without being coupled to a churning code path. **Bind it to `api/package.json` and F4 would fire on every patch bump - a gate that blocks every release gets deleted rather than obeyed**, which is the same trap the script's own F1 claim-handling comment warns about. Proven in both directions before being trusted: reverting the header to 0.55.8 fails with `[F1] header advertises 0.55.8 but the product is 0.55.17`, and `-Fix` restamps it. **The general rule: a doc whose wrongness misleads the START of an investigation deserves a gate regardless of who its audience is** - a confidently wrong context file is worse than a missing one, which is the same lesson as rule 1.11.

**Use an empty `sources` array rather than inventing a prefix.** A prefix that matches nothing makes F4 silently dead - green because it can never fire. F5 exists to catch that, so do not give it something to catch.
...
3b.3. **`endpointConfigFlagAudit` prompt** - Verifies the 14-boolean-flag + logLevel system stays architecturally complete (registry + default + validator + enforcement + tests at every layer + doc + UI Switch + UI test = 10 cells per flag). Run when adding/modifying any flag. **Both directions are gated, in two separate specs (added 2026-08-26).** [endpoint-config-conformance.spec.ts](api/src/modules/endpoint/endpoint-config-conformance.spec.ts) F-T4 asks *does the UI offer a key the server does not know?*; [endpoint-config-ui-coverage.spec.ts](api/src/modules/endpoint/endpoint-config-ui-coverage.spec.ts) U-T1 asks the reverse, *is every registered flag reachable by an operator?* **Eleven flags had drifted into the second state** - registered, bounded, validated and enforced, yet absent from the UI - precisely because only the first direction was ever checked. They are kept separate because the failures mean different things: F-T4 means the UI misleads the operator about what they control; U-T1 means a control exists that the operator cannot find. **A registry-to-UI check must match a DECLARED control (`key: '<flag>'`), never a bare substring and never free prose.** Both mistakes were made and both were caught only by the negative control: `includes()` matched `MaxActiveWifTrustsREMOVED` while looking for `MaxActiveWifTrusts` (so a removal was undetectable), and a word-boundary scan across all of `web/src` then matched the flag name inside its own `description:` string (so documentation counted as coverage). Genuinely bespoke controls are declared in `BESPOKE_UI` with the reason and location, and U-T2/U-T3 stop that allowlist outliving the flag it excuses.
...
6.1. **Version bump** - `api/package.json` + `web/package.json` + lockfiles regenerated **in CI or on a non-corp-managed machine**, never locally on a corp device. **CORRECTED 2026-07-30:** this rule previously required regeneration *inside* `node:24-alpine`, which **cannot work on a Microsoft corp-managed device** (containers do not inherit the host `~/.npmrc`, and the public registry is egress-blocked). But the host is not a safe substitute either: a measured local `npm install --package-lock-only` rewrote a `resolved` URL to an internal feed **and downgraded that entry's `integrity` from sha512 to sha1**. Before staging ANY lockfile change, verify `resolved` hosts are only `registry.npmjs.org` and `integrity` values are only `sha512`; **revert** a contaminated entry rather than patching it. See [docs/strategy/NPM_SUPPLY_CHAIN_QUARANTINE_POLICY.md](docs/strategy/NPM_SUPPLY_CHAIN_QUARANTINE_POLICY.md).
...
| Base-image support status (EOL runtime with no CVE yet) | 1.10 | **ACTIVE** via [scripts/audit-base-images.ps1](scripts/audit-base-images.ps1). Complements Trivy, which cannot see support status. |
| Deployed-artifact runtime support status (prod still on an EOL runtime after the source is fixed) | 1.11 (`-Live`) | **ACTIVE** via [scripts/audit-deployment-doc.ps1](scripts/audit-deployment-doc.ps1) check C4. Gates what is RUNNING, not what is built. |
| Infra documentation currency (silently stale deployment facts) | 1.11 | **ACTIVE** via [scripts/audit-deployment-doc.ps1](scripts/audit-deployment-doc.ps1) checks C1/C2/C3. |
...
| Forwarded-header trust (`trust proxy: true`) | 3b.4 | **KNOWN-OPEN, do not use a forwarded header as a security input.** [api/src/main.ts](api/src/main.ts) sets `app.set('trust proxy', true)`, so EVERY `X-Forwarded-*` header from ANY source is trusted. That is fine for its stated purpose (`req.protocol` / `req.hostname` / `req.ip` when behind Azure Container Apps ingress) and NOT fine as the basis of any authorization or policy decision - an unauthenticated caller can set those headers freely. Surfaced 2026-07-29 by [docs/tls/PER_ENDPOINT_TLS_POLICY_OPTIONS.md](docs/tls/PER_ENDPOINT_TLS_POLICY_OPTIONS.md) §7 while designing a per-endpoint TLS floor: the control would have been decoration, because `X-Forwarded-TLS-Version: TLSv1.3` is caller-supplied. **Standing rule: any future feature that reads a forwarded header as a security input MUST first require an explicit trusted-terminator allowlist (peer IP checked), default the control to OFF, and report itself UNAVAILABLE when no trustworthy signal exists - never fall back to trusting the header.** A control that cannot be evaluated must never look like a control that passed. |
| Non-uuid input reaching a `@db.Uuid` column | 3a.3 + 2.5 | **ACTIVE** via [api/src/modules/logging/storable-uuid.ts](api/src/modules/logging/storable-uuid.ts) (`requestId` + `endpointId`) and `isUuid` guards in [logging.service.ts](api/src/modules/logging/logging.service.ts). Two escapes (v0.54.85 `requestId`, v0.54.89 `endpointId`) - see the standing rule below. |
...
- **Stage 1.10 base-image LTS audit** (2026-07-29 EOL-runtime escape) - the shipped image ran EOL Node 25 for ~2 months while every gate stayed green, because CVE scanning cannot detect support status and the drift was "resolved" against the wrong Dockerfile. Added `scripts/audit-base-images.ps1` (proven with a negative control before being trusted).
- **Stage 1.11 deployment infra doc audit** (2026-07-29, same escape, documentation half) - the repo had no gate on documentation currency at all, so the canonical infra facts could drift from reality without anything failing. `scripts/audit-deployment-doc.ps1` binds [docs/DEPLOYMENT_INFRASTRUCTURE_AND_FORM_FACTORS.md](docs/DEPLOYMENT_INFRASTRUCTURE_AND_FORM_FACTORS.md) to the infrastructure it describes, and its C3 check is **self-extending** (new infra elements automatically become required documentation). Its C4 check closes the deployed-artifact gap that let both prods keep serving EOL Node after the source was fixed.
- **Stage 1.12 user-facing doc freshness audit** (2026-07-31 operator-surfaced escape) - 12 of 18 user-facing docs still advertised v0.53.0 while 0.55.1 shipped, because 1.11 gated exactly one file and everything else depended on a discretionary prompt. `scripts/audit-doc-freshness.ps1` + [docs/.doc-manifest.json](docs/.doc-manifest.json) generalize it to the whole set and mechanize "Update Docs with Code" as the F4 coupling check. Its F5 check guards the checker's own inputs after the first manifest draft shipped 4 non-existent source paths, which would have made F4 silently dead.
...
- **Migration to distroless or rootless base image** - currently node:24-alpine running as the `scim` user UID 1001 (rootless already); evaluate distroless Node image as a further hardening step (smaller attack surface, no shell). Cost > value at current scale; harder live-debug.

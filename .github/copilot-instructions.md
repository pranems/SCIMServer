...
**CRITICAL - reconcile against the full session transcript at build end.** Even with capture-at-fix-time, a long build crosses compaction boundaries. Before declaring a build complete, the agent MUST reconcile the per-build ledger against the **full session transcript** (`…/GitHub.copilot-chat/transcripts/<sessionId>.jsonl`, with a debug log alongside; `VSCODE_TARGET_SESSION_LOG` points at it when present), not in-context recollection alone. Scan it for issue signals (`error TS`, `415`/`422`/`401`, `ECONNRESET`/`ENOENT`/`Cannot find module`, `RED`, `false positive`, `root cause`, `no-op`, `silently`, lint-ceiling bumps, `parity`). Every diagnosed problem MUST map to a ledger entry; add any that do not. Record the reconciliation method + result in the ledger's provenance note so a future reader knows it is transcript-verified, not memory-only.

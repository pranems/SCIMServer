...
(e.g., `G8B_*.md`, `G8C_*.md`, `G8E_*.md`, `G8F_*.md`, `G8G_*.md`, `PHASE_09_*.md`, `phases/*.md`)
...
3. **Flag count consistency**: All docs that reference "11 boolean flags" or list flag names are in sync with `endpoint-config.interface.ts`.
...
3. **New config flags**: If the flag count changed from 11, update all references in this prompt.

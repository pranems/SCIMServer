...
- **v0.55.31 - Back restores complete operator context.** One history-first Back primitive now restores the exact prior URL when an in-app entry exists and uses a caller-owned safe fallback for direct deep links. Endpoint detail returns to filtered endpoint lists; endpoint Edit Save/Cancel restore the originating tab, pagination, filters, and open drawer.
  ...
  - **Local consolidation.** Web coverage **1,534/1,534** across **115** files at **85.46% lines / 74.93% branches / 75.32% functions / 82.64% statements**; focused Back and URL-state Vitest **155/155**; Playwright **3/3**; search schemas **23/23**; live SCIM **1,516/1,516** against dev. Web production build, all route budgets, zero touched-file diagnostics, docs freshness/content, and Mermaid **713/713** in both themes pass. PR, merge, and dev deployment remain pending. Canary and customer prod are unchanged.

...
- **v0.55.32 - WIF/JWKS settings display runtime truth.** The endpoint Connect WIF panel shows all 11 effective egress values with configured override, source, unit, runtime bounds, and requested-vs-effective clamping. The API read model is derived from the same resolvers used by runtime enforcement and exposes no secret-bearing config.
  ...
  - **Review hardening and validation.** Independent review found that URI-only JWKS cache identity let a lenient endpoint seed keys reused by a stricter endpoint. Cache, single-flight, stale, refresh, and unknown-kid state are now partitioned by URI plus the complete effective policy; cross-endpoint TTL, key-cap, and refresh tests are RED before and GREEN after. API unit **5,166/5,166** across 174 suites; API E2E **1,529/1,529** across 97 suites; web coverage **1,538/1,538** across 115 files; focused API **160/160** plus JWKS/provider **80/80**; focused web **99/99**; Playwright **1/1** with measured 900 px containment; full local live **1,524/1,524**, including effective-policy T1-T8; all six modes pass. PR, merge, and dev deployment remain pending. Canary and customer prod are unchanged.

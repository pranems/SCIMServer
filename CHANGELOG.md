...
- **v0.55.24 - endpoint settings and credential controls use progressive disclosure without duplicating ownership.** User and Group tabs retain only their resource-specific behavior controls, while common validation, coercion, primary-enforcement, general PATCH, and ETag settings remain in the complete Endpoint Settings inventory.
  ...
  - **TDD and consolidation validation.** RED-first tests proved the absent disclosure, duplicated User/Group ownership, flat-vs-authoritative auth mismatch, WIF target-client/subject conflation, crowded action regions, silent card failures, and pre-deploy browser layout. GREEN: API unit **4,924/4,924 across 173 suites**, API E2E **1,522/1,522 across 96 suites**, web Vitest **1,306/1,306 across 105 files**, changed web **109/109**, connection-info E2E **9/9**, overview E2E **6/6**, local live **1,487/1,487**, focused local Playwright **30/30**, API + web production builds, all **23** size budgets, endpoint-config conformance, docs content/freshness, and Mermaid grammar/render. Merged-master dev evidence is recorded when complete.

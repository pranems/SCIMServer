...
- **v0.55.35 - vulnerable transitive Hono pins remediated.** The API override map moves `@hono/node-server` from 1.19.10 to 1.19.15 and `hono` from 4.12.25 to 4.13.5, clearing all 12 medium/low findings reported by issue #144. Both targets were published more than seven days before adoption. The packages are reachable only through `prisma -> @prisma/dev`, but they ship in the runtime image because startup executes `prisma migrate deploy`, so the frozen vulnerable versions were not dismissed as dev-only. The pin checker moved from **12 findings / exit 1** to **0 findings / exit 0** with zero unchecked packages. Lockfiles are regenerated only by the public-runner `regen-lockfile` workflow and must preserve public npm hosts plus SHA-512 integrity. No application behavior or public contract changes.
  ...
  - **Dev and canary deployment complete.** The immutable v0.55.35 digest `sha256:ad8594e2e2b7263e88dc4ae813436a20797cbea30b78457340fdb35dfe7eedfc` serves dev and canary. Dev passed live **1,532/1,532**, full Playwright, and endpoint integrity **60 -> 60**. Canary true blue/green passed green and post-flip live **1,532/1,532** plus post-flip Playwright **223 passed / 5 skipped / 0 failed**. Before, green, and post-flip inventories contain the same **62** endpoint IDs with identical set hash `1fda0504f1809fd963d6e72337866691d285ed28228e006dcf82a4fe767e694b`. Revision `scimserver--green-0925-1317` serves 100%; v0.55.23 remains at 0% for instant rollback; revision hygiene retains exactly two. Customer production remains on v0.55.20 and requires explicit approval.
- **v0.55.34 - global history and intent-based workflow context.** The app header now owns Back and Forward controls for every route and URL-owned page, tab, subtab, filter, comparison, and drawer transition. Forward availability tracks TanStack's in-app history index and resets when a new navigation branch replaces forward entries. At narrow widths the sidebar becomes an icon rail, preserving a measured 342px main area at a 390px viewport.
  - **Workflow corrections.** Logs **Errors only** accepts native router booleans and URL text booleans, eliminating the reported `hasError` `invalid_value`. Self-service `/Me` now preflights for a JWT `sub`, sends no doomed request under the shared admin secret, distinguishes an unmatched subject from generic PATCH `noTarget`, and links to OAuth setup. Operations is labeled as current cross-endpoint inventory; Logs is labeled as recorded request history; each links to the other. Manual Provision is explicitly the choose-target-first workspace and hands successful creates into endpoint-local resource views.
  - **Endpoint context by intent.** Action-target selectors show display name, stable name, and Active/Inactive state; Logs keeps a compact filter; Discovery retains comparison cards; Operations and Logs rows show endpoint display names as provenance; endpoint-local routes do not repeat a selector.
  - **Intended visual baseline update.** The Manual Provision expected/actual/diff triplet was inspected after dev deployment. The change is intentional: global history controls, Self-service `/Me` naming, workflow-purpose copy, endpoint guidance, and the ownership-oriented placeholder are the v0.55.34 UX. No unrelated overlap, clipping, missing content, or layout shift is present.
  - **Validation and deployment.** Full web **1,567/1,567** across **118** files at **85.39% lines / 75.35% branches / 75.22% functions / 82.66% statements**; focused review closure **78/78** plus filter-security **29/29**; final dev Playwright **246 passed / 5 skipped / 0 failed**; dev live HTTP **1,532/1,532** with zero leaked endpoints; all six persistence/web modes, API/web builds, API lint, route budgets, docs, Mermaid **716/716**, and exact-tip CodeQL pass. PR #179 merged as `b58af31d`; dev revision `scimserver-dev--vb58af31d` serves v0.55.34 at 100% with image content digest `sha256:2a0950aee2f42363f4341e21de1dfd122dde267acde5375116c025d507457fda`. Endpoint integrity is **60 -> 60**, revision hygiene retains two active revisions, and canary/customer prod are unchanged. The optional local ACR mirror was repaired post-deploy and reran green with immutable local digest `sha256:caf2ade4b800cb6f8b1eda49caaaa9aceeaacb85896f76fa2c713ee2038b311b`. Operator quick visual verification was received on 2026-09-25 for the changed navigation and workflow surfaces. See [docs/GLOBAL_NAVIGATION_AND_WORKFLOW_CONTEXT.md](docs/GLOBAL_NAVIGATION_AND_WORKFLOW_CONTEXT.md) and [docs/UX_NAVIGATION_AND_WORKFLOW_EXECUTION_RCA.md](docs/UX_NAVIGATION_AND_WORKFLOW_EXECUTION_RCA.md).
- **v0.55.33 - API outputs now reflect runtime truth across casing, discovery, Settings, and authenticated secret exports.** Endpoint UUID/name segments, built-in Bulk paths, custom ResourceType identifiers and endpoints, Schema URNs, and SCIM PATCH attribute/valuePath segments resolve case-insensitively while responses preserve canonical casing. Endpoint-name case-only duplicates are rejected by an additive unique index on `lower(name)`. Mixed-case extension URNs cannot bypass `returned:never`, and User/Group/custom responses recursively suppress internal persistence fields.
  - **Discovery and Settings fidelity.** `ServiceProviderConfig.authenticationSchemes` uses effective authentication enablement, including dedicated OAuth client and WIF settings. Endpoint Settings consumes the authoritative 11-field egress-policy read model and displays actual effective values, configured/inherited state, unit, source, bounds, and clamping; JSON export contains effective values instead of blank placeholders.
  - **Authenticated secrets with a hard public/durable boundary.** Recoverable bearer and OAuth client secrets render automatically and are included in credential, method, endpoint, clipboard, and download JSON on authenticated admin surfaces. WIF trusts remain secretless. New endpoint/server writes using retired `CredentialSecretVisibility=once` return `400`; previously purged values are marked rotation-required. Public discovery, endpoint overview/list projections, RequestLog persistence, Workbench history, and structured logs remain plaintext-secret-free. Durable logs always redact regardless of legacy `PersistRequestSecrets` values.
  - **Back behavior verified.** TanStack `useCanGoBack()` uses its in-app `__TSR_index`, not browser history length. External-origin/direct-link loads therefore use the explicit fallback; in-app navigation restores the complete prior URL. A regression test locks this framework contract without changing correct production code.
  - **Validation and deployment.** API unit **5,192/5,192** across **174** suites; API E2E **1,533/1,533** across **97** suites; web Vitest **1,544/1,544** across **116** files; local and dev live HTTP **1,532/1,532** with zero leaked endpoints; all four executable persistence/web lanes pass, while the two Prisma lanes were skipped because `DATABASE_URL` was unavailable; final dev Playwright **241 passed / 4 intentionally skipped / 0 failed**; API lint **0 errors**; API/web builds, route budgets, migration lint, docs, and Mermaid gates pass. Exact-tip CI, both image builds, and CodeQL are green. PR #177 merged as `6565dc60`; dev revision `scimserver-dev--v6565dc60` serves v0.55.33 at 100% with image content digest `sha256:0ff03ce18ce5f01c944776cb09ee4e12a29f6b6af6371449582424921a8b6f53`. Endpoint integrity is **60 -> 60** with zero missing IDs, and revision hygiene retains two active revisions. See [docs/API_OUTPUT_FIDELITY_AND_SECRET_SURFACE_CONTRACT.md](docs/API_OUTPUT_FIDELITY_AND_SECRET_SURFACE_CONTRACT.md) and [docs/API_OUTPUT_FIDELITY_EXECUTION_RCA.md](docs/API_OUTPUT_FIDELITY_EXECUTION_RCA.md). Canary and customer prod are unchanged.
- **v0.55.32 - WIF/JWKS settings display runtime truth.** The endpoint Connect WIF panel shows all 11 effective egress values with configured override, source, unit, runtime bounds, and requested-vs-effective clamping. The API read model is derived from the same resolvers used by runtime enforcement and exposes no secret-bearing config.
  - **Transactional editing.** Edit opens a draft; Save sends only changed overrides; Cancel discards changes; Reset to inherit removes one stored override only after Save. Endpoint PATCH accepts `null` as explicit per-key removal while preserving sibling settings and every other profile section.
  - **Review hardening and deployment.** Independent review found that URI-only JWKS cache identity let a lenient endpoint seed keys reused by a stricter endpoint. Cache, single-flight, stale, refresh, unknown-kid, and startup prewarm state are now partitioned by URI plus the complete effective policy; redirect memory is bounded to 1,000 LRU entries and zero TTL expires immediately. Cross-endpoint TTL, key-cap, refresh, prewarm, redirect-bound, and same-millisecond zero-TTL tests are RED before and GREEN after. API unit **5,170/5,170** across 174 suites; API E2E **1,529/1,529** across 97 suites; web coverage **1,538/1,538** across 115 files; focused API **160/160** plus final JWKS/provider/wiring **90/90**; focused web **99/99**; dev live **1,524/1,524**, including effective-policy T1-T8; final dev Playwright **242 passed / 4 intentionally skipped / 0 failed**; all six modes pass. PR #175 merged as `566ebd45`; dev revision `scimserver-dev--v566ebd45` serves v0.55.32 at 100% with image content digest `sha256:1e64ef0dc9ebcea7ee8d2c5e0b2bc9acccf360efc0817522dc2fd97f46720e42`. Endpoint integrity is **60 -> 60** with zero missing IDs, and revision hygiene retains two revisions. Canary and customer prod are unchanged.
- **v0.55.31 - Back restores complete operator context.** One history-first Back primitive now restores the exact prior URL when an in-app entry exists and uses a caller-owned safe fallback for direct deep links. Endpoint detail returns to filtered endpoint lists; endpoint Edit Save/Cancel restore the originating tab, pagination, filters, and open drawer.
  - **Workflow state is shareable.** Connect method, Operations subtab/search/active/pages, and Discovery primary/compare/secondary/subtab state now use typed URL schemas instead of component-local state. Browser Back, Forward, refresh, and copied links preserve them.
  - **Validation and deployment.** Web coverage **1,534/1,534** across **115** files at **85.46% lines / 74.93% branches / 75.32% functions / 82.64% statements**; focused Back and URL-state Vitest **155/155**; search schemas **23/23**; live SCIM **1,516/1,516**; final dev Playwright **241 passed / 4 intentionally skipped / 0 failed**. The inspected Users-tab visual change is intended: the old fixed link is now the shared Back control. Positional live-data masks were replaced with semantic selectors, and explicit detail/Users readiness assertions reject a shell-only baseline by a measured 29% diff. PR #173 merged as `574d9135`; dev revision `scimserver-dev--v574d9135` serves v0.55.31 at 100% with image content digest `sha256:580c80a5de591b0097e24925f368261ff97408d52b4c8a330d22d20860c78eea`. Endpoint integrity is **60 -> 60** with zero missing IDs, and revision hygiene retains two revisions. Canary and customer prod are unchanged.
- **v0.55.30 - credential and WIF trust lifecycle is explicit and durable.** Connect now uses an explicit deactivate route while preserving legacy DELETE soft-deactivation, offers Activate for inactive rows, and exposes confirmed permanent deletion only after deactivation. Bearer, OAuth, and WIF cards share the same lifecycle language; Rotate remains active-only and secretless WIF trusts never offer it.
  - **Atomic rotation.** Prisma deactivation and replacement creation run in one transaction, with synchronous InMemory parity. Replacement failure rolls back deactivation; inactive or raced sources return `409`. OAuth keeps its public `client_id`, and old/new mint outcomes are verified on the wire.
  - **Verified purge.** Hard delete returns a structural success result instead of swallowing persistence failures. The inactive predicate is enforced at the delete sink, so active purge and an activate-vs-purge race return `409`; WIF purge invalidates the trust cache.
  - **Validation and deployment.** API unit **5,159/5,159** across **174** suites; API E2E **1,528/1,528** across **96** suites; focused repository/controller **98/98**; web coverage **1,525/1,525** across **114** files at **85.38% lines / 74.87% branches / 75.27% functions / 82.54% statements**; focused web **163/163**; full local and dev live **1,516/1,516**, including lifecycle T1-T13; final dev Playwright **238 passed / 4 intentionally skipped / 0 failed**; all six persistence/web modes pass. API/web builds, API lint with zero errors, route budgets, docs freshness/content, and Mermaid **712/712** in both themes pass. PR #171 merged as `524906d0`; dev revision `scimserver-dev--v524906d0` serves v0.55.30 at 100% with digest `sha256:eef89a068f2e915ec4b6ce180b213dc2b26fc31c860f9ae02e634f6a39217a66`. Endpoint integrity is **60 -> 60** with zero missing IDs, and revision hygiene retains two revisions. Canary and customer prod are unchanged.
- **v0.55.29 - custom ResourceTypes have first-class Activity and Logs observability.** Endpoint-scoped custom POST, PUT/PATCH, GET, and DELETE requests are classified as `type: resource` with `resourceType`, `resourceEndpoint`, and resource identifier. Prisma Activity queries include custom endpoint paths and push User/Group/resource plus error filters before pagination. Generic PUT/PATCH now emits `scim.resource.updated`, forwarded through SSE so custom lists, Activity, and Logs refresh after edits.
  - **One Logs filter contract.** Global Logs and endpoint Logs share a single toolbar and URL model for URL substring, method, status, time range, errors-only, minimum duration, and request ID. Global Logs adds endpoint selection; endpoint Logs enforces its route endpoint. Query keys include every active filter without breaking legacy cache keys.
  - **Review hardening.** Derived Activity type/severity filters now run before pagination in both backends, and custom names come from the endpoint profile rather than URL singularization. Relative time keys stay stable, filters remain available in empty states, global Logs exposes one Reset command, and narrow Auth chips own a measured single-line ellipsis contract. Docker build contexts explicitly re-include the intentional production `logs` source directory; a new Fast pre-push audit rejects any populated production source directory shadowed by a broad artifact rule without matching exceptions.
  - **Validation and deployment.** API unit **5,150/5,150** across **174** suites; API E2E **1,525/1,525** across **96** suites; affected API/event slice **144/144**; web Vitest/coverage **1,521/1,521** across **114** files at **85.32% lines / 74.79% branches / 75.13% functions / 82.49% statements**; affected web **103/103**; exact-branch browser lifecycle **1/1**; full local and dev live **1,507/1,507**, including custom-resource **T1-T13 + cleanup**; final dev Playwright **238 passed / 2 intentionally skipped / 0 failed**. API/web builds, API lint with zero errors, unit-2 TypeScript diagnostics, and all **25** route budgets pass. A registry-free scratch Docker build copied the shared toolbar from the real context. Populated 1002 px browser review measured no page overflow, exactly one Reset command, and Auth chip `scrollHeight == clientHeight`. PR #167 merged the feature, PR #168 merged the deployment-pipeline unblock, and PR #169 merged the final browser-contract hardening. Dev serves **v0.55.29** at revision `scimserver-dev--v5ccafc7c` with 100% traffic; endpoint integrity is **60 -> 60** with zero missing IDs, and revision hygiene retains two revisions. Canary and customer prod are unchanged.
- **v0.55.28 - endpoint resource UI follows the live profile end to end.** User, Group, custom ResourceType, and Manual Provision create flows now share one editable request-body surface synchronized with profile-generated controls in both directions. Valid operator-authored JSON members are preserved; malformed or non-object JSON blocks submission. A dropdown renders only for a non-empty published `canonicalValues` list.
  - **Runtime discovery stays coherent.** Endpoint profile writes and endpoint-update SSE events invalidate detail, Schemas, ResourceTypes, ServiceProviderConfig, and custom-resource caches. Removed User, Group, or custom ResourceType routes redirect to Resource types instead of leaving a stale panel mounted. Overview now says **Custom Resources**.
  - **Endpoint Service Provider Config.** A dedicated endpoint tab presents capability support, filter and bulk limits, authentication schemes, documentation links, and the complete copyable endpoint-scoped discovery JSON. Its lazy route is budgeted at **1.47 kB gzipped / 110 kB**.
  - **AI governance reference fixture.** `PRTest-Auth-Methods-ISV-1` gains a project-defined `AIAgent` ResourceType with stable identity, lifecycle/risk canonical values, provider/model/purpose, multi-valued capabilities, owner reference, active state, and review timestamp. Device `platform` is now an unconstrained string, matching the text-control acceptance case.
  - **Validation to date.** Full web Vitest/coverage **1,514/1,514** across 113 files at **85.14% lines / 74.34% branches / 74.87% functions / 82.24% statements**; focused web **149/149** and review-fix **125/125**; profile browser workflow **1/1**, including AIAgent control mapping; SPC click/deep-link **2/2**; fixture parser **0 errors** and self-test **16/16**; production build and all **25** route budgets pass. Full live, PR, merge, and dev deployment evidence remain pending. Canary and customer prod are unchanged.
- **v0.55.27 - Workbench ships executable server, admin, endpoint, and profile-aware SCIM examples.** A new Examples panel applies complete request drafts without sending: static health/admin requests work immediately; endpoint selection adds endpoint detail/overview/settings/discovery requests; ResourceType selection derives a profile-valid create POST and an existing-resource PATCH with concrete id and returned `If-Match`.
  - **One request contract.** Applying an example updates method, path, body, and template-specific headers. Enabled header rows now reach the network; one shared sanitizer trims names, drops empty names, and removes every editable Authorization variant before the stored admin bearer is applied. Endpoint names and unique string schema fields receive run-specific values so create examples remain runnable on shared estates.
  - **Deployment-time reference fixture.** `configure-profile-resource-forms-dev-fixture.ps1` additively configures Device plus User/Group provisioning extensions on `PRTest-Auth-Methods-ISV-1`, with active-estate resolution, canonical-id check, ETag-protected PATCH, idempotent owned-definition merge, `-WhatIf`, and discovery verification. It runs only after the merged image is deployed to dev.
  - **Validation.** Web Vitest + coverage **1,505/1,505** across 112 files; coverage **85.34% lines / 74.56% branches / 74.9% functions / 82.49% statements**; focused Workbench/profile/history/snippet **37 targeted tests** plus serializer and shared auth-header coverage; local Playwright **1/1** executed health, admin POST, endpoint PATCH, custom Device PATCH and stale-ETag rejection with cleanup; exact local and post-fixture dev live SCIM **1,500/1,500** each; fixture PowerShell AST parse **0 errors**, pure identity/idempotency/canonical-hash self-test **8/8**, disposable first-apply plus rerun **34/34**, and named dev first-apply plus rerun **34/34** discovery/preservation/convergence assertions. The named-dev proof executed the clean committed script at **`565aa19776fe03333c7df5a4940c84bd513b2f26`** twice. Final dev Playwright **233 passed / 4 intentionally skipped / 0 failed**; API/web production builds and all 24 size budgets pass, with Workbench at **9.74 kB gzipped / 110 kB**; Mermaid parse/render **709/709** in both themes; touched-file TypeScript diagnostics **0**. PR #162 merged, v0.55.27 is deployed to dev, and PR #163 merged the inspected intended Manual Provision baseline update. Canary and customer prod are unchanged.
- **v0.55.26 - resource create and edit workflows are generated from each endpoint profile.** Users and Groups now expose Create actions in populated and empty states, and their detail drawer edits every writable core and extension field published by that endpoint. Custom ResourceTypes appear as first-class endpoint tabs with list/create/edit/delete workflows. Manual Provision uses the same generated form for every discovered ResourceType, and Resource Types shows the effective combined core plus extension schema.
  - **One schema-to-form contract.** `profile-resource-shape.ts` resolves core plus extension fields, excludes read-only attributes from editing, generates executable examples, nests extension values under their URN, and emits changed-field-only SCIM PATCH operations. Text, number, boolean, canonical-value, complex, and multi-valued fields share one renderer and one copyable request preview.
  - **Generic ResourceType API hooks.** A normalized resource endpoint adapter supports arbitrary list, POST, PATCH with `If-Match`, and DELETE operations with one cache namespace. Built-in hooks remain compatible.
  - **Generic ETags now round-trip.** Custom resources previously returned `W/"N"` while write enforcement compared against `W/"vN"`, so an immediate edit falsely returned `412`. Generic responses now emit the canonical version form used by Users, Groups, and `enforceIfMatch()`.
  - **Validation.** API unit **5,140/5,140** across 174 suites; API E2E **1,524/1,524** across 96 suites; generic service **68/68**; web Vitest **1,481/1,481** across 110 files; focused web consolidation **178/178**; local profile-resource Playwright **1/1**, including custom-resource 204 delete; full local live SCIM **1,500/1,500**, including active `9z-CN.T1-T6` and endpoint cleanup; API/web production builds; all route size budgets, with `GenericResourcesTab` at **1.6 kB gzipped / 110 kB**; zero touched-file TypeScript errors; docs content/freshness and Mermaid grammar plus Chromium render **708/708** in both themes.
  - **Deployment status:** merged to master as `7e43ad0a01a3b060d814c765bec02d14a1302b03`. Dev, canary, and customer prod remain unchanged. Workbench templates and the named dev fixture remain a separate rollback unit.
- **v0.55.25 - authentication enablement has one setting per method.** The retired bearer/OAuth umbrella is removed from the public registry, profile types, runtime resolver, connection-info provenance, Settings, Connect, fixtures, live tests, OpenAPI, and current documentation. New writes using the removed key return `400` with both dedicated replacements.
  - **Existing endpoints keep their behavior.** A PostgreSQL JSONB migration copies the former combined value into each missing dedicated bearer/OAuth setting, preserves explicit dedicated overrides, then removes the old key. The same normalization runs at the endpoint profile boundary for InMemory parity and pre-migration reads. A disposable PostgreSQL 17 replay passed legacy true/false, explicit bearer/OAuth override, and unrelated-profile cases.
  - **Connect updates immediately and does not retain stale auth data.** Optimistic setting changes override only stale default/dedicated connection-info; explicit authentication-method entries remain authoritative. Credential/auth mutations, endpoint changes, and cross-tab SSE events invalidate overview, dedicated connection-info, and retained-secret caches. Endpoint visibility changes to `once` purge that endpoint's retained envelopes; server visibility changes purge globally and broadcast cache invalidation. Unit coverage proves the tab appears immediately, and Playwright holds the PATCH response open while requiring the bearer tab to render before the server replies.
  - **Internal delivery labels stay internal.** WIF discovery and credential-visibility copy use product language. A new AST-based Vitest gate scans production TypeScript/TSX literals and passes 142/142; the permanent rule is in `.github/copilot-instructions.md`.
  - **Validation.** API unit **5,140/5,140** across 174 suites; API E2E **1,524/1,524** across 96 suites; web Vitest **1,453/1,453** across 106 files; local live SCIM **1,493/1,493**; focused Connect Playwright **6/6**; migration policy **20/20**; real PostgreSQL 17 migration replay; API/web builds; 23 web size budgets; PowerShell parser; docs content/freshness; Mermaid grammar + Chromium render **706/706** in both themes. The complete local Playwright run passed 221 with five deployment-sensitive skips; the nine residuals were classified by screenshots and rerun individually: Connect, sidebar, all-tabs smoke, Command Palette, and shortcuts all pass after test-harness corrections. One Helmet SPA probe is N/A against Vite because `/` does not traverse Nest middleware, and three populated-dev visual baselines are intentionally not regenerated from the one-endpoint local estate.
  - **Deployment status:** local branch only. Dev, canary, and customer prod remain unchanged.
- **v0.55.24 - endpoint settings and credential controls use progressive disclosure without duplicating ownership.** User and Group tabs retain only their resource-specific behavior controls, while common validation, coercion, primary-enforcement, general PATCH, and ETag settings remain in the complete Endpoint Settings inventory.
  - **Canonical endpoint-profile, discovery, and authentication design.** `PORTABLE_ENDPOINT_PROFILE_AUTHENTICATION_AND_DISCOVERY_DESIGN.md` defines one portable profile boundary, separates desired/effective/configured/ready/healthy auth state, preserves credentials and server policy outside the profile, specifies discovery translation and fidelity reporting, maps APIs to database storage, and sequences profile-v2/import/provider-registry work as later rollback units. The documentation index now points to this authority, while stale inert-model, lossless-round-trip, four-section profile, old WIF identity, and historical UI claims are corrected or explicitly labeled historical.
  - **Contextual settings are collapsed but still visible.** Every related-settings pane uses an accessible Fluent Accordion whose persistent header shows the title and setting count. Users expose exactly two lifecycle controls; Groups expose exactly three membership/deletion controls; Schemas, Resource types, Logs, and method-specific Connect settings keep their existing scoped definitions behind the same disclosure.
  - **Four real authentication methods, one compatibility fallback.** Connect orders OAuth2, WIF, global shared secret, then per-endpoint bearer. `PerEndpointCredentialsEnabled` no longer appears as a fifth method and is isolated under Settings -> Legacy compatibility.
  - **Displayed auth state now follows server truth.** The central resolver reports whether an effective state came from an `authentication.methods[]` entry, a dedicated setting, the legacy fallback, or a default. Connection info exposes that non-secret `enablementSource`; Connect and Endpoint Settings derive checked state from it and disable a flat switch when a method entry is authoritative. WIF now participates in the same resolver across trust creation, diagnostics, token minting, connection info, and discovery; any enabled WIF entry enables the method, while all declared WIF entries disabled overrides `WifCredentialsEnabled=true`.
  ...
  - **Keyed-credential non-recoverability test is deterministic.** P1-X2 now parses the issued token with the production parser before asserting that the persisted row excludes the complete secret. The previous `_` split was invalid for base64url and could false-fail on a legal random secret; the corrected focused E2E suite passes 15/15.
...
- **v0.55.23 - W3.5: warm WIF token mints skip credential-table reads and unknown issuers stop before unrelated JWKS fetches.** A bounded per-endpoint cache loads only active `wif` rows through the new typed repository method, compiles an exact issuer map, coalesces concurrent cold reads, refreshes at credential expiry, and rejects an unknown decoded issuer with `wif_issuer_mismatch` before validator/network work.
  ...
  - **Typed persistence path.** `findActiveByEndpointAndType(endpointId, credentialType)` is implemented with matching active/expiry semantics in Prisma and InMemory. Migration `20260916035422_add_endpoint_credential_type_active_index` adds `(endpointId, credentialType, active)`.
  - **Migration audit found and closed pre-existing drift.** Prisma initially proposed dropping `CredentialDek_active_idx` and `RequestLog_authOutcome_idx` because both existed in migration history but not in `schema.prisma`. Their declarations and regression guards are restored; the W3.5 migration is strictly additive. A disposable PostgreSQL 17 replay applied all **20 migrations** and physically verified all three indexes.
  - **TDD and validation.** RED-first coverage includes provider typed-call selection, both repositories, warm no-DB reads, expiry, LRU capacity, stale in-flight invalidation, every mutation path, endpoint deletion, unknown-issuer zero-validator behavior, and schema constraints. API unit **4,922 / 173 suites**; API E2E **1,521 / 96 suites**; complete six-mode matrix passes including Prisma; full WIF E2E **37/37**; local live **1,484/1,484**; migration linter **20/20**; API build/lint 0 errors; web Vitest and coverage pass unchanged. Live `9z-AT.T5` asserts the early `wif_issuer_mismatch` contract, while `9z-BZ.T13/T14` assert both cache bounds. Playwright is N/A because no `web/` behavior changed.
  - **Deployment status:** local implementation and gates only. Dev, canary, and customer prod remain unchanged until the standard merged-master deployment pipeline runs. Customer prod still requires explicit operator approval.
- **v0.55.22 - endpoint settings are available where operators use them without weakening Settings as the complete inventory.** The Settings tab remains the structured all-in-one surface for all **38 server-registered endpoint settings**. Users, Groups, Schemas, Resource types, Logs, and the active Connect method now also render contextual subsets beside the behavior they control.
  - **One registry, two presentations.** `endpoint-settings-definitions.ts` owns all boolean, enum, and numeric definitions. Settings renders the complete registry plus its dedicated `CredentialSecretVisibility` radio; contextual tabs render key subsets from `TAB_SETTING_KEYS`. No default, bound, or description is duplicated across tabs.
  - **Contextual ownership.** Users owns validation/concurrency/PATCH/User lifecycle; Groups adds membership PATCH and Group lifecycle; Schemas owns discovery + strict validation; Resource types owns discovery + enforcement; Logs owns request persistence/file output/log level; Connect follows the selected bearer, OAuth-client, or WIF method with its credential caps and JWKS controls.
  - **Settings completeness is enforced.** `U-T7` compares all server registry keys against the shared Settings catalog and asserts that Settings still renders every control family plus `CredentialSecretVisibility`. Current result: **38/38**.
  - **Two touched-file TypeScript defects closed.** User/Group detail drawers now resolve a selected `ScimResource` once before rendering instead of repeating a `.find()` whose result stayed `Record | undefined`. The repository TypeScript baseline improved **96 -> 94** with zero errors in changed files.
  - **Dependency sweep closed 11 High findings without unsafe framework downgrades.** A clean GitHub-runner lockfile raises Multer to **2.3.0**, fast-uri to **4.1.3**, deepmerge-ts to **8.0.0**, and mysql2 to **3.23.1**; sql-escaper is pinned at the feed-compatible **1.5.1**. The graph installs through the managed corporate feed, retains public-registry + sha512 provenance, and `npm audit --omit=dev --audit-level=high` is GREEN at **0 Critical / 0 High** (6 Moderate / 1 Low remain).
  - **Deployment publish identity is now enforced rather than inferred.** Stage 4.3 selects the dispatched GitHub run by exact master SHA + dispatch time instead of taking the newest visible run; Stage 4.4 proves the version and `latest` tags resolve to identical local image IDs. An 8-assertion contract runs in Fast pre-push.
  - **Intended visual baseline updates:** the deployed Schemas and Users tabs now include their contextual behavior-settings panels above the existing content. Both expected/actual/diff PNG triplets were inspected before updating those two baselines; no unrelated chrome, inventory, or empty-state regression was present.
  - **TDD and coverage.** The shared-panel test was RED on the missing production module before implementation, then GREEN across boolean/enum/numeric persistence and ownership. The 3 Playwright journeys were added after local implementation and correctly fail against the pre-feature dev deployment; they remain the deployment regression gate, not the initial RED. CI-equivalent Validate is GREEN: API unit **4,905 / 171 suites**, API E2E pre-push corpus **1,461 / 93 suites**, web vitest **1,302 / 105 files**, endpoint-config **12/12**, API build/lint, web type/build, Prisma generation/migration lint, and all static docs/infra/supply-chain gates. The full six-mode matrix is GREEN: API unit on InMemory + Prisma, API E2E **1,520 / 96 suites** on InMemory + Prisma, web vitest, and coverage. Web coverage remains above every ratchet floor; all size budgets pass. Existing live section `9z-Z` already proves PATCH/deep-merge/follow-up GET/overview behavior and remains the live API layer.
  - **Deployed to dev and canary.** Image digest `sha256:7590c82e39657f8ee58372016061e7ce26b898201f195a3c194e090d6b564f61` was built from merged master. Dev: **1,483/1,483 live**, **230 Playwright passed / 2 skipped**, 60 -> 60 endpoints. Canary true blue/green: green-label and post-flip each **1,483/1,483 live** plus **205 Playwright passed / 4 skipped**, 61 endpoints intact; revision `scimserver--green-0915-2109` serves 100% and v0.55.21 remains the rollback target. Customer prod remains on v0.55.20 pending explicit visual verification/promotion approval.
- **v0.55.21 - P7: the UI could PERFORM the bcrypt-to-keyed migration but could not tell you that you needed to.** Found by reading `web/src` rather than assuming: `hashAlgo` appeared nowhere, `/scim/admin/credentials/migration-status` was not surfaced, and the create dialog sent only `{label, description, credentialType}`.
  - **You could not see which credentials were legacy.** The per-credential verifier is now on the endpoint-overview projection and rendered as a **Legacy (bcrypt)** / **Keyed** badge, plus a banner counting this endpoint's legacy rows. `isLegacyHash` treats a **missing** `hashAlgo` as legacy - rows predating the migration carry no algo, and reading absence as "already fine" would under-report exactly the rows needing work.
  - **You could not issue a second secret under an existing `client_id`.** The create dialog now takes an optional Client identifier, which is what makes a zero-downtime handover possible; blank still gets a fresh id. `rotate` remains a hard cutover and the banner says so.
  - **Connect click-path.** The four IdP connection values render up front instead of behind a toggle, and the now-redundant Connect button is removed. The **secret** still costs one deliberate click: auto-revealing on render would put every retained secret on screen by default and fire one request per card, which is a different security posture and not one to change silently. **Rotate** moves out of the overflow menu onto the card, rendered `primary` on a legacy row.
  - **Auth-method switches now live on the Connect tab**, since enabling a method is a prerequisite for everything else there. The five definitions moved to `web/src/pages/endpoint-auth-flags.ts` and **both** pages import them; a second copy is the drift the flag audit exists to catch. That move made all five read as unreachable to the `U-T1` coverage gate until its source list was updated - the gate working correctly, since a control the scan cannot see is indistinguishable from one that does not exist.
  - WIF trusts are untouched: they hold no secret, so no badge and no banner count. Verified live - 7 active WIF credentials on dev and 7 on canary, all with complete trust config.
  - Coverage: 6 new vitest (badge, missing-algo-is-legacy, banner count + disappearance, inline switches persist, clientId sent / omitted) + 1 E2E asserting the hashAlgo **value** (a present-but-wrong algo would paint a green badge over a legacy row) + 4 Playwright.
  - Gates: api tsc 0; web tsc **96** (baseline unchanged); web vitest **1,292** / 104 files (was 1,285); endpoint-config 362.
- **v0.55.20 - P6: a second `oauth_client` credential used to SHADOW the first, which made a graceful credential migration impossible.** Found while answering "can users rotate at their own convenience instead of being cut off?" - the answer was no, and the reason was a latent bug rather than a missing feature.
  - **Every `oauth_client` credential on an endpoint shares one `clientId`** (`client-id-<endpointId>`), and the token endpoint selected the candidate with `.find()` - which returns the **first** match. So only ONE credential per endpoint could ever authenticate, while the P2 caps allow **five**, and a live estate already had **two active**. The second was dead weight that looked healthy in the UI.
  - **Worse, which one won was unspecified.** `findActiveByEndpoint` has no `orderBy`, so on PostgreSQL the row order is not guaranteed - the working secret could change on its own after a table rewrite or vacuum.
  - **Fix:** a keyed secret now matches on its **own `lookupKey`** - exact, O(1), unambiguous at any number of credentials. Only legacy `client-secret-<uuid>` secrets need iteration, because they carry nothing identifying, and that loop is bounded by `MaxActiveOAuthClientCredentials` and shrinks as rotation progresses. `client_found` and `secret_match` stay distinct in the decision trace, so an operator can still tell which half failed.
  - **This unlocks the zero-downtime migration path.** Rotation is destructive - it invalidates the owner's secret the instant it runs. For an integration we do not control (a live Entra provisioning job), the humane path is to issue the new keyed secret **alongside** the old, let the owner switch when convenient, then retire the old. That overlap is now possible and tested end to end.
  - Coverage: 5 unit (`P6-T1..T5`) + 3 E2E (`P6-X1..X3`, both backends) + 4 live (`9z-CL.T15..T18`). **Proven by negative control:** the corrected tests fail 3/8 against the pre-fix `.find()` code and pass 8/8 after.
  - Gates: api unit **4,895** / 170 suites; api E2E 95 suites (inmemory + Prisma); eslint 0 errors.

- **v0.55.19 - the phase-4 report is now actionable, and using it falsified phase 5.** Two findings, both from operating the thing rather than reasoning about it.
  - **`hashAlgo` is now on the credential list.** The status report says how many legacy credentials remain; it could not say **which**. An operator reading "4 legacy" had to guess which rows to rotate, and guessing is how the wrong live credential gets rotated. Exposing the algorithm name leaks nothing (`P4-L3` asserts the hash, `secretHash` and `lookupKey` all stay out).
  - **`legacy.total === 0` is unreachable, so phase 5 as specified can never run.** Rotation deactivates the old row and creates a new one; `DELETE` is a soft-deactivate; there is no hard-delete route. Every migration path converts *active* legacy into *inactive* legacy, and the gate deliberately counts inactive rows - **so it is shut permanently by its own success criterion.** Recommended change, written up in [P1_KEYED_CREDENTIAL_LOOKUP_DESIGN.md](docs/auth/P1_KEYED_CREDENTIAL_LOOKUP_DESIGN.md) §4.5 and deliberately **not applied yet** because it is one-way: gate on `legacy.active === 0` (which rotation can achieve, and which is the honest security condition since the O(N) scan only ever iterated active rows), and make `activate` refuse to reactivate a legacy row once the verifier is gone. Strictly safer than the original plan *and* achievable, and it keeps the audit trail - hard-deleting rows to satisfy a gate would be destroying history to make a number look right.
  - **External-traffic audit before touching any credential.** Rotation invalidates the current secret, so source addresses were read from the retained request logs rather than assumed. Four of the five legacy-holding endpoints see only this machine. The fifth - `Test-AV_CustomExtension_ScalarMV` on canary - is taking **live Microsoft Entra ID provisioning traffic**: 966 of 1,000 requests from `20.190.174.40` / `40.126.18.174`, with `userName eq "AzureAD..."` filters, group syncs and `POST .../oauth/token`. **Rotating its credential would break a live integration.** Two caveats recorded: our own egress is a rotating **NAT pool** (`40.117.66.208-215`), so identifying "us" by one IP under-counts badly; and log retention is ~30 days, so "no external traffic" means none *in the retained window*.
  - Gates: api unit **4,888** / 170 suites; api E2E **1,510** / 95 suites; eslint 0 errors; doc content + freshness audits pass.

  - **The D3 retention fix proved itself in production on this very promote.** The first attempt terminated mid-verification and left an orphan green revision carrying the **same image** as the one being deployed - reproducing the D3 setup by accident. The relaunched prune skipped that duplicate and kept the 0.55.18 revision as the rollback target (`rollback target: scimserver--green-0903-0320`); the pre-D3 selector would have kept both 0.55.19 revisions and pruned the only thing worth rolling back to. The interrupted attempt also confirmed the safety property: blue kept serving 100% and the flip never happened, because a verification that never completes cannot promote.
  - Gates: api unit **4,890** / 170 suites; api E2E **1,510** / 95 suites (both backends); web vitest **1,285** / 104 files; web tsc 96 (baseline); eslint 0 errors. Live **1477 / 0** on all four form factors - local node (inmemory), Docker compose (Prisma), Azure dev, Azure canary - plus Playwright **198 passed / 4 skipped** against dev, green and post-flip.

- **v0.55.18 - P1 phase 4: the legacy credential tail is now MEASURED, and measuring it immediately corrected the measurement.** Phase 5 deletes the bcrypt verifier and cannot be undone, so it is gated on a number rather than on elapsed time - "it has been three months, probably fine" is the reasoning that produced the EOL-Node escape. New admin-only `GET /scim/admin/credentials/migration-status` reports the population split by hash algorithm, with a per-endpoint list of the endpoints still holding legacy rows.
  - **The first live run reported 32 legacy credentials on an estate where every credential had just been minted in the keyed format.** All 32 were **WIF trusts**. A WIF row stores no secret at all (`credentialHash: ''`) and sets no `hashAlgo`, so it inherits the column default of `bcrypt` and *looks* legacy - while actually being verified as a JWT against a JWKS, never touching the bcrypt path. Counted naively it is a legacy row that **can never be migrated**, so the phase-5 gate would have been shut permanently. The report classifies by credential type first (a `secretless` bucket), which fixes existing and future rows with **no data migration**. Regression tests: `P4-S9`, `P4-S10`, `P4-X9`.
  - **This is the argument for phase 4 existing at all.** A `TODO` reading "confirm the tail is empty before phase 5" would have been discharged by a number that was wrong in the *safe* direction this time, and nothing guarantees the next miscount is as harmless.
  - **Inactive rows still count, deliberately.** `POST .../credentials/:id/activate` can bring a deactivated credential back; if phase 5 had already removed the bcrypt verifier it would return and silently fail to authenticate. So the gate is `legacy.total === 0`, not `legacy.active === 0` (`P4-S2`, `P4-X4`).
  - **Unrecognised algorithms fail closed** - anything not provably `hmac-sha256-v1` and not secretless counts as legacy, so adding a third algorithm cannot silently open a one-way gate (`P4-S6`).
  - **Counts are grouped in the database**, not by loading the credential table, so the report stays cheap on a large estate. `countByHashAlgo()` is implemented in both backends and proven at parity by running the same E2E spec against inmemory **and** Prisma.
  - **Kept out of `AdminCredentialController`** - that file is already a god-class at 1,200+ lines (register item **D1**), and this is a distinct responsibility. New thin controller + small service instead. No UI surface: the report is a maintainer instrument for a one-way decision, and it becomes permanently zero after phase 5.
  - Coverage: 11 unit (`P4-S1..S11`) + 9 E2E (`P4-X1..X9`, run on **both** backends) + 9 live (`9z-CM.T1..T9`, including a negative control that the report is live rather than cached). Gates: api unit **4,885** / 170 suites; api E2E **1,510** / 95 suites; live **1,473 / 0**; eslint 0 errors.

- **v0.55.17 - P1 hybrid: `oauth_client` secrets are keyed too, without losing the readable prefix.** The v0.55.16 increment deliberately left `oauth_client` on bcrypt, because that secret is presented at the **token endpoint** rather than as a resource bearer and because the readable `client-secret-<uuid>` form was an explicit operator request. The hybrid gives up neither property: `client-secret-<24 hex lookupKey>-<43 char base64url secret>`. Both credential types are now keyed.
  - **The hex key matters more here than on the bearer format.** The separator is `-`, and the base64url alphabet **contains** `-` - a real captured secret has four of them. A hex key cannot contain `-`, so the boundary is unambiguous and the secret is simply everything after it.
  - **Hex also buys the migration its safety property for free:** a legacy `client-secret-<uuid>` can never false-match, because a UUID's longest run of hex is 12 characters and the key needs 24. If it *could* match, a legacy secret would be looked up by a bogus key, miss, and stop authenticating - a silent customer outage. `P1-H7` asserts this over **500 real UUIDs** rather than arguing it.
  - **Existing secrets keep working.** Legacy `client-secret-<uuid>` values still verify via bcrypt (`P1-X8d`, `9z-CL` T9-T14), and rotation upgrades them in place while **preserving the public `clientId`** - an integration re-reads one value, not two.
  - **Where the win actually is.** The token endpoint was *already* O(1) - it selects the candidate by `clientId` and does one compare. The amplification was on the **resource plane**, where `oauth_client` rows joined the O(N) bcrypt scan; they are now skipped once migrated.
  - **Two existing assertions pinned the old UUID shape and failed, exactly as they should have** - a controller unit test and live `9z-AX.T2`. Both were updated to pin the hybrid contract, and the unit test now also asserts the old shape does **not** come back.
  - Coverage: 9 unit (`P1-H1..H9`) + 5 E2E (`P1-X8a..X8e`) + 6 live (`9z-CL.T9..T14`). Gates: api unit **4,874** / 169 suites; api E2E **1,501** / 94 suites; live **1,464 / 0**.

- **v0.55.16 - P1: the O(N) bcrypt scan on the auth hot path is gone.** A wrong token against **10 active credentials answers in 7 ms**, measured on a live node. Before this it cost ~2.9 s (10 x 287 ms of bcrypt), and that loop was reachable by an **unauthenticated** caller on a 0.5-vCPU replica with no rate limiting - a denial-of-service amplifier, not a latency annoyance.
  - **Why a fast hash is correct here, since it reads as a downgrade.** bcrypt is deliberately slow to make brute-forcing a *human-chosen password* expensive. A per-endpoint credential is `crypto.randomBytes(32)` - **256 bits of entropy**, no dictionary, no reuse, no human pattern - so a slow KDF buys **nothing** against the only attack it was designed to stop, while costing 287 ms on every verification. The token now carries a **public** `lookupKey`, so the server does one indexed read and one constant-time HMAC comparison. This is the same conclusion GitHub, Stripe and AWS reached for API tokens.
  - **Format:** `scim_<lookupKey>_<secret>`. The `scim_` prefix is deliberate so secret scanners can recognise a leak. A server-side pepper (`CREDENTIAL_HASH_PEPPER`, same convention as `CREDENTIAL_KEK`) preserves the one property bcrypt did give us - a dump of the credentials table alone is not enough to verify tokens offline.
  - **TDD caught a real bug immediately.** The first implementation used base64url for both halves and the round-trip test failed at once: **the base64url alphabet includes `_`**, which is the separator, so a key containing `_` split in the wrong place and truncated. The lookup key is now hex, which cannot collide with the separator.
  - **Migration: existing credentials keep working, and rotation is the upgrade path.** The columns are strictly additive (`hashAlgo` defaults to `bcrypt`, which is exactly what every existing row is), legacy rows still verify via bcrypt, and `POST .../rotate` reissues in the new format - so the existing operator workflow migrates rows with no new concept to learn. The legacy scan now **skips** already-migrated rows, so the remaining cost shrinks as rotation progresses instead of staying flat.
  - **Scope, stated rather than glossed:** `bearer` only - the type actually presented as a resource-plane bearer. `oauth_client` keeps its readable `client-secret-<uuid>` form and bcrypt, because that secret is presented at the **token endpoint** and the readable form was an explicit operator request. An endpoint with `oauth_client` credentials therefore still pays for those; migrating them needs an operator decision on the format.
  - **The perf assertion carries a negative control.** 25 keyed credentials must beat **3** legacy bcrypt rows - without that, a fast result could not be attributed to the fast path. Coverage: 25 unit (token primitives) + 7 unit (authenticator, asserting *which* repository method runs, because "it works" would also pass if the fast path silently fell through) + 7 E2E + 8 live (`9z-CL`).

- **v0.55.16 - N8: stop advertising authentication schemes the server cannot enforce.** `mtls`, `dpop` and `oauth-authcode` were declarable on an endpoint and appeared in `ServiceProviderConfig.authenticationSchemes`, but no authenticator implements any of them. Discovery is a promise a client is entitled to rely on, so advertising them was worse than not offering them - an integrator configures `mtls`, sees it advertised, and only finds out it was decorative when requests behave in ways the document does not explain. They remain **declarable** (existing profiles keep loading) but are no longer advertised, and each carries a written reason because a bare denylist entry rots into folklore. **The filter is an allowlist of enforceable types, and that is the point:** a denylist would default a *newly added* method type to "advertised", silently recreating this bug for the next type registered before it is implemented. A self-extending gate fails if any known type is classified in neither set or both, so adding one forces the decision.

- **v0.55.16 - pre-push now runs API unit + E2E tests.** Previously the default mode ran **no tests at all** - all 12 gates were static - which is how a dead route was pushed green. The gates existed but only in `Validate` mode, which the hook never selects: **a gate defined in a mode nothing invokes is indistinguishable from a gate that does not exist.** Measured on real pushes: unit ~42-52 s, E2E ~146-160 s, whole push ~7 min. E2E uses the inmemory backend exactly as CI does, so pre-push needs no database, Docker or network. Reverse-checked: with the orphaned decorator reintroduced, unit fails 1 suite and E2E fails 8+, so the push would have been blocked.
- **v0.55.15 - per-type credential caps, observable pruning, and a gate for the direction nothing was checking.**
  - **The register's premise did not survive measurement.** A3'/P2 recorded "nothing prunes request-log rows". Retention has been enforced since it was written (`LOG_AUTO_PRUNE` default true, `LOG_RETENTION_DAYS` default 21, scheduled at init and on an interval). What did not exist was any report that it had **run**, so a working prune and an absent one looked identical. `lastRunAt` / `lastPrunedCount` are now recorded in **both** backends and surfaced on `GET /log-config`; a zero-row prune counts as a run, because otherwise the steady state is indistinguishable from "never fired".
  - **A live test was silently capping production at one day.** Section `9z-L` "restored" prune config by `PUT`ting a **hardcoded** `retentionDays: 1`, then asserted `-Success $true` - a literal that cannot fail. Every estate the suite touched, **including customer prod on every promotion**, was left pruning at 1 day while `LOG_RETENTION_DAYS` said 30. Measured on all three: env **30**, running **1**. It now restores the value **captured** at `9z-L.1` and proves it by reading back; the estates were restored to 30.
  - **Per-type active-credential caps** - `MaxActiveBearerCredentials` (5), `MaxActiveOAuthClientCredentials` (5), `MaxActiveWifTrusts` (10), bounds 1-25. Counts only **active, same-type** credentials, which is exactly the set `findActiveByEndpoint` feeds into the bcrypt loop. WIF is separate and more generous because it is JWKS-verified and never enters that loop. **Why it matters, measured:** bcrypt cost 12 is **293 ms per compare**, and the authenticator compares against every active credential - so **three** already exceed the 800 ms latency gate, and the loop is reachable by an unauthenticated caller sending any non-JWT token. The cap **bounds** that amplification; the fix is a keyed O(1) lookup with a fast hash, since these are 256-bit server-generated secrets rather than passwords.
  - **A second flag gate, for the reverse direction.** F-T4 checked that the UI offers no unknown key; nothing checked that every known key reaches the UI, and **eleven flags** had drifted into being enforced-but-invisible. All eleven are now controls grouped into the existing category cards, and [endpoint-config-ui-coverage.spec.ts](api/src/modules/endpoint/endpoint-config-ui-coverage.spec.ts) keeps them there. **It took three attempts and the negative control caught both misses:** a bare-substring match satisfied `MaxActiveWifTrustsREMOVED`, and a broad word-boundary scan then matched the flag name inside its own `description:` prose - counting documentation as coverage.
  - **The caps change had broken credential creation outright, and the failure mode was `201 Created`.** Writing the E2E spec for the caps is what found it: two `private` helpers had been inserted **between** `@Post(':endpointId/credentials')` and `createCredential`, and TypeScript binds a decorator to whatever declaration **follows** it. The route bound to `registeredCapDefault(flag: string)`, which has no parameter decorators - so Nest called it with `undefined`, it returned `undefined`, and every POST answered `201` with an empty body while creating nothing. Three of four pre-existing `credential-lifecycle` E2E cases were failing with `404` on an id they had just "created". **Why no gate caught it:** controller unit tests call the method directly, so routing is never exercised and an orphaned decorator is invisible to them **by construction** - no amount of extra unit testing could have found it - and the API E2E suite that *would* have caught it is not part of pre-push.
  - **New static gate: [route-decorator-binding.spec.ts](api/src/modules/scim/controllers/route-decorator-binding.spec.ts).** Decorator-to-handler binding is a property of the **source**, so it is checked statically, in the unit suite where pre-push already runs. Two checks, deliberately the two ends of the same defect: **B-T1** no HTTP decorator may land on a `private`/`protected` method; **B-T2** no method carrying parameter decorators may lack an HTTP decorator. The orphaning trips **both**, from opposite directions. **Reverse-checked against the real defect, not just synthetic source** - the fix was temporarily reverted and the gate failed naming `admin-credential.controller.ts:425` (B-T1) and `:472` (B-T2). Four negative controls plus **B-T0**, which asserts the scan finds controllers at all so the gate cannot pass by scanning nothing.
  - **The cap check itself was fail-open, and this item's own principle caught it.** `assertTypeCapNotReached` opened with `if (!this.credentialRepo) return;` - silently **skipping** the cap when the repository was absent. The injection is non-optional so the branch is unreachable today, but it encoded "absence means unenforced", which is precisely the defect P2 exists to remove. It now fails **closed**: a control that cannot be *evaluated* must never look like a control that *passed*.
  - **`createEndpointWithConfig` accepted only `string | boolean`,** which made all **14 numeric flags** unreachable from any E2E test - a coverage hole the type system was creating rather than catching. Now accepts `number`.
  - **The new caps broke the X9 latency gate, which is the caps working.** `9z-BQ` deliberately seeds **6** bearer credentials so a reintroduced bcrypt loop would be unmissable; the new default cap is 5, so the sixth was refused. `9z-BQ` now raises its own cap explicitly, keeping its premise while demonstrating the cap is a configurable bound rather than a hard ceiling. Found by the standing **author-and-smoke-run-before-batch** rule - nothing else would have surfaced an interaction between two individually-correct, separately-tested features before dev.
  - **A live assertion failed for a reason unrelated to what it tested.** `9z-CK` used `Invoke-WebRequest`, whose `$r.Content` in PS7 is a **`byte[]`** for `application/scim+json` (not a recognized text type). `ConvertFrom-Json` on a byte array yields an object with no properties, so the credential id was silently **empty** and the DELETE three lines later `404`d. Now uses `Invoke-RestMethod` plus the established `Get-HttpErrorStatus` helper, and `9z-CK.T5` asserts the id is non-empty **where it is produced** rather than letting it fail downstream.
  - **Coverage:** API unit **4,823** / 168 suites (+6, +1 suite). Credential E2E **56** / 6 suites (was 52 pass + 4 fail). Live **1,448 / 0 failed** on local inmemory (was 1,436) - `9z-CK` contributes 11 assertions including a negative control that proves the cap, not a broken route, is what refuses.
  - **The Stage 3b.4 security audit found the cap was bypassable, and that is the strongest argument for running these audits before merge.** `POST .../credentials/:id/activate` performed no cap check. Because deactivating correctly *frees* a slot, the reverse operation is a second door into the same budget: fill to the cap, deactivate all, fill again, then reactivate the first batch - **twice the cap active, repeatable without limit**. Every test written for the feature passed, because they all exercised the entry point that *was* guarded. Reactivation is now capped; **rotation stays deliberately exempt** and says so in code, because it is net-neutral and refusing it at the cap would block the one operation you most want available when a secret is compromised. **Mutation-tested:** neutering the guard makes `P2-E7` fail with `expected 400, got 200`, so the new test provably discriminates.
  - **`9z-X.8` was a latent time bomb.** "Current-hour bucket monotonically non-decreasing" compares the LAST series bucket, which legitimately **resets to 0** at the top of every hour - so any run crossing an hour boundary fails. It surfaced at 14:01 (`1211 -> 0`). Same class as the v0.55.14 `9z-X.9` defect: measuring a quantity the test does not control. It now records the hour at baseline and, on a rollover, asserts the new bucket counted the two calls just made - keeping a real check rather than degrading to an unconditional pass.
  - **Stage 3a.2 found `/log-config` had no key allowlist.** Every assertion was `toHaveProperty`, which can only fail on a MISSING key - never on an extra one, which is the direction that leaks. An allowlist now covers the top level and `autoPrune`, plus a populated-shape guard so it cannot pass vacuously. (Writing it also caught me inventing key names instead of reading the contract - the first draft asserted `level`/`categories`/`payloadLogging`, none of which exist.)
  - **Stage 3c.1 flagged a single-source-of-truth slip of mine:** the refusal message hardcoded `bounds: 1 - 25` while the same function carefully read the *default* from the registry. Bounds are now registry-derived too. A cap refusal is also logged at WARN - hitting a DoS bound is security-relevant, and a refusal that leaves no trace is indistinguishable from one that never happened.
  - **Deployed 2026-08-27.** Image built from **master @ `2b6feeb1`**, published as `0.55.15` + `latest`. dev and the proudbush canary both serve **0.55.15** (confirmed from `/scim/admin/version`, not from the image tag); calmsand remains on 0.55.14 pending an explicit go-ahead. dev: live **1,450 / 0**, Playwright **219 passed / 4 skipped**. Canary blue/green: green verification passed, flip, post-flip live **1,450 / 0** + Playwright **196 passed**; revisions pruned to the policy's 2 (serving + rollback target).
  - **The first canary attempt flipped and AUTO-ROLLED BACK, and that is the system working.** One Playwright test failed post-flip (`keyboard-nav > shortcuts skip when focus is in an input`) and the machinery returned traffic to blue on its own - no customer exposure, no manual intervention. Per R3 the response was investigation, not retry: the same test passed **5/5 against the 0.55.15 green revision**, **3/3 against blue**, and in the full dev run on the identical image, while live SCIM passed 1,450/0 on green *and* post-flip. A code regression was therefore ruled out **by measurement**. Root cause: the test waits for `endpoints-page` to be *visible*, then clicks the search box and immediately types - but visible is not interactive, and on a just-flipped cold revision the endpoint-list re-render can swallow the click, sending the keystrokes to the document so the shortcut layer navigates. **The test was accusing the wrong component.** It now asserts `toBeFocused()` before typing, so a lost click fails as "not focused" instead of masquerading as a shortcut-layer defect. Test-only change; the image was not rebuilt.

- **v0.55.14 - a healthy customer-prod deploy was aborted by a test that measured a counter it did not control.** The blue/green verification failed `9z-X.9` with `delta=4 < 3` and correctly refused the flip, keeping customers on blue with zero impact. **The product was right and the test was wrong.** `9z-X.9` made 3 `/scim/admin/version` calls and asserted the dashboard's 24h series moved by fewer than 3. That series is derived from the whole request log, and on customer prod the **blue revision serves real users off the same database the green revision reads** - so ambient customer traffic lands in the same hour bucket and the delta cannot be attributed to the test's own calls. The old assertion even admitted this in a comment ("other traffic from parallel test runs may [contribute]") and tried to absorb it with an arbitrary tolerance; an arbitrary tolerance on an unbounded quantity is not a bound. It now asserts the **exclusion contract directly** - after the admin calls, `/admin/logs` (default view, which feeds the series) must contain **zero** `/scim/admin/version` rows - plus a **positive control** proving those same rows ARE visible under `includeAdmin=true`, so the check cannot pass vacuously by the rows simply not existing. Verified on the live GHCR container: 0 in the default view, 4 with `includeAdmin=true`. **Standing lesson: never measure a shared mutable counter and attribute the movement to yourself.**

- **v0.55.14 - ships the `fast-uri` security fix, and closes two gates that could not fail.** Three defects, all of the same shape: a check that reports something other than what it verifies.
  - **`fast-uri` 3.1.4 -> 4.1.2 (CVE-2026-18446, HIGH) now actually ships.** The fix was committed as source in v0.55.13 but no image carried it. Its `.trivyignore` entry had been a `quarantine-window` whose `Fix-available-from` passed on 2026-08-10, while the override kept pinning the vulnerable version. Because 3 -> 4 is a **major** bump on the chain `prisma migrate deploy` runs at container start, it is validated on every form factor rather than assumed.
  - **Excluded Playwright tests are no longer invisible.** [verify-deployment.ps1](scripts/verify-deployment.ps1) drops the pixel-baseline specs via `--grep-invert`, which is correct (they are coupled to fixture data), but an excluded test appears in **no** pass/skip/fail count - so every customer-prod verification reported "196 passed" while silently carrying zero visual coverage. It now prints `223 collected, 23 excluded, 200 will run`, **fails** if the filter leaves nothing, and **fails if the three counts do not balance**, so an over-broad pattern cannot quietly delete coverage. Negative control: adding `|Settings` to the pattern excludes **57** instead of 23, which the accounting now surfaces.
  - **Three pipeline audits that were recorded as PENDING now execute.** [dev-deployment-pipeline.ps1](scripts/dev-deployment-pipeline.ps1) recorded all 11 Stage 3 audits as `PENDING` at 0 s through an unconditional loop with no execution path - they could never pass, and were explicitly non-blocking. `endpointConfigFlagAudit` and `dependencyCveSweep` (production-only audit, CRITICAL blocks, plus `.trivyignore` staleness) now run; the remaining eight are genuine reviewer-judgement gates and say so. **Recording an executable check as PENDING is indistinguishable from having no check.**
  - **Stages 1.9 / 1.10 / 1.11 were never wired into the pipeline** despite being mandated. `prismaMigrationAudit` (schema and migrations must move together), base-image LTS, and the deployment-doc audit now run there too, not only at pre-push.

- **v0.55.13 - the UI stopped gating on a flag the server retired, and the drift is now gated.** `CustomResourceTypesEnabled` was deleted from the flag registry in settings-v8 as one of six deprecated flags, marked `@deprecated DERIVED - use profile.resourceTypes instead`. The API duly derives availability from `profile.resourceTypes[]`. **The UI never got the memo.**
  - **The defect was the opposite of what it looked like.** It first presented as "an unregistered flag", which suggests registering it. Measured on both live estates: **1** endpoint has the flag set, **2** have custom resource types, and **all 2 of those have the flag off**. Re-registering and enforcing it would have **404'd every endpoint actually using the feature**. What was actually broken is that [ResourceTypesTab](web/src/pages/ResourceTypesTab.tsx) hid Create/Delete and showed a "feature disabled" panel for a capability that works, and [SettingsTab](web/src/pages/SettingsTab.tsx) offered a toggle that wrote a setting nothing reads.
  - **Why only this one.** Of the six retired flags, three were **renamed** and are handled by `normalizeStaleSettingsKeys`, which deletes the legacy key on read. The other three were **derived away**, so there was nothing to rename them to - and this one alone still had a live UI dependency. It was also absent from the stale-key cleanup, so it persisted on profiles indefinitely, which is why it showed up on 117 live endpoints.
  - **Fixed:** the UI derives like the server does; the retired flag is dropped from profiles alongside the three renames (`BulkOperationsEnabled` and `ReprovisionOnConflictForSoftDeletedResource` too); the stale "Gated behind..." comment in the wildcard controller is corrected.
  - **Gated so it cannot recur.** New [endpoint-config-conformance.spec.ts](api/src/modules/endpoint/endpoint-config-conformance.spec.ts) compares the registry against the Settings UI in **both** directions, requires every numeric flag to declare both bounds, and pins the six retired flags as retired. **Proven falsifiable**: reinstating the orphan makes 2 of its 5 tests fail. `normalizeStaleSettingsKeys` had **no test at any level** before this; it now has 6 E2E cases including the rename-does-not-overwrite case and a positive control.
  - **Full 35-flag conformance audit** (requested): registry is internally consistent (35 constants = 35 definitions); **zero** Settings-UI keys are unregistered after this fix; all 35 are documented. Two residual findings recorded rather than silently fixed - the doc header claimed "30 flags" while its own breakdown summed to 34 and the registry holds 35 (corrected to the measured 21 boolean + 11 numeric + 3 string), and **9 flags have no UI at all** (8 JWKS numeric tuning knobs + `logFileEnabled`), which is a real gap against the ten-cell flag rule.
  - **Tests:** API unit 4,796 -> **4,801**, API E2E 1,470 -> **1,476**, web vitest **1,285**, local live 1,429 -> **1,435** (new section `9z-CJ`). Lint held at 0 errors / 510 warnings; `tsc` held at 96.
  - **Pipeline made end-to-end runnable on this device:** `full-validation-pipeline.ps1` now probes container egress and falls back to the **host's own** npm registry rather than a hardcoded corporate URL, and `dev-deployment-pipeline.ps1` fails fast with the real cause when ARM returns an empty FQDN because the tenant expired.

- **v0.55.12 - B/C/D: endpoint config writes stop losing edits silently.** Endpoint profile writes were last-write-wins and the loser received `200 OK`, so a lost update produced no signal anywhere. Scoped deliberately narrow after measuring which writes can actually lose data.
  - **The common case never had the bug.** `profile.settings` and `profile.serviceProviderConfig` merge **per key** server-side, so two operators changing different flags both survive today. Only `schemas`, `resourceTypes` and `authentication` are replaced wholesale. A blanket precondition would have added a `412` to the frequent, already-correct path - so it was not added there.
  - **B - the server publishes a version.** `GET /admin/endpoints/:id` now returns a weak `ETag` derived from a content hash of `{displayName, description, active, profile}`. `id` and `name` are excluded on purpose: `name` is immutable after create and `id` identifies the row, so neither participates in a lost update and including them would manufacture spurious conflicts. A content hash was chosen over a row version (no migration, works on both persistence backends) and over a timestamp (millisecond collisions); it also means an idempotent re-save correctly matches instead of conflicting.
  - **C - `If-Match` is honoured, opt-in.** A write naming a stale version is refused with `412` + `scimType: versionMismatch` and a detail naming **both** ETags. A write with no `If-Match` behaves exactly as before, so every existing caller is untouched. In the UI only [ResourceTypesTab](web/src/pages/ResourceTypesTab.tsx) opts in - it is the one surface that merges the whole `resourceTypes[]` + `schemas[]` arrays client-side from a page-load read, which is a textbook mid-air collision. Conflicts surface through the existing Phase K5 `ConflictDialog` (refresh-and-reapply / force-overwrite / cancel) rather than a new bespoke surface.
  - **D - a server-side race that no caller could have resolved.** `POST` and `DELETE` on `/admin/endpoints/:id/authentication/methods` did read-modify-write of the whole `authentication` block; three simultaneous adds all returned `201` and **one** method survived. `If-Match` is the wrong tool for this - the race is between two requests milliseconds apart, not two operators - so the fix is a per-endpoint [KeyedMutex](api/src/common/keyed-mutex.ts). Its scope is stated in-file: an in-process lock is a complete fix only while one process serves an endpoint, and both dev and prod run `minReplicas = maxReplicas = 1`.
  - **Every concurrency test was proven falsifiable before being trusted.** The unit test was run against an unlocked copy of the same body (2 of 3 writes lost) and `D-E1` was re-run with the lock neutralized (fails), because a concurrency test that passes either way is worse than no test. `C-P1` (conflict raised) and `C-P3` (no conflict on an ordinary edit) are the same flow differing only by a competing edit, so the pair discriminates.
  - **Tests:** API unit 4781 -> **4796** (163 -> 165 suites), API E2E 1460 -> **1470** (89 -> 90 suites), Web vitest **1283** (104 files), local live 1419 -> **1429** (new section `9z-CI`), new Playwright spec [endpoint-write-conflict.spec.ts](web/e2e/endpoint-write-conflict.spec.ts) **3/3** in Chromium. Lint held at the 0 errors / 510 warnings ratchet; `tsc` held at the 96 baseline.
  - **Tooling note:** Chromium refuses `http://localhost:6000` with `ERR_UNSAFE_PORT` (port 6000 is X11), so the repo's usual local live-test port cannot be used as a Playwright target. Browser runs use port 4000.

- **v0.55.11 - N6: the slice-dependent assertion audience is answered by a runbook, not by code.** SyncFabric emits `api://<appId>` on the legacy `CustomerApplication` chain and `api://<appId>/<host>` on the `FirstPartyApplication` chain, selected by a feature flag enabled on some slices and disabled globally. A job moving between slices therefore changes its assertion `aud` **with no change on this side**, and SCIMServer matches `expectedAudience` exactly - so it surfaces as a sudden `401 wif_audience_mismatch` after a period of working normally.
  - **The remedy already existed and needed no new capability.** Multi-trust iteration (WI-16) means registering BOTH audience shapes as two trusts works: each still matches **exactly**, and both are explicitly declared by an operator. The endpoint accepts two chosen audience strings instead of one - the same posture Microsoft's own validators take with `ValidAudiences`.
  - **Prefix and wildcard audience matching were explicitly rejected.** They would convert an exact check into a pattern check on the claim that binds a token to this server. The audience-matching code is unchanged.
  - **The error now names its own cause.** The `wif_audience_mismatch` remediation text explains the two shapes and the two-trust fix, so an operator meets the answer at the point of failure rather than having to find a document.
  - **A runbook that is not tested is a claim that quietly rots**, so the remedy is locked at three levels: unit (fall-through with the **same issuer** on both trusts, in **either** registration order, still rejecting an unrelated audience), E2E (real RS256 assertions of both shapes mint over HTTP, and a token minted via the first-party trust **authorizes a real SCIM call**), and live `9z-CH` (a second trust differing only by audience is accepted as a distinct credential and does not overwrite the first). The unit tests were **mutation-checked** - replacing the provider's `continue` with a throw makes them fail.
  - Note the pre-existing `WI-16` test covers multi-trust with **different issuers**; trusts are ordered by issuer, so the same-issuer case these tests cover is a genuinely different path that nothing exercised before.
  - Coverage: **+3 unit**, **+4 E2E** (new `n6-slice-audience.e2e-spec.ts`), **+4 live** (`9z-CH`). Runbook: [docs/auth/N6_SLICE_DEPENDENT_AUDIENCE_RUNBOOK.md](docs/auth/N6_SLICE_DEPENDENT_AUDIENCE_RUNBOOK.md). User-facing guidance added to [docs/AUTHENTICATION_GUIDE.md](docs/AUTHENTICATION_GUIDE.md) sections 6.3 and 8.4.
### Added
- **v0.55.10 - W1.2: the first WIF mint after a deploy no longer pays a cold JWKS fetch. Wave 1 is complete.** W1.4 keeps the JWKS cache warm once it holds an entry, but nothing put the first entry there, so the **first** token mint after every restart paid a synchronous outbound fetch on a real user's request. A new `JwksPrewarmService` enumerates every active `wif` trust at boot, deduplicates their `jwksUri`, and warms each one. This was the **last open item in Wave 1**.
  - **It is a separate service, not more work inside the validator.** `ExternalJwksValidatorService` has no business knowing that trusts are stored as endpoint credentials: the validator warms a URI, the prewarm service decides which URIs exist. The new `prewarm(jwksUri)` goes through the same `fetchJwks` path as everything else, so it shares single-flight coalescing and the atomic cache swap - a prewarm racing a real mint cannot produce two fetches or a half-updated entry.
  - **Boot cannot be broken by it.** A database that is not ready, or an IdP that is unreachable, is logged and skipped. `Promise.allSettled` is used rather than `Promise.all` deliberately: `prewarm` already promises never to reject, but a boot path must not depend on a collaborator keeping that promise.
  - **The completion line is logged even when nothing was warmed**, because a boot-time action leaves no other trace and there would otherwise be no way to distinguish "ran and found no trusts" from "never ran". That distinction is the whole risk here - the service takes the credential repository as an `@Optional()` token, so a missing module import would have shipped the feature completely inert with every unit test still green. `RepositoryModule.register()` is now imported by `OAuthModule`, and E2E `W1.2-E1` asserts the line exists, making it the wiring test for that token.
  - New repository method `findAllActiveByType(credentialType)` - the first cross-endpoint query on `IEndpointCredentialRepository` - implemented for both backends. The Prisma implementation is exercised on **every E2E boot** because the prewarm calls it during `onModuleInit`; the in-memory one has its own unit spec.
  - **No live-test section, deliberately.** The boot log line rolls out of the ring buffer on a long-running node, so a live assertion would be a flake generator rather than a gate - the same reasoning that put W1.6's latency check in `wif-e2e-proof.ps1` instead of `live-test.ps1`. The E2E proves the wiring on a fresh boot, which is the condition that matters.
  - Coverage: **+6 unit** (`jwks-prewarm.service.spec.ts`), **+5 unit** (first spec for `InMemoryEndpointCredentialRepository`), **+2 E2E** (`jwks-prewarm.e2e-spec.ts`). Feature doc: [docs/auth/W1_2_JWKS_STARTUP_PREWARM.md](docs/auth/W1_2_JWKS_STARTUP_PREWARM.md).
### Fixed
- **v0.55.9 - A10: a partial `profile.authentication` block silently deleted every authentication method.** `PATCH /scim/admin/endpoints/{id}` replaces `authentication` **wholesale** (unlike `settings` and `serviceProviderConfig`, which merge per key), and `expandAuthentication` normalizes a missing `methods` key to `[]`. Each is reasonable alone. Together, a caller sending `{ "authentication": { "defaultMethodId": "m-abc" } }` - intending to set one sibling field - wiped **every configured authentication method** and got a `200 OK`. The blast radius is the endpoint's authentication configuration itself.
  - **The guard refuses partials, not the section.** A complete block is still accepted, including an explicit `methods: []` when emptying is genuinely intended - the distinction drawn is *stated intent* versus *accidental omission*, so it never has to guess. The error names the recovery (read the current block from the methods API and resend it, or omit the key entirely).
  - **Parity is structural.** The check sits in the shared `mergeProfilePartial` helper that both the create and update paths call, so the rule cannot differ between them or between the Prisma and InMemory backends. It is still proven on both: E2E on Prisma, live `9z-CG` on InMemory.
  - **The assertion that matters is not the `400`.** E2E and live `9z-CG.T2` seed a method, attempt the partial PATCH, then **re-read the method list** to prove it survived. Asserting only the status would pass against a server that returned `400` and wiped the data anyway. Controls at unit and live level prove a complete block is still accepted, so an over-tightening also fails.
  - Coverage: **+4 unit**, **+2 E2E**, **+4 live** (`9z-CG`). Feature doc: [docs/auth/A10_PARTIAL_AUTHENTICATION_BLOCK.md](docs/auth/A10_PARTIAL_AUTHENTICATION_BLOCK.md). RCA: [docs/auth/EXECUTION_ISSUES_AND_RCA.md](docs/auth/EXECUTION_ISSUES_AND_RCA.md) section 10E (I-41).
### Added
- **v0.55.8 - A8: changing an endpoint's authentication methods now emits the structured audit event.** `POST` and `DELETE` on `/admin/endpoints/{id}/authentication/methods` mutate the most security-sensitive configuration the product exposes, and emitted only free-text `INFO` - which cannot be alerted on, counted, or filtered. They now emit the canonical `AUTH_ADMIN_EVENT` through the existing `emitAuthAdminEvent`, exactly as the credential and JWKS-host controllers already did, with two new actions `auth_method_add` / `auth_method_remove`.
  - **Failures emit too, not just successes.** An unknown method type and a remove of a non-existent method each emit `outcome: 'failure'` before throwing, so a probing or misconfigured caller is visible rather than silent.
  - **The event carries ids and `type` only, never `config`.** A method's `config` can hold operator-supplied material, so it is deliberately excluded from the event; a unit test asserts a planted secret value and its key name are both absent from the serialized event.
  - **No new pattern.** This is the fourth caller of an emitter that already existed, so the change is EXTEND-not-EDIT on an established seam. Coverage: +5 unit (new `admin-authentication-method.controller.spec.ts`, which is also the controller's first spec).
- **A12 - corrected a comment that misdescribed the merge it labels.** `endpoint.service.ts` said `// Deep-merge settings (additive)` above `{ ...current.settings, ...partial.settings }`, a **shallow** per-key spread in which a nested object value REPLACES rather than merges. A reader trusting the comment would predict the wrong outcome for nested settings.

### Fixed
- **v0.55.7 - `StrictSchemaValidation` no longer rejects a payload that conforms to its own schema.** Strict validation honoured the `multiValued` characteristic at the attribute level and ignored it at the **sub-attribute** level, so a sub-attribute declared `type: "string", multiValued: true` was type-checked as a single value and an array in it was rejected with `Attribute 'skus' must be a string, got object.` - a **cardinality** mismatch misreported as a **type** error, against a shape the server itself published at `/Schemas`. RFC 7643 [§1.2](https://www.rfc-editor.org/rfc/rfc7643#section-1.2) defines a simple attribute as "singular **or** multi-valued", erratum 5607 confirms it for `referenceTypes`, and RFC 9967 ships one in the wild (`securityEvents.eventUris`).
  - **The defect was an inconsistency inside one validator**, not a policy: `validateSingleValue` fans out over array elements when `attrDef.multiValued` is set at level 1, while `validateSubAttributes` carried the comment *"Sub-attributes are always single-valued in SCIM"* and dispatched every level-2 value as singular. The comment asserted as fact something three RFC sources contradict.
  - **This half moved out of `RfcCompliantSubAttributes` and into base strict behavior.** The flag shipped in v0.54.87 bundling two rules pointing in opposite directions: R1 (reject complex sub-attributes, §2.3.8) **tightens**, R2 (honour multi-valued simple sub-attributes) **loosens**. Bundling a defect fix with a policy tightening meant a flag named "RfcCompliant..." made the server simultaneously stricter and more permissive. The flag now governs **R1 only**, giving it a contract statable in one line: *everything it does rejects something.* This **reduces** flag surface - no flag was added or split.
  - **Measured blast radius: zero.** Across the dev estate on 2026-08-18: 58 endpoints, 354 schemas, **2,826 declared sub-attributes, of which 0 carry `multiValued: true`** and 0 are complex. No existing endpoint can change behavior, and the change only ever accepts more, so no working caller can break.
  - **Cardinality is still enforced in both directions.** An array in a sub-attribute the schema declares **singular** remains a `400`, and each element of a multi-valued sub-attribute is still type-checked individually and reported at its index (`licenses[0].skus[1]`). Honouring `multiValued` did not become "skip validation".
  - **Why no gate caught it for a release.** Every test exercised exactly one flag setting, so the R2 assertions all sat under `flag ON` and the `flag OFF` case was locked in as *expected* rejection - the polluted-baseline failure mode (R10: a gate only proves what it asserts, and codifying a broken state as the baseline makes it permanent). The specs are now parameterised over **every** flag setting (`absent`, `false`, `true` at unit level; both at E2E and live level), so re-gating R2 on the flag turns them red.
  - Coverage: domain unit 23 -> **33**, API E2E 9 -> **10**, live `9z-CA` 11 -> **15** (adds round-trip, element-index and singular-cardinality assertions at flag OFF). Totals: API unit **4,758/4,758** (160 suites), API E2E **1,449/1,449** (87 suites), web vitest **1,274/1,274** (103 files), local-node live **1,405/1,405**.
  - Docs: [docs/RFC_COMPLIANT_SUBATTRIBUTES.md](docs/RFC_COMPLIANT_SUBATTRIBUTES.md) §1/§2/§9, [docs/rfcs/SCIM_SUBATTRIBUTE_TYPE_RULES.md](docs/rfcs/SCIM_SUBATTRIBUTE_TYPE_RULES.md) §15, and [docs/ENDPOINT_CONFIG_FLAGS_REFERENCE.md](docs/ENDPOINT_CONFIG_FLAGS_REFERENCE.md), which gains a full **`StrictSchemaValidation` vs `RfcCompliantSubAttributes`** comparison plus the previously-undocumented full scope of `StrictSchemaValidation` (it had documented only the URN check, understating that lenient mode performs **no type checking at all**).
  - **Two gate defects fixed in passing, both of the same shape: a precise detector wired to a blunt writer.**
    - **The docs freshness fixer rewrote history.** `-Fix` string-replaced the stale version token across the whole 15-line header, so it turned "every image was re-captured on **2026-07-31** ... running **v0.55.6**" into `v0.55.7` - a dated, evidentiary statement converted into a false one, in 4 docs. It now restamps product-version **markers** by match offset and never a dated **verification claim**, and a claim is a finding only when a whole MINOR behind (failing on a patch bump would have got the rule deleted rather than obeyed). Self-test 20 -> **24** with 4 negative controls, including one asserting `-Fix` leaves capture provenance untouched.
    - **A visual-regression baseline encoded whichever endpoint happened to be first.** `Endpoint detail - Overview tab` snapshotted `[data-testid^="endpoint-"]` `.first()`, so its baseline captured that endpoint's Recent Activity row count. `mask` hides a card's **content** but not its **height**, and under `fullPage: true` a collapse from ~15 rows to ~4 is a large unmasked geometry change - it failed the 3 % tolerance for a reason unrelated to any UI change. Now uses the purpose-created fixture the sibling tab tests already use, so activity is deterministic. The tolerance was **not** widened: widening it would have left a gate that no longer discriminates.

### Added
- **v0.55.6 - Dev and canary prod moved to a new Azure AD tenant, carrying everything.** The ephemeral subscription hosting both estates was expiring, so the full estate was rebuilt in tenant `9751e42f` (Provisioning IAM Team 09, subscription `8cb58fd6-...`, environment domain `purplecliff-91e4026d`). Plan: [docs/TENANT_09_MIGRATION_PLAN.md](docs/TENANT_09_MIGRATION_PLAN.md). Issues + RCA: [docs/TENANT_09_MIGRATION_EXECUTION_ISSUES_AND_RCA.md](docs/TENANT_09_MIGRATION_EXECUTION_ISSUES_AND_RCA.md).
  - **Everything was carried, and verified per surface rather than by count alone.** 58 endpoints / 728 users / 347 groups to canary prod, 58 / 734 / 347 to dev, primary keys preserved so existing SCIM client configurations keep resolving. Deep verification walked **all 58 endpoints x 6 surfaces** (profile, schemas, resource types, ServiceProviderConfig, settings, config flags) plus the global surfaces on both estates: **OVERALL PASS** each, with 296 and 319 attribute definitions confirmed RFC-valid.
  - **Test counts.** Live SCIM **1387/1387** against tenant-09 dev (canary prod is 1380/1387; the 7 are JWKS numeric-bound assertions for a feature introduced in 0.55.3 while the canary still runs 0.55.1 - a version lag, confirmed by locating the introducing commit, not an estate defect). Playwright **207 passed / 0 failed** in real Chromium. Pre-push **12/12 gates PASS**. Mermaid **648/648** parse and render in both themes.
  - **A migration can carry every row and still lose server-level state.** Counts, IDs and all 348 per-endpoint surfaces passed while the JWKS host allow-list had silently reverted to its seeded default. Every check was resource-shaped; server-level singletons are invisible to all of them. Fixed at both layers: [mirror-prod-to-dev.ts](api/src/scripts/mirror-prod-to-dev.ts) now carries DEKs, JWKS hosts, server settings and the retained `secretEnvelope`, with a new coverage spec that pins the mirrored model list against the Prisma schema so a model added and not mirrored **fails a test**; and `login.windows.net` (the **v1** Entra host) was added to `WELL_KNOWN_JWKS_HOST_SEED`. The previous allow-list test iterated the seed constant, so by construction it could never detect a removal - it now names all 7 hosts individually.
  - **Subscription expiry kills ARM, not the data plane.** The old tenant's control plane expired mid-migration on 2026-08-11. Recovery was possible only because the PostgreSQL connection strings had been captured before that boundary and stored outside the tenant. The corollary is in gap **G16**: the old estates are still serving and can no longer be stopped, patched or deleted.
  - **Durable fixes.** [infra/containerapp.bicep](infra/containerapp.bicep) accepts an optional `environmentResourceId` so a Container App can bind an environment in another resource group. [deploy-azure.ps1](scripts/deploy-azure.ps1) provisions the **superset** `CITEXT,PG_TRGM,PGCRYPTO,UUID-OSSP`, closing gap G9 - `pg_dump` emits `CREATE EXTENSION "uuid-ossp"` and Azure rejects it against a target that omits it, which made a data-specific fault present as an environment-specific one. New [rotate-tenant-data.ps1](scripts/rotate-tenant-data.ps1) moves a database between tenants via an in-Azure ephemeral job, guards against the 0-byte dump that restores `rc=0` over a live target, and probes both raw and percent-decoded passwords because two estates needed opposite handling of the same generated credential.
  - **`az` stores extensions under `AZURE_CONFIG_DIR`.** The per-tenant profile isolation in [az-tenant.ps1](scripts/az-tenant.ps1) therefore gave each tenant an empty extension set, surfacing as `'environment' is misspelled or not recognized by the system` and an empty resource inventory for a completely healthy estate. `Set-ScimAzExtensionDir` now pins `AZURE_EXTENSION_DIR` to one shared directory on every profile switch. Tenant resolution also keys on subscription **ID** and refuses an ambiguous name, because two subscriptions share the display name `ProvIAM_Subscription`.
  - **Gaps G1, G3, G3a, G5, G6, G7, G13 closed**; **G15** (resource names encode a tenant generation that expires) and **G16** opened. Customer prod (calmsand) was never contacted beyond a read-only version check and is untouched.

- **v0.55.5 - W1.4: JWKS cache cadence (24 h TTL + background refresh + rate-limited unknown-`kid` + hard stale ceiling).** Replaces a 10-minute, lazily-refreshed, unbounded-fallback cache with [Microsoft's published algorithm for its own signing keys](https://learn.microsoft.com/en-us/entra/identity-platform/signing-key-rollover). Feature doc: [docs/auth/W1_4_JWKS_CACHE_CADENCE.md](docs/auth/W1_4_JWKS_CACHE_CADENCE.md). Issues + RCA: [docs/auth/EXECUTION_ISSUES_AND_RCA.md](docs/auth/EXECUTION_ISSUES_AND_RCA.md) section 10C (I-35 to I-37).
  - **The hard constraint was honored.** The delivery plan gated the TTL raise behind a precondition, because $\frac{24\ \text{h}}{10\ \text{min}} = 144$ - raising the TTL alone multiplies the key-rotation blast radius by 144x. The raise (600,000 -> **86,400,000 ms**) therefore ships in the same commit as the background refresher, the rate-limited unknown-`kid` path, and the **overlap-window rotation test**.
  - **The overlap-window test is the one that matters.** `W1.4-T3` walks a real rotation and asserts that after a token with the NEW `kid` forces a refetch, a token with the **retiring** `kid` still verifies from cache with no further fetch. A cache that replaced rather than merged, or that tracked only the newest `kid`, passes the first three steps and fails that one - and would break every in-flight token issued before a rotation.
  - **Three pre-existing gaps closed.** (1) Fail-to-stale had **no age test at all**, so a rotated-out or revoked key stayed acceptable for exactly as long as the IdP stayed unreachable - now bounded by `JwksStaleIfErrorMs` (48 h default). (2) `Cache-Control` was ignored - now honored **downward only**: `expiresAt = now + min(configured, Cache-Control max-age)`, so an IdP may ask for a shorter cache but can never pin keys beyond the lifetime we chose to trust them for. (3) The unknown-`kid` refetch was unrate-limited and fully caller-controlled - now floored by `JwksUnknownKidMinIntervalMs` (5 min default), closing an amplification vector against the IdP.
  - **RCA 10.2 resolved.** An allowlist rejection is now a distinct `JwksHostNotPermittedError` that is explicitly **not stale-eligible**, while a network failure still is. Serving stale keys through a real outage is an availability property worth keeping; serving them after the operator has REVOKED the host turns a security action into a no-op for the whole cache lifetime. Nothing is purged, so real outages are unaffected. The pre-existing W1.3 test that asserted the opposite was updated with the reasoning recorded inline (I-36).
  - **Steady state is now always a cache hit.** A background sweep runs at `max(60s, refreshInterval/4)`, refreshes only entries older than `JwksRefreshIntervalMs` (1 h default), is `unref()`ed so it never holds the event loop open, and **never rejects** - a failed refresh leaves the still-valid entry to be retried. Previously every TTL expiry put a synchronous outbound fetch on a user's token mint.
  - **Settings:** three new numeric endpoint settings + env vars - `JwksRefreshIntervalMs`/`JWKS_REFRESH_INTERVAL_MS` (60,000-86,400,000), `JwksUnknownKidMinIntervalMs`/`JWKS_UNKNOWN_KID_MIN_INTERVAL_MS` (0-3,600,000), `JwksStaleIfErrorMs`/`JWKS_STALE_IF_ERROR_MS` (0-604,800,000). Zero is legal on the latter two and means "disable the rate limit" and "disable fail-to-stale entirely" respectively. Endpoint settings controls: **32 -> 35** (11 numerics).
  - **The I-30 negative control paid for itself immediately (I-35).** Running the new `9z-CE` assertions against dev at 0.55.3 (no W1.4) measured: round-trip **PASS** (not testing W1.4), bounds rejections **FAIL** 0/6 (load-bearing), 24 h-ceiling-accepted **PASS**. That last one was written as feature evidence and is actually a regression guard - the ceiling predates W1.4, which raises the *default* to it. It is now labelled as such in the suite rather than left to become folklore. **The cheapest way to find out what a test is testing is to run it against a build that lacks the feature.**
  - **Coverage:** +12 unit (new [external-jwks-cache.w14.spec.ts](api/src/oauth/external-jwks-cache.w14.spec.ts), including the non-vacuous `T10` that stops `T9` passing against an unconditional sweep), +8 E2E ([config-flags.e2e-spec.ts](api/test/e2e/config-flags.e2e-spec.ts), including an unregistered-key control), +14 live (`9z-CE`). API unit **4,720 -> 4,732**; live SCIM local **1,387 -> 1,401**.

### Fixed
- **Live-test isolation: nine assertions passed only on environments carrying leftover state (I-38).** The suite reported 1401/1401 on Azure dev and on a local node, then 1396/1401 on Docker against a **fresh** database (`9z-AV.T7/T8`, `9z-BK.T2-T4`, `9z-BE.T1-T4`).
  - **Not container egress** - disproved directly: `docker exec ... wget https://login.microsoftonline.com/common/discovery/v2.0/keys` returned a key set.
  - **Root cause.** Those sections verify against the **legacy** Entra JWKS host `login.windows.net`, which is deliberately absent from the well-known seed allowlist. Each environment satisfied the precondition by accident and differently: dev had it **persisted** from an earlier run; the local node had it in `JWKS_HOST_ALLOWLIST`; fresh Docker had neither, so the SSRF guard correctly rejected. The assertions were therefore not testing what they appeared to on two of three form factors.
  - **Confirmed pre-existing, not a W1.4 regression**, by running the identical suite against the **previous published image (0.55.3)** on a fresh volume: the failure sets were identical (`none - every 0.55.5 failure also fails on 0.55.3`).
  - **Fix.** New idempotent `Ensure-JwksHostAllowed` helper in [scripts/live-test.ps1](scripts/live-test.ps1), called by `9z-AV` and `9z-BK`, which reads the effective allowlist and only adds the host when missing. Proven non-vacuously: the local node was restarted with `login.windows.net` deliberately **absent**, verified absent via the API, and the suite then passed **1401/1401**. All three form factors now pass from a cold state.
  - **Generalizable rule.** This is the live-test analogue of the Playwright fixture work in the same release: a test that depends on ambient environment state passes or fails for reasons unrelated to the code. Corollary worth keeping: **three green form factors are not three independent confirmations if each is green for a different accidental reason** - only a fresh-volume run exercises the cold path.
- **v0.55.4 - eliminated 11 of 13 Playwright skips, and the two cross-spec races they were hiding.** Full suite vs dev goes from **207 passed / 13 skipped / 0 failed** to **218 passed / 2 skipped / 0 failed**. No production code changed; this is entirely test-determinism work.
  - **The finding.** 8 specs were silently skipping on an environment serving **58 endpoints**, with reasons like "Tenant has zero endpoints" and "No endpoints available". Two independent root causes, both invisible to a green gate:
    - **`.count()` does not auto-wait.** `test.skip((await cards.count()) === 0, 'Tenant has zero endpoints.')` read `0` before React had painted the grid, so the guard fired for a reason that was never true. The tests had been dead for as long as the guard existed.
    - **"The first endpoint card" is not a stable target.** The admin list is ordered `createdAt DESC`, so an endpoint created by ANY spec running in parallel immediately becomes "the first endpoint" for every other spec - and is then deleted underneath it at cleanup. This produced intermittent, hard-to-attribute failures in `credential-secret-visibility.spec.ts` (30s timeout) and `discovery-explorer.spec.ts` (`discovery-spc-section` never mounted). Both were **latent before this change** and were surfaced by measuring the suite rather than by any gate.
  - **A third, quieter defect: conditionally-vacuous assertions.** `credential-reveal.spec.ts` wrapped its assertions in `if ((await moreButtons.count()) > 0)`, so on an endpoint with no credential card the test passed having asserted nothing. That is the same failure as a skip, minus the report line that would have revealed it.
  - **Fix - preconditions by construction.** New shared module [web/e2e/endpoint-fixture.ts](web/e2e/endpoint-fixture.ts) provides `createFixtureEndpoint` / `createFixtureEndpointWithUsers` / `deleteFixtureEndpoint` / `openCredentialsTab`. A spec that needs an endpoint in a particular shape now **creates one in that shape**, asserts against it, and deletes it. Seeded users are deliberately Entra-shaped (>= 40-char `userName` plus `name`/`emails`/`externalId`/enterprise extension) so truncation and drawer-enrichment assertions are always exercisable.
  - **Specs converted:** `export`, `wif-credentials` (7 skips), `credential-reveal` (1 skip + 2 vacuous conditionals), `credential-secret-visibility`, `connect-tab`, `endpoint-detail-tabs`, `endpoint-crud` (2 guards), `discovery-explorer` (4 unreachable skips removed), `copy-and-truncate` (3 data-dependent skips).
  - **Method-tab gating documented.** Credential cards are filtered by the ACTIVE method sub-tab, and each tab is gated by its own flag (`WifCredentialsEnabled`, `SecretTokenBearerAuthEnabled`, `OAuthClientCredentialsAuthEnabled`, `SharedSecretBearerAuthEnabled`). A fixture must both enable the flag and select the tab; enabling alone leaves the credential filtered out of view.
  - **Create-wizard coverage turned on for dev.** `E2E_ALLOW_MUTATIONS` is retained as a safety valve for customer-facing estates but is now set by [scripts/dev-deployment-pipeline.ps1](scripts/dev-deployment-pipeline.ps1) Stage 5.3, so the primary endpoint-creation flow finally has browser coverage instead of skipping by default.
  - **Two skips remain, both deliberate and justified.** `router-behavior` hover-prefetch (network-unobservable since N2's `OnboardingWizard` warms the query on every route mount; contract locked at the unit layer by `router-loaders.test.ts`) and `sse-cross-tab` (MSW-only, runs against a local vite dev server; covered against real backends by `scim-event-sse-bridge.e2e-spec.ts` + live `9z-V`). The suggested "assert no loading skeleton" rewrite of the former was evaluated and **rejected as vacuous** - it would pass whether or not prefetch worked, and a test that cannot fail for its stated reason is worse than no test.
  - **Generalizable rule.** A skip is a test result nobody reads. Any conditional skip whose predicate depends on ambient environment data is a dead test waiting to happen - the predicate must be made true by construction, or the skip must be unconditional and documented with its alternative coverage. Corollary: **`.count()` is never a valid readiness signal in a Playwright guard**, because it does not auto-wait.

### Added
- **Dev deploy validation of 0.55.3, and two live assertions that could not fail (I-30).** Deployed to dev (0.55.1 -> 0.55.3, node v24.18.1 -> v24.19.0), 58/58 endpoints intact. Live **1,387 / 1,387**; Playwright vs dev **207 passed / 13 skipped / 0 failed**.
  - **The finding.** Capturing the pre-deploy state ran the W1.5 assertions against the OLD build (0.55.1, which contains no W1.5 code) and `JwksTotalDeadlineMs` was **accepted** there. Cause: the endpoint settings validator validates **registered** keys and stores **unregistered** ones untouched - an invented `TotallyUnregisteredSettingXyz` round-trips identically. So `9z-CD.T1` to `T4`, which assert the four caps persist and re-read, were **persistence checks, not feature checks**: four of the section's twelve assertions would have passed against a build with no W1.5 in it.
  - **Why the section still held.** `T5` (six bounds rejections), `T6` (the non-vacuous guard) and `T7` (the 1,000-key maximum) do discriminate - rejecting an out-of-range value requires a registry entry. The section was partly load-bearing, and nothing marked which half.
  - **Fix.** Two controls added and the reasoning written into the suite: **T8** proves an unregistered key round-trips; **T9** proves that same key has **no bounds** while a registered one does. That difference is the registry entry, asserted rather than assumed.
  - **Generalizable rule.** When a feature adds configuration, the round-trip is provided by the config store, not by the feature. Assert what only the feature can do - validation, enforcement, or an observable behaviour change. The cheap detector: **run new assertions against the previous build before deploying; anything that still passes is not testing the change.** This is R10 arriving from a new direction - a round-trip check feels like an outcome assertion and is not one.


### Added
- **W1.5: the JWKS safety envelope - a total deadline plus response caps (v0.55.3).** A per-attempt timeout is not a bound on the operation. With `retries: 5` and a 200 ms base backoff the retry ladder alone sleeps `200+400+800+1600+3200 = 6.2 s` before the last attempt even starts, and every one of those seconds sits on the token-mint hot path. W1.5 adds the budget that actually caps it, and the caps that bound what a JWKS response may cost.
  - **Four knobs, configurable from birth** rather than hardcoded literals to be retrofitted: `JWKS_TOTAL_DEADLINE_MS` (10000, 100-120000), `JWKS_MAX_RESPONSE_BYTES` (1048576, 1024-10485760), `JWKS_MAX_KEYS` (100, 1-1000) and `JWKS_MAX_CACHE_ENTRIES` (50, 1-1000), each with a matching per-endpoint override (`JwksTotalDeadlineMs`, `JwksMaxResponseBytes`, `JwksMaxKeys`, `JwksMaxCacheEntries`) that wins over the server default and is clamped to the same bounds.
  - **A fourth cap the plan did not ask for.** `JWKS_MAX_CACHE_ENTRIES` was added because the JWKS cache was an unbounded `Map` keyed by a caller-influenced URI, so a large or hostile trust set grew process memory without limit. Past the cap the oldest entry is evicted.
  - **`maxKeys` defaults to 100, not 10**, deliberately: Microsoft states a signing-key cache should hold 10-1000 keys across issuers, so a tight cap would reject a legitimate multi-issuer key set.
  - **Three implementation decisions worth knowing.** The backoff sleep is **clamped to the remaining budget** - an unbounded sleep was the single largest contributor to the worst case. The per-attempt timeout becomes `min(perAttempt, remaining)`, so an attempt cannot outlive the budget either. And a cap breach raises a **non-retryable** `JwksPolicyViolationError`: an oversized body is deterministic, so retrying only burns the deadline and then reports the generic "JWKS unavailable" message, hiding the real cause.
  - **Fail-to-stale still applies when the budget runs out.** Exceeding the deadline is an availability event, so a cached copy is still served (logged with `deadlineExceeded`); only the no-cache case fails closed, and it then names the deadline explicitly. The stale-age ceiling remains **W1.4's** - the fail-to-stale path still serves a cached copy of any age.
  - **TDD, and one false green caught.** The `maxKeys` test initially passed *before* the cap existed: its fixture reused one key three times, so `createLocalJWKSet` rejected on duplicate `kid` and matched the expected `/keys/i` for entirely the wrong reason. Rewritten with three distinct kids so the key set is valid and only the cap can reject it, plus an exact-boundary test so the check cannot be off-by-one.
  - **Coverage:** +17 unit ([egress-policy.spec.ts](api/src/oauth/egress-policy.spec.ts), [external-jwks-validator.service.spec.ts](api/src/oauth/external-jwks-validator.service.spec.ts)) and new live section **`9z-CD`** (+12 assertions: round-trip, bounds rejection for all four caps, a non-vacuous guard asserting all 6 bound cases actually ran, and the documented 1000-key maximum).
  - **Validation:** API unit **158 suites / 4,720**; API E2E **87 / 1,440 on both backends** (parity held); live **1,385 / 1,385**; ESLint **510 warnings - exactly the master baseline**, after typing the one `any` this change introduced rather than raising the ceiling.


### Added
- **Two new supply-chain gates, both prompted by CVE-2026-18446 blocking a push, and the second one immediately found 6 more problems nobody knew about.** Trivy blocks the moment a HIGH advisory has a published fix; the corporate policy forbids consuming any version younger than 7 days. For a fast-moving advisory those two are simultaneously true, so there is a guaranteed window of up to 7 days in which **no compliant action makes the gate green**. That window is designed-in, not bad luck, so it now has a first-class representation instead of an ad-hoc suppression each time. Full procedure + decision diagram: [NPM_SUPPLY_CHAIN_QUARANTINE_POLICY.md](docs/strategy/NPM_SUPPLY_CHAIN_QUARANTINE_POLICY.md) section 11.
  - **(a) `.trivyignore` entries now declare a `Class`, because the two kinds of suppression differ in cadence by 13x.** A `judgment` entry (upstream deprecated its own fix) is open-ended and 90 days is right. A `quarantine-window` entry is a **timing** hold on a fix we intend to take, and inheriting that 90-day default silently turned a one-week wait into a quarter. Quarantine entries must additionally declare `Fixed-version:` and `Fix-available-from:`, are capped at 14 days, and are flagged the moment `Fix-available-from` passes **independent of `Re-check-by`** - so a known-fixed HIGH CVE cannot sit suppressed while its review date is still in the future. `Class:` defaults to `judgment`, so pre-existing entries keep working. +15 tests, including three contract tests that parse the **real committed `.trivyignore`** rather than a fixture, because synthetic fixtures cannot catch a hand-written entry with a typo'd field. **Negative control proven**: neutralising the window cap turns the cap test RED.
  - **[trivyignore-review.yml](.github/workflows/trivyignore-review.yml) moved from weekly to daily.** A weekly cron cannot police a seven-day deadline: a hold expiring on a Tuesday would go unnoticed until the following Monday. The job costs seconds.
  - **(b) New pinned-dependency watcher - [check-dependency-pins.ts](api/src/scripts/check-dependency-pins.ts) + [dependency-pins-review.yml](.github/workflows/dependency-pins-review.yml).** `api/package.json` pins 8 transitive packages via `overrides`, each added to fix a HIGH CVE. An override **freezes** the version so the tree can never float off it, and **Dependabot does not manage the `overrides` block**, so the version pinned as the *fix* for one advisory quietly becomes the *vulnerable* version of the next. `fast-uri` is precisely that: pinned at `3.1.4` to fix an earlier CVE, and `3.1.4` is vulnerable to CVE-2026-18446. Nothing watched for it - Trivy caught it only after building an image, by failing a required status check on a push.
  - **Its first real run flagged 7 vulnerable pins out of 8, and 6 of them were invisible to Trivy** because the image scan gates on HIGH+CRITICAL only: `hono@4.12.25` (4 medium, move to 4.12.34), `@hono/node-server@1.19.10` (2 medium, move to 1.19.13), `fast-uri@3.1.4` (1 high, move to 3.1.5). Measured live against the GitHub Advisory Database, not asserted.
  - **No semver dependency, deliberately.** `semver` exists only as a transitive package with no `@types`, so importing it would be a phantom dependency and adding it directly would require regenerating a lockfile - the exact thing this policy forbids on a corp device. GHSA ranges are a tiny grammar, so the comparator is hand-written and unit-tested on the cases a lexical compare gets wrong (`3.1.10` vs `3.1.9`, `10.0.0` vs `9.0.0`) and on the real GHSA-7p8r-x3mc-p8w7 range strings kept verbatim as fixtures. +16 tests.
  - **Neither check blocks a push.** Both need the network and the GitHub Advisory API; a flaky blocking gate would trade a real security signal for an unreliable one. Both open (and auto-close) a labelled Issue instead.
  - **Time-boxed suppression added for CVE-2026-18446** so `master` is unblocked: `Class: quarantine-window`, `Fixed-version: 4.1.2`, `Fix-available-from: 2026-08-10`. Confirmed inside the window by measurement, not assumption - the corp proxy serves up to `4.1.1` and omits `3.1.5`/`4.1.2` entirely. Reachability recorded: `fast-uri` parses JSON-Schema `$ref` URIs inside `ajv`, reached only via Prisma CLI tooling, never on a request path.
  - **New [scripts/validate-workflows.mjs](scripts/validate-workflows.mjs).** A malformed workflow fails no build - it simply never runs, or silently drops a job - so all 9 are now parsed and structurally checked. Per the house rule that a new checking tool needs its own sanity check, it was proven with a negative control (a job missing `runs-on` -> exit 1) before its green was believed.
  - Also corrected: `.trivyignore` cited its parser as `api/scripts/check-trivyignore.mjs`; the file is [api/src/scripts/check-trivyignore.ts](api/src/scripts/check-trivyignore.ts). Four em-dashes removed from `.trivyignore` and six escaped ones from the checker's report strings.
...
### Fixed
- **`serviceProviderConfig` was documented as "replaced wholesale" in five places; the code has always done a per-key merge.** Surfaced by an operator question about updating an endpoint's schemas and resource types. `mergeProfilePartial()` spreads SPC (`{ ...current, ...partial }`), so top-level capability keys absent from the request **survive** a PATCH - the opposite of what [ENDPOINT_PROFILE_ARCHITECTURE.md](docs/ENDPOINT_PROFILE_ARCHITECTURE.md), [COMPLETE_API_REFERENCE.md](docs/COMPLETE_API_REFERENCE.md), [SCHEMA_TEMPLATES_DESIGN.md](docs/SCHEMA_TEMPLATES_DESIGN.md) D3, the `UpdateEndpointDto` comment and the service docstring all claimed. An operator following the docs would have defensively resent all six capability keys on every settings change.
  - **Why no gate caught it, and the fix for that (R10 - presence is not correctness).** The unit test named `should replace SPC when provided in partial profile` sent **all six** capability keys, so it passed identically under a wholesale replace *and* under a per-key merge - it could not discriminate between the two behaviours it was named after. A merge/replace test is only meaningful when the partial is a **strict subset**. Added `should PER-KEY merge SPC - capability keys absent from the partial survive` (sends one key, asserts the other five persist) and `should REPLACE schemas wholesale - entries absent from the partial are dropped` (locks the asymmetry). **Negative control proven**: flipping the source to `merged.serviceProviderConfig = partial...` turns the new test RED; the original test stays green.
  - **The `authentication` block was absent from every merge table**, and no document stated the rule that actually protects operators: every branch is guarded by `if (partial.X !== undefined)`, so **omitting a section can never wipe it**. Both are now documented as first-class rules.
  - **Docs rewritten around the corrected semantics.** [ENDPOINT_PROFILE_ARCHITECTURE.md](docs/ENDPOINT_PROFILE_ARCHITECTURE.md) gains the canonical five-row table with per-section source line numbers, a Mermaid decision flow, a read-modify-write worked example, the live-captured `400` envelope, and an evidence table. [COMPLETE_API_REFERENCE.md](docs/COMPLETE_API_REFERENCE.md) had **no** schemas/resourceTypes example on the PATCH route at all. [ENDPOINT_CREATION_WIKI.md](docs/ENDPOINT_CREATION_WIKI.md) gains "Recipe 4 - change schemas or resource types on an endpoint that already exists"; the wiki previously covered creation only, which is the exact gap the question came from.
  - **Verified live, at outcome level** - [scripts/verify-patch-merge-semantics.ps1](scripts/verify-patch-merge-semantics.ps1), 17 assertions, 0 failures against a running v0.55.0 node. It measures array contents, preserved key sets and persisted state after a rejection, not status codes. C3 is the assertion that proves the correction: sending only `sort` preserved `patch`, `bulk`, `filter`, `changePassword` and `etag`. C5 confirms a rejected PATCH is atomic - the merge throws before `prisma.endpoint.update()`, so the stored profile is untouched.
  - **Two user-facing docs were ungated** and drifted accordingly ([SCHEMA_ATTRIBUTE_CUSTOMIZATION_GUIDE.md](docs/SCHEMA_ATTRIBUTE_CUSTOMIZATION_GUIDE.md) still advertised v0.40.0). Both, plus [ENDPOINT_PROFILE_ARCHITECTURE.md](docs/ENDPOINT_PROFILE_ARCHITECTURE.md), are now bound to their sources in [.doc-manifest.json](docs/.doc-manifest.json) - the Stage 1.12 gate covers **24** docs, up from 22. Its F5 check immediately rejected a wrong source filename supplied with the new entries, which is exactly the failure mode F5 exists for.
  - Seven schematic JSON fences containing `...` placeholders were re-fenced as `jsonc` per the house rule, restoring the "every ```json block parses" invariant across the touched docs (80 blocks, 0 invalid).
  - **Process defect found while writing this entry, recorded because it nearly caused silent data loss.** The first attempt to add it to this file deleted 143 lines: `read_file` returned a *condensed* view of the 5,443-line CHANGELOG (showing `## [Unreleased]` immediately followed by a `### Fixed` that actually lives at line 154), and that condensed view was used as a literal edit anchor, collapsing the four intervening subsections. Caught by reading the commit's `--numstat` rather than trusting the commit summary. **Standing rule: never derive an edit anchor from a condensed read of a large file - verify the anchor's real line numbers first, and after editing a large file assert the diff shows the expected insertions and ZERO unexpected deletions.**
...
- **Three silent audit-log-loss defects, found because a GREEN test run was printing errors (v0.55.0).** The Prisma-backend E2E suite passed 87 suites / 1,439 tests while emitting 78 `prisma:error` lines to stdout. Nothing failed, because every consumer of those failures is a `catch` that logs and continues. They were investigated only because [ENGINEERING_LESSONS_AND_PATTERNS.md](docs/strategy/ENGINEERING_LESSONS_AND_PATTERNS.md) PA-7 records that process stdout is an unasserted signal channel.
  ...
  - **A timer armed *after* teardown.** `recordRequest` schedules the flush timer, so a request still in flight when shutdown began could arm a fresh timer **after** `onModuleDestroy` had already cleared them, leaving it unowned. Clearing timers is therefore not sufficient - shutdown has to be one-way. A `shuttingDown` flag is now set before any `await` in `onModuleDestroy`, closing the window.
  - **Cross-module shutdown ordering.** `PrismaService` ended the connection pool in its own `onModuleDestroy`, while `LoggingService.onModuleDestroy` flushes drained audit rows through that same pool. Nest runs every module's `onModuleDestroy` in one phase and guarantees **no ordering between modules**, so the pool could close first. Teardown moved to `onApplicationShutdown`, which Nest documents as running after all `onModuleDestroy` hooks settle. `main.ts` enables shutdown hooks, so this fires on SIGTERM - exactly what Azure Container Apps sends on every revision swap.
  - **A failed flush no longer destroys the batch it was carrying.** `flushLogs` drains the buffer with `splice(0)` and only then attempts the insert, so *any* write failure silently discarded up to `LOG_FLUSH_MAX_BUFFER` audit rows. **This is the third time this exact shape has caused silent audit loss in this one service** - the v0.54.85 `requestId` vector, the v0.54.89 `endpointId` vector, and a `Transaction already closed` error from the Prisma driver adapter. Root-causing each instance was not converging, so the durable fix is that a failed write is **requeued at the front and retried** rather than mourned. The requeue is bounded at `10x` the flush buffer; past the cap the oldest are dropped **and counted**, so loss becomes explicit and measurable instead of silent.
  - **Measured outcome, not assumed** (full Prisma E2E run, before -> after):

    | Signal | Before | After |
    |---|---|---|
    | `Cannot use a pool after calling end on the pool` | 52 | **0** |
    | `Auto-prune failed` | 26 | **0** |
    | `prisma:error` (total) | 48 | 26 |
    | Flush failures | 20 | 24 (now **retried**) |
    | **Audit rows actually lost** | up to 20 batches | **0** |

  - **KNOWN REMAINING (not fixed, deliberately not claimed):** the Prisma driver adapter still raises `Transaction API error: Transaction already closed` on ~24 `requestLog.createMany` calls per full E2E run. It is **pre-existing** (60 occurrences measured before any of these changes, 63 after - run-to-run variance) and its real cause is masked by Prisma behind a failed rollback, so it is not visible in the logs. It no longer causes data loss because of the requeue above. Tracked for root-cause work; do not close it on the basis that the symptom is now harmless.
  - **The first two diagnoses were wrong, and re-measuring is what caught them.** The ordering fix alone left all 52 pool errors in place; only attributing the errors by stack frame revealed the two timer defects. A fix is not verified by the fact that it *should* work.
  - +13 tests: [prisma-shutdown-ordering.spec.ts](api/src/modules/prisma/prisma-shutdown-ordering.spec.ts) (5, written RED - failed with `Property 'onApplicationShutdown' does not exist`) and [logging-shutdown-race.spec.ts](api/src/modules/logging/logging-shutdown-race.spec.ts) (8, asserting no DB call escapes shutdown, pre-shutdown rows still drain, requeue preserves order, retry persists, and the bound reports dropped rows).

### Changed
- **Version unified at 0.55.2 across every manifest.** `api/package.json`, `web/package.json` and both lockfiles' two version fields now agree, and all 22 version-bearing user-facing doc headers were re-synced by `npm run docs:freshness:fix`. The bump is a **patch**: everything since 0.55.1 is documentation, tests and source comments with zero runtime behaviour change, matching this repo's practice of patch-bumping within a line. The root `package.json` (`scimserver-repo-tooling`, private) stays at `0.0.0` deliberately - it is tooling-only and ships nothing.
  - **The three commits that landed the PATCH-merge-semantics correction shipped without a version bump, which is a Stage 6.1 miss.** 0.55.2 is the corrective bump; the work itself is unchanged.
  - **Lockfile version fields were edited surgically, not regenerated.** Changing the two `"version"` lines in a lockfile header is not dependency resolution, so it is safe on a corp-managed device where regeneration is not (see [NPM_SUPPLY_CHAIN_QUARANTINE_POLICY.md](docs/strategy/NPM_SUPPLY_CHAIN_QUARANTINE_POLICY.md)). Asserted before commit: the diff is exactly `1/1` per `package.json` and `2/2` per lockfile, every `resolved` host is `registry.npmjs.org`, every `integrity` is sha512, and **all 46 changed lines contain the new version string** - so nothing unrelated drifted.
  - New helper [scripts/set-version.ps1](scripts/set-version.ps1) performs the edit by line index rather than text match, because the lockfiles are large enough that a condensed read is not trustworthy as an edit anchor (the failure that cost 143 CHANGELOG lines earlier in this same change).
- **Version 0.55.0.** First release of the 0.55 line, cut from master after merging #140, #139 and #138.
- **Dependency PR merge register** ([docs/strategy/DEPENDENCY_PR_MERGE_REGISTER.md](docs/strategy/DEPENDENCY_PR_MERGE_REGISTER.md)). Dependabot #128, #136 and #137 are **postponed, not closed**: each proposes at least one version the corporate npm feed proxy does not serve, because it enforces a 7-day minimum release age by **omitting** too-new versions entirely. GitHub runners resolve from `registry.npmjs.org`, so those PRs are fully green in CI while being uninstallable on every corp-managed developer machine. #138 (GitHub Actions only, no npm) was merged.
- **New gate `docs: patterns pie matches catalog`** ([scripts/check-patterns-pie.mjs](scripts/check-patterns-pie.mjs)). Merging two branches that had each appended patterns produced a conflict where **both sides' counts were wrong** (22 and 14, against a real 24). A stale pie renders perfectly - Mermaid has no idea what the numbers mean - so `docs:mermaid:render` passes on any integers and nothing else reads them. Proven by [a negative control](scripts/check-patterns-pie.negctl.mjs) covering both real wrong values plus a per-category skew that still sums correctly, which a naive title-vs-sum check would wave through. Its own first run failed on a CRLF-anchored regex, which is exactly why the control exists.
- **Stale `node:25-alpine` reference removed** from the Stage 6 gate name in [scripts/run-all-gates.ps1](scripts/run-all-gates.ps1). Node 25 went EOL 2026-06-01 and the rule it described was corrected days earlier (lockfiles are regenerated in CI, never on a corp device). A gate whose *name* describes a procedure since proven impossible is a trap for the next reader. Every other Node reference in the repo (4 Dockerfiles, 5 workflows, `engines`, compose, standalone launcher, docs) verified already on 24 LTS.

### Added
- **Per-endpoint TLS version policy: CLOSED as incident-only, and the decision record is now on `master`.** The operator confirmed on 2026-07-30 that the TLS-1.3-only ask was an **incident**, not a product capability, which closes every phase of the proposal. [docs/tls/PER_ENDPOINT_TLS_POLICY_OPTIONS.md](docs/tls/PER_ENDPOINT_TLS_POLICY_OPTIONS.md) + [scripts/tls-sni-policy-probe.mjs](scripts/tls-sni-policy-probe.mjs) are cherry-picked from `feat/per-endpoint-tls-policy` anyway, because the durable value is the **"why not"**, not the plan: TLS version is negotiated before any HTTP exists, so the URL path that identifies an endpoint today is invisible at handshake time - an answer that holds permanently, for any future asker. The doc now carries a per-option disposition, with **O3 (in-process SNI router) dropped permanently** rather than deferred, since its gate ("a customer needs 1.3-only without a gateway") can no longer open.
  - **The probe records a silent-no-op API.** Setting `minVersion: 'TLSv1.3'` on a per-SNI `SecureContext` is accepted by Node, throws nothing, logs nothing, and **enforces nothing** - the protocol bounds are fixed on the connection before the SNI callback runs. Anyone implementing per-endpoint TLS the obvious way would ship a switch that appears to work and enforces nothing. The SNI-router variant does produce a genuine `protocol_version` alert. Both measurements carry negative controls, and the probe was **re-run on `master`** as part of landing it (`node scripts/tls-sni-policy-probe.mjs`, exit 0, both controls green) rather than trusted from the branch.
  - The branch's TLS 1.3 standalone stack (`docker/tls13/`) was deliberately **not** brought along - it is a form-factor harness, not a design record, and an incident-only answer does not justify a second Docker stack on `master`. It stays on the branch, recoverable if the question ever returns.
  - **One item flagged as worth reconsidering on its own merits:** O7 (record the negotiated TLS version per request) was never really about this incident - it answers "what does this client actually negotiate?" for every integration, permanently, and costs nothing. Dropped only because it has no requester today.
- **Four new patterns in [docs/strategy/ENGINEERING_LESSONS_AND_PATTERNS.md](docs/strategy/ENGINEERING_LESSONS_AND_PATTERNS.md)**, all earned by the v0.54.88/89 chain. **PA-6:** a test double must fail the way the real dependency fails, or the RED is vacuous - a permissive Prisma mock passed **18 of 19 against unfixed code**, twice in one session. **PA-7:** the process's own stdout is an unasserted signal channel, so anything the application catches-and-logs is invisible to every gate - both defects this release were found by reading the server log through a fully green 1,368-assertion run. **PA-8:** prove a new live-test section by running it against a deliberately reverted build, so it fails for the right reason and only that one. **PC-3:** a `try/catch` spanning a multi-step fallback can make later steps unreachable, and the catch disguises the dead path as a clean degrade. **PD-2** (fix the class - enumerate every column of the constrained type - not the instance) is promoted straight to a hard rule on its 2nd high-severity escape.

### Security
- **New Security Gate Map row: forwarded-header trust.** [api/src/main.ts](api/src/main.ts) sets `app.set('trust proxy', true)`, so every `X-Forwarded-*` header from any source is trusted. That is correct for its stated purpose (`req.protocol` / `req.hostname` / `req.ip` behind Container Apps ingress) and **not** acceptable as the basis of a security decision. Surfaced while designing a per-endpoint TLS floor: the control would have been decoration, because `X-Forwarded-TLS-Version: TLSv1.3` is caller-supplied. Recorded as **KNOWN-OPEN** with a standing rule - any future feature reading a forwarded header as a security input MUST require an explicit trusted-terminator allowlist (peer IP checked), default the control to OFF, and report itself **unavailable** when no trustworthy signal exists, never falling back to trusting the header. A control that cannot be evaluated must never look like a control that passed.
- **New Security Gate Map row: non-uuid input reaching a `@db.Uuid` column**, now covered by one shared guard across both affected columns (see the v0.54.89 entry below).

### Security
- **The Trivy gate was red on every branch for days, and dependency bumps could never have fixed it (v0.54.90).** `build-test.yml` had been failing on `feat/wif`, `feat/subattribute-nesting-flag`, all three Dependabot branches, and every new branch since - and `feat/subattribute-nesting-flag` was **merged to master while red**. A gate that fails on every branch has stopped discriminating between a good change and a bad one; the repo was already routinely merging past it.
  - **Triage found the reason nothing worked: 5 of the 7 findings, including the only CRITICAL, were npm's OWN bundled dependencies**, not ours. The distinguishing evidence is the image path Trivy prints - `usr/local/lib/node_modules/**npm**/...` is the base image's npm, while `app/node_modules/...` is our tree.

    | Package | Version | Owner | Findings |
    |---|---|---|---|
    | `tar` | 7.5.15 | npm-bundled | CVE-2026-59873 (**CRITICAL**, gzip-bomb DoS), CVE-2026-59874 |
    | `brace-expansion` | 5.0.6 | npm-bundled | CVE-2026-13149, CVE-2026-14257 |
    | `undici` | 6.26.0 | npm-bundled | CVE-2026-12151 |
    | `fast-uri` | 3.1.2 | **ours** (api) | CVE-2026-13676, CVE-2026-16221 |

    No `package.json` or lockfile change can reach a package the base image ships inside npm itself, which is exactly why the Dependabot PRs raised against this gate also failed. Chasing them would have burned arbitrarily many cycles on an unfixable target.
  - **Fix: delete the tool.** npm is a **build-time** dependency; the running container starts with `node dist/main.js` and never needs a package manager. The runtime stage now removes `npm`, `npx` and `yarn`, which removes their bundled CVEs outright and shrinks the image. The single former usage - `npx prisma migrate deploy` in [api/docker-entrypoint.sh](api/docker-entrypoint.sh) - is now `node node_modules/prisma/build/index.js migrate deploy`, which is **equivalent rather than a workaround**: prisma's `bin` field is `{"prisma": "build/index.js"}`, so `npx prisma` already resolved to that same file, the CLI is grafted into the image by the `prod-deps` stage, and the standalone Windows launcher has always used this form.
  - **The container smoke test is the important half of this change.** Until now the only post-build gate was Trivy, which scans a **filesystem** and never starts the process - so removing something the entrypoint needed would have produced a *greener* scan and a container that cannot boot. "Fewer CVEs" must never be purchasable with "does not run". [build-test.yml](.github/workflows/build-test.yml) now boots the built image (inmemory backend, no database required) and requires `/scim/health` to report `status=ok`, requires `/scim/v2/ServiceProviderConfig` to carry its SCIM schema URN (an outcome, not merely an open port), and asserts `npm`/`npx` are absent - verifying the change under test directly instead of inferring it from the scan result.
  - **`fast-uri` (ours, 2 HIGH) is fixed too, and getting there exposed a second problem.** It reaches the production tree via `@prisma/client -> prisma -> @prisma/dev -> @prisma/streams-local -> ajv -> fast-uri`, so the fix is an `overrides` pin to **3.1.4** (published 2026-07-19, 11.7 days old - comfortably outside the corporate 7-day quarantine, which was never the obstacle). **With this, Trivy on the runtime image reports 0 findings, down from 7.**
    - **A corp-managed device cannot produce a committable lockfile.** An earlier note here claimed *"the npm registry is TLS-blocked on this workstation"*. That was wrong - npm works fine through the corporate feed proxy; the original probe had passed `--registry https://registry.npmjs.org` and so measured the deliberately egress-blocked public endpoint rather than the configured one. The real problem is narrower and worse: the proxy serves only a legacy `shasum` and no `integrity`, so regenerating a single entry rewrote its `resolved` to an internal `ms-feed-12.pkgs.visualstudio.com` host **and downgraded its `integrity` from sha512 to sha1** among 725 sha512 siblings. Committing that would publish an internal endpoint from a public repo and weaken the lockfile's own tamper-evidence, and it cannot be hand-patched - the correct sha512 is unobtainable on the device, and hand-writing an integrity hash is never acceptable.
    - **New capability: [regen-lockfile.yml](.github/workflows/regen-lockfile.yml).** Regenerates lockfiles on a clean runner against the public registry and returns them as an artifact. It holds `permissions: contents: read` and deliberately does not push, so a lockfile change stays a reviewed human act. It hard-fails on both measured contamination modes and prints the publish age of every changed package. Stage 6.1 has said "regenerate in CI" for a while without providing a CI path; this is that path. The lockfile shipped here came from it, and the diff is exactly five lines.
    - **New gate: `supply chain: lockfile provenance`.** [scripts/check-lockfile-provenance.mjs](scripts/check-lockfile-provenance.mjs) rejects any lockfile entry with a non-public `resolved` host or a non-sha512 `integrity`. This closes a gap **no existing gate could see** - eslint, tsc, Trivy, CodeQL and both infra audits all pass happily on a lockfile pointing at an internal feed with sha1 hashes. It runs at pre-push **and** in CI before `npm ci`, because pre-push is blind to Dependabot PRs, fork PRs and `--no-verify`, and four of the six then-open PRs were Dependabot lockfile changes. Proven rather than assumed: [check-lockfile-provenance.negctl.mjs](scripts/check-lockfile-provenance.negctl.mjs) injects both real contamination modes and asserts rejection of each, plus acceptance of the untouched file - 3/3 hold. Same discipline as the base-image gate above.
    - **The publish-age reporter was itself green-but-blind on its first run**, printing only a header because it parsed `git diff -U0`, where the enclosing package key is never in the hunk. Rewritten to diff the two lockfile maps directly, which cannot be silently empty. Caught in the same run that proved the contamination checks work.

### Security
- **The shipped image was running an end-of-life Node runtime (v0.54.86).** Prompted by the question "have we moved to node-25?" - we had, and it was the wrong direction. Verified against [nodejs.org/en/about/previous-releases](https://nodejs.org/en/about/previous-releases) and [endoflife.date/nodejs](https://endoflife.date/nodejs): **Node 25 reached EOL on 2026-06-01**. `node:25-alpine` therefore pins **v25.9.0, frozen since 2026-04-01**, and a live probe of `/scim/admin/version` on dev confirmed it was serving exactly that - an unpatched runtime for ~2 months, through the July 2026 Node security releases. Node's own guidance: *"Production applications should only use Active LTS or Maintenance LTS releases."* The root `Dockerfile`, `Dockerfile.optimized` and `Dockerfile.ultra` are back on **`node:24-alpine`** (Krypton LTS, maintenance to 2028-04-30).
  - **The original bump was speculative and never revisited.** Commit `e7c78673` (2026-05-01) moved 24 -> 25 stating *"May resolve CVE-2026-33671 (.trivyignore picomatch entry) automatically; will revisit after Trivy gate runs in CI."* The revisit never happened, and Node 25 went EOL a month later. Probing both images shows the bump was not merely useless but **actively counterproductive**: `node:24-alpine` ships npm 11.13.0 with **picomatch 4.0.4 (patched)**, while `node:25-alpine` ships npm 11.12.1 with **picomatch 4.0.3 (still vulnerable)**. The change made to fix that CVE was the only one of the two that still carried it.
  - **`.trivyignore` CVE-2026-33671 suppression removed.** Its own documented removal condition - picomatch `>= 4.0.4` in the base image - is now met and verified directly in the image. Its `Re-check-by` was 2026-07-30, so this closes it one day early rather than auto-filing a staleness issue.
  - **Why no gate caught it.** Trivy scans the image for **CVEs**, never for the **support status** of the base image, so an EOL runtime with no CVE filed yet is invisible to it. Compounding that, every other surface (all 4 workflows, `api/package.json` engines, `api/Dockerfile`) was already on 24, so a check against the wrong file concluded the repo was consistent - which is precisely what the 2026-05-17 Stage X.2 intake did, "resolving" the drift by amending the instruction to 24 while the **shipped** image stayed on 25. New **Stage 1.10** gate [scripts/audit-base-images.ps1](scripts/audit-base-images.ps1) fails any `Dockerfile*` pinning a non-LTS Node major.
  - **The gate was proven before being trusted.** A negative control (a temporary `Dockerfile` on node:25) exposed a real bug in the first version: `Get-Content` returns a **scalar string** for a single-line file, so `$lines[0]` indexed the first *character* rather than the line, and the gate reported PASS on input it was written to reject. Fixed with `@()`, then re-proven: **fails on node:25, passes on node:24**. Same lesson as the `9z-BZ` byte-array escape - a gate that has never been observed to fail is not evidence of anything.
  - **Standing-rule correction.** Stage 6.1 claimed lockfiles are regenerated in node:24-alpine because that *"matches the deployed runtime in `api/Dockerfile`"*. `api/Dockerfile` is **not what ships** - CI builds the **root** `Dockerfile` (`publish-ghcr.yml` -> `file: ./Dockerfile`). The justification was wrong even while the number was right, which is what let the 2026-05-17 audit reach a confident wrong conclusion. Corrected, along with two prompt files still instructing node:25.

- **Audit-log batch loss via a client-controlled `X-Request-Id` (v0.54.85).** Found while triaging the pre-existing Prisma E2E failures. `RequestLog.requestId` is a `@db.Uuid` column, but the correlation middleware runs **before guards** and propagated the caller's `X-Request-Id` header verbatim into it - so any caller, authenticated or not, could put arbitrary text on that path. That is not cosmetic: rows are flushed in batches of up to 50 via a single `createMany`, the batch is **drained from the buffer before the insert is attempted**, and a failure is only logged - so **one poisoned row silently discards up to 49 unrelated audit-log rows**, and a caller could trigger it deliberately to erase the record of their own requests.
  - **Fix at the persistence boundary, not the middleware.** There are two consumers of `X-Request-Id` with different constraints: tracing (the response-header echo and the in-memory ring buffer, which take the value verbatim - a contract `log-config.e2e-spec.ts` explicitly locks, and correct, since tracing ids are not required to be UUIDs) and persistence (the `uuid` column). Normalising in the middleware would satisfy one by breaking the other, which is exactly what a first attempt did. New [storable-request-id.ts](api/src/modules/logging/storable-request-id.ts) nulls a non-UUID id **only** when the row is built, so tracing keeps the caller's id and the column can never be poisoned. Applied to **both** persistence backends so InMemory cannot match rows Prisma would miss - that divergence is what hid this.
  - +8 unit tests covering near-miss UUIDs, hostile values (SQL-shaped, traversal, 10 KB, script tags) and array-valued headers.

### Security
- **The audit-log batch-loss vector was only half-closed - `endpointId` was still open (v0.54.89).** v0.54.85 closed the `requestId` path but stopped there. `RequestLog.endpointId` is the SECOND `@db.Uuid` column fed from caller-controlled input: [request-logging.interceptor.ts](api/src/modules/logging/request-logging.interceptor.ts) derives it with `originalUrl?.match(/\/endpoints\/([^/]+)/)?.[1]` - a raw path segment, unvalidated. So `GET /scim/endpoints/<anything>/Users` writes `<anything>` into a uuid column, and the consequence is identical to the one already documented: rows flush in batches of up to 50 through one `createMany`, the batch is **drained from the buffer before the insert is attempted**, and a failure is only logged - so **one poisoned row silently destroys up to 49 unrelated audit-log rows**. The interceptor wraps unauthenticated traffic too (401s are logged), so this needs no credentials.
  - **Found by reading the server log, not by a gate.** It surfaced as `prisma:error ... invalid input syntax for type uuid: "by-name"` during a Prisma live run while validating an unrelated fix. Both live vectors are ordinary routes, not exotic probes: `/scim/admin/endpoints/by-name/<name>` yields `by-name`, and a query string surviving the path match yields `<uuid>?view=summary`. **Every gate was green while this was happening** - the driver error goes to stdout, no test asserts on stdout, and the discarded rows are invisible by construction.
  - **One guard, both columns.** `storable-request-id.ts` is renamed [storable-uuid.ts](api/src/modules/logging/storable-uuid.ts) - the name had drifted the moment it guarded a second column - and now exports `toStorableRequestId` + `toStorableEndpointId` over one shared implementation, so the two can never diverge. Applied to **both** persistence backends, for the same parity reason as the original fix. The duplicated UUID regex introduced earlier in this release was also folded into the existing leaf predicate [shared/uuid.ts](api/src/shared/uuid.ts), which now backs all four call sites (`getLog`, `resolveUserDisplayName`, the `requestId` list filter, and the storable guards).
  - **The RED proves the destruction, not just the value.** One assertion buffers 11 rows, poisons exactly one, and asserts the `createMany` **resolves with `count: 11`** - against the unfixed service the whole promise rejects, which is the batch loss itself reproduced in a test. +10 unit ([storable-endpoint-id.spec.ts](api/src/modules/logging/storable-endpoint-id.spec.ts)) with a negative control asserting the mock really does raise P2023.
  - **Two existing fixtures had to change** (`'ep-redact'`, `'A'`/`'B'`): they used placeholder endpoint ids that only ever worked because the column was unguarded. Both now use real UUIDs, which is also the shape production always has.

### Fixed
- **Log display-name resolution: the `userName` fallback was unreachable on Prisma (v0.54.89).** `resolveUserDisplayName` looks up by `scimId` (`@db.Uuid`) and falls back to `userName` (`@db.Citext`). Because the fallback only runs when the first lookup returns **null**, and a userName-shaped identifier makes that first query **throw** P2023 instead, control jumped straight to the outer `catch` - so the fallback could never execute and every userName-identified log row silently kept its raw identifier. The `try/catch` turned a functional defect into what looked like a clean degrade-to-null, which is why no gate ever saw it. Guarded with the shared `isUuid` predicate so the uuid column is only queried when the identifier could actually be one.
  - **The live section was proven a real gate, not assumed to be one.** Running the new `9z-CC` against a deliberately guard-removed build fails **T3 and T4** (`found 0 resolved of 2`) while T1/T2/T5 stay green, so the section fails for the right reason and only that reason. +8 unit ([logging-display-name-resolution.spec.ts](api/src/modules/logging/logging-display-name-resolution.spec.ts)) asserting the OUTCOME (`listLogs` -> `reportableIdentifier`) rather than the private helper, +5 live `9z-CC` (backend-aware, since resolution is a Prisma-branch feature).
  - **Measured effect:** `prisma:error` lines emitted during a full live run went from **5 to 0**, and the driver-error spam on every `GET /scim/admin/logs` is gone.
  - **Validation:** API unit **154 suites / 4,660**; API E2E **87 / 1,439**; ESLint 0 errors / 504 warnings (new specs contribute none after cleanup). Live **1,373 / 1,373 on real Prisma** and **1,373 / 1,373 on InMemory**.

- **`GET /scim/admin/logs/<anything-not-a-uuid>` returned HTTP 500 on every Prisma estate (v0.54.88).** Surfaced by the TLS 1.3 standalone exercise as an incidental finding and confirmed live on dev, proudbush and calmsand. There is no `logs/stream` route, so `GET /scim/admin/logs/stream` fell through to `@Get('logs/:id')` with `id = 'stream'`; `RequestLog.id` is `@db.Uuid`, so Postgres raised `PrismaClientKnownRequestError` **P2023** ("Inconsistent column data: Error creating UUID"), which escaped the handler unhandled as `{"detail":"Internal server error","status":"500"}`. The handler already intended a 404 - `if (!log) throw new NotFoundException('Log not found')` - it simply never got the chance to run.
  - **Two defects in one.** A malformed path param produced a 500 instead of a 404, and an unhandled persistence error reaching the wire is a disclosure risk in its own right. It was also a **cross-backend parity gap** of the Finding-B class: the InMemory branch does `rows.find(r => r.id === id)` and returned a clean 404 for the identical request, so the divergence was invisible to every InMemory-mode gate. Measured before the fix: `inmemory -> 404`, `prisma -> 500`.
  - **Fix at the boundary, scoped to the actual blast radius.** `LoggingService.getLog` now resolves a syntactically impossible id to `null` before querying, so no malformed input reaches the database and both backends agree. A live probe of the sibling routes (`/scim/admin/endpoints/:id`, `.../overview`, `.../credentials`, `/scim/endpoints/:id/Users`, `.../Users/:id`) confirmed **all already returned 404**, so this is deliberately a local guard rather than a new shared pipe - a repo-wide interceptor would be speculative generality for a one-route defect.
  - **The RED had to be made honest first.** The initial spec passed 18/19 against the *unfixed* service, because a `null`-resolving Prisma mock cannot reproduce a Postgres cast failure - the R10 presence-not-correctness trap in test-double form. The mock now rejects with a real P2023-shaped error and carries its own negative control asserting it does so; the suite then failed **8** tests for the right reason before the one-line guard turned it green.
  - **Coverage:** +20 unit ([logging-get-log-nonuuid.spec.ts](api/src/modules/logging/logging-get-log-nonuuid.spec.ts), both backends, including a parity block), +8 E2E ([admin-log-malformed-id.e2e-spec.ts](api/test/e2e/admin-log-malformed-id.e2e-spec.ts), asserting the 404 **and** the SCIM envelope **and** that no `Prisma` / `P2023` / `Inconsistent column data` string reaches the client), and new live section **`9z-CB`** (16 assertions) carrying a **positive control** - a real log id must still resolve - so the guard cannot pass by rejecting everything.
  - **Validation:** API unit **152 suites / 4,642**; API E2E **87 / 1,439**; ESLint 0 errors / 504 warnings (changed files contribute none). Live suite **1,368 / 1,368 on a real Prisma backend** (compiled build against PostgreSQL 17) **and 1,368 / 1,368 on InMemory** - the same numbers on both backends, which is the property the bug broke.

- **The 5 pre-existing Prisma E2E failures - root-caused, not silenced (v0.54.85).** They were **proven pre-existing** first (reverting `api/src` + `api/test` to `6f318ef8` reproduced them identically), then fixed:
  - **Two specs encoded the same mistake - treating an eventually-durable write as immediately durable.** `request-body-capture` slept a fixed 300 ms then queried once, but the log buffer flushes on a **3 s** timer, so the row was reliably absent. `log-config` called the force-flush endpoint then queried once, which looks safe but is not: a request's row is enqueued **asynchronously after its response is sent**, so the flush can run before the row it is meant to flush was ever buffered. New shared [log-wait.helper.ts](api/test/e2e/helpers/log-wait.helper.ts) polls to a deadline, forcing a flush per attempt, so neither bug can be reinvented.
  - **A cross-backend parity trap.** `log-config` used `X-Request-Id: e2e-flush-<timestamp>`, not a UUID. On Prisma the `requestId` filter swaps a non-UUID for an all-zeros sentinel that matches nothing; on InMemory it is a plain string compare that works. The test now uses `randomUUID()` - and the underlying hazard is closed by the Security entry above.
  - The E2E harness sets `LOG_FLUSH_INTERVAL_MS=100` (a **W1.7b knob**, its first real payoff) so polls converge in one or two attempts instead of waiting out a 3 s timer. Buffer size is left at default so batching stays representative.
  - **Validation:** API E2E **prisma 85/1,422 and inmemory 85/1,422 - now identical**, restoring cross-backend parity. API unit **150 suites / 4,599**. ESLint 504 warnings, unchanged.
- **Visual-regression: the Schemas snapshot now carries the same data-drift tolerance as its siblings.** The dev-deploy Playwright gate failed on `Endpoint detail - Schemas tab` (28% pixels). Inspecting the diff showed two non-regressions - the `Credentials` -> `Connect` tab rename shipped in v0.54.46, and a masked panel ~305 px shorter on dev - and the real cause: `Users` and `Overview` both carry `maxDiffPixelRatio: 0.03` for exactly this reason and **Schemas was the only one that never got it**, which is why its baseline sat at 2026-06-03 while theirs were refreshed 2026-07-23. Tolerance applied, baseline regenerated after the written classification. Playwright vs dev **192 passed / 7 skipped / 0 failed**.

### Added
- **The deployment infrastructure doc is now a gated living document.** [docs/DEPLOYMENT_INFRASTRUCTURE_AND_FORM_FACTORS.md](docs/DEPLOYMENT_INFRASTRUCTURE_AND_FORM_FACTORS.md) is the canonical record of every infra element, all six form factors, and the measured state of all three live Azure estates. Documentation had **never been gated in this repo**, so its facts could drift from reality with nothing failing - and a confidently wrong infra doc is worse than none, because it terminates an investigation in the wrong place. That is not hypothetical: it is exactly how the 2026-05-17 security intake "resolved" the Node base-image drift by checking `api/Dockerfile` (which does not ship) instead of the root `Dockerfile` (which does), leaving an EOL runtime in production for two more months.
  - **New Stage 1.11 gate** [scripts/audit-deployment-doc.ps1](scripts/audit-deployment-doc.ps1) with four checks: **C1** an infra path changed => the doc must change in the same diff; **C2** the `**Last verified:**` header is within 90 days; **C3** every `Dockerfile*`, `docker-compose*.yml` and `infra/*.bicep` on disk is named in the doc; **C4** (`-Live`) every reachable estate runs a supported Node LTS and reports a version the doc mentions. "Infrastructure" is defined mechanically, not by judgement - the watched set is `Dockerfile*`, `docker-compose*.yml`, `infra/**`, `.github/workflows/**`, `api/docker-entrypoint.sh`, the two Container App templates, and the six provisioning/promotion/verification scripts.
  - **C3 makes the gate self-extending.** Add a new Bicep template or Dockerfile and the gate starts demanding documentation for it on the next run, with no edit to the gate itself. This is the structural answer to "keep it current across the board" - coverage grows with the infrastructure rather than depending on someone remembering.
  - **C4 closes the deployed-artifact half of the Node-LTS rule.** Stage 1.10 gates the **source** Dockerfile; C4 gates **what is actually running**. Both read one LTS table, extracted to [scripts/node-lts.ps1](scripts/node-lts.ps1), so the two halves cannot drift apart - a second copy is precisely how they would. This closes finding **G3** (both prods still serving EOL Node v25.9.0 while dev is on v24.18.0) at the gate level; the finding itself stays open until the prods are promoted.
  - **Doc changes:** new **Section 0.1 maintenance contract** stating the rule ("any change to the infrastructure updates this document in the same commit"), the mechanical definition of infra, a per-change-type map of which sections to revisit, four standing rules for facts (measured never remembered; verify against the artifact that ships; record gaps rather than silently fixing them; promote recurring gaps to hard gates), and a Mermaid flow of the gate. Section 14 now marks the doc-rot and G3 gaps **closed/applied** and G10/G11 **partly closed**.
  - **Proven with negative controls** for all three static checks before being trusted, per the standing rule that a gate never observed to fail proves nothing.

### Added
- **W1.7c: `GET /scim/admin/runtime-config` - W1.7 is now complete (v0.54.84).** The surface that answers "what configuration actually took effect?" with a fact instead of "what do you have set?". Returns every tunable with its effective value, provenance (`env` / `legacy-env` / `default`), published bounds, and - the case an operator most needs - whether it had to be **clamped** and what was originally `requested`, plus the cross-key `invariantWarnings[]`. Admin-gated (no `@Public`) and `Cache-Control: no-store`, since it reflects live process state.
  - **Security by construction, not by filtering.** The payload is assembled from `RUNTIME_CONFIG_SPECS`, a fixed table of numeric and byte-size tuning values. `DATABASE_URL`, `OAUTH_CLIENT_SECRET`, `SCIM_SHARED_SECRET`, `JWT_SECRET`, `JWKS_HOST_ALLOWLIST`, `CREDENTIAL_KEK` and `OAUTH_JWT_PRIVATE_KEY` are not in that table and so cannot appear - only values the table names are ever read. Locked by key-allowlist assertions at the unit, E2E **and** live layers (asserting the FULL key set, not that a field exists).
  - **Coverage:** +9 unit ([runtime-config.controller.spec.ts](api/src/modules/scim/controllers/runtime-config.controller.spec.ts)), +8 E2E ([admin-runtime-config.e2e-spec.ts](api/test/e2e/admin-runtime-config.e2e-spec.ts)), and new live section **`9z-BZ`** (12 assertions) covering the auth gate, `no-store`, the envelope allowlist, the schema URN, all four groups, self-consistency (**every** effective value inside its **own** published bounds), valid provenance on every setting, no secret leakage, and direct X15-F2 / X15-F3 closure probes.
  - **Validation:** API unit **149 suites / 4,591**; API E2E **85 / 1,422**; build clean; ESLint 504 warnings (byte-identical baseline). Local live suite **1,341 / 1,341 PASS**. Boot log verified on a real node, including `poolMax=7(env)` proving an override lands end-to-end.
  - **An RCA worth reading ([EXECUTION_ISSUES_AND_RCA.md](docs/auth/EXECUTION_ISSUES_AND_RCA.md) section 10.0).** The smoke-run of `9z-BZ` caught that `Invoke-WebRequest` returns `.Content` as a `Byte[]`, and piping a byte array into `ConvertFrom-Json` **enumerates** it - so the parse yielded 1,732 integers. Four assertions failed loudly, but **three passed vacuously**, including the **secret-leak check**, which searched a byte array and found nothing because there was nothing to search. Every loop-based assertion now requires a non-zero count and prints what it examined (`all 15 effective values...`, `...in the 1732-char payload`). This is the live-test instance of the standing rule that presence is not correctness: *an assertion that iterates a collection must also assert the collection is non-empty*, or "no violations found" and "nothing was examined" are indistinguishable.

### Added
- **W1.7b completed: the logging-flush and SCIM-pagination call sites now read the runtime config too (v0.54.83).** The previous commit specified, bounded and logged these knobs but left their call sites reading their own constants - a half-wired setting is exactly the drift the design gate warns about, because the boot log would then advertise a value the code does not actually use. Both are now genuinely wired:
  - **Log buffering** ([logging.service.ts](api/src/modules/logging/logging.service.ts)): the `FLUSH_INTERVAL_MS = 3_000` / `MAX_BUFFER_SIZE = 50` statics become instance fields resolved from `LOG_FLUSH_INTERVAL_MS` / `LOG_FLUSH_MAX_BUFFER`. This is a throughput-versus-durability tradeoff that genuinely moves with the deployment: a busy production replica wants a **shorter** interval (bounding crash-loss to ~1 s) with a **larger** batch (fewer round-trips, less connection-pool pressure) than a developer laptop.
  - **Pagination ceilings** ([scim-constants.ts](api/src/modules/scim/common/scim-constants.ts)): `DEFAULT_COUNT` / `MAX_COUNT` resolve once at import from `SCIM_DEFAULT_COUNT` / `SCIM_MAX_COUNT`, clamped to 1-1000 so `MAX_COUNT` can never be configured into a memory problem. **This is the server floor only** - the per-endpoint ServiceProviderConfig `filter.maxResults` cascade still layers on top via `resolveNumericLimit`, which is the RFC 7644 section 3.7 mechanism, so the tier model is preserved rather than bypassed.
  - **Value-preserving:** every default is the value that was previously hardcoded, so an unconfigured deployment behaves identically.
  - **Validation:** TDD RED-first (2 of the 5 new pagination assertions confirmed failing before the wiring). API unit **148 suites / 4,582** (was 147 / 4,577); API E2E **84 / 1,414** unchanged; build clean; ESLint **504 warnings, byte-identical to the pre-change count**, so zero new warnings - measured rather than inspected.

### Added
- **W1.7a + W1.7b: the runtime perf/resilience values are now configurable settings, and two real defects are closed with them (v0.54.82).** The X15 audit established that every timeout, pool size, buffer and cap here is a function of the deployment (container CPU, DB tier, replica count, network path to the IdP) - X11 proved it by measuring the SAME WIF mint at 2,161 ms and at 92 ms. New [runtime-config.ts](api/src/bootstrap/runtime-config.ts) publishes `RUNTIME_CONFIG_SPECS`: 15 settings across `http` / `database` / `logging` / `scim`, each with its env key, default and clamp range, so the table doubles as the documentation. `resolveRuntimeConfig` is pure (injected env getter, no `process.env` access, never throws), **falls through to the documented default on invalid input** rather than failing startup, **clamps every value** so no configuration path can disable a bound, and **records provenance** (`source` = `env` / `legacy-env` / `default`, plus `requested` and `clamped` when it had to clamp). It deliberately mirrors the existing `oauth/egress-policy.ts` shape - no new pattern is introduced. Four **cross-key invariants** (transaction timeout inside the request timeout, pool acquire inside the request timeout, default page size within max, keep-alive buffer under keep-alive) emit WARNINGS, never a startup failure: a server that refuses to boot on a tuning mistake is worse than one that boots and says so. `formatRuntimeConfigLines` prints one `[Config]` line per group at boot naming every effective value and where it came from, because a configurable system without that is strictly harder to operate than a hardcoded one.
  - **X15-F2 CLOSED - `REQUEST_TIMEOUT_MS` did not bound request duration.** Node's `http.Server` has four distinct timeouts and [main.ts](api/src/main.ts) set one and a half of them: `setTimeout()` (which is **socket inactivity**, so a client dribbling a byte every 60 s never trips it) and `keepAliveTimeout`, while `server.requestTimeout` and `server.headersTimeout` were **never set** - leaving request duration bounded by Node's implicit **300 s**, not the 120 s the operator believed they had configured. All four are now set explicitly (plus `keepAliveTimeoutBuffer` where the runtime has it), and `keepAliveTimeout` is **decoupled** from the request timeout because its correct value is a function of the upstream ingress idle timeout - if it is shorter than the proxy's idle timeout the proxy reuses a socket the server is closing and the client sees a 502 / `ECONNRESET`. **Value-preserving:** `REQUEST_TIMEOUT_MS` remains a legacy alias driving both the request and keep-alive timeouts, so a deployment that only sets it sees exactly today's behaviour for those two - it additionally *gains* an explicit `requestTimeout`/`headersTimeout` where it previously had Node's implicit defaults, which is the one intentional behaviour change.
  - **X15-F3 CLOSED - the Prisma v7 migration had silently dropped the pool acquire timeout.** [prisma.service.ts](api/src/modules/prisma/prisma.service.ts) constructed `new pg.Pool({ connectionString, max: 5 })`, so every option not passed took the `pg` default - including `connectionTimeoutMillis: 0`, meaning **wait forever** for a connection, where Prisma v6 had defaulted `pool_timeout` to 10 s. Combined with F2 that turned pool exhaustion into hung requests instead of fast, legible failure. `max`, `connectionTimeoutMillis` and `idleTimeoutMillis` are now all passed **explicitly**, and new [prisma-pool-options.spec.ts](api/src/modules/prisma/prisma-pool-options.spec.ts) is the standing regression lock for pattern **PG-2** (*when you depend on a library default, assert it; when you override it, assert the override*) - it fails if anyone constructs the pool without passing every option.
  - Body-parser limits are now `HTTP_JSON_BODY_LIMIT` / `HTTP_FORM_BODY_LIMIT` (a malformed value falls back to the previous hardcoded default rather than making body-parser throw at request time). The logging-flush and SCIM-pagination knobs are specified, bounded and logged, but their call sites still read their own constants - wiring them is the remainder of W1.7b.
  - **Validation:** API unit **147 suites / 4,577** (was 145 / 4,549: +22 runtime-config, +6 pool-options); API E2E **84 / 1,414** unchanged; `npm run build` clean; ESLint **0 errors** and **no new warnings** from the changed files (the 2 reported on `prisma.service.ts` were verified pre-existing on HEAD). TDD RED-first: the spec was written and confirmed failing before the module existed. Recommended values per deployment form factor are in [RUNTIME_TUNING_AND_CONFIGURATION_REFERENCE.md](docs/perf/RUNTIME_TUNING_AND_CONFIGURATION_REFERENCE.md) section 7.
- **`npm run docs:html` - read the docs with every diagram rendered, without depending on VS Code at all.** After three rounds of the Markdown preview failing to show Mermaid diagrams for three *different* renderer-side reasons, [scripts/export-docs-html.mjs](scripts/export-docs-html.mjs) removes the editor from the loop: it converts Markdown to standalone HTML with each Mermaid block **pre-rendered as inline SVG** by the same headless Chromium and the same pinned Mermaid version the render gate uses. Output goes to `test-results/docs-html/` (gitignored build artifacts) with an `index.html` listing every document and its diagram count. No extension, no workspace trust, no preview, no version pinning involved - it opens in any browser. Defaults to `docs/auth` + `docs/perf`; accepts explicit paths. **Verified: 150 diagrams across 60 documents in those two folders, all rendered cleanly.** This is now the standard companion to `docs:mermaid:doctor` when missing diagrams are reported, because together they separate "the diagrams are broken" from "your viewer is broken" in one step. Recorded as Mermaid Diagram Rule 11, including the implementation trap that produced 15 *false* failures on the first run: the id given to `mermaid.render(id, src)` is used internally in a CSS selector, so filename-derived ids containing spaces, dots or parentheses throw for reasons unrelated to the diagram - a new checking tool needs its own sanity check before its output is believed.

### Fixed
- **The recurring "I cannot see the Mermaid diagrams in VS Code" issue - ACTUAL root cause found: two Mermaid renderers were racing in the same preview webview.** Third report. The first pass fixed 20 syntax errors; the second pass built a renderer-side doctor and found the workspace-trust and gitignore defects; neither made the diagrams appear, because **the real cause was a renderer collision nothing had looked for**. VS Code 1.104+ ships `vscode.mermaid-markdown-features` **built in** (inside the editor's own install directory, bundling **mermaid 11.15.0**). The separately-installed `bierner.markdown-mermaid` 1.32.1 (bundling **11.12.2**) predates it and does the identical job - same `markdown-mermaid.*` configuration namespace, same `markdown.previewScripts` contribution. With both present, **two different Mermaid builds are injected into one preview webview and both try to render the same `<pre class="mermaid">` nodes**; the loser replaces or re-parses the winner's output and the diagram comes out blank with no error. Every other signal read healthy the whole time, which is exactly why three investigations missed it.
  - **Fix applied:** `bierner.markdown-mermaid` uninstalled (redundant since VS Code 1.104), leaving the built-in as the sole renderer. `.vscode/extensions.json` now lists it under **`unwantedRecommendations`** rather than `recommendations`, so no contributor re-creates the collision.
  - **The gate was pinned to the wrong renderer.** `scripts/render-mermaid.mjs` pinned mermaid to 11.12.2 by reading only `~/.vscode/extensions` - it had no idea the editor ships its own renderer. Repinned to **11.15.0**. New shared [scripts/mermaid-renderers.mjs](scripts/mermaid-renderers.mjs) discovers built-in **and** marketplace renderers (handling Windows' multiple versioned install directories and the `.obsolete` marker for uninstalled-but-not-yet-deleted extensions), and both the gate and the doctor consume it so they can never disagree about what is authoritative. The built-in is authoritative because it ships with the editor and is therefore what every reader gets.
  - **The doctor gained the check that would have caught this in seconds:** it now enumerates every renderer and reports a BLOCKER when more than one can claim the preview. It also corrects the earlier trust finding - the built-in declares `capabilities.untrustedWorkspaces.supported`, so Workspace Trust is **not** a factor when it is the active renderer (that only applies to the marketplace extension, which does not opt in).
  - **Verified:** exactly one renderer, gate and renderer both on 11.15.0, and **150 diagrams across `docs/auth` + `docs/perf`** (596 repo-wide) render cleanly under `securityLevel: 'strict'` in both light and dark themes. Content was healthy at 11.12.2 **and** 11.15.0 throughout - it was never the problem after the first pass.
  - Mermaid Diagram Rule: rule 2 rewritten (the **built-in** is the pin target, with the shared discovery module), and the rule 9 taxonomy gains **R0 - two renderers racing** as the *first* thing to check, since it is the only cause where every other signal reads perfectly healthy. Pattern **PA-5** in [ENGINEERING_LESSONS_AND_PATTERNS.md](docs/strategy/ENGINEERING_LESSONS_AND_PATTERNS.md) extended with the specific lesson: when diagnosing a viewer, first enumerate **every** component that can claim the role - including ones the platform ships itself - because a duplicate-provider conflict presents as total silence.

### Fixed
- **The recurring "I cannot see the Mermaid diagrams in VS Code" issue, at its actual root cause.** This was reported twice. The 2026-07-27 investigation found 20 real syntax errors, fixed them, built a parse gate and a headless-Chromium render gate, and concluded syntax was the cause. It recurred anyway - with the render gate green at **596/596** and, on re-verification under the extension's *own* config in *both* themes, **every diagram provably healthy**. Content was never the problem the second time. Three structural defects were:
  - **The instructions asserted a false fact.** [copilot-instructions.md](.github/copilot-instructions.md) stated that `bierner.markdown-mermaid` "is a workspace recommendation in `.vscode/extensions.json`". `.gitignore` line 38 was `.vscode/`, so that file was **never committed** - `git ls-files .vscode` returned empty. It existed on exactly one machine. Every fresh clone got no recommendation, and, worse, every investigation started from "the environment is handled, so it must be syntax" and re-chased syntax. A false premise in the instruction file is more damaging than a missing one because it terminates the investigation in the wrong place. `.vscode/extensions.json` is now committed, and `.gitignore` uses `.vscode/*` + `!.vscode/extensions.json` - it **must** be `.vscode/*`, because git cannot re-include a file whose parent *directory* is excluded, which would have made the negation silently dead.
  - **Every gate validated CONTENT while all remaining failure modes are RENDERER-SIDE.** No content gate can detect a disabled extension. New [scripts/doctor-mermaid.mjs](scripts/doctor-mermaid.mjs) (`npm run docs:mermaid:doctor`) closes that class: it checks whether the extension is installed, whether its bundled Mermaid version matches the gate's, **whether the workspace is trusted** (reading VS Code's `content.trust.model.key` store directly), whether the workspace recommendation exists, whether a competing preview extension is shadowing the built-in one, and whether `markdown-mermaid.maxHeight` / `languages` are set hostilely - then prints a ranked verdict with the exact remedy for each, and can measure a named file's diagrams in pixels so "it looks blank" gets a factual answer. **Top finding on this machine: the workspace is not in VS Code's trusted-folders list.** `bierner.markdown-mermaid` declares no `capabilities.untrustedWorkspaces`, and per [VS Code's Workspace Trust docs](https://code.visualstudio.com/docs/editing/workspaces/workspace-trust) "extensions that have not explicitly opted into Workspace Trust are disabled by default in Restricted Mode" - so mermaid fences fall back to plain code blocks with **no diagram and no error**, and `security.workspace.trust.startupPrompt` defaults to `never` so the only signal is a dismissible banner.
  - **The render gate did not reproduce the real renderer.** [render-mermaid.mjs](scripts/render-mermaid.mjs) initialized with `securityLevel: 'loose'` while the extension uses Mermaid's default `'strict'` (verified by reading its `dist-preview` bundle), under which labels pass through DOMPurify - so the gate could be green while the preview mangled content. It now renders every block under `strict` in **both** the light (`default`) and dark (`dark`) themes the extension chooses between, honours the extension's `maxTextSize`, and fails a **degenerate** render (under 40x20px, or zero text because every label was sanitized away) rather than only a throwing one. Re-verified: **596 blocks x 2 themes, all clean.**
  - Mermaid Diagram Rule gains **rule 9** (triage order: environment FIRST via the doctor, content SECOND, plus an 8-row renderer-side cause taxonomy R1-R8) and **rule 10** (the gate must use the renderer's real config), and its origin narrative is corrected so the next reader does not repeat the syntax-only conclusion. Distilled as pattern **PA-5** in [ENGINEERING_LESSONS_AND_PATTERNS.md](docs/strategy/ENGINEERING_LESSONS_AND_PATTERNS.md): *when the artifact is provably fine but the human still cannot see it, the fault is in the viewer, and no artifact gate can ever catch that* - promoted to a rule immediately on its 2nd escape.

### Documentation
- **X15: a complete runtime-tuning and configuration reference, plus three findings that change the Wave 1 design.** Every latency, timeout, retry, cache TTL, pool size, buffer, and cap in this server is a function of the environment it runs in (container CPU, DB tier, network path to the IdP, replica count, live load) - X11 proved it by measuring the SAME WIF mint at 2,161 ms cold and 92 ms warm. The new [docs/perf/RUNTIME_TUNING_AND_CONFIGURATION_REFERENCE.md](docs/perf/RUNTIME_TUNING_AND_CONFIGURATION_REFERENCE.md) is the configuration layer under those options: a **source-audited inventory** at api v0.54.81 of every such value with `file:line`, split into already-configurable vs hardcoded; a **three-tier model** (tier 1 server env, tier 2 server env + per-endpoint, tier 3 hardcoded security invariant) with a mandatory clamp contract that extends the existing `EGRESS_POLICY_BOUNDS` cascade rather than inventing a new pattern; **best practices with fetched vendor sources**; a **14-row issue/mitigation matrix**; **recommended values for every knob across four form factors** (local dev, Docker compose, Azure dev, Azure prod); and the **W1.7** implementation proposal (bounds table, cross-key invariants, an effective-config boot log + `GET /scim/admin/runtime-config` surface, and a value-preserving `REQUEST_TIMEOUT_MS` legacy alias). 10 Mermaid diagrams; render gate 586 -> 596 blocks. Three findings, all scheduled, none fixed in this commit (documentation only):
  - **X15-F1 (High) - our JWKS cache defaults contradict Microsoft's own published guidance.** [Entra's signing-key-rollover guidance](https://learn.microsoft.com/en-us/entra/identity-platform/signing-key-rollover) for consumers of its keys is a **24 h TTL with a 1 h background refresh**, per-`kid` caching, and an unknown-`kid` refetch **rate-limited to once per 5 minutes**. `JWKS_CACHE_MAX_AGE_MS` defaults to **10 minutes**, refreshes **synchronously on the token-mint hot path**, caches per `jwksUri` rather than per `kid`, and rate-limits the unknown-`kid` refetch **not at all** (an amplification vector). This is the direct cause of the periodic cold mint X11 measured, and it **redesigns W1.4** from "refresh at 60% of TTL" to Microsoft's algorithm. Hard constraint recorded: the TTL raise must ship in the same commit as the refresher and an overlap-window rotation test, because a 24 h TTL without a working refresher multiplies the key-rotation blast radius 144x.
  - **X15-F2 (Medium) - `REQUEST_TIMEOUT_MS` does not bound request duration.** [main.ts](api/src/main.ts) applies it to `httpServer.setTimeout()` (which is **socket inactivity**) and to `keepAliveTimeout`, but `server.requestTimeout` and `server.headersTimeout` are **never set**, so request duration is actually bounded by Node's implicit 300 s default - not the 120 s an operator reasonably believes they configured. Node's docs call setting these explicitly a denial-of-service protection. Separately, coupling `keepAliveTimeout` to the request timeout is a latent 502/`ECONNRESET` race: keep-alive must be sized against the **upstream ingress idle timeout**, so tuning `REQUEST_TIMEOUT_MS` down for a fast-fail policy would silently create the race.
  - **X15-F3 (Medium) - the Prisma v7 driver-adapter migration silently dropped the pool acquire timeout.** [prisma.service.ts](api/src/modules/prisma/prisma.service.ts) constructs `new pg.Pool({ connectionString, max: 5 })`, so every option not passed takes the `pg` default - including `connectionTimeoutMillis: 0`, meaning **wait forever** for a connection. Prisma v6 defaulted `pool_timeout` to 10 s, so a bound that used to exist is gone, and combined with F2 a pool-exhaustion incident degrades into requests hanging rather than failing fast. The pool size is also a hardcoded `5` with no recorded derivation, when it is a global budget constrained by `pool * replicas <= max_connections - reserved`.
  - **Self-improvement (R7):** all three findings are the same class - *a silent default is not a decision*. Scheduled gaps: (1) nothing detects a hardcoded literal where an environment-dependent value belongs; (2) nothing detects **dependency default drift** (F3's general fix: when you depend on a library default, assert it; when you override it, assert the override - landing as a `pg.Pool` options unit lock with W1.7a); (3) nothing detects **guidance drift** from an integration partner (F1), proposed as a new "integration-partner operational guidance" intake category on the Stage X.1 `gateStrategySelfAudit` prompt alongside the existing security-landscape intake.
- **Related docs updated in lockstep.** [AUTH_CONSOLIDATED_DELIVERY_PLAN.md](docs/auth/AUTH_CONSOLIDATED_DELIVERY_PLAN.md): X15 added as the 4th consolidated stream, **W1.7** added (a/plumbing + bounds + effective-config log, b/DB + HTTP knobs, c/runtime-config endpoint), **W1.4 redesigned**, **W1.5 resequenced BEFORE W1.4** so the deadline/caps safety envelope exists before the riskier cache change and its caps ship configurable from birth, Wave 1 24 -> 36 points and core 116 -> 128, plus the dependency diagram, wave rollup, current-state row, references, and change log. [WIF_TOKEN_MINT_LATENCY_ANALYSIS.md](docs/perf/WIF_TOKEN_MINT_LATENCY_ANALYSIS.md): a configuration-companion banner noting that its Option A assumed the 10-minute TTL that F1 now contradicts. [EXTERNAL_JWKS_VALIDATOR.md](docs/auth/EXTERNAL_JWKS_VALIDATOR.md): the F1 guidance-gap table and an explicit "do not raise this TTL on today's code" warning on `JWKS_CACHE_MAX_AGE_MS`. [ENDPOINT_CONFIG_FLAGS_REFERENCE.md](docs/ENDPOINT_CONFIG_FLAGS_REFERENCE.md): the runtime-egress family named as the reference implementation of the tier-2 pattern, plus the same TTL warning. [EXECUTION_ISSUES_AND_RCA.md](docs/auth/EXECUTION_ISSUES_AND_RCA.md) section 10.2: the open allowlist-revocation question now notes that F1 would widen its exposure window from 10 minutes to 24 hours, so the two must be decided together in W1.4, and records a recommended middle option (make an SSRF rejection specifically non-stale-eligible while a network failure stays stale-eligible). [INDEX.md](docs/INDEX.md): new entry. **No code change; no version bump.**

### Performance
- **W1.1 + W1.3 + W1.6: the first two Wave 1 perf items, plus the mint-latency gate that makes them measurable.** The X11 analysis measured the SAME WIF mint at **~2,161 ms cold vs ~92 ms warm**, and the JWKS fetch sits synchronously on the mint path behind a 10-minute cache with no background refresh - so roughly every 10 minutes a real caller pays the cold cost. **W1.1:** `jose` was lazy-`import()`ed *inside* `verify()` and nothing pre-loaded it, so the first mint after every restart paid the ESM module load; [external-jwks-validator.service.ts](api/src/oauth/external-jwks-validator.service.ts) now memoizes the import and warms it in `onModuleInit`. The pre-load is deliberately **non-fatal** - a transient module-resolution failure logs and falls back to loading on first use, so it can never break startup. **W1.3:** a trust storing the legacy `login.windows.net` JWKS host paid a redirect hop (measured ~130-160 ms) on **every** cold fetch, because redirects are followed manually per attempt; the resolved canonical target is now remembered per configured `jwksUri`, so the hop is paid once per process. The remembered target is **re-validated against the SSRF allowlist on every use**, so the shortcut can never widen what the fetcher may reach (asserted by a test that revokes the host after it was remembered and proves no request is issued to it). **W1.6:** the proof harness gained a warm mint-latency stage (7 samples, median, configurable `-MintLatencyBudgetMs`) - the existing `9z-BQ` gate measures the *resource* path, so until now there was **no mint benchmark at all**. Cold timing is deliberately not asserted: the JWKS cache is process-wide per `jwksUri`, so whether a run starts cold depends on what else already hit that IdP on that replica. **Audit correction:** the plan listed single-flight coalescing and stale-if-error as Wave 1 work; both were already implemented, so Wave 1 is smaller than planned. **Validation:** `external-jwks-validator` unit +4 (redirect remembered / remembered target re-validated / `onModuleInit` pre-loads / a failed pre-load is non-fatal), verified stable over 4 consecutive runs; full API unit 145 suites / 4549; ESLint 0 errors. **Measured on dev v0.54.81: warm mint median 36 ms** (min 31, max 42, reproducible across two runs), so the gate budget was tightened from the initial 400 ms to the plan's **150 ms** target - ~4x headroom for container jitter while still tripping immediately if a network fetch or module load returns to the mint path. Live-test regression 1329/1329; proof harness 50/50. RCA note on a boundary-condition test flake found and fixed during this work: [EXECUTION_ISSUES_AND_RCA.md](docs/auth/EXECUTION_ISSUES_AND_RCA.md) section 10. Version 0.54.80 -> 0.54.81.

### Fixed
- **W3.1 (right-sized): a WIF trust is now ROUTED on the protocol profile it declares, closing the last proof-run finding.** The trust already stored an `assertionProfile` (`jwt-bearer` / `token-exchange`), but nothing ever read it at mint time - so a trust an operator explicitly scoped to **token-exchange still minted via the RFC 7523 `jwt-bearer` path**, and the endpoint's metadata still advertised `private_key_jwt` + the `syncfabric-rfc7523` profile for it. Two new pure helpers on the provider seam ([assertion-token-provider.ts](api/src/modules/scim/controllers/assertion-token-provider.ts)) - `resolveTrustProfiles` + `trustEnablesProfile` - resolve which profiles a stored trust serves, reading the versioned `enabledProfiles[]` (the guide 12.1 shape, now an accepted public trust field) when present and otherwise **projecting** the legacy singular `assertionProfile` onto it. The WIF provider filters candidate trusts to those enabling `syncfabric-rfc7523` **before** validation (cheap, on raw metadata), and [endpoint-oauth-metadata.controller.ts](api/src/modules/scim/controllers/endpoint-oauth-metadata.controller.ts) derives the 7523 capability the same way, so advertised stays == enforced. **Value-preserving:** a trust with `assertionProfile: 'jwt-bearer'` OR with no profile field at all (every trust created before this) resolves to RFC 7523 exactly as today - only a trust that explicitly declares `token-exchange` changes behavior, and that trust was non-functional anyway (there is no RFC 8693 handler until Wave 4). **This is the seam Wave 4 needs:** the RFC 8693 provider will select on `syncfabric-rfc8693` without touching the 7523 path (EXTEND, not EDIT). **Right-sized per the DA-gate:** the full `WifTrustV2` aggregate + 7-state reversible migration machine is still NOT built - the projection gives per-variation routing (the only part that is load-bearing) without a migration no production data needs. **Validation:** WIF provider unit +7 (token-exchange scoped does not mint; explicit v2 profile matches; both-profiles matches; the two backward-compat cases; mixed set tries only the 7523 trust), `wif-assertion` E2E +2 (401 for a token-exchange-scoped trust with a perfectly valid assertion + its metadata advertises nothing; explicit `enabledProfiles` mints), live-test `9z-BX`, proof harness `S7.T13b/T13c`. Full API unit 145 suites / 4545; API E2E 84 / 1414; ESLint 0 errors. Proof: [WIF_END_TO_END_PROOF_AND_AUTH_METHOD_REFERENCE.md](docs/auth/WIF_END_TO_END_PROOF_AND_AUTH_METHOD_REFERENCE.md) finding F6. Version 0.54.79 -> 0.54.80.

### Added
- **W3.8: every issued token now carries a unique `jti` plus the guide-13.4 provenance claims.** The real-Entra proof run showed AT2 carried only `src_iss` + `src_sub`, so a consumer could not tell an RFC 7523 token from a future RFC 8693 one, a multi-tenant trust could not attribute the tenant, and no per-token identifier existed for log correlation or a future replay denylist. [`generateEndpointAccessToken`](api/src/oauth/oauth.service.ts) now stamps **`jti`** (a fresh UUID on every mint) and, when supplied, **`auth_method`** / **`source_tid`** / **`source_oid`** / **`source_azp`**. The WIF provider passes `syncfabric-rfc7523` plus the verified assertion's `tid` / `oid` / `azp` (falling back to `appid` for v1.0 assertions); the [`ClientSecretTokenProvider`](api/src/modules/scim/controllers/client-secret-token-provider.ts) passes `client_secret`. The `source_*` claims are **omitted entirely** on non-federated mints, so a client-credentials token stays clean - and that asymmetry is itself the documented WIF/non-WIF discriminator. None of these claims is an authorization input; they are attribution only. **Validation:** `oauth.service` unit +3 (unique `jti`; provenance stamped; `source_*` omitted without a federated source), WIF provider unit +2 (`azp` and the v1.0 `appid` fallback), `wif-assertion` E2E +1 (two mints of the same assertion yield different `jti`) + the mint test strengthened, live-test `9z-BX`. Proof: [WIF_END_TO_END_PROOF_AND_AUTH_METHOD_REFERENCE.md](docs/auth/WIF_END_TO_END_PROOF_AND_AUTH_METHOD_REFERENCE.md) findings F3 + F4.

### Fixed
- **W3.9: connection-info now shows the OAuth client identity as Entra's "Client identifier", not the federated assertion subject.** The WIF projection mapped `entraFields.clientIdentifier` to the trust's `expectedSubject`. Since W3.2 made `targetClientId` the value the issued token actually carries as `client_id`, that projection was inconsistent with the token itself - the operator was shown one identity while the wire carried another (SyncFabric guide 16.2). `clientIdentifier` is now `targetClientId ?? endpointId` (mirroring the mint exactly, so advertised == minted), and the expected assertion subject is surfaced as a **new distinct** `expectedAssertionSubject` field rather than being conflated into the client identity (guide 11.2: "separate values with separate validation rules"). **Validation:** connection-info unit +1 (explicit `targetClientId` surfaces) + 1 corrected (no `targetClientId` falls back to the endpointId and keeps the subject distinct), connection-info E2E key-allowlist extended for the new field, live-test `9z-BX.T6/T7`. Full API unit 145 suites / 4538; API E2E 84 / 1412; ESLint 0 errors. Version 0.54.78 -> 0.54.79.

### Security
- **W3.6: a WIF-minted token can no longer outlive the assertion that authorized it (SyncFabric guide 13.5).** Found by a new end-to-end proof harness run against a **real Microsoft Entra application**: with the documented 6h ceiling (`issuedTokenTtlSec: 21600`) against a 1h Entra assertion, the minted token's `exp` exceeded the assertion's `exp` by **18,002 seconds (~5 hours)** - so if the customer disabled the workload identity or the assertion was revoked, a SCIMServer-issued token could keep provisioning for up to 6 hours. The lifetime was clamped only to the static 3600-21600 window; the assertion's own `exp` was never an input. **Fix:** [`generateEndpointAccessToken`](api/src/oauth/oauth.service.ts) accepts an `assertionExpiresAt` option and applies `expiresIn = min(clamped ttl, assertionExp - now)` **after** the static clamp, so the 1h floor can never raise the lifetime back above the assertion (an assertion with 2 minutes left yields a 2-minute token, not an hour). The WIF provider passes the verified `claims.exp`. Non-federated `oauth_client` mints are unaffected (no `assertionExpiresAt` -> unchanged behavior). **Validation:** `oauth.service` unit +3 (6h TTL capped to 1h assertion; sub-floor assertion caps below the 1h floor; no assertion expiry leaves the ttl unchanged), WIF provider unit +1 (passes `claims.exp`), `wif-assertion` E2E +1 (a 6h trust yields <= the assertion's 10m lifetime) + 2 pre-existing assertions corrected (they had codified the uncapped `expires_in: 7200`). Proof: [WIF_END_TO_END_PROOF_AND_AUTH_METHOD_REFERENCE.md](docs/auth/WIF_END_TO_END_PROOF_AND_AUTH_METHOD_REFERENCE.md) finding F1.

### Added
- **W3.7: the RFC 7523 form `client_id` is now bound to the trust's `targetClientId` (completes W3.2).** The per-endpoint RFC 8414 metadata advertises `x_scimserver_wif_profiles[0].client_id_binding: "target-client-id"`, but nothing validated it: the real-Entra proof run showed a **valid assertion presented with `client_id=totally-wrong-client-id` still minted a token**. The W3.2 report had deferred this half on the rationale that "the SyncFabric RFC 7523 flow does not currently send a form `client_id`" - **that rationale was factually wrong**; SyncFabric guide section 7.1 lists `client_id=<ISV-issued target client identifier>` as a *confirmed current* RFC 7523 form field and section 7.4 maps `Oauth2ClientId` as **Required** for RFC 7523 (it is only omitted for RFC 8693). **Fix:** the parser now captures the form `client_id` on the `client_assertion` variant, and the WIF provider rejects a mismatch with `wif_client_id_mismatch` + a `target_client_id_match` decision-trace check. **Backward compatible in both directions:** a trust with no `targetClientId` has nothing to bind against, and a request that sends no `client_id` is unaffected - only a trust that pins a target AND a request that sends a *different* one is rejected. **Design:** `IAssertionTokenProvider.mintFromAssertion`'s growing positional parameters (`resource` was the 3rd, `clientId` would have been the 4th) are replaced by an `AssertionMintRequest` options object, so the next variation adds a FIELD instead of another positional argument (DA-gate Open/Closed). **Validation:** WIF provider unit +4 (mismatch rejected / match accepted / no client_id unaffected / no targetClientId never binds), controller unit +1 (threads the form `client_id`), parser unit already covers capture, `wif-assertion` E2E +1 (401 `wif_client_id_mismatch` -> 200 on match -> 200 with no `client_id`). Proof: finding F2.
- **`scripts/wif-e2e-proof.ps1` - a real-Entra WIF end-to-end proof harness (the delivery plan's empirical gate).** Acquires a genuine Microsoft-signed Entra application token, establishes a SCIMServer WIF trust from that token's own claims, mints via the RFC 7523 wire contract, decodes both tokens, provisions real SCIM Users + Groups with the minted token, and probes every variation (resource policy, client_id binding, lifetime cap, RFC 8693 refusal, metadata truthfulness, connection-info, and the OAuth2 client-credentials sibling method). Self-cleaning, never prints a raw token or the client secret, and exits non-zero on any failed assertion so it can run as a gate. Full analysis + verbatim wire captures: [WIF_END_TO_END_PROOF_AND_AUTH_METHOD_REFERENCE.md](docs/auth/WIF_END_TO_END_PROOF_AND_AUTH_METHOD_REFERENCE.md). Version 0.54.77 -> 0.54.78.

### Added
- **W3.4: a WIF trust can now enforce the RFC 8707 `resource` request indicator (SAP SuccessFactors).** SuccessFactors sends a `resource` form parameter on its token request; SCIMServer stored `expectedResource` on the trust but never checked it. A new per-trust `resourceMode` (all public, additive on the `wif` `EndpointCredential.metadata`) governs the check: **`ignore`** (default, legacy) accepts any/absent resource and only emits an advisory shadow log on mismatch; **`optionalExact`** rejects a present-but-mismatched resource but accepts an absent one; **`requiredExact`** requires the request to present a resource that exactly equals `expectedResource`. The `resource` is a REQUEST parameter (not an assertion claim), so it is threaded from the parser ([endpoint-token-request-parser.ts](api/src/modules/scim/controllers/endpoint-token-request-parser.ts)) through the controller ([endpoint-oauth.controller.ts](api/src/modules/scim/controllers/endpoint-oauth.controller.ts)) and provider ([wif-assertion-token.provider.ts](api/src/modules/scim/controllers/wif-assertion-token.provider.ts)) into the validator ([wif-assertion-validator.service.ts](api/src/oauth/wif-assertion-validator.service.ts)), where a new `resource_match` check sits in the same ordered decision trace as `audience_match`/`tenant_match` and rejects with `wif_resource_required` / `wif_resource_mismatch`. **Behavior-preserving:** `ignore` is the default, so every existing trust accepts exactly what it does today; the three modes are finite typed values, not a policy DSL (YAGNI). **Validation:** validator unit +6 (all three modes x present/absent/match/mismatch), parser unit +1 (captures `resource`), WIF provider unit +1 (threads `resource`), controller unit +1 (+1 arity fix); `wif-assertion` E2E +2 (`requiredExact` 401-missing -> 401-wrong -> 200-exact; `ignore` accepts a mismatch); live-test **9z-BY** (a `requiredExact` trust persists `resourceMode` + `expectedResource`, no secret leak). Full API unit 145 suites / 4523; API E2E 84 / 1409; ESLint 0 errors. Report [WAVE3_W3_4_IMPLEMENTATION_REPORT.md](docs/auth/WAVE3_W3_4_IMPLEMENTATION_REPORT.md). Version 0.54.76 -> 0.54.77.

### Fixed
- **W3.2: the WIF-minted token no longer conflates the federated assertion subject with the issued OAuth `client_id` (RFC 6749 + RFC 7523 identity separation).** When an endpoint minted its token from a WIF (`jwt-bearer`) assertion, the provider passed the assertion's `sub` (the Entra workload identity's object id) straight into `generateEndpointAccessToken` as the `clientId`, so the issued token's `sub` AND `client_id` were both the federated subject ([wif-assertion-token.provider.ts](api/src/modules/scim/controllers/wif-assertion-token.provider.ts)). That conflates two identities the RFCs keep distinct - the OAuth client the token represents vs the federated principal presented once at the token endpoint - and it made the per-endpoint OAuth AS metadata **untruthful** (it already advertised `x_scimserver_wif_profiles[0].client_id_binding: 'target-client-id'` + `assertion_subject_binding: 'independent'` while the runtime did the opposite). **Fix (additive, non-breaking):** a new optional public trust field `targetClientId` on the `wif` `EndpointCredential.metadata`; the issued token's `sub`/`client_id` are now `trust.targetClientId ?? endpointId` (the stable per-endpoint identity, which is also the default `oauth_client` `client_id`), **never** the assertion subject. The federated subject is preserved as a new distinct `src_sub` claim (sibling of `src_iss`) for attribution ([oauth.service.ts](api/src/oauth/oauth.service.ts) gains a `sourceSubject` option). **Safe:** the issued `client_id`/`sub` are used only for log enrichment on the resource path ([oauth-jwt.authenticator.ts](api/src/modules/auth/authenticators/oauth-jwt.authenticator.ts) lines 117-119); authorization is by the `endpoint_id` claim (lines 76-79), so the issued identity change cannot alter what a token may do. Existing trusts (no `targetClientId`) keep working - their issued identity moves from the leaked assertion subject to the correct endpointId. **Right-sized (DA-gate YAGNI):** did NOT build the full `WifTrustV2` versioned aggregate + 7-state reversible migration machine (W3.1, scheduled - no production trust data needs a reversible migration yet); the single additive field is the minimal correctness fix. **W3.3 (remove the endpointId audience default) deferred** - that default is a documented operator decision (blocks cross-endpoint replay) and `buildTrust` already requires a non-empty `expectedAudience` at mint time, so there is no silent-derivation hole. **Validation:** `oauth.service` unit +2 (`src_sub` distinct from `sub`/`client_id`; omitted without `sourceSubject`), `wif-assertion-token.provider` unit +1 (explicit `targetClientId`) + 2 corrected; `wif-assertion` E2E mint test strengthened (issued `sub`==endpointId, `!=` assertion subject, `src_sub`==subject) + 1 new (`targetClientId`); live-test **9z-BX** (trust with `targetClientId` persists, no secret leak; metadata advertises the truthful binding). Full API unit 145 suites / 4514; API E2E 84 / 1407; ESLint 0 errors. Report [WAVE3_W3_2_IMPLEMENTATION_REPORT.md](docs/auth/WAVE3_W3_2_IMPLEMENTATION_REPORT.md). Version 0.54.75 -> 0.54.76.
- **The per-log DETAIL response now carries `endpointId` (parity with the list row).** The logs LIST projection ([logging.service.ts](api/src/modules/logging/logging.service.ts) `listLogs`) surfaces `endpointId` on every row (X6 endpoint-name column), but the single-log DETAIL (`GET /scim/admin/logs/:id`, `getLog`) omitted it - so a client that opened one log by id could not correlate it to its endpoint without re-reading the list, and the `9z-BW` FK-drop regression assertion had to read the endpointId back off the list row instead of the detail it actually cared about. `getLog` now includes `endpointId: row.endpointId ?? undefined` in BOTH backends (the Prisma branch and the in-memory branch, at parity), immediately after `id`. Purely additive - every existing detail-contract test uses additive-safe `toHaveProperty`/allowlist assertions, so no response shrank. **Validation:** logging unit +1 (in-memory: the detail's `endpointId` equals the list row's); global-logs-filters E2E strengthened (the detail asserts `endpointId === ep` alongside the existing `requestId` round-trip); live-test `9z-BW.T3` now reads the endpointId off the DETAIL (was the list) to prove the persisted-after-delete row keeps its correlation on the wire; full API unit 145 suites; log detail E2E 18. Version 0.54.74 -> 0.54.75.
- **ROOT CAUSE: request logs are no longer lost when an endpoint is deleted (drop the `RequestLog -> Endpoint` FK) - the real fix for the flaky request-log-readback assertions + a production audit-log data-loss bug.** After the flush-throughput (0.54.72) and force-flush (0.54.73) fixes did not clear the `9z-BD`/`9z-BN`/`9z-BO` flakes, a dev ring-buffer inspection revealed the batch flush was actually THROWING: `Foreign key constraint violated: RequestLog_endpointId_fkey`. `RequestLog.endpointId` had a FK to `Endpoint` (`onDelete: SetNull`). That handled deletion for EXISTING rows, but INSERTING a buffered row whose endpoint was deleted between buffering and the batched flush violated the constraint - and because the flush is one atomic `createMany`, the WHOLE batch was rejected, silently dropping every audit row in it (including unrelated sections' readback targets). The live-test creates + deletes many endpoints, so a buffered row for a since-deleted endpoint routinely nuked the batch; in isolation the endpoint still existed, so it worked. **This was also a real production bug:** deleting an endpoint while its rows were buffered lost that request/audit history. **Fix:** removed the FK both directions ([schema.prisma](api/prisma/schema.prisma): `RequestLog.endpoint` relation + `Endpoint.logs` back-relation) with migration [20260724000000_drop_requestlog_endpoint_fk](api/prisma/migrations/20260724000000_drop_requestlog_endpoint_fk/migration.sql) (`DROP CONSTRAINT ... RequestLog_endpointId_fkey`). `endpointId` stays a plain indexed correlation column, so logs for a since-deleted endpoint persist and stay queryable by that id - exactly what an append-only audit trail must do (the live-test already documented "RequestLog rows can outlive their endpoint"). **Validation:** Prisma client regenerated; full API unit 145 suites / 4510; live-test regression section `9z-BW` (drive a request, DELETE the endpoint, force-flush, assert the row still persists with its endpointId). Version 0.54.73 -> 0.54.74.
- **Request-log force-flush now reliably drains under load (completes the flush-backlog fix) + UUID-safe `requestId` filter.** The first cut of the fix (0.54.72) added `POST /scim/admin/logs/flush` but wired it to `flushLogs()`, which deliberately no-ops while a background flush is in progress (`if (flushInProgress) return`) - so under sustained live-test load, when a timer flush was usually mid-write, the force-flush returned 204 having drained nothing, and the `9z-BD`/`9z-BN`/`9z-BO` readbacks still flaked. Added [`LoggingService.flushPending()`](api/src/modules/logging/logging.service.ts) which spins until the buffer is empty AND no flush is in flight (bounded by a 10s deadline), and pointed the admin endpoint at it - so on return every entry buffered before the call is durable + queryable. Also hardened the `GET /scim/admin/logs?requestId=` filter: `requestId` is a `@db.Uuid` column, so a non-UUID value made Postgres throw on the cast (a 500); it now maps a non-UUID to the nil-UUID no-match filter and returns an empty set (parity with the in-memory string-equality branch). **Validation:** logging + admin unit +4 (flushPending drains / returns-fast-when-empty; requestId non-UUID -> nil-UUID / valid UUID passthrough); global-logs-filters E2E +1 (non-UUID requestId returns 200 empty, not 500); full API unit 145 suites / 4510; log E2E 46. Version 0.54.72 -> 0.54.73.
- **Request-log flush backlog (RCA of the 10 flaky live-test request-log-readback assertions).** The Prisma request-log flush ([logging.service.ts](api/src/modules/logging/logging.service.ts) `flushLogs`) did a batch `createMany` **plus N sequential per-identifier `UPDATE` round-trips** (a `SELECT` of the most-recent-N rows by `createdAt` + one `UPDATE` per derived identifier). On a latency-bound node each flush cost N+1 round-trips, so under sustained request volume the flush throughput fell behind, the `flushInProgress` guard skipped new flushes, and rows sat un-persisted in the in-memory buffer - a just-created row was not queryable for tens of seconds (the flush-backlog that made `9z-BD` / `9z-BN` / `9z-BO` request-log-readback assertions flaky on dev). The `createdAt`-desc correlation was also fragile: interleaved inserts could misassign identifiers. **Fix:** the derived `identifier` is now written INLINE in the `createMany` payload (it is a real indexed column), so the flush is a single fast batch insert with no backfill - faster AND correct. Added **`POST /scim/admin/logs/flush`** ([admin.controller.ts](api/src/modules/scim/controllers/admin.controller.ts)) to force-drain the buffer on demand (for operators chasing a fresh row and for tests that read back a row right after producing it). The live-test's `Get-LogDetailByUrlStatus` + new `Get-LogDetailByRequestId` helpers now force-flush before polling, making request-log-readback deterministic. **Validation:** logging + admin unit +2 (identifier-inline-in-`createMany`; flush endpoint delegates); log-config E2E +3 (flush 204 / requires auth / requestId queryable-after-flush); full API unit 145 suites / 4506; ESLint 0 errors. Version 0.54.71 -> 0.54.72.

### Added
- **W2.4: one auth-decision emit+record choke point (behavior-preserving).** The `emitAuthDecisionEvent(logger, trace, AUTH) + decisionStore.record(trace)` pair was hand-rolled in FOUR places - the resource guard ([shared-secret.guard.ts](api/src/modules/auth/shared-secret.guard.ts)), the per-endpoint mint controller ([endpoint-oauth.controller.ts](api/src/modules/scim/controllers/endpoint-oauth.controller.ts)), the WIF assertion provider ([wif-assertion-token.provider.ts](api/src/modules/scim/controllers/wif-assertion-token.provider.ts)), and the global token controller ([oauth.controller.ts](api/src/oauth/oauth.controller.ts)) - which risked drift (emit-without-record, or a second canonical AUTH event). W2.4 gives the sequence ONE definition: [`emitAndRecordAuthDecision`](api/src/oauth/auth-decision-trace.ts) (+ a minimal `AuthDecisionRecorder` shape so it needs no store-class import). Each site keeps its OWN pre-conditions (the guard's no-store early-return + accept-noise-control, the WIF provider's per-trust aggregation); only the final emit+record step is unified. Chose a pure function over an injectable `AuthDecisionEmitter` service because the four call sites live in four different modules and already `@Optional`-inject the store - a service would need cross-module DI for zero behavior benefit (the YAGNI-correct choice; promote to a service only if injected policy is ever needed). This closes [AUTH_SOURCE_REFACTORING_ANALYSIS.md](docs/auth/AUTH_SOURCE_REFACTORING_ANALYSIS.md) finding #3. The provider file relocation to `oauth/token-mint/` is deferred (pure churn, no behavior benefit). **Validation:** API build + ESLint 0 errors; `auth-decision-trace.spec` +3 (emit+record; null store; accept); the 6 emit-site suites (guard 39 + controller 12 + trace + WIF + global) 129; auth/token E2E 74; full API unit 145 suites / 4504. Report + DA-gate: [docs/auth/WAVE2_W2_4_IMPLEMENTATION_REPORT.md](docs/auth/WAVE2_W2_4_IMPLEMENTATION_REPORT.md). Version 0.54.70 -> 0.54.71.
 The credential lookup + bcrypt verification + token mint that were inlined in `EndpointOAuthController.handleClientSecret` now live in a dedicated [`ClientSecretTokenProvider`](api/src/modules/scim/controllers/client-secret-token-provider.ts) (a plain `@Injectable` - one implementation, so no speculative interface + DI token per the YAGNI gate), symmetric with the WIF `IAssertionTokenProvider`. The provider owns the credential repo + `OAuthService` + bcrypt and the per-check trace through `secret_match`, returning a three-outcome result (`accept` with token + checks / `reject` with reason + checks) that never throws for an auth failure. The controller keeps only routing, the W2.5 mint-shadow read, the `token_ttl` + decision-event emission, the RFC 6749 error shaping, and the HTTP response - it no longer imports `bcrypt`, `OAuthService`, or the credential repository. **Behavior-preserving**: the auth-decision `checks[]` order (`grant_type`, `credential_location`, `client_id_present`, `client_found`, `secret_match`, `method_enabled_shadow`, `token_ttl`), the reject reason (`oauth_client_auth_failed`), the accept response, and the W0.2 `@HttpCode(200)` + no-store headers are unchanged. **Validation:** API build + ESLint 0 errors; new `client-secret-token-provider.spec` 3/3 (accept + 2 reject shapes; never echoes the secret); `endpoint-oauth.controller.spec` 12/12; token-mint + connection-info E2E 82; full API unit 145 suites / 4501. Report + DA-gate: [docs/auth/WAVE2_W2_3_IMPLEMENTATION_REPORT.md](docs/auth/WAVE2_W2_3_IMPLEMENTATION_REPORT.md). Version 0.54.69 -> 0.54.70.
 The token-mint controller ([endpoint-oauth.controller.ts](api/src/modules/scim/controllers/endpoint-oauth.controller.ts)) no longer re-derives the credential method inline. A new pure parser [`parseEndpointTokenRequest`](api/src/modules/scim/controllers/endpoint-token-request-parser.ts) turns the raw RFC 6749 form body + the `Authorization` header into exactly one variant of a [`ParsedEndpointTokenRequest`](api/src/modules/scim/controllers/endpoint-token-request.types.ts) discriminated union - `client_assertion` (WIF/RFC 7523), `client_secret` (RFC 6749 client_credentials), or `invalid` - and the controller only ROUTES the well-formed variants + shapes the error response. Parsing is free of crypto + DB (RFC 6749 section 2.3.1 credential-location normalization - Basic vs body, body wins; grant-type enforcement; mutual-exclusion of assertion + secret; assertion-type validation; missing-credential rejection); secret validation (bcrypt) + assertion validation (JWKS) stay in the providers. **Behavior-preserving**: every `error` / `error_description` / `reason_code` / HTTP status and the auth-decision `checks[]` trace shape are reproduced exactly; the W0.2 `@HttpCode(200)` + `no-store` headers and the W2.5 mint-shadow read are unchanged. This makes the mint plane Open/Closed - a new credential shape (private_key_jwt, mTLS) adds a union variant + a parser branch, not controller surgery. **Validation:** API build + ESLint 0 errors; new `endpoint-token-request-parser.spec` 10/10 (all variants + every error shape); `endpoint-oauth.controller.spec` 12/12; token-mint E2E 74 (endpoint-oauth-client + wif-assertion + authentication); full API unit 144 suites / 4498. Report + RCA + DA-gate: [docs/auth/WAVE2_W2_2_IMPLEMENTATION_REPORT.md](docs/auth/WAVE2_W2_2_IMPLEMENTATION_REPORT.md). Version 0.54.68 -> 0.54.69.
 A single resolver [`resolveEndpointAuthEnablement`](api/src/modules/endpoint/endpoint-config.interface.ts) is now the ONE per-method enablement source. For each facet it prefers an explicit `profile.authentication.methods[]` entry of the corresponding `type` (`bearer` -> `secretTokenBearer`, `oauth-client` -> `oauthClientCredentials`, `shared-secret` -> `sharedSecretBearer`) using the A2 convention (`enabled !== false`), and falls back to the flat-flag `getEffectiveAuthEnablement` (`SecretTokenBearerAuthEnabled` / `OAuthClientCredentialsAuthEnabled` / `SharedSecretBearerAuthEnabled` -> legacy `PerEndpointCredentialsEnabled`) when no such entry exists. The resource-plane guard ([endpoint-credential](api/src/modules/auth/authenticators/endpoint-credential.authenticator.ts) + [global-shared-secret](api/src/modules/auth/authenticators/global-shared-secret.authenticator.ts) authenticators), the credential create-gate ([admin-credential.controller.ts](api/src/modules/scim/controllers/admin-credential.controller.ts)), and [connection-info.service.ts](api/src/modules/scim/services/connection-info.service.ts) all read this one resolver, so "advertised == enforced" cannot drift. **Value-preserving:** `profile.authentication.methods[]` is never auto-seeded, so every endpoint that has not been managed through the A1 authentication-method API resolves to the EXACT flat-flag values it does today (proven by unit + a co-location E2E). **Mint plane consults the same source in SHADOW (design 7.4):** [endpoint-oauth.controller.ts](api/src/modules/scim/controllers/endpoint-oauth.controller.ts) now reads the resolver and records a `method_enabled_shadow` fail check + a `W2.5 shadow` warning when the `oauth_client` method is disabled (the disabled-with-credential state), but STILL mints - fixing the design 7.1 mint-vs-resource asymmetry non-blockingly so the future enforcement flip can be validated against real traffic first (the shadow read fails open and never blocks a mint). **Scheduled follow-up (value-preserving):** materialize + retire `PerEndpointCredentialsEnabled` (Prisma migration + InMemory parity), flip the mint plane from shadow to enforce, gate OAuth metadata on enablement, and give WIF an explicit enable. **Validation:** API build + ESLint 0 errors; resolver unit +7 (`endpoint-config.interface.spec` 351/351); mint-shadow unit +2 (`endpoint-oauth.controller.spec` 12/12); resource-guard + authenticators + create-gate + connection-info specs 160/160; full API unit 143 suites / 4488 tests (4479 + 9); auth E2E 82; `per-endpoint-credentials` E2E +3 co-location (27/27); live-test **9z-BV** (co-location on the wire). Report + RCA + DA-gate: [docs/auth/WAVE2_W2_5_IMPLEMENTATION_REPORT.md](docs/auth/WAVE2_W2_5_IMPLEMENTATION_REPORT.md). Version 0.54.67 -> 0.54.68.

- **W2.1: the resource-plane auth is now a `ResourceAuthenticator` probe-chain (behavior-preserving refactor).** The 491-line `SharedSecretGuard` god-guard is refactored into a ~185-line thin orchestrator over three ordered, single-purpose strategy classes under [api/src/modules/auth/authenticators/](api/src/modules/auth/authenticators/): `EndpointCredentialAuthenticator` (order 10, the per-endpoint bearer + oauth_client bcrypt loop with the X9 JWT/global-secret fast-paths), `OAuthJwtAuthenticator` (order 20, the OAuth JWT validation + the endpoint-scope reject-STOP), and `GlobalSharedSecretAuthenticator` (order 30, the legacy global secret + the WI-11 refusal reject-STOP). Each returns a three-outcome `AuthAttempt` (accept / reject-STOP / not-applicable-continue) - the Spring Security `ProviderManager` + `AuthenticationProvider` shape, and the exact model the mint plane already uses via `IAssertionTokenProvider`. The guard composes the chain internally from its existing deps (constructor signature unchanged), so the comprehensive 39-test security spec passes with zero construction churn, and owns only the cross-cutting concerns (shared-secret resolution, decision-trace accumulation + terminal emission, the RFC 6750 reject response). **This is a pure structural refactor**: both reject-STOP cases (`bearer_token_scoped_other_endpoint`, `bearer_shared_secret_refused`), the F3 fall-through sub-reason, the X9 short-circuits, and the request enrichment are reproduced exactly. `isEnabled` still reads today's `getEffectiveAuthEnablement` flags - the per-method enablement co-location + legacy-umbrella retirement is W2.5. It unlocks Open/Closed for the roadmap: RFC 8693 / private_key_jwt / mTLS become a new authenticator class + one constructor entry, not a guard edit. `mapBearerJwtErrorToReason` moved to the OAuth authenticator and is re-exported from the guard for compat. **Validation:** API build + ESLint 0 errors; `shared-secret.guard.spec` 39/39 (regression net) + 20 new per-strategy specs (accept / reject-STOP / not-applicable, incl. real-bcrypt match + the X9 fast-paths); full API unit 4479/4479 (was 4459 + 20); auth E2E (authentication + wif-assertion + endpoint-oauth-client + oauth-discovery, inmemory) 7 suites / 80 tests. Report + RCA + DA-gate: [docs/auth/WAVE2_W2_1_IMPLEMENTATION_REPORT.md](docs/auth/WAVE2_W2_1_IMPLEMENTATION_REPORT.md). Version 0.54.66 -> 0.54.67.

- **Token endpoint returns HTTP 200 + no-store (W0.2, RFC 6749 section 5.1).** Every token-mint SUCCESS response now returns **200** (was 201) with the two mandatory cache headers `Cache-Control: no-store` + `Pragma: no-cache`, on ALL three success paths - the global `client_secret` handler ([oauth.controller.ts](api/src/oauth/oauth.controller.ts)) and the per-endpoint handler ([endpoint-oauth.controller.ts](api/src/modules/scim/controllers/endpoint-oauth.controller.ts)), which covers BOTH its `client_secret` and its WIF `client_assertion` (RFC 7523) sub-routes. Error responses are unchanged: a bad grant_type or bad client still returns its RFC 6749 section 5.2 400/401 (a thrown `HttpException` keeps its status and bypasses the static `@Header` decorators). Implemented with `@HttpCode(200)` + `@Header('Cache-Control', 'no-store')` + `@Header('Pragma', 'no-cache')` on both handlers. Why: 201 was a literal (if minor) violation of section 5.1 and the absent no-store was a second independent one; 201 was almost certainly working with real IdPs (Entra's provisioning client, like any conformant OAuth client, accepts any 2xx) but diverged from the exact 200 shape Entra's own AS returns and that a gallery / conformance validator asserts - so this is a correctness + hardening fix, not a fix for a confirmed outage (full IdP-behavior analysis, options, and recommendation in the feature doc). **Validation:** the shared E2E auth helper + every global/per-endpoint/WIF token-mint assertion flipped 201 -> 200; one representative test per path additionally asserts `Cache-Control: no-store` + `Pragma: no-cache` ([authentication.e2e-spec.ts](api/test/e2e/authentication.e2e-spec.ts), [endpoint-oauth-client.e2e-spec.ts](api/test/e2e/endpoint-oauth-client.e2e-spec.ts), [wif-assertion.e2e-spec.ts](api/test/e2e/wif-assertion.e2e-spec.ts)); a new live-test section **9z-BU** (+7 wire assertions) uses `Invoke-WebRequest` to assert the ACTUAL status + cache headers on the global and per-endpoint routes and that a bad grant_type still returns 400 (error path unaffected). Feature doc: [docs/auth/W0_2_TOKEN_ENDPOINT_200_NO_STORE.md](docs/auth/W0_2_TOKEN_ENDPOINT_200_NO_STORE.md) (linked from [docs/INDEX.md](docs/INDEX.md)). API build + eslint clean (0 errors); API unit 4459 pass; API E2E 1399 pass (in-memory). DA-gate: no new coupling - two static decorators on existing handlers; the per-endpoint decorator intentionally covers both sub-routes because they share one handler (SRP preserved). Running the local live gate (**1,313/1,313 PASS**, 0 fail, 0 skip) surfaced and fixed 7 PRE-EXISTING live-test authoring bugs (unrelated to W0.2, none a product/parity bug): 9z-AZ + 9z-BT created `oauth_client` credentials without first enabling `OAuthClientCredentialsAuthEnabled` (WI-11 per-method gate -> 403); 9z-AT6.T3/T4 asserted stale client_id shapes (actual first-client default is `client-id-<endpointId>`, second is a distinct generated `client-id-*`); 9z-AV.T4 re-read a wif field the credential-list projection omits (`wif` is null in the list) - now re-reads the persisted label the PUT changed; 9z-AT5.T3 probed shared-secret refusal with the OAuth JWT (accepted via the OAuth path) instead of the actual shared secret (new `-SharedSecret` param + baseline skip-guard); 9z-AT8.T4 flagged the intended WI-8 global-shared-secret recipe disclosure (`secretToken`, admin-only + audit-logged when `CredentialSecretVisibility=always`) as a leak - now checks the per-endpoint `oauth_client` `clientSecretState`. The local live gate needs `JWKS_HOST_ALLOWLIST=login.microsoftonline.com,login.windows.net` + `SCIM_SHARED_SECRET=changeme-scim` (dev-equivalent env) to exercise the WIF-verify + shared-secret sections. Version 0.54.65 -> 0.54.66.

- **Capability-derived (truthful) per-endpoint OAuth AS metadata (W0.3).** The per-endpoint RFC 8414 discovery document ([endpoint-oauth-metadata.controller.ts](api/src/modules/scim/controllers/endpoint-oauth-metadata.controller.ts), `GET /scim/endpoints/:id/.well-known/oauth-authorization-server`) is now DERIVED from what the runtime actually implements AND what the endpoint has an active credential/trust for, instead of a hardcoded list. This removes a live untruth: it previously advertised the `token-exchange` grant (RFC 8693) even though there is no runtime handler (the request is rejected), and advertised `private_key_jwt` even for an endpoint with no WIF trust (SyncFabric guide Section 17.1 defect). Now: (a) `token-exchange` + the `none` method are **never** advertised until the RFC 8693 handler ships (delivery-plan Wave 4); (b) `client_secret_basic` + `client_secret_post` appear only with an active `oauth_client` credential; (c) `private_key_jwt` + `token_endpoint_auth_signing_alg_values_supported: [RS256, ES256]` + a collision-resistant `x_scimserver_wif_profiles` disclosure (per guide 17.4: the advertised `private_key_jwt` is a SyncFabric client-assertion profile whose assertion subject is validated against the trust and need not equal `client_id`) appear only with an active `wif` trust; (d) the lookup fails open to an empty capability set (a valid document that advertises nothing) rather than falsely claiming a capability. The controller now injects the credential repository + `ConfigService` (`ConfigModule` is global) and the handler is async. **Validation:** metadata unit 12 (all branches: bare/secret/wif/both, no token-exchange, fail-open, scopes, proxy headers); `wif-assertion` E2E WI-12 strengthened to assert the WIF endpoint advertises `private_key_jwt` + the disclosure and never token-exchange; live-test `9z-BT` (bare endpoint advertises nothing -> adding an oauth_client makes the secret methods appear -> token-exchange never advertised). API build clean. Version 0.54.63 -> 0.54.64.

- **Copy-as-JSON + Download-JSON of the COMPLETE record on every detail drawer (X14).** A new reusable [DownloadJsonButton](web/src/components/primitives/DownloadJsonButton.tsx) primitive (the download sibling of `CopyJsonButton`: serialises pretty-printed JSON and triggers a browser download, preferring a Blob + object URL and falling back to a data-URI so it never throws under jsdom) is wired into the shared [DetailDrawer](web/src/components/primitives/DetailDrawer.tsx) primitive via an optional `jsonData` prop. When a drawer passes its raw underlying record, the drawer header now renders **Copy as JSON** + **Download JSON** actions that serialise the ENTIRE record - all fields, including any the drawer body does not render (e.g. a log's `requestHeaders` / `requestBody` / `responseHeaders` / `responseBody`). Applied to all three production detail drawers uniformly: the global **Logs** detail drawer ([LogsPage.tsx](web/src/pages/LogsPage.tsx)), the endpoint **Logs** detail drawer ([LogsTab.tsx](web/src/pages/LogsTab.tsx)), and the shared Users/Groups **resource** detail drawer ([ResourceDetailDrawer.tsx](web/src/components/detail/ResourceDetailDrawer.tsx)). Filenames preserve case (IDs are case-sensitive) and always carry a `.json` suffix. **Validation:** `DownloadJsonButton` unit +4 (asserts the OUTCOME per R10 - the downloaded file's name + parsed complete-record content, across both the Blob and data-URI paths); `DetailDrawer` unit +2 (actions render only when `jsonData` is set); LogsPage/LogsTab/ResourceDetailDrawer suites still green (51 pass); `logs-auth-inline` Playwright +1 (opens the log drawer, asserts both header actions, and downloads + parses the file to prove the complete record incl. the four body-hidden fields). Web tsc prod baseline unchanged (0 new prod errors; new specs compile clean); the DA-gate reused the existing `CopyJsonButton`/`useCopyToClipboard` + `DetailDrawer` seams (no new coupling). Version 0.54.61 -> 0.54.62.

- **Broadened the clickable-affordance + resizable-column coverage across the remaining UI (X1 + X5/X6 + X7 follow-up).** After the initial passes, an exhaustive audit found the remaining surfaces and this change closes them: **(X7)** the Discovery **schema-characteristics diff table** (Attribute + 7 characteristic columns) is now drag-resizable via the `useResizableColumns` hook (its sibling ServiceProviderConfig diff table already was). **(X1)** a new shared [interactive.ts](web/src/utils/interactive.ts) helper (`clickableProps` + `toggleChipProps` + `onActivateKeyDown`) gives every non-button clickable element the SAME uniform affordance as a real button - a `button` role, an Enter/Space keyboard handler, and a tab stop: applied to the **Users / Groups / global-Logs / endpoint-Logs / Workbench-history table rows** (previously mouse-only), and the **Logs status + time-range filter chips** are now proper toggle buttons (`role="button"` + `aria-pressed` + pointer cursor + keyboard), not static badges. The **Discovery / Create-endpoint-wizard / Me-profile picker cards** gained a `:hover` background so they read as clickable, and the SchemasTab sub-attribute expand toggle moved from `appearance="transparent"` to `subtle` for a visible hover affordance. **(X5/X6 audit outcome)** confirmed the auth-method chip is already on every applicable request-log surface (global Logs, endpoint Logs, dashboard + endpoint recent-activity); the ActivityTab (parsed event summaries), NotificationsDrawer (notifications), and live LogStream (app logs) are intentionally not request-log rows so the auth chip does not apply. **Validation:** interactive-helpers unit +4; the 8 affected page vitest suites still green (97 pass, no regressions); `logs-auth-inline` Playwright +1 (log row `role="button"` + status chip `role`/`aria-pressed`). Web tsc 96 baseline (0 new); web build clean. Version 0.54.57 -> 0.54.58.

- **Uniform "More" overflow-menu affordance (X1).** A new reusable [OverflowMenu](web/src/components/primitives/OverflowMenu.tsx) primitive standardizes the three-dot overflow control the WIF-trust + credential cards introduced (W7): every overflow menu now renders a `MoreHorizontal` three-dot icon PLUS a visible **"More"** text label (not a bare, ambiguous icon), a Fluent `subtle` Button hover/focus affordance, a tooltip, and a stable trigger `data-testid`; the menu items are passed as children. Both hand-rolled overflow menus in the Connect tab (the per-credential card + the WIF trust card - the only three-dot menus in the app) now use it, so the "clickable More menu" reads identically everywhere and is unmistakably interactive. This complements the clickable-row affordances added in X5 (dashboard + endpoint recent-activity rows became `role="button"` + `cursor: pointer` + keyboard-activatable, opening the request in the Logs detail) and the drag-handle affordance in X7. Fluent `Button`s + router links continue to carry the app's button/link affordances elsewhere. **Validation:** OverflowMenu unit +3 (labelled trigger, custom label, items-hidden-until-opened); CredentialsTab vitest 70 still green (the W7 reveal/rotate/deactivate/delete specs open via the unchanged `credential-more-*` / `wif-credential-more-*` testids); `wif-credentials` Playwright +1 assertion (the trigger shows the "More" label). Web tsc 96 baseline; web build clean. Version 0.54.56 -> 0.54.57.

- **Drag-to-resize table columns across the data grids (X7).** A new reusable [useResizableColumns](web/src/hooks/useResizableColumns.ts) hook + [ColumnResizeHandle](web/src/components/primitives/ColumnResizeHandle.tsx) primitive add mouse drag-resize to the app's hand-rolled `table-layout: fixed` grids without pulling in a data-grid dependency. Each resizable column header renders a thin `col-resize` grip on its right edge that widens/tints on hover (a discoverable clickable affordance); dragging sets that column's pixel width (min 48px), double-clicking the grip resets just that column, and widths persist to `localStorage` per table so a resize survives reloads. Applied to the global **Logs** table (now with its X6 Endpoint column), the endpoint **Logs** tab, **Users**, **Groups**, both **Operations** tables (all-users + all-groups), the **Workbench** request-history table (colgroup-driven), and the **Discovery** ServiceProviderConfig diff table. The dynamic-column schema-characteristics diff table is intentionally left non-resizable (its column set varies at runtime). **Validation:** useResizableColumns hook unit +7 + ColumnResizeHandle render +1; the 7 affected page vitest suites still green (95 pass, no regressions); `logs-auth-inline` Playwright +1 (drag the URL-column handle and assert the header widened by ~the drag distance). Web tsc 96 baseline (0 new); web build clean. Version 0.54.55 -> 0.54.56.

### Documentation
- **Wave 2 deep design analysis (auth strategy seam + enablement consolidation).** New [docs/auth/WAVE2_DESIGN_ANALYSIS.md](docs/auth/WAVE2_DESIGN_ANALYSIS.md) (linked from [docs/INDEX.md](docs/INDEX.md)) answers the operator's "is Wave 2 good design or are there better ways, and how will settings work after W2.5" questions with cross-framework research. Findings: the resource-plane `ResourceAuthenticator` seam is not a new model - it formalizes what the 491-line `SharedSecretGuard` already does implicitly, and that shape is the industry-standard **Spring Security `ProviderManager` + `AuthenticationProvider` probe-chain** (each provider signals success / fail / cannot-decide - the exact three-outcome contract). The doc contrasts ASP.NET Core's named-scheme model ("no automatic probing"), maps GoF Chain-of-Responsibility + Strategy, and concludes: **probe-chain + a token-shape `isApplicable` fast-path on the resource plane** (a client sends one opaque Bearer header, so it must probe; the X9 short-circuits become the fast-path), and a **keyed Strategy-select on the mint plane** (request shape self-identifies the method - already the `IAssertionTokenProvider` model). It establishes the load-bearing invariant that the contract MUST distinguish "not mine, continue" from "mine but invalid, stop" (or the reject-STOP cases - wrong-endpoint-scoped token, refused global secret - would regress into downgrade-confusion), plus 7 pitfalls with mitigations (ordering-as-policy, probe cost, telemetry threading, DI registration, over-abstraction/YAGNI, global-route null-endpoint). W2.5 is worked fully: the enabled/has-cred/**active** 4-state model, unifying mint + resource + metadata on one per-method source of truth (fixing today's asymmetry where the mint plane ignores the enablement flags), **Option A** (an explicit `enabled` on each `AuthenticationMethod`, migrated value-preservingly, legacy umbrella retired) with the operator UX and migration gaps. Per-item recommendations (W2.1-W2.5) + sequencing (W2.1+W2.5 together). Grounded with URL citations. Analysis only - no runtime change. Version 0.54.64 -> 0.54.65.
- **X13 delivery-plan revision (operator decisions on Wave 0 + auth-enablement flags).** Updated [docs/auth/AUTH_CONSOLIDATED_DELIVERY_PLAN.md](docs/auth/AUTH_CONSOLIDATED_DELIVERY_PLAN.md): **W0.1 secret redaction is DECLINED** per operator decision - `PERSIST_REQUEST_SECRETS` stays default `true` (request-secret capture is intentionally on for troubleshooting; operators opt out at runtime via the server env or the per-endpoint `PersistRequestSecrets` override), so Wave 0 is now just the HTTP 200 + `Cache-Control: no-store` and capability-truthful-metadata correctness items (15 -> 7 points; ~116 core). Added work item **W2.5** - consolidate the two overlapping enablement flags (the legacy umbrella `PerEndpointCredentialsEnabled`, which gates both `bearer` and `oauth_client` via fallback, vs the WI-11 per-method `SecretTokenBearerAuthEnabled` / `OAuthClientCredentialsAuthEnabled`) down to the per-method flags by finishing the WI-11 migration and retiring the legacy umbrella, AND co-locate enablement with each method (each `ResourceAuthenticator` owns `isEnabled()` reading the `AuthenticationMethod` model, with OAuth metadata derived from the same source so advertised and enforced cannot drift) - with a design answer on why co-locating is good design (cohesion/SRP + Open-Closed) and its two guardrails (keep enabled-vs-has-credential distinct; reuse the existing inert `profile.authentication` backbone, no new DSL). Version 0.54.62 -> 0.54.63.
- **Consolidated auth + WIF + performance delivery plan (X13).** New [docs/auth/AUTH_CONSOLIDATED_DELIVERY_PLAN.md](docs/auth/AUTH_CONSOLIDATED_DELIVERY_PLAN.md) (linked from [docs/INDEX.md](docs/INDEX.md)) interlocks the three previously-separate analysis streams - X11 token-mint latency, X12 auth-source refactoring, and the SyncFabric roadmap (RFC 8693, RFC 7523 binding corrections, persona model, security + metadata truthfulness, migration Phases -1..6) - into ONE sequenced release train. It is grounded in the confirmed current `feat/wif` source state (token endpoint returns 201 not 200; `PERSIST_REQUEST_SECRETS` defaults true; metadata advertises token-exchange with no runtime handler; no seam / persona / jti / index). Structure: Waves 0-6 with ~25 consolidated work items (each carrying goal, tasks, acceptance criteria, dependencies, relative-complexity estimate, and risk); a current-source state table; dependency + critical-path Mermaid diagrams; a story-point estimate rollup (~119 core points + ~40 optional/future); empirical gates (real SyncFabric assertion capture for customer + 1P modes); a per-item Definition-of-Done contract tied to the standing gates; and the Design & Architecture gate disposition. Core sequencing rule: **the X12 seam (Wave 2) precedes RFC 8693 (Wave 4)** so new methods EXTEND the strategy chain instead of EDITing the god-guard/controller; the security hotfix (Wave 0) ships first; the perf foundation (Wave 1) de-risks every external-JWKS method. Cross-links added from X11, X12, and the SyncFabric guide. Plan only - no runtime change. Version 0.54.60 -> 0.54.61.
- **Auth-source refactoring analysis + a standing Design & Architecture gate (X12).** New [docs/auth/AUTH_SOURCE_REFACTORING_ANALYSIS.md](docs/auth/AUTH_SOURCE_REFACTORING_ANALYSIS.md) (linked from [docs/INDEX.md](docs/INDEX.md)) answers the operator's "should the auth sources be refactored, decoupled, and how" question with a source-grounded analysis: it maps the current two-plane structure (token-mint + resource) and the patterns already used (repository + DI token, the `IAssertionTokenProvider` strategy seam, guard, small single-purpose services), ranks 5 coupling/SRP findings (the 491-line `SharedSecretGuard` god-guard is #1; the token-mint `client_secret` inline-vs-WIF-strategy asymmetry is #2; decision-trace emission duplicated 3x is #3), and recommends exactly ONE high-leverage, pattern-consistent refactor - a resource-plane `ResourceAuthenticator` strategy chain that mirrors the existing mint-plane seam, plus a symmetric `client_secret` extraction and a centralized decision emitter - decoupling **by auth method** (where the real independence lives). It records explicit YAGNI non-goals (no auth-policy DSL, no `OAuthService` split, no further repository abstraction) and a 3-phase backward-compatible TDD plan. Mermaid current-vs-proposed diagrams, a file-inventory + size table, a ranked findings dashboard, and a decoupling matrix. **Standing gate:** the doc defines a **Design & Architecture Self-Improvement Gate** (SRP / coupling / pattern-consistency / Open-Closed / YAGNI counter-check, with a mandatory applied-scheduled-accepted disposition), now added as a hard rule in [.github/copilot-instructions.md](.github/copilot-instructions.md) and as pattern **PF-1** (Category F) in [docs/strategy/ENGINEERING_LESSONS_AND_PATTERNS.md](docs/strategy/ENGINEERING_LESSONS_AND_PATTERNS.md). Analysis + gate only - no runtime change. Version 0.54.59 -> 0.54.60.
- **WIF token-mint latency deep analysis (X11).** New [docs/perf/WIF_TOKEN_MINT_LATENCY_ANALYSIS.md](docs/perf/WIF_TOKEN_MINT_LATENCY_ANALYSIS.md) (linked from [docs/INDEX.md](docs/INDEX.md)) breaks down the operator-reported 2,161 ms WIF token mint (`POST /scim/endpoints/:id/oauth/token` with a `client_assertion`) to the minutest per-stage granularity, grounded in measured data: the identical mint is **92 ms warm** (dev log `durationMs`), so the ~2,069 ms gap is one-time cold cost, not per-request cost. Root cause = (1) the external JWKS fetch runs synchronously on the hot path with **no background refresh** (10-min TTL, so ~every 10 min a request re-pays the cold fetch), (2) `jose` is lazy-`import()`ed on the first mint (nothing loads it at boot), and (3) the trust stored the legacy `login.windows.net` JWKS host which **redirects** to `login.microsoftonline.com` (measured ~130-160 ms slower/fetch). The doc includes a per-stage attribution table with file:line, cold-vs-warm waterfall + sequence Mermaid diagrams, a pre-caching inventory (JWKS keys, `jose`, trust config, credential set, DB pool), industry best practices (refresh-ahead, honor `Cache-Control`, keep-alive, startup pre-warm, client-side token caching - Entra/Google/Okta/Auth0/Spring patterns), use-case coverage (RFC 7523 variations, RFC 8693 token exchange, Entra 1P, Google GCP/Workspace, SAP SuccessFactors, Zoom, AWS Cognito/STS, Okta), 9 ranked options to reach tens of ms (eager `jose` import + startup JWKS/DB pre-warm + canonical `jwks_uri` eliminate cold; background refresh removes the periodic cold; per-endpoint credential cache + async logging take warm from ~92 ms to ~30-40 ms), and a suggested token-mint latency gate mirroring X9's `9z-BQ`. Analysis only - no runtime change in this commit. Version 0.54.58 -> 0.54.59.
- **Auth-methods standards comparison research doc (X10).** New [docs/auth/AUTH_METHODS_STANDARDS_COMPARISON.md](docs/auth/AUTH_METHODS_STANDARDS_COMPARISON.md) (linked from [docs/INDEX.md](docs/INDEX.md)) answers the operator's auth-standards questions: it maps SCIMServer's four auth methods (global shared secret, per-endpoint bearer, OAuth2 client-credentials, WIF) to the exact RFCs they implement; confirms the WIF-minted-JWT-as-`jwt-bearer` flow is correct per RFC 7521/7523 (validate the external assertion) + RFC 6750/9068 (mint + present a JWT bearer); confirms the client-secret approach is standard per RFC 6749 §4.4 + §2.3.1 (with SCIMServer's bcrypt-at-rest + rotation + basic/post hygiene rated above average); analyzes the token-return options (self-contained JWT vs opaque reference token vs hybrid) and recommends an OPT-IN endpoint-level `IssuedTokenFormat` (JWT default, opaque + RFC 7662 introspection + RFC 7009 revocation for instant-revocation needs); compares Entra / Google (RFC 8693 STS) / AWS (SigV4 - the outlier, not bearer) / Zoom / SAP / Okta; and gives a prioritized recommendation list (ship the RFC 8693 token-exchange profile; optional opaque-token format; `private_key_jwt` + mTLS per RFC 9700; a JWT `jti` denylist; DPoP). Grounded against the [Microsoft Entra client-credentials doc](https://learn.microsoft.com/en-us/entra/identity-platform/v2-oauth2-client-creds-grant-flow) + the OAuth RFC family. Docs-only (no runtime change). Version 0.54.54 -> 0.54.55.

### Added
- **Auth method + endpoint name on every log / activity row, and clickable activity rows (X5 + X6).** A new reusable [AuthMethodChip](web/src/components/primitives/AuthMethodChip.tsx) renders the authentication decision consistently on EVERY request-log + recent-activity surface: a resource-CRUD row shows **"auth ok - <method>"** (e.g. "auth ok - OAuth JWT") or the reject reason; a token-mint row (a `POST` to `/oauth/token`) shows **"JWT - <method>"**, naming the auth method that authorized the mint and the minted token type. **X5** - the chip now appears not just on the global Logs page + endpoint Logs tab (which had a bare "auth ok" badge) but also on the **dashboard recent-activity** rows and the **endpoint-overview recent-activity** rows, which previously had no auth indicator; those activity rows are now **clickable**, opening the request in the canonical Logs detail view. **X6** - the global Logs page gains an **Endpoint** column showing the endpoint NAME (resolved from the id) plus a quick-open button, and each dashboard activity row shows the endpoint name + quick-open. **Backend** - the log-list projection now surfaces `endpointId` (both Prisma + in-memory backends at parity) ([logging.service.ts](api/src/modules/logging/logging.service.ts)); the dashboard global + per-endpoint recent-activity projections now carry the persisted auth summary (`authOutcome`/`authMethod`/`authReason`/`requestId`) and the global one resolves the endpoint NAME ([dashboard.controller.ts](api/src/modules/dashboard/dashboard.controller.ts), [dashboard.types.ts](api/src/shared/types/dashboard.types.ts)). **Validation:** AuthMethodChip unit +6; LogsPage vitest +1 (chip method + endpoint name/quick-open); OverviewTab vitest updated (chip + clickable role); DashboardPage vitest +1 (chip + endpoint quick-open + clickable); `logs-auth-inline` Playwright +2 (resource vs token-mint chip; endpoint name/quick-open); live-test `9z-BS`. API build 0 (dashboard + logging unit 308 pass; dashboard E2E 8 pass on in-memory); web tsc 96 baseline; web build clean. Version 0.54.53 -> 0.54.54.
- **Credential + WIF-trust label + description fields (X3 + X4).** Every credential now carries an optional operator-supplied free-text **description** (never a secret), persisted in `metadata.description` on any credential type (no schema migration - reuses the existing credential `metadata` JSON) and surfaced in the endpoint overview projection ([admin-credential.controller.ts](api/src/modules/scim/controllers/admin-credential.controller.ts), [dashboard.controller.ts](api/src/modules/dashboard/dashboard.controller.ts), [dashboard.types.ts](api/src/shared/types/dashboard.types.ts)). **X4** - the credential create dialog gains a **Description** textarea alongside the existing Label. **X3** - the WIF trust create/edit form (which previously had neither) gains both a **Label** and a **Description** field; editing a saved trust loads its label + description, and Save carries them through the WIF `PUT` (which reuses the same `metadata.description` slot) ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx), [queries.ts](web/src/api/queries.ts)). The description is shown on each credential + WIF trust card and included in the per-card JSON export. A blank/whitespace description normalizes to null. **Validation:** admin-credential controller unit +1 (persist/echo/trim/blank-to-null); dashboard-overview E2E +1 (create echoes + overview surfaces + blank-to-null, cross-backend green on inmemory); CredentialsTab vitest +2 (X4 create-dialog description; X3 WIF label+description load+save+card); `connect-tab` Playwright +1 (route-mocked: create dialog + WIF form both expose Label + Description); live-test `9z-BR`. API build 0; web tsc 96 baseline; web build clean. Version 0.54.52 -> 0.54.53.
- **Connect-tab auth-method sub-tab ordering + naming (X8).** The Connect tab's per-method sub-tabs are re-ordered to the operator-requested sequence **OAuth2 Client-Credential -> WIF -> Per-endpoint bearer -> Shared secret** (previously shared-secret-first), and the global shared-secret method is renamed **"Shared secret" -> "Global Shared secret"** (its info banner title becomes "Global Shared secret (bearer)") to make clear it is the endpoint-wide secret, not a per-endpoint one ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx)). Because the enabled sub-tabs now lead with OAuth2 client-credentials, the default selected method tab follows the same rule (first non-shared-secret method when one is enabled), so an endpoint configured for OAuth opens on its OAuth tab. **Validation:** CredentialsTab vitest +1 (X8: asserts the exact tab order via `data-testid` + the "Global Shared secret" label) - 67 pass; `wif-credentials` Playwright updated for the new default tab + explicit bearer-tab clicks (`connect-tab` specs click the shared-secret tab by its unchanged `data-testid`, so they are unaffected by the label rename). Web tsc 96 baseline; web build clean. Version 0.54.51 -> 0.54.52.

### Fixed
- **Connect-tab: a retained BEARER credential's Secret Token now shows in the per-card Connect subpanel (X2).** The reveal API returns a bearer credential's retained secret in the `token` field (an `oauth_client` credential uses `clientSecret`), but the per-card Connect subpanel only ever read `clientSecret` - so a retained bearer Secret Token was silently never displayed (only the "shown when the endpoint retains it" note appeared, even when it WAS retained). The subpanel now reads the type-appropriate field (`token` for bearer, `clientSecret` for oauth_client) and shows the secret inline with the other connection parameters ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx)). A pre-existing vitest that mocked the bearer reveal with `clientSecret` (an unrealistic shape that masked the bug) was corrected to use `token`. **Validation:** CredentialsTab vitest (X2: bearer reveal `token` renders inline; the P5 W8 test corrected to the realistic `token` shape); `connect-tab` Playwright +1 (route-mocked: a retained bearer Secret Token shows in the subpanel). Web tsc 96 baseline; web build clean. Version 0.54.52 -> 0.54.53.
- **PERF (gate + RCA): the X9 per-endpoint latency regression is now documented and locked by a live-test gate.** A dedicated root-cause doc ([docs/perf/DEV_LATENCY_REGRESSION_RCA.md](docs/perf/DEV_LATENCY_REGRESSION_RCA.md), linked from [docs/INDEX.md](docs/INDEX.md)) captures the full investigation - symptom (dev p95 1,854 ms / max 2,821 ms vs identical-infra prod ~20 ms), the hypotheses ruled out (identical compute + identical `Standard_B1ms` DB + ~10% DB CPU, so NOT infra), the smoking-gun container log (the ~1,431 ms elapsed inside the auth guard BEFORE OAuth validation), the root cause (the guard bcrypt-compared the presented bearer against every active per-endpoint secret credential; a JWT / the global secret matches none, so it paid `O(active-secret-creds) x bcrypt` of pure wasted work), the two-part fix (0.54.49 JWT skip + 0.54.50 global-secret skip), the measured ~40x improvement, why no gate caught it, and the standards/norms (auth must be O(1)-per-request, short-circuit on token shape, latency is a gated SLO). **New regression gate** `9z-BQ` in [scripts/live-test.ps1](scripts/live-test.ps1) seeds an endpoint with 6 active bearer credentials and asserts a JWT-authenticated per-endpoint op's median latency stays under 800 ms (it is ~36 ms with the fix; a loop-reintroducing regression would be ~2,100 ms and trip the gate). The runtime companion is the already-present per-request `durationMs` slow-request logging. **Validation:** `9z-BQ` validated against dev v0.54.50 (median 36 ms with 6 seeded credentials, PASS). Version 0.54.50 -> 0.54.51.
- **PERF (follow-up): the global shared-secret bearer path also skips the per-endpoint bcrypt loop.** The primary fix (0.54.49) short-circuited JWT bearers, but a request presenting the operator-configured GLOBAL shared secret (an opaque token, not a JWT) still paid the full O(active-credentials) x bcrypt cost before falling through to the legacy acceptor (measured ~1,560 ms on the same dev endpoint that ran ~40 ms for a JWT after 0.54.49). A per-endpoint credential is an auto-generated random secret, so it can NEVER equal the configured global secret - comparing the global secret against each credential's bcrypt hash is therefore always wasted work. `SharedSecretGuard.tryEndpointCredential` now receives the configured `expectedSecret` and, when the presented token `safeCompare`-matches it, skips the credential fetch + bcrypt loop and falls straight through to the legacy global-secret acceptor (which still enforces the WI-11 `SharedSecretBearerAuthEnabled=false` per-endpoint refusal) ([shared-secret.guard.ts](api/src/modules/auth/shared-secret.guard.ts)). Genuine per-endpoint opaque bearer/oauth_client secrets are unaffected (still matched exactly). **Validation:** shared-secret guard unit +1 (the global secret skips `findActiveByEndpoint` + the bcrypt loop and authenticates via `legacy`) - 39 pass; API build 0. Version 0.54.49 -> 0.54.50.
- **PERF (major): per-endpoint request latency no longer scales with the endpoint's credential count - a JWT bearer skips the per-endpoint secret bcrypt loop.** Investigation of an operator-reported dev regression (per-endpoint SCIM ops at ~1,400-2,800 ms while the same ops on an identical-infra prod ran ~20 ms) found the cost was entirely in the auth guard, BEFORE the resource query: `SharedSecretGuard.tryEndpointCredential` bcrypt-compared the presented bearer against EVERY active per-endpoint secret credential (`bearer` + `oauth_client`) for the endpoint, and bcrypt is deliberately expensive (~hundreds of ms each). A per-endpoint credential is an OPAQUE random secret, but an Entra OAuth access token (and a WIF-minted token) is a **JWT** (`eyJ...` three-segment shape) that can NEVER match a bcrypt'd secret - so for the dominant OAuth-JWT traffic the whole O(active-credentials) loop was wasted work (a dev endpoint with 4 secret credentials spent ~1.4 s here before falling back to OAuth). The guard now short-circuits: when the token is a JWT (`looksLikeJwt`), it skips the credential fetch + bcrypt loop entirely and falls straight through to OAuth/JWKS validation ([shared-secret.guard.ts](api/src/modules/auth/shared-secret.guard.ts)). Opaque per-endpoint bearer/oauth_client secrets are unaffected (still matched exactly as before). **Root cause was data-shaped, not infra:** dev and the parallel prod share identical compute (0.5 vCPU) + DB (Standard_B1ms Burstable Postgres), the dev DB CPU was only ~10%, and a FRESH endpoint on the same dev build was fast (~30 ms) - the slow endpoint simply had 4 accumulated secret credentials. **Validation:** shared-secret guard unit +1 (a JWT skips `findActiveByEndpoint` + the bcrypt loop and authenticates via OAuth) - 38 pass; API build 0. Version 0.54.48 -> 0.54.49.

### Added
- **Connect-tab UX, part 3 (W7): the crowded credential-card action row is organized into a primary set + a "More" overflow menu.** Each credential / trust card previously showed every action inline (Reveal, Rotate, Connect, Edit, Deactivate, Copy/Download, Delete) - a scattered row. Now the frequently-used actions stay visible - **Connect** (the primary IdP-setup action, promoted to a `secondary`-appearance button), **Edit**, and the **Copy/Download JSON** export - while the less-common + destructive actions move into a Fluent `Menu` overflow (`credential-more-<id>` / `wif-credential-more-<id>`): **Reveal secret**, **Rotate secret**, **Deactivate/Activate**, and **Revoke** (with the destructive Revoke last) ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx)). The action test-ids are unchanged (the moved actions are the same `data-testid`s, now on `MenuItem`s), so integrations that target them still work after opening the menu. Item 7 of the [durable-auth-logs + Connect-UX plan](docs/auth/AUTH_DURABLE_LOGS_AND_CONNECT_UX_PLAN.md) (W1-W12) - completes the batch. **Validation:** CredentialsTab vitest +1 (primary actions visible; secondary/destructive hidden until the overflow menu opens) + the reveal/rotate/deactivate/delete specs updated to open the menu - 66 pass; `wif-credentials` + `credential-reveal` Playwright updated for the overflow menu. Web tsc 96 baseline; web build clean. Version 0.54.47 -> 0.54.48.
- **Connect-tab UX, part 2 (W11 + W12): the redundant "All" method tab and the redundant endpoint-level Connect card are gone.** **W11** - the Connect tab no longer has an "All" sub-tab; the per-method sub-tabs (shared secret / bearer / oauth_client / wif, only the enabled ones) are the single method axis. The default selection is the first configured PER-ENDPOINT method (bearer / oauth_client / wif) when one exists - the auth the operator deliberately set up for this endpoint - otherwise the global shared-secret method; a rare all-methods-disabled endpoint shows a "no auth methods enabled" hint. The obsolete `PerEndpointCredentialsEnabled`-off banner is removed (a disabled method simply has no tab). **W12** - the endpoint-level `ConnectionPanel` (the "Connect this endpoint to Entra ID" card) is kept ONLY on the shared-secret tab, which has no per-credential card + subpanel to carry its connection info; on the bearer / oauth_client / wif tabs it is removed because the per-card Connect subpanels (W6/W8) already provide the same values, so it was redundant ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx)). Items 11-12 (final) of the [durable-auth-logs + Connect-UX plan](docs/auth/AUTH_DURABLE_LOGS_AND_CONNECT_UX_PLAN.md) (W1-W12). **Validation:** CredentialsTab vitest updated across the R6 + P5 suites (no "All" tab, per-endpoint default, panel-on-shared-secret, retained-secret via the per-card subpanel) - 65 pass; `connect-tab` + `wif-credentials` Playwright updated (no "All" tab, shared-secret-scoped panel, wif-tab selection helper). Web tsc 96 baseline; web build clean. Version 0.54.46 -> 0.54.47.
- **Connect-tab UX, part 1 (W8 + W9 + W10): the Connect subpanel now covers bearer credentials, is renamed, and carries per-parameter help.** **W8** - the retractable in-card "Connect" subpanel (previously oauth_client-only) now also appears on per-endpoint **bearer** credential cards; a bearer card shows the Application API URL + (when retained) its Secret Token, while oauth_client keeps the OAuth token endpoint + client identifier. **W9** - the subpanel header is renamed from "Connect to Entra" to **"Connect this endpoint to IdP like Entra ID"** (Entra is one example IdP, not the only one), and each card's **Connect** button carries that same text as a hover `Tooltip` ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx)). **W10** - the per-parameter explanatory prose is moved onto each parameter label as a Fluent `InfoLabel` (an info icon with a hover/click popover) - `applicationApiUrl` / `oauthTokenEndpoint` / `clientIdentifier` / secret each get a one-line explanation via a shared `CONNECT_PARAM_HELP` map - instead of a wall of text. Items 8-10 of the [durable-auth-logs + Connect-UX plan](docs/auth/AUTH_DURABLE_LOGS_AND_CONNECT_UX_PLAN.md) (W1-W12). **Validation:** CredentialsTab vitest +1 (bearer Connect subpanel: renamed header, app-URL only, InfoLabel, export) + U2 extended (oauth_client rename + 3 InfoLabels) - 65 pass; `wif-credentials` Playwright U2/W8 generalized to bearer|oauth_client + rename + InfoLabel assertions. Web tsc 96 baseline; web build clean. Version 0.54.45 -> 0.54.46.
- **Connect-tab copy/download at the credential-card + connection-subpanel levels (W5 + W6).** Completes the "copy/export at every Connect level" story (W3 endpoint + W4 method already shipped). **W5** - each credential/trust card's per-card copy button is upgraded from copy-only to a full `SettingsJsonExport` (Copy-JSON + Download-.json) of that card's public object (`credential-export-<id>` on the generic card, `wif-credential-export-<id>` on the WIF trust card), reusing the shared non-secret `projectCredentialPublic` projection. **W6** - each card's expandable "Connect to Entra" subpanel now carries a `SettingsJsonExport` (`credential-connect-export-<id>` / `wif-connect-export-<id>`) of the whole IdP-connection bundle for that credential/trust (Application API URL + OAuth token endpoint + client identifier, plus the full WIF trust params on a WIF card) - so an operator can grab the exact connection payload in one click instead of copying each field. No secret values in any bundle. Fifth+sixth items of the [durable-auth-logs + Connect-UX plan](docs/auth/AUTH_DURABLE_LOGS_AND_CONNECT_UX_PLAN.md) (W1-W12). **Validation:** CredentialsTab vitest updated (V5/W5 card export copy+download; U2/W6 subpanel export copy+download) - 64 pass; `wif-credentials` Playwright updated (W5 card export, W6 subpanel export). Web tsc 96 baseline; web build clean. Version 0.54.44 -> 0.54.45.
- **Connect-tab copy/download-as-JSON at the endpoint + method levels (W3 + W4).** The Connect tab could copy an individual credential's public object (W5, per-card) but had no one-click "take the whole thing away" affordance at the endpoint or per-method level. **W3** - the Connect header now carries an endpoint-level export (`connect-endpoint-export`, via the `SettingsJsonExport` primitive so it is Copy-as-JSON + Download-.json) of the WHOLE endpoint auth + connection posture: every enabled method, the full `connectionInfo` (URLs + Entra field mappings), every credential/trust (public projection), and the auth-related config flags (`PerEndpointCredentialsEnabled`, `SharedSecretBearerAuthEnabled`, `SecretTokenBearerAuthEnabled`, `OAuthClientCredentialsAuthEnabled`, `WifCredentialsEnabled`, `CredentialSecretVisibility`) - never a secret value. **W4** - selecting a specific method sub-tab reveals a per-method export (`connect-method-export-<method>`) scoped to that one method's connection info + the credentials/trusts backing it. Both bundles are assembled client-side from the already-loaded endpoint overview (no new API round trip) via `buildEndpointConnectBundle` / `buildMethodConnectBundle` ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx)), reusing the same non-secret credential projection as the per-card W5 copy. Third+fourth items of the [durable-auth-logs + Connect-UX plan](docs/auth/AUTH_DURABLE_LOGS_AND_CONNECT_UX_PLAN.md) (W1-W12). **Validation:** CredentialsTab vitest +2 (W3 endpoint export copy+download present; W4 per-method export appears only on a specific method sub-tab); `connect-tab` Playwright +2 (W3 endpoint export; W4 per-method export). Web tsc 96 baseline; web build clean. Version 0.54.43 -> 0.54.44.
- **Decode encoded/JWT values everywhere (W2) - a "Decode" affordance for Bearer / `client_assertion` / `access_token` values, in the UI and via an admin API.** A JWT is signed, not encrypted, so its header + claims are readable by the holder; operators needed a way to inspect what a token in a request/response header or body actually contains. **Shared decoder:** a pure, total (`never throws`) `decodeJwt` + `looksLikeJwt` on both sides ([api/src/oauth/jwt-decode.util.ts](api/src/oauth/jwt-decode.util.ts), [web/src/utils/jwt-decode.ts](web/src/utils/jwt-decode.ts)) - it base64url-decodes the two non-secret segments and returns `{ isJwt, header, payload, signaturePresent, reason }`; the signature is never returned (only a presence flag), and it never verifies. **Admin API:** `POST /scim/admin/decode-jwt` ([admin.controller.ts](api/src/modules/scim/controllers/admin.controller.ts)) decodes a pasted token (admin-gated; always 200, the caller inspects `isJwt`; the submitted `token` body key is redacted by the request logger when `PersistRequestSecrets` is off). **UI:** a new `JwtDecodeButton` primitive ([JwtDecodeButton.tsx](web/src/components/primitives/JwtDecodeButton.tsx)) renders a "Decode JWT" button that toggles an inline, copyable view of the decoded header + claims; and `CopyableJsonBlock` ([CopyableJsonBlock.tsx](web/src/components/primitives/CopyableJsonBlock.tsx)) now scans the JSON it displays (via `findJwtsInValue`) and offers a per-token decode button keyed by path - so every request/response header + body viewer in the log detail (and anywhere else that renders JSON) gets inline JWT decoding for free. Second item of the [durable-auth-logs + Connect-UX plan](docs/auth/AUTH_DURABLE_LOGS_AND_CONNECT_UX_PLAN.md) (W1-W12). **Validation:** jwt-decode util unit (API 8 / web 11 incl. `findJwtsInValue` path extraction); E2E +4 ([decode-jwt.e2e-spec.ts](api/test/e2e/decode-jwt.e2e-spec.ts) - decode / Bearer-prefixed / non-JWT / admin-gated); web vitest +5 (button reveals header+claims; CopyableJsonBlock offers inline decode); live-test `9z-BP`. API build 0; web tsc 96 baseline; web build clean. Version 0.54.42 -> 0.54.43.
- **Durable auth diagnostics on the request-log row (W1) - the full auth decision is now permanent, fixing "No auth decision for this request".** Operator report: a dev log detail showed the persisted "Authenticated via `bearer_jwt`" summary (V10/V11) but the U11 expected-vs-received panel said "No auth decision for this request" because that panel read the short-TTL (30-minute) `AuthDecisionRecordStore` via `useAuthDecisions`, which had expired. Auth is part of a request's own processing, so its full diagnostics now live ON the request row. **Backend:** a new nullable `authDecision` column on `RequestLog` ([schema.prisma](api/prisma/schema.prisma) + migration `20260722120000_add_requestlog_auth_decision`, InMemory parity) holds the complete, redacted `AuthDecisionTrace` (every `checks[]` step with `expected`/`received`, `decodedClaims`, `joseHeader`, `plane`, `selectedTrustId`, `subTraces` - all non-secret by the trace contract; the raw assertion/token never enters the trace). `emitAuthDecisionEvent` ([auth-decision-trace.ts](api/src/oauth/auth-decision-trace.ts)) stamps the serialized trace onto the correlation context at the single emit choke point; `LoggingService.recordRequest` persists it (size-capped) on both backends; `getLog` returns it parsed. **UI:** `AuthDecisionForRequest` ([AuthDiagnosticsPanel.tsx](web/src/components/primitives/AuthDiagnosticsPanel.tsx)) gains a `persistedDecision` prop - when the log detail carries the persisted trace it is the source of truth (the short-TTL store is not even queried, `enabled:false`), so the diff renders permanently; the "short-lived diagnostics" hint becomes "persisted with the request log. Non-secret; permanent." Both the global Logs page ([LogsPage.tsx](web/src/pages/LogsPage.tsx)) and the endpoint Logs tab ([LogsTab.tsx](web/src/pages/LogsTab.tsx)) pass `detailQuery.data.authDecision`. Because every request that emits an auth decision (including resource-plane `bearer_jwt` accepts on `/Users`) stamps the trace, the panel is now populated for exactly the cases the operator saw blank. First item of the [durable-auth-logs + Connect-UX plan](docs/auth/AUTH_DURABLE_LOGS_AND_CONNECT_UX_PLAN.md) (W1-W12). **Validation:** auth-decision-trace unit +1 (full-trace stamp); logging-auth-summary unit +1 (persist + parse round-trip); `activity.controller` mock fixtures extended; E2E +1 (the row detail carries the full trace + no secret leak); web vitest +1 (drawer renders the diff from the persisted decision with an EMPTY store); live-test `9z-BO`. API build 0; web tsc 96 baseline held. Version 0.54.41 -> 0.54.42.
- **Robust request-body capture on pre-parse failures - a rejected request's body is now captured or explicitly marked, never silently empty.** Closes the edge case where a request that fails BEFORE body parsing (a malformed JSON `400` or a wrong-`Content-Type` `415`) persisted a `RequestLog` row with an empty `requestBody`. The row was always stored (the exception filters run), but the body was lost. Now: **Tier 1 (marker)** - when the body cannot be captured, the stored `requestBody` is a `{ "_bodyNotCaptured": true, "reason": "content-type-rejected" | "unparseable", "contentType", "contentLength" }` marker so the absence is self-explanatory ([request-body-capture.ts](api/src/modules/logging/request-body-capture.ts)). **Tier 2 (raw capture)** - the body parsers now install a `verify` hook ([body-parsers.ts](api/src/bootstrap/body-parsers.ts), shared by [main.ts](api/src/main.ts) + the E2E harness [app.helper.ts](api/test/e2e/helpers/app.helper.ts) for parity) that stashes the raw buffer BEFORE parsing, so a malformed-JSON `400` surfaces the actual bytes as a capped `_rawPreview` (a wrong-content-type `415` skips the parser, so it gets the marker without a preview). **Cross-cutting safeties:** every stored body (request AND response) is size-capped via `capStoredBodyString` (over-cap -> a `{ "_truncated": true, "originalLength", "preview" }` marker, so a multi-MB payload cannot bloat the `RequestLog` table), and the free-text `_rawPreview` is redacted to `[REDACTED]` when the effective `PersistRequestSecrets` flag is off (a key-based redactor cannot reach a blob's contents). Both exception filters ([scim-exception.filter.ts](api/src/modules/scim/filters/scim-exception.filter.ts), [global-exception.filter.ts](api/src/modules/scim/filters/global-exception.filter.ts)) resolve the body through the shared helper. Tier 3 (a terminal Express error handler) was investigated and deliberately NOT added - a probe confirmed the `GlobalExceptionFilter` (`@Catch()`) already catches body-parser `SyntaxError`s and persists the row, so no backstop is needed. **Validation:** helper unit +11 ([request-body-capture.spec.ts](api/src/modules/logging/request-body-capture.spec.ts)); recordRequest safety unit +3 ([logging-body-capture.spec.ts](api/src/modules/logging/logging-body-capture.spec.ts) - raw-preview kept when secrets on, redacted when off, over-cap truncation); E2E +3 ([request-body-capture.e2e-spec.ts](api/test/e2e/request-body-capture.e2e-spec.ts) - malformed -> unparseable marker + raw bytes; 415 -> content-type-rejected marker; valid -> real parsed body); live-test `9z-BN` (T1-T5). API build 0; filters + logging suites 324 pass. Version 0.54.40 -> 0.54.41.
- **Credential-lifecycle + auth-in-logs batch, Track C: the authentication decision is now PERSISTED on the request-log row (V10), naming the winning method/credential (V11), so a rejected request stays legible in the logs list durably (V12).** Third and final track of the [credential-lifecycle + auth-in-logs plan](docs/auth/CREDENTIAL_LIFECYCLE_AND_AUTH_IN_LOGS_PLAN.md). Previously the per-row auth chip on the Logs surface read from the short-TTL `AuthDecisionRecordStore` via a SECOND query (`useAuthDecisions`), joined by `requestId === correlationId` - so the chip went blank once the 30-minute store expired, and every render paid for an extra fetch. **V10** - the `RequestLog` model gains four nullable columns `authOutcome` / `authMethod` / `authReason` / `authCredentialId` ([schema.prisma](api/prisma/schema.prisma) + migration `20260721180000_add_requestlog_auth_summary`, indexed on `authOutcome`). The auth decision is stamped onto the `AsyncLocalStorage` correlation context at the single emit choke point `emitAuthDecisionEvent` ([auth-decision-trace.ts](api/src/oauth/auth-decision-trace.ts)) - reading `outcome` / `method` / `reasonCode` / `selectedTrustId` from the trace - then `LoggingService.recordRequest` ([logging.service.ts](api/src/modules/logging/logging.service.ts)) reads the context and persists the four fields on BOTH the Prisma and InMemory backends at parity. The logs list + detail projections (`listLogs`, `getLog`, and by delegation the endpoint history + admin logs controllers) now return them. **V11** - the winning credential/trust id is carried onto the trace (`selectedTrustId` for WIF, the guard-stamped `authCredentialId` for bearer/oauth) and the request-log detail drawer renders a durable one-line summary "Authenticated via {method} using {credential} because {reason}" (`log-detail-auth-summary`) on both the global Logs page ([LogsPage.tsx](web/src/pages/LogsPage.tsx)) and the endpoint Logs tab ([LogsTab.tsx](web/src/pages/LogsTab.tsx)). **V12** - because the outcome is now persisted on the row, the per-row chip (`log-row-auth-{id}`) reads the row's own `authOutcome`/`authReason` first (falling back to the live decision map only for pre-V10 rows), so a rejected request keeps its red reason-code chip long after the ephemeral decision record is gone. The short-TTL store is retained for the deep expected-vs-received diff (U11). **Validation:** API unit +7 (`logging-auth-summary.spec.ts` 4 - persist+project accept/reject/detail/none on InMemory; `auth-decision-trace.spec.ts` 3 - the emit choke point stamps the context on accept/reject and is a no-op outside a context); E2E +1 (`endpoint-oauth-client` - a rejected oauth_client token request persists `authOutcome=reject`/`authMethod=oauth_client`/`authReason=oauth_client_auth_failed` on its RequestLog row); web vitest +3 (`LogsPage.test.tsx` - durable chip renders with an EMPTY decision store, accept chip, drawer "Authenticated via" summary); Playwright +2 (`logs-auth-inline` - durable chip + drawer summary with an empty decision store); live-test `9z-BM` (T1-T6, reject+accept token requests -> auth summary on the log rows). `activity.controller.spec.ts` mock RequestLog fixtures extended with the four nullable columns. API build 0; full API unit 4408 pass; web tsc 96 baseline; web build clean. Version 0.54.39 -> 0.54.40.
- **Credential-lifecycle + auth-in-logs batch, Track A: WIF verify-persistence bug fix (V7), a Verify button on the trust card (V8), a togglable Edit button (V9), and the OAuth2 sub-tab rename (V6).** First track of the [credential-lifecycle + auth-in-logs plan](docs/auth/CREDENTIAL_LIFECYCLE_AND_AUTH_IN_LOGS_PLAN.md). **V7 (bug fix)** - a standalone `POST .../wif/verify` was a stateless reachability check, so a successful verify left the trust card showing "Unverified". It now accepts an optional `credentialId`; when supplied AND the verify passes, it stamps `metadata.lastVerifiedAt` on that credential ([admin-credential.controller.ts](api/src/modules/scim/controllers/admin-credential.controller.ts)) and returns it on the `WifVerifyResult`; the web `useVerifyWifTrust` mutation invalidates the endpoint overview so the card flips Unverified -> Verified. A verify with no `credentialId` stays a pure dry-run. **V8** - each WIF trust card gains a **Verify** button (`wif-credential-verify-{id}`) that runs the persisting verify for that trust (using its stored issuer/JWKS) and renders the checklist in-card (`wif-credential-verify-result-{id}`) - no Edit needed. **V9** - the WIF card **Edit** button now toggles (a second click closes the in-card edit form), matching the Connect button. **V6** - the Connect tab's OAuth2 sub-tab is renamed **"OAuth2 Client-Credential"** ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx)). **Validation:** admin-credential controller unit +4 (persist on pass / no-persist on dry-run / no-persist on fail / no-persist for non-wif); CredentialsTab vitest +3 (V6 label, V8 card verify, V9 edit toggle); wif-trust-management Playwright +2 (V8 card verify, V9 edit toggle); live-test `9z-BK` (verify persists lastVerifiedAt against a real Entra tenant). API build 0; web tsc 96 baseline; web build clean. Version 0.54.37 -> 0.54.38.
- **Connect + Logs UX overhaul, U11 + U12: the authentication decision is now part of a request's own log detail, with an auth chip in the list.** Track 4 of the [Connect + Logs UX overhaul](docs/auth/CONNECT_AND_LOGS_UX_OVERHAUL_PLAN.md), making auth a first-class, inspectable step of each request instead of a disjointed sibling panel. **U11** - a new reusable `AuthDecisionForRequest` primitive ([AuthDiagnosticsPanel.tsx](web/src/components/primitives/AuthDiagnosticsPanel.tsx)) renders the auth decision for a request INSIDE its `DetailDrawer` (`log-detail-auth-section`), joined by `requestId === correlationId`, showing the outcome badge + the expected-vs-received per-check diff + reason code + remediation inline - on both the global Logs page ([LogsPage.tsx](web/src/pages/LogsPage.tsx)) and the endpoint Logs tab ([LogsTab.tsx](web/src/pages/LogsTab.tsx)). The old "View auth decision" focus-jump button is removed. **U12** - each request-log row shows a glanceable **auth-outcome chip** (`log-row-auth-{id}`: green `auth ok` / red reason code) derived from the recent-decision store, and the standalone `AuthDiagnosticsPanel` is removed from the logs surface (re-scoped to the endpoint **Connect -> Health** view, where it already lives). This is the browser-devtools "one request -> its headers/auth in one detail" model, satisfying locality of reference + progressive disclosure with no new API plumbing (reuses the shipped `AuthDecisionRecordStore` + `requestId <-> correlationId` bridge). **Validation:** LogsPage vitest +2 (inline record + row chip); LogsTab vitest updated (inline section); AuthDiagnosticsPanel vitest held; new route-mocked `logs-auth-inline` Playwright spec (chip + inline section + panel-absent); web tsc baseline 96 held; web build clean. Version 0.54.36 -> 0.54.37.
- **Connect + Logs UX overhaul, U2: each OAuth2 client credential has its own "Connect to Entra" params panel.** Track 3 of the [Connect + Logs UX overhaul](docs/auth/CONNECT_AND_LOGS_UX_OVERHAUL_PLAN.md). Every `oauth_client` credential row now carries a **Connect** button (`credential-connect-{id}`) that expands, in-card, the connection bundle for THAT credential (`credential-connect-panel-{id}`): Application API URL -> OAuth token endpoint -> Client identifier = this credential's public client id, in the U10 order ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx)). The endpoint-overview credential projection ([dashboard.controller.ts](api/src/modules/dashboard/dashboard.controller.ts) + [dashboard.types.ts](api/src/shared/types/dashboard.types.ts)) surfaces a new public `oauthClientId` (from `metadata.clientId`, never a secret) so each row can render its own bundle without a second round trip; the operator uses the existing per-credential **Reveal** to view the secret when the endpoint retains it. **Validation:** CredentialsTab vitest +1 (Connect reveals the panel with this credential's client id); dashboard-overview E2E +1 (`oauthClientId` surfaced, no secret); dashboard controller unit held; wif-credentials Playwright +1 (skip-safe in-card panel); live-test `9z-BJ` (T1-T3). API build 0; web tsc baseline 96 held; web build clean. Version 0.54.35 -> 0.54.36.
- **Connect + Logs UX overhaul, U3 + U4 + U6 + U1: WIF sub-tab progressive disclosure - add-behind-a-button, edit-in-card, per-trust Connect, and an Advanced accordion.** Track 2 (WIF sub-tab UX) of the [Connect + Logs UX overhaul](docs/auth/CONNECT_AND_LOGS_UX_OVERHAUL_PLAN.md), delivering the operator-approved option (b) targeted progressive disclosure instead of one monolithic accordion. The WIF trust form was extracted into a reusable `WifTrustFieldGrid` and the section restructured ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx)): **U3** - the add-trust form is now collapsed behind an **"Add trust"** button (`wif-add-trust-button` / `wif-add-trust-form` / `wif-cancel-add-button`), so viewing the configured trusts is not buried under a form. **U4** - **Edit** opens an in-card edit form (`wif-trust-edit-form-{id}` with `wif-trust-edit-save-{id}` / `-cancel-{id}` / `-verify-{id}`) **below that trust**, instead of hoisting it into the shared top form. **U6** - each trust gains a **Connect** button (`wif-credential-connect-{id}`) that expands the per-trust Entra connection params in-card (`wif-credential-connect-panel-{id}`, U10 order Application API URL -> OAuth token endpoint -> Client identifier = that trust's subject). **U1** - the assertion debugger moved behind an **"Advanced / troubleshooting"** accordion (`wif-advanced-accordion` / `wif-advanced-toggle`), collapsed by default, so the occasional tool no longer dominates the common case. **Validation:** CredentialsTab vitest 52 (existing 14 WIF specs updated for the new flow + new U3 add-behind-button + U6 per-trust-connect specs); ConnectionPanel vitest held; wif-credentials + wif-trust-management Playwright specs updated for add-behind-button + in-card edit + new U1/U3/U6 specs; web tsc baseline 96 held (0 in changed prod files); web build clean. Version 0.54.34 -> 0.54.35.
- **Connect + Logs UX overhaul, U9 + U10 + U5 + U7/U8 UI: WIF form field order, WIF connection labels, and per-trust validity on the Connect tab.** Track 2 (WIF sub-tab UX) of the [Connect + Logs UX overhaul](docs/auth/CONNECT_AND_LOGS_UX_OVERHAUL_PLAN.md). **U9** - the WIF trust form fields are re-ordered to the operator-requested sequence Issuer (iss) -> JWKS URI -> Subject (sub) -> Audience (aud), then the optional fields ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx)). **U10** - the WIF connection bundle relabels the endpoint SCIM URL field from the generic "Tenant URL" to Entra's WIF-specific **"Application API URL"** ([ConnectionPanel.tsx](web/src/components/primitives/ConnectionPanel.tsx)); the order (Application API URL -> OAuth token endpoint -> Client identifier) already matched. **U5** - each displayed trust field (issuer / JWKS / subject / audience / tenant) now carries an inline per-field validity badge (`wif-credential-{id}-{field}-status`: OK / warning / error) computed from https-format + JWKS-host-allowlist + gleaned-source + last-verified signals. **U7 (UI)** - each configured trust shows a Verified / Unverified badge + "Last verified {date}" line (`wif-credential-{id}-validity`). **U8 (UI)** - when the tenant was gleaned (U8 API), the tenant field status marks it "Inferred from the issuer / JWKS URI". **Validation:** CredentialsTab vitest +2 (verified + per-field status + inferred marker; unverified badge); ConnectionPanel vitest +1 (U10 Application API URL label); web tsc baseline 96 held (0 in changed prod files); web build + build size clean. Version 0.54.33 -> 0.54.34.

- **Connect + Logs UX overhaul, U7: every connection method now reports `validity` + `lastVerifiedAt` + `lastUsedAt`.** Track 1 (API foundations) of the [Connect + Logs UX overhaul](docs/auth/CONNECT_AND_LOGS_UX_OVERHAUL_PLAN.md). The connection-info contract ([connection-info.types.ts](api/src/shared/types/connection-info.types.ts)) gains a per-method operator-facing status trio: `validity` (`ok` verified-or-recently-accepted | `failing` last runtime auth rejected | `unverified` never verified and no recent success | `invalid` reserved), `lastVerifiedAt` (ISO time of the last passing verify-on-save), and `lastUsedAt` (ISO time of the last runtime auth ACCEPT from the short-TTL decision store). `ConnectionInfoService.assemble()` ([connection-info.service.ts](api/src/modules/scim/services/connection-info.service.ts)) computes them per method via a new `enrichMethodStatus()` (validity prefers the most-recent runtime outcome, falling back to whether the trust was ever verified; `lastUsedAt` is the last accept we still hold; `lastVerifiedAt` is read from the backing credential metadata). The WIF create + edit paths ([admin-credential.controller.ts](api/src/modules/scim/controllers/admin-credential.controller.ts)) stamp `metadata.lastVerifiedAt` when a `verify:true` save passes and carry a prior timestamp forward across edits; the WIF trust overview projection ([dashboard.controller.ts](api/src/modules/dashboard/dashboard.controller.ts) + [dashboard.types.ts](api/src/shared/types/dashboard.types.ts)) surfaces `lastVerifiedAt`. **Validation:** connection-info service unit +4 (failing/ok/unverified/verified-not-used); admin-credential controller unit +2 (stamp on verify, none without verify); connection-info E2E +1 + key-allowlist extended (`lastVerifiedAt`/`lastUsedAt`/`validity`); dashboard projection key-allowlist updated; live-test `9z-BI`. API build 0. Version 0.54.32 -> 0.54.33.

- **Connect + Logs UX overhaul, U8: WIF `allowedTenantId` is gleaned from the trust's issuer / JWKS URI when the operator omits it.** Track 1 (API foundations) of the operator-approved [Connect + Logs UX overhaul](docs/auth/CONNECT_AND_LOGS_UX_OVERHAUL_PLAN.md). A new pure helper `inferAllowedTenantId(issuer, jwksUri)` ([infer-allowed-tenant-id.ts](api/src/oauth/infer-allowed-tenant-id.ts)) extracts the Entra directory tenant GUID from the first GUID-shaped path segment of the issuer (preferred) or the JWKS URI, covering every Entra cloud (commercial `login.microsoftonline.com`, US-gov `login.microsoftonline.us`, china `login.partner.microsoftonline.cn`) and both the v2 issuer and v1 `sts.windows.net` shapes host-agnostically; a URL with no tenant GUID (e.g. a Google issuer or a `common`/`organizations` placeholder) yields `null` and the tenant stays required-explicit. Wired into BOTH the WIF create and edit paths in [admin-credential.controller.ts](api/src/modules/scim/controllers/admin-credential.controller.ts) via `applyTenantGleaning()`: when `allowedTenantId` is blank it is inferred and a non-secret `allowedTenantIdSource` marker (`issuer` | `jwksUri`) is stored on the trust metadata; an explicit tenant is never overridden and carries no marker; a non-inferable trust with no explicit tenant is still rejected `400`. The gleaned source is surfaced through the endpoint-overview WIF trust projection ([dashboard.controller.ts](api/src/modules/dashboard/dashboard.controller.ts) + [dashboard.types.ts](api/src/shared/types/dashboard.types.ts), closed-allowlist preserved). **Validation:** helper unit 10; admin-credential controller unit +4 (glean from issuer / JWKS / explicit-not-overridden / non-inferable rejected); dashboard controller unit updated (projection key allowlist); E2E [wif-tenant-gleaning.e2e-spec.ts](api/test/e2e/wif-tenant-gleaning.e2e-spec.ts) 4; live-test `9z-BH` (T1-T5). API build 0. Version 0.54.31 -> 0.54.32.
- **Auth traceability: the `requestId` correlator now works for GUARD-level rejections (auth 401/403), via an early correlation middleware.** Closes the follow-up left open by the F1-F4 gap closure. Previously the request-id + `AsyncLocalStorage` correlation context were established by the `RequestLoggingInterceptor`, which runs AFTER guards - so a guard-rejected 401 (exactly the auth failures operators chase) carried the `reason_code` but no `requestId`, and its RequestLog row + `Auth decision` event had no correlator. A new early **correlation middleware** ([correlation-middleware.ts](api/src/bootstrap/correlation-middleware.ts)), applied via `app.use()` in both production ([main.ts](api/src/main.ts)) and the E2E harness ([app.helper.ts](api/test/e2e/helpers/app.helper.ts)) so both behave identically, runs BEFORE guards: it sets the `X-Request-Id` header (single source of the id), establishes the ALS correlation context (so guards, services, and exception filters can read `requestId`/`endpointId`), and stashes a base `RequestLoggingMeta` on the request. The `RequestLoggingInterceptor` ([request-logging.interceptor.ts](api/src/modules/logging/request-logging.interceptor.ts)) was refactored to REUSE that request-id when the middleware ran (it only enriches the meta with the parsed body) and keep its self-sufficient context/header fallback for unit isolation. `ScimExceptionFilter` ([scim-exception.filter.ts](api/src/modules/scim/filters/scim-exception.filter.ts)) now reads the request-id from the correlation context OR the stashed meta (belt-and-suspenders, robust regardless of ALS propagation), so a guard-rejected 401 diagnostics block now carries `reason_code` + `requestId` (=== the `X-Request-Id` header) + `logsUrl`, the canonical `Auth decision` event carries the `correlationId`, and the RequestLog row is written with the `requestId`. **Validation:** interceptor + filters + logging unit 309 pass (dual-path interceptor); E2E `auth-reason-codes` 6/6 (added: a guard-rejected 401 carries requestId in diagnostics + header, and they match); live-test `9z-BG.T3b`. API build 0, ESLint 0 errors (494 warnings, net -1); full API unit 4389 pass (135 suites). Docs: [AUTH_ERROR_DIAGNOSTICS_AND_OBSERVABILITY.md](docs/auth/AUTH_ERROR_DIAGNOSTICS_AND_OBSERVABILITY.md) (follow-up marked CLOSED). Version 0.54.30 -> 0.54.31.

- **Auth error-handling / observability / logging gap closure (F1-F4) - complete reason + sub-reason at every layer (API, UI, logs) + configurable request-log privacy.** A norms audit of every auth flow found four residual gaps beyond the shipped WI-D1..D8 / P1-P5 epic; all four are now closed. **F1 - RequestLog secret persistence is now a deliberate, configurable behavior.** The RequestLog previously stored request/response headers + bodies (including `Authorization`, `client_secret`, `access_token`) verbatim. Per operator direction it now keeps the COMPLETE request/response for fast RCA **by default**, governed by a new server env `PERSIST_REQUEST_SECRETS` (default `true`) + a per-endpoint `PersistRequestSecrets` config flag (endpoint OVERRIDES server, `effective = endpoint ?? server-env ?? true`); when set to `false` a shared recursive redactor ([redact-sensitive.ts](api/src/security/redact-sensitive.ts)) masks secret-bearing header/body values before the row is persisted (and therefore before API/UI display). The same redactor is now applied by `ScimLogger.sanitizeData` ([scim-logger.service.ts](api/src/modules/logging/scim-logger.service.ts)) so shipped console/file structured logs ALWAYS redact NESTED secrets (defense in depth) regardless of the flag - closing a `{ body: { client_secret } }` TRACE-log leak. `LoggingService.recordRequest` ([logging.service.ts](api/src/modules/logging/logging.service.ts)) resolves the effective flag via a cycle-safe `ModuleRef` lazy read of `EndpointService.getCachedProfileSettings()` and redacts the four stored fields when off. **F2 - the GLOBAL `/scim/oauth/token` endpoint** ([oauth.controller.ts](api/src/oauth/oauth.controller.ts)) now carries a stable `reason_code` on every RFC-6749 error body (`grant_type_unsupported` / `missing_credentials` / `oauth_client_auth_failed`) and emits the canonical `Auth decision` event + short-TTL record - parity with the per-endpoint token endpoint. **F3 - the resource-plane OAuth-JWT sub-reason is preserved:** `OAuthService.validateAccessToken` ([oauth.service.ts](api/src/oauth/oauth.service.ts)) attaches a jose-style `code` to its thrown error and the guard ([shared-secret.guard.ts](api/src/modules/auth/shared-secret.guard.ts)) classifies it into `bearer_oauth_expired` / `bearer_oauth_signature_invalid` instead of collapsing every bearer failure to `bearer_invalid`. **F4 - the resource-plane `reason_code` is now on the wire:** the bearer guard carries the specific reason inside the SCIM `Diagnostics` extension URN (a documented member, so the SCIM error contract stays intact), `ScimExceptionFilter` ([scim-exception.filter.ts](api/src/modules/scim/filters/scim-exception.filter.ts)) MERGES it with the auto-enriched `requestId`, and the UI ([queries.ts](web/src/api/queries.ts) reads `reason_code` from the diagnostics extension; [scim-error.ts](web/src/api/scim-error.ts) gains the 5 resource-plane `bearer_*` catalog entries) renders the specific bearer remediation. **UI:** a new "Logging & privacy" category on the endpoint Settings tab ([SettingsTab.tsx](web/src/pages/SettingsTab.tsx)) with the `PersistRequestSecrets` Switch (default ON). **Validation:** redactor unit +9; ScimLogger nested-redaction +1; endpoint-config `PersistRequestSecrets` +6; RequestLog redaction +4 (default store-all / server-env off / endpoint-override both directions); global oauth controller +4 (reason_code + accept/reject decisions, no secret in trace); guard F3/F4 +5 (expired/signature/junk sub-reasons + bearer_missing/scoped diagnostics); E2E `auth-reason-codes` +5 (F2 wire + F4 diagnostics); SettingsTab vitest +2; scim-error catalog parity +5; live-test `9z-BG` (T1-T5). API build 0, ESLint 0 errors (495 warnings, net 0); full API unit 4389 pass (135 suites); web tsc 96 baseline held. Docs: [AUTH_ERROR_DIAGNOSTICS_AND_OBSERVABILITY.md](docs/auth/AUTH_ERROR_DIAGNOSTICS_AND_OBSERVABILITY.md) (gap-closure section), [ENDPOINT_CONFIG_FLAGS_REFERENCE.md](docs/ENDPOINT_CONFIG_FLAGS_REFERENCE.md) (27 flags + PersistRequestSecrets), [README.md](README.md) (`PERSIST_REQUEST_SECRETS` env). Known follow-up (out of scope): the `requestId` correlator is not yet enriched for GUARD-level rejections (the request-logging interceptor runs after guards). Version 0.54.29 -> 0.54.30.

- **Runtime egress robustness for the WIF JWKS fetch - server-level + per-endpoint configurable timeout / retries / backoff / cache-max-age, with endpoint overriding server.** The runtime JWKS fetch made during a WIF (RFC 7523 `jwt-bearer`) token-mint previously used the bare global `fetch` with no timeout, no retry, no redirect re-validation, and no request coalescing - a slow or flaky IdP could hang or thrash the mint path. A new `EgressPolicy` ([egress-policy.ts](api/src/oauth/egress-policy.ts)) hardens the runtime fetch in `ExternalJwksValidatorService` ([external-jwks-validator.service.ts](api/src/oauth/external-jwks-validator.service.ts)): **G1 timeout** (`AbortSignal.timeout` per attempt - a hung IdP is aborted, not blocking the mint), **G5 retry + exponential backoff + jitter** (total tries = `retries + 1`), **G2 redirect re-validation** (`redirect: 'manual'` - each 3xx `Location` is re-checked against the SSRF host allowlist before it is followed, ≤ 3 hops, so a trusted host cannot redirect the fetch to an internal address), and **G3 single-flight** (concurrent fetches for the same `jwksUri` are coalesced into one). It preserves the existing fail-to-stale + fail-closed semantics (it NEVER skips the signature check). Every knob is **configurable at two scopes with endpoint-overrides-server precedence** (`effective = endpoint setting ?? server env default ?? hardcoded default`): server-level env `JWKS_FETCH_TIMEOUT_MS` (5000) / `JWKS_FETCH_RETRIES` (2) / `JWKS_FETCH_RETRY_BACKOFF_MS` (200) / `JWKS_CACHE_MAX_AGE_MS` (600000, pre-existing), and per-endpoint numeric config flags `JwksFetchTimeoutMs` / `JwksFetchRetries` / `JwksFetchRetryBackoffMs` / `JwksCacheMaxAgeMs` (new `number` flag value-type in [endpoint-config.interface.ts](api/src/modules/endpoint/endpoint-config.interface.ts) with bounds-checking via `validateNumberFlag`; keys default to unset so they cleanly fall through to the server default). The overrides are threaded runtime-only: the token provider ([wif-assertion-token.provider.ts](api/src/modules/scim/controllers/wif-assertion-token.provider.ts)) resolves them from the endpoint's `profile.settings` via `resolveEndpointEgressOverrides()` and passes them through the WIF validator ([wif-assertion-validator.service.ts](api/src/oauth/wif-assertion-validator.service.ts)) into `verify()`. **Scope: runtime token-mint fetch only** - the config-time discovery/verify paths are unchanged. **UI:** a new "Runtime egress (WIF JWKS fetch)" card on the endpoint Settings tab ([SettingsTab.tsx](web/src/pages/SettingsTab.tsx)) renders a bounds-validated number input per knob (blank = inherit the server default). **Validation:** egress-policy unit +10 (env resolution + clamping + endpoint-over-server merge); external-jwks-validator unit 8 -> 14 (+6: G1 timeout/redirect-manual passed to fetch; G5 retry-then-succeed + endpoint retries=0 override; G2 SSRF redirect-to-disallowed-host rejected + redirect-to-allowed-host followed; G3 single-flight coalescing); wif-assertion-validator +1 (forwards overrides); provider +2 (threads endpoint overrides / empty when unset); endpoint-config +egress block (number-flag bounds accept/reject, `getConfigNumber`, `resolveEndpointEgressOverrides`, not-in-DEFAULT_ENDPOINT_CONFIG); E2E `config-flags` +4 (in-range persist + GET round-trip; below-min / above-max / non-numeric rejected 400); SettingsTab vitest +4 (renders 4 inputs; reflects persisted value + blank when unset; blur fires update; out-of-range shows error + no update); live-test `9z-BF` (T1-T6). API build 0, ESLint 0 errors (495 warnings, net -3); web tsc baseline held. Docs: [ENDPOINT_CONFIG_FLAGS_REFERENCE.md](docs/ENDPOINT_CONFIG_FLAGS_REFERENCE.md) (26 flags, new `number` type + Runtime-egress details), [EXTERNAL_JWKS_VALIDATOR.md](docs/auth/EXTERNAL_JWKS_VALIDATOR.md) (runtime-egress-hardening section), [README.md](README.md) (4 env vars). Version 0.54.28 -> 0.54.29.

- **Auth observability Phase 5: merged Credentials + Connect into ONE method-centric "Connect" tab (Setup → Connect → Health per method), with the actual secret shown for every method when visibility is Always.** The endpoint detail had TWO overlapping auth surfaces - a read-only **Connect** tab (what to paste into the IdP) and a **Credentials** tab (create/rotate/reveal/WIF) - each with its own method selector (the Connect radio + the Credentials sub-tabs). P5 collapses them into a single method-centric surface. The former `CredentialsTab`'s per-method sub-tabs become the single method axis; the tab is retitled **"Connect"** and now renders, for the active method: the existing credential/WIF **Setup** (create / rotate / reveal / revoke, the WIF trust form + assertion debugger + JWKS allowlist), the **Connect** bundle (the `ConnectionPanel` scoped to that one method - the exact IdP values + Copy-JSON / .env / download export), and **Health** (the `AuthDiagnosticsPanel` with the expected-vs-received diff + the P3 request-log deep-link). `ConnectionPanel` gains a `hideMethodSelector` prop ([ConnectionPanel.tsx](web/src/components/primitives/ConnectionPanel.tsx)) so the tab-level axis is the sole selector - no competing radio. **Always-show-secret:** the merged tab fetches retained secrets via `useConnectionRetainedSecrets`, so when the effective `CredentialSecretVisibility` is `always` the actual secret is rendered inline (re-viewable) for EVERY method - the operator's complete IdP-config bundle in one place (reusing the v0.54.18 `ConnectionSecretResolverService` inline-secret path). **Cutover:** the separate **Credentials** tab is removed from the endpoint detail tab registry ([EndpointDetailPage.tsx](web/src/pages/EndpointDetailPage.tsx)); the **Connect** tab hosts the unified surface ([endpoints.$endpointId.connect.tsx](web/src/routes/endpoints.$endpointId.connect.tsx) now lazy-loads the unified component); the legacy `/credentials` route permanently redirects to `/connect` ([endpoints.$endpointId.credentials.tsx](web/src/routes/endpoints.$endpointId.credentials.tsx)) so existing bookmarks + the `AuthDiagnosticsPanel` "Fix in Connect" cross-link keep working; the standalone `ConnectTab` component + its size-limit chunk budget are retired. All existing credential/WIF `data-testid`s are preserved (the tab was edited in place, not rewritten), so the WIF + reveal + rotate Playwright specs keep passing via the redirect. Validation: `CredentialsTab` vitest +3 (merged Connect panel + Health present; single-axis scoping hides the panel's own selector; retained secret rendered when visibility Always); `ConnectionPanel` vitest +1 (`hideMethodSelector` hides the radio + pins to `defaultMethod`); `AuthDiagnosticsPanel` vitest + `auth-diagnostics`/`connect-tab`/`endpoint-detail-tabs`/`lazy-routes` specs updated for the merged surface; ConnectTab.test.tsx retired. Web tsc baseline 96 held; web build clean; size-limit all pass (CredentialsTab 8.99 kB of 110 kB). Version 0.54.27 -> 0.54.28.

- **Auth observability Phase 4: config-time auth audit events - one canonical `LogCategory.AUTH` "Auth config change" event for every auth-administration action.** Runtime auth attempts already emitted a canonical `Auth decision` event (WI-D4); the CONFIG-TIME side (an operator verifying a WIF trust, editing the JWKS host allowlist, or toggling an auth flag) was only partially covered. A new canonical emitter `emitAuthAdminEvent()` ([auth-admin-event.ts](api/src/oauth/auth-admin-event.ts)) - the sibling of `emitAuthDecisionEvent` - emits exactly ONE `LogCategory.AUTH` event with the fixed message `Auth config change` per operation (INFO on `success`, WARN on `failure`/`denied`; non-secret payload of action / endpointId / credentialId / host / changedFlags / dryRun / reasonCode / correlationId; undefined keys dropped), flowing through the existing ring buffer + SSE + file - not a parallel mechanism. Credential lifecycle (create / reveal / rotate / revoke, WIF create / edit) already emitted `LogCategory.AUTH`; P4 closes the four remaining gaps: **(1) WIF verify** ([admin-credential.controller.ts](api/src/modules/scim/controllers/admin-credential.controller.ts) `verifyWifTrust`) emits `wif_verify` success/failure from the reachability result; **(2) WIF debug-assertion** (`debugWifAssertion`) emits `wif_debug_assertion` with `dryRun: true` + the first reject's reason code (it evaluates against every trust but NEVER mints); **(3) JWKS host allowlist** add / update / patch / remove ([admin-jwks-host.controller.ts](api/src/modules/scim/controllers/admin-jwks-host.controller.ts), a JWKS host is a WIF trust root) emit `jwks_host_add|update|patch|remove` with the host(s); **(4) auth-affecting endpoint flag changes** - a shared `detectAuthFlagChanges()` on BOTH `updateEndpoint` backends ([endpoint.service.ts](api/src/modules/endpoint/services/endpoint.service.ts)) diffs the 6 auth flags (`PerEndpointCredentialsEnabled`, `SecretTokenBearerAuthEnabled`, `OAuthClientCredentialsAuthEnabled`, `SharedSecretBearerAuthEnabled`, `WifCredentialsEnabled`, `CredentialSecretVisibility`) and emits `auth_flags_changed` carrying `{ flag, from, to }` deltas - a non-auth setting change emits nothing. Every event carries the `correlationId` so it bridges to the P3 request log. Validation: emitter unit +7; JWKS host controller unit +6 (new [admin-jwks-host.controller.spec.ts](api/src/modules/scim/controllers/admin-jwks-host.controller.spec.ts)); admin-credential unit +5 (verify success/failure + debug dry-run accept/reject); endpoint.service unit +3 inmemory + 2 prisma (auth-flag delta, cross-backend parity); E2E +3 ([auth-admin-events.e2e-spec.ts](api/test/e2e/auth-admin-events.e2e-spec.ts): JWKS add/remove + flag flip + non-auth no-emit, all via the ring buffer); live-test `9z-BE` (T1-T5). API build 0, ESLint 0 (test-file warning ceiling +8 from new spec casts, all in `*.spec.ts`); web unaffected. Version 0.54.26 -> 0.54.27.

- **Auth observability Phase 3: the `correlationId ↔ requestId` bridge - jump between a request log and its auth decision in one click.** The auth-diagnostics surfaces and the request-logs surfaces were two disconnected views of the same event: a request log carried no auth-decision link and an auth decision carried no request-log link, even though both already share the `X-Request-Id` correlation id. Phase 3 wires them together. **Backend:** every `RequestLog` now persists the correlation id in a new indexed `requestId` column ([schema.prisma](api/prisma/schema.prisma) + migration `20260717120000_add_requestlog_requestid`). It is threaded through the whole write path - the request-logging interceptor stashes it on the request-meta and passes it to `recordRequest()`, BOTH exception filters ([global-exception.filter.ts](api/src/modules/scim/filters/global-exception.filter.ts), [scim-exception.filter.ts](api/src/modules/scim/filters/scim-exception.filter.ts)) forward it for error responses (the 401 auth-reject case, which is exactly the one operators chase), and the InMemory backend stores + filters it at parity ([logging.service.ts](api/src/modules/logging/logging.service.ts)). `GET /scim/admin/logs?requestId=<id>` ([admin.controller.ts](api/src/modules/scim/controllers/admin.controller.ts)) and `GET /scim/endpoints/:id/history?requestId=<id>` ([endpoint-log.controller.ts](api/src/modules/scim/controllers/endpoint-log.controller.ts)) filter to the matching request; both the list items and the per-log detail echo `requestId`. **UI - forward link (auth decision → request):** the `AuthDiagnosticsPanel` decision detail ([AuthDiagnosticsPanel.tsx](web/src/components/primitives/AuthDiagnosticsPanel.tsx)) adds a **"View request log"** link next to the correlation id that navigates to `/logs?requestId=<correlationId>`. **UI - reverse link (request → auth decision):** the request-log `DetailDrawer` on both the global Logs page ([LogsPage.tsx](web/src/pages/LogsPage.tsx)) and the endpoint Logs tab ([LogsTab.tsx](web/src/pages/LogsTab.tsx)) now shows the request's correlation id + a **"View auth decision"** button that focuses the embedded `AuthDiagnosticsPanel` on that id via a new `focusCorrelationId` prop (filters the list to the matching decision, auto-expands it, offers a "Show all" reset). The `/logs` route search schema gains a `requestId` param so the deep-link is shareable. All values are non-secret. Validation: logging-service unit +4 (store + list-filter + getLog + absent, InMemory parity); global-logs E2E +1 (`X-Request-Id` round-trip: request → `?requestId=` list → detail echo); `AuthDiagnosticsPanel` vitest +4 (View-request-log nav + focus filter + focused-empty + Show-all clear); `LogsPage` vitest +4 (requestId passthrough + drawer correlation section + View-auth-decision focus + absent); `LogsTab` vitest +1 (drawer correlation + focus); live-test `9z-BD` (T1-T4). Interceptor + filter + controller specs 100/100 held; API build 0, ESLint 0; web tsc baseline 96 held. Version 0.54.25 -> 0.54.26.

- **Auth observability Phase 2: resource-plane auth-decision tracing + the "which auth method was selected and why" cascade.** The resource plane (a bearer token validating against `/Users`, `/Groups`, etc.) previously produced NO auth-decision trace at all - only the token-mint plane was observable. Now the `SharedSecretGuard` ([shared-secret.guard.ts](api/src/modules/auth/shared-secret.guard.ts)) records ONE `AuthDecisionTrace` (`plane: 'resource'`) per endpoint-scoped auth attempt capturing the entire method-selection cascade as ordered checks - `token_presented`, `endpoint_bearer` (per-endpoint bcrypt bearer), `oauth_jwt` (OAuth 2.0 JWT incl. the endpoint-scope check), `shared_secret` (global SCIM secret) - each with `status` (pass/skip/fail) + `expected` + `received` explaining WHY it did or did not win. This is the operator's "which auth method is being selected and on what basis" made visible. The winning method is the trace `method` (`endpoint_bearer` / `bearer_jwt` / `shared_secret`); rejects carry the catalog reason (`bearer_missing`, `bearer_token_scoped_other_endpoint`, `bearer_shared_secret_refused`, `bearer_invalid`). Recording is best-effort (never changes the auth outcome, never throws) and noise-controlled (every reject recorded; accepts recorded only for endpoint-scoped routes, not global-admin UI-poll traffic). No raw token is ever stored. `AuthMethodKind` gains `endpoint_bearer` (mapped to the `bearer` connection chip in `ConnectionInfoService.buildAuthHealth()`). The `AuthDecisionRecordStore` is injected `@Optional()` (resolves via the already-imported `OAuthModule`). Validation: guard unit +4 (accept endpoint_bearer + shared_secret; reject cascade; wrong-endpoint-scope reason); E2E +2 (per-endpoint-credentials: reject cascade no-token-leak; accept endpoint_bearer); live-test `9z-BC` (T1-T3). Full API unit 4303 -> 4307 (130 suites); API build 0, ESLint 0; web tsc baseline held. Version 0.54.24 -> 0.54.25.

- **Auth observability Phase 1: every auth-decision check now carries BOTH `expected` AND `received` (pass and fail) - the diagnostics table stops showing "-".** The Auth Diagnostics expected-vs-received table was empty on the accept path and for oauth_client because passing checks omitted `received`, the token provider synthesized a lossy 2-check accept summary, and the oauth_client decision recorded `checks: []`. Fixes: **(1)** the WIF validator ([wif-assertion-validator.service.ts](api/src/oauth/wif-assertion-validator.service.ts)) now sets `received` on EVERY passing claim check (the matched value), so a pass renders `expected: api://x · received: api://x` instead of `· -`; a new `validateWithTrace()` returns the full accept trace and the internal `runChecks()` core is shared by `validate()`/`validateWithTrace()`/`debug()`. **(2)** the WIF token provider ([wif-assertion-token.provider.ts](api/src/modules/scim/controllers/wif-assertion-token.provider.ts)) records the validator's FULL trace (enriched with correlationId/endpointId/selectedTrustId) instead of a 2-check summary, so the mint-path decision shows the real per-claim diff. **(3)** the oauth_client path ([endpoint-oauth.controller.ts](api/src/modules/scim/controllers/endpoint-oauth.controller.ts)) now emits real per-step checks - `grant_type`, `credential_location` (basic vs post, answering "how did the client present its secret"), `client_id_present`, `client_found`, `secret_match` (`match`/`mismatch`, NEVER the secret value), and `token_ttl` - so a reject names WHICH step failed. All checks are non-secret. First phase of the operator-approved auth observability + Connect/Credentials-merge epic. Validation: validator unit +5 (pass checks carry received; validateWithTrace; debug reuses full trace; roles pass shows received); oauth controller unit +1 (unknown client_id names client_found); provider spec updated to `validateWithTrace`; E2E +2 (oauth_client reject records populated per-check no-secret; WIF accept trace received populated); AuthDiagnosticsPanel vitest +1 (passing check renders received); live-test `9z-AZ.T9b`. Full API unit 4298 -> 4303 (130 suites); API build 0, ESLint 0 errors; web tsc baseline held. Version 0.54.23 -> 0.54.24.

- **Integrated latest `origin/master` into the `feat/wif` line (v0.54.23).** Merged the two master-only commits - the `EnforceResourceTypes` endpoint flag (`e4e5488`) and the per-endpoint `*/oauth/token` content-type-415 exemption (`0450d7b`) - onto the connection-info / auth-diagnostics epic so the branch sits on top of the latest master across both the API and UI layers. Six conflicts were resolved deliberately (both `package.json` + lockfiles kept the `feat/wif` `0.54.x` version line; `endpoint-config.interface.ts` kept BOTH new flags `CredentialSecretVisibility` + `EnforceResourceTypes`; the content-type middleware took master's broader `*/oauth/token` regex exemption comment; `CHANGELOG.md` + `ENDPOINT_CONFIG_FLAGS_REFERENCE.md` kept both feature histories with a de-collided flag-table row number). **Settings-category placement fix:** master added the `EnforceResourceTypes` Switch to [SettingsTab.tsx](web/src/pages/SettingsTab.tsx) WITHOUT the `category` field that the `feat/wif` categorized-settings UI requires; the merged entry is now filed under the **Discovery** category (alongside `SchemaDiscoveryEnabled` + `CustomResourceTypesEnabled`, the other resource-type/discovery flags) so every endpoint setting still renders in its appropriate category card. The WI-D6/D8 R8 auth-diagnostics cross-links (Settings > JWKS host allowlist / Credentials deep-links) are preserved unchanged. Validation post-merge: API build 0; full API unit 4298/4298 (130 suites); merged-feature unit 420/420 + E2E 35/35 (profile-enforcement-gaps + endpoint-oauth-client); web tsc baseline 96 held; web build clean; SettingsTab vitest 22/22. Version 0.54.22 -> 0.54.23.

- **`RfcCompliantSubAttributes` endpoint flag (default `false`, standalone) - opt in to RFC 7643 sub-attribute shape rules without changing any existing endpoint.** The sub-attribute RFC survey ([docs/rfcs/SCIM_SUBATTRIBUTE_TYPE_RULES.md](docs/rfcs/SCIM_SUBATTRIBUTE_TYPE_RULES.md)) found the validator diverged from RFC 7643 in two opposite directions at once: it **accepted** a complex attribute whose sub-attribute is itself complex (forbidden by [§2.3.8](docs/rfcs/rfc7643.txt), reinforced by **erratum 8415**, Verified 2025-10-28, and independently by RFC 7644 §3.5.2's `attrPath = [URI ":"] ATTRNAME *1subAttr` which caps addressable depth at two), and it **rejected** a sub-attribute that is simple and multi-valued (permitted by [§1.2](docs/rfcs/rfc7643.txt) "a simple attribute, which may be singular or multi-valued", clarified by **erratum 5607**). Neither could be corrected silently - an endpoint whose custom schema already declares a nested complex sub-attribute would start failing provisioning - so the corrected behavior ships behind ONE per-endpoint flag defaulting to today's behavior. When `true`: **R1** a payload populating a complex sub-attribute is rejected `400 invalidValue` naming the offending path; **R2** a multi-valued SIMPLE sub-attribute is accepted and each element is type-checked with an `[index]` path. **Standalone by design** - deliberately NOT gated on `StrictSchemaValidation`, because the two answer different questions (how carefully do I police this payload vs is this schema shape legal at all); turning the flag on does not enable strict validation, and turning strict off does not lift an R1 rejection. Applies to `POST`/`PUT`/`PATCH` on `/Users`, `/Groups` and custom resource types. **UI:** `RfcCompliantSubAttributes` Switch on the endpoint Settings tab ([SettingsTab.tsx](web/src/pages/SettingsTab.tsx)); every flag Switch also gained a stable `data-testid` (`settings-flag-<Key>`) so specs assert by test id rather than label text (rule R9.4). Implementation: flag registry ([endpoint-config.interface.ts](api/src/modules/endpoint/endpoint-config.interface.ts)); `ValidationOptions.rfcCompliantSubAttributes` ([validation-types.ts](api/src/domain/validation/validation-types.ts)); `SchemaValidator.validateSubAttributeNesting` + `isComplexSubAttribute` + a single `subAttributeNestingError` builder ([schema-validator.ts](api/src/domain/validation/schema-validator.ts)); one enforcement point per service via `enforceSubAttributeNesting` ([scim-service-helpers.ts](api/src/modules/scim/common/scim-service-helpers.ts) and its dynamic-URN twin in [endpoint-scim-generic.service.ts](api/src/modules/scim/services/endpoint-scim-generic.service.ts)). Validation: domain unit **+23** ([rfc-compliant-subattributes.spec.ts](api/src/domain/validation/rfc-compliant-subattributes.spec.ts)); API E2E **+9** ([rfc-compliant-subattributes.e2e-spec.ts](api/test/e2e/rfc-compliant-subattributes.e2e-spec.ts)); web vitest **+3** ([SettingsTab.test.tsx](web/src/pages/SettingsTab.test.tsx)); Playwright **+5** ([rfc-compliant-subattributes-flag.spec.ts](web/e2e/rfc-compliant-subattributes-flag.spec.ts)); live `live-test.ps1` section **9z-AT** **+11**. API unit **4,031/4,031** (112 suites); API E2E **1,289/1,289** (73 suites) on BOTH inmemory and Prisma backends; web vitest **1,092/1,092** (93 files); local-node live **1,116/1,116**. Feature doc: [docs/RFC_COMPLIANT_SUBATTRIBUTES.md](docs/RFC_COMPLIANT_SUBATTRIBUTES.md). Version 0.54.86 -> 0.54.87.

### Fixed
- **An `RfcCompliantSubAttributes` rejection is now attributed to the flag that caused it, not to `StrictSchemaValidation`.** Found by the new Playwright spec, which is the only test that exercised the UI toggle and the server decision in one flow: with `StrictSchemaValidation` ON the R1 nesting error travelled through the strict validator, so the diagnostics envelope reported `triggeredBy: "StrictSchemaValidation"` - misleading, because turning strict off does **not** lift the rejection, so the operator would be sent to a switch that cannot fix it. Every API-level test missed it because each exercised only one strict setting at a time and asserted `triggeredBy` only on the lenient path. Fix: the nesting pass was hoisted out of the `!isStrict` branch into a single private `enforceSubAttributeNesting` in both SCIM service paths and now runs BEFORE the strict branch, giving one enforcement point, one error builder, and correct attribution in all four (strict x flag) combinations. `activeConfig` now reports the real `StrictSchemaValidation` value instead of a hard-coded `false`. Locked by a new E2E assertion on the strict-ON path.

### Added
- **`EnforceResourceTypes` endpoint flag (default `true`) - relax LIST/query on an un-served resource type to `200` empty + warning (unblocks Entra Test Connection for user-only endpoints).** RCA: Entra's provisioning Test Connection probes BOTH `/Users` and `/Groups` and expects `200` empty `ListResponse` (RFC 7644 §3.4.2); the v0.53.3 profile enforcement returns `404 RESOURCE_TYPE_NOT_SUPPORTED` on `/Groups` for a **user-only** endpoint, which Entra reports as `SystemForCrossDomainIdentityManagementServiceIncompatible` ("HTTP/404 ... rather than the expected HTTP/200"). New endpoint config flag `EnforceResourceTypes` (default `true` = strict Gap-1 behavior, fully backward-compatible). When `false`, a **LIST/query** on an un-served resource type returns a `200` empty `ListResponse` instead of `404`; **item-by-id reads and all writes still reject with `404`** (a query for zero of an un-served type is semantically empty, but creating one is not). Symmetric for `/Users` and `/Groups`. Every relaxed empty-list emits ONE warning object ([resource-type-enforcement.ts](api/src/modules/scim/common/resource-type-enforcement.ts)) projected onto three channels: **W1** structured `LogCategory` warning, **W2** `urn:scimserver:api:messages:2.0:Warning` body member, **W3** `X-SCIM-Warning` response header (custom vendor header, NOT the deprecated RFC 9111 `Warning`). All three are Entra-safe (unknown body members + headers are ignored). **UI:** `EnforceResourceTypes` Switch added to the endpoint Settings tab ([SettingsTab.tsx](web/src/pages/SettingsTab.tsx)). Validation: helper unit +7 ([resource-type-enforcement.spec.ts](api/src/modules/scim/common/resource-type-enforcement.spec.ts)); config-flag unit +4; groups+users controller unit +12 (relaxed list/search 200 empty; item read + create still 404; default-enforce preserved); E2E profile-enforcement-gaps +6 (200 empty + W2 body + W3 header on GET/.search; item + create still 404); live `live-test.ps1` 9z-AS.5b-5g; SettingsTab vitest +1. Full API unit 4008 pass (111 suites); API + web build clean. Docs: [ENDPOINT_PROFILE_ENFORCEMENT_DESIGN.md §8.1a](docs/ENDPOINT_PROFILE_ENFORCEMENT_DESIGN.md), [ENDPOINT_CONFIG_FLAGS_REFERENCE.md](docs/ENDPOINT_CONFIG_FLAGS_REFERENCE.md). RCA: [docs/auth/EXECUTION_ISSUES_AND_RCA.md](docs/auth/EXECUTION_ISSUES_AND_RCA.md) Issue 3. Version 0.54.0-alpha.10 -> 0.54.0-alpha.11.

### Fixed

- **Clickable endpoint-log detail returns a clean 404 (not a 500) for a malformed log id.** The WI-D6 clickable-logs endpoint `GET /scim/endpoints/:id/logs/:logId` ([endpoint-log.controller.ts](api/src/modules/scim/controllers/endpoint-log.controller.ts)) 500'd when the `logId` was a non-UUID string (e.g. `does-not-exist`): on Postgres the `RequestLog.id` lookup throws on a value that cannot be a valid id, and the controller assumed `getLog()` always returns null for a miss. An id that cannot resolve is simply "not found", so the lookup is now wrapped and any throw degrades to a `404 NotFoundException` (matching the null-row path). Caught by the dev-deployment pipeline's prisma E2E gate (the InMemory-vs-Postgres parity difference only surfaced when the persistent history was momentarily empty and the test fell through to the malformed-id branch). Validation: endpoint-scoped-logs E2E +1 explicit regression test ("returns 404 (not 500) for a MALFORMED log id"), green in BOTH inmemory + prisma backends (13/13 each). Version 0.54.21 -> 0.54.22.

- **Connect tab WIF method: correct Entra auth-method label + `Client identifier` = sub claim; shared-secret shows the inline token; endpoint logs are clickable.** Three operator-reported Connect-tab fixes plus an endpoint-logs upgrade. **(1) WIF auth-method label** - the WIF method previously showed `Authentication Method: OAuth2 Client Credentials Grant`, which is the wrong Entra dropdown selection; it now reports `Workload Identity based authentication` (matching Entra's "Workload Identity based authentication (Active)" option) via the new `entraAuthenticationMethod` value ([connection-info.types.ts](api/src/shared/types/connection-info.types.ts), [connection-info.service.ts](api/src/modules/scim/services/connection-info.service.ts)). **(2) WIF Client identifier = sub claim** - Entra's WIF connectivity "Enter values from your application" form has a 3rd field `Client identifier` that takes the **subject (`sub`) claim**, not the audience. The assembler now maps the WIF trust's `expectedSubject` to `entraFields.clientIdentifier`, and the ConnectionPanel labels it `Client identifier (sub claim)` with a WIF-specific description ([ConnectionPanel.tsx](web/src/components/primitives/ConnectionPanel.tsx)); the audience stays its own row (Entra shows it under "Configure trust"). **(3) Shared-secret token now shows when visibility=always** - the shared_secret method's `secretToken` is inlined by the v0.54.18 resolver, and the panel already renders a non-null `entraFields` value, so the global SCIM shared secret now appears on the Connect tab instead of "Shown once at creation" (verified by a new ConnectionPanel test). **(4) Clickable endpoint logs** - the endpoint Logs tab rows are now clickable and open a DetailDrawer with the full request/response headers + parsed bodies, mirroring the SCIMServer-level Logs page. A new tenant-isolated `GET /scim/endpoints/:id/logs/:logId` ([endpoint-log.controller.ts](api/src/modules/scim/controllers/endpoint-log.controller.ts), 404 unless the log's URL is scoped to the endpoint) backs a new `useEndpointLog` hook ([queries.ts](web/src/api/queries.ts)) + the drawer in [LogsTab.tsx](web/src/pages/LogsTab.tsx). Validation: assembler unit +3 (WIF auth-method label / expectedSubject -> clientIdentifier / null-subject); ConnectionPanel vitest +3 (WIF auth-method title, Client identifier sub-claim label, shared_secret inline token); endpoint-scoped-logs E2E +2 (detail returns headers/body scoped to endpoint, 404 for unknown id); LogsTab vitest +1 (row-click opens the detail drawer); full API unit 4246 -> 4249; web tsc baseline 96 held; web build + ConnectionPanel/LogsTab vitest green. Version 0.54.18 -> 0.54.19.

### Added

- **WI-D8: humanized auth-failure messages + per-method `authHealth` chip - the last mile of the auth-diagnostics epic.** Three additive pieces that make an auth failure glanceable and self-explanatory in the UI. **(1) Client-side reason-code catalog** - 21 auth `reason_code` entries (the WIF `jwt-bearer`, `oauth_client`, and resource `bearer` planes) added to [scim-error.ts](web/src/api/scim-error.ts) `SCIM_ERROR_CATALOG`, each with a title + plain-English explanation mirroring the WI-D2 API catalog, so a token failure renders operator-readable remediation instead of a raw code. **(2) OAuth token-error parsing** - `fetchWithAuth` ([queries.ts](web/src/api/queries.ts)) now parses the flat RFC-6749 token-error body (`error` / `error_description` / `reason_code`, the WI-D1 shape) and stamps `reasonCode` on `ScimApiError`; `parseScimError` prefers the reason-code catalog entry over the `scimType` / HTTP-status fallback (reasonCode > scimType > status), and degrades to the status fallback for an unknown code. **(3) `authHealth` chip** - a new additive `authHealth` block per enabled connection method ([connection-info.types.ts](api/src/shared/types/connection-info.types.ts): `lastOutcome` / `lastReasonCode` / `lastAttemptAt` / `lastCorrelationId`, all non-secret), resolved from the WI-D5 decision store via the new `AuthDecisionRecordStore.latestByMethodForEndpoint()` + `ConnectionInfoService.buildAuthHealth()` (maps the trace `method` vocabulary onto `ConnectionMethod`, `bearer_jwt` -> `bearer`), wired through BOTH the connection-info controller ([admin-connection-info.controller.ts](api/src/modules/scim/controllers/admin-connection-info.controller.ts)) and the overview BFF ([dashboard.controller.ts](api/src/modules/dashboard/dashboard.controller.ts)) via an `@Optional()` store injection so no separate call is needed; [ConnectionPanel.tsx](web/src/components/primitives/ConnectionPanel.tsx) renders a green/red "Last auth: OK/FAILED" chip + last reason code + correlation id + timestamp. Validation: scim-error vitest +4 (21-key catalog presence; reasonCode-first lookup; reasonCode beats scimType; unknown-code status fallback); store unit +2 (latest-per-method newest-wins + endpoint scoping); connection-info service unit +3 (authHealth attached to matching method only; omitted when no map; buildAuthHealth bearer_jwt->bearer mapping); connection-info E2E +1 (a failed oauth_client attempt surfaces authHealth with the oauth_client_auth_failed reason + key-allowlist); ConnectionPanel vitest +2 (chip renders FAILED + reason + correlation id; absent when no decision); new live-test section `9z-BB`. API build 0; full API unit 4257 -> 4262; web tsc baseline 96 held. Version 0.54.20 -> 0.54.21.

 A server-evaluated debugger so an operator can answer "will THIS exact assertion be accepted, and if not which claim is wrong?" BEFORE the source IdP is fully wired. `POST /scim/admin/endpoints/:id/wif/debug-assertion` ([admin-credential.controller.ts](api/src/modules/scim/controllers/admin-credential.controller.ts), admin-only, same `WifCredentialsEnabled` gate as the other `wif/*` admin routes) decodes the pasted assertion and runs it against EVERY configured WIF trust for the endpoint using the exact same server-side checks a real token mint runs - real JWKS fetch + signature + alg-pin + time window + `iss`/`sub`/`aud`/`tid`/roles matching - via a new non-throwing `WifAssertionValidatorService.debug()` ([wif-assertion-validator.service.ts](api/src/oauth/wif-assertion-validator.service.ts)) that returns `{ outcome, reasonCode?, trace }` instead of throwing. It NEVER mints a token and NEVER 4xxs on a bad assertion (a reject is a result). The response is one per-trust `WifDebugTrustResult` ([wif-debug.types.ts](api/src/shared/types/wif-debug.types.ts)) carrying the WI-D3 `AuthDecisionTrace` - the per-check expected-vs-received table + the decoded non-secret claims + jose header (the raw assertion is never echoed). A misconfigured trust row surfaces as a `wif_no_trust_configured` reject rather than throwing. The UI is the new "Assertion debugger" panel in the R6 WIF sub-tab of the Credentials tab ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx), `useDebugWifAssertion` in [queries.ts](web/src/api/queries.ts)): paste an assertion into a textarea, click "Debug assertion", and see each trust's ACCEPT/REJECT badge + reason code + a per-check PASS/FAIL list (expected vs received) + the decoded claims via the **R9** `CopyableJsonBlock`. Validation: validator unit 20 -> 23 (+3: accept trace / reject-without-throw + reasonCode + failing check / signature-failure no raw-assertion leak); admin-credential controller unit 52 -> 57 (+5: dry-run per trust + overall accept/reject + empty-body 400 + WIF-off 403 + misconfigured-trust reject); WIF E2E 25 -> 28 (+3: valid assertion accept-without-mint / wrong-audience reject with `wif_audience_mismatch` + failing check / empty-body 400); CredentialsTab vitest 44 -> 45 (+1: posts the assertion + renders the per-check reject trace + decoded claims, R10 outcome assertion); new live-test section `9z-BA` (WIF-off 403, empty-body 400, bogus-assertion reject result, no `access_token` in the response, admin-auth required). API build 0, web tsc baseline held (96). Version 0.54.19 -> 0.54.20.

 Previously the connection surfaces never carried a secret VALUE: `connection-info` returned `secretToken`/`clientSecret` as `null`, the global SCIM shared secret was shown nowhere, and an operator had to make a separate reveal call per credential. Now, when the effective visibility is `always` (operator opt-in), the actual secrets are inlined in the same responses that carry the URLs, so every connection parameter for an Entra gallery app can be copied from one place. A new `ConnectionSecretResolverService` ([connection-secret-resolver.service.ts](api/src/modules/scim/services/connection-secret-resolver.service.ts)) is the single gate: it resolves per-endpoint `bearer`/`oauth_client` secrets (decrypted via the same `CredentialEncryptionService` the reveal endpoint uses) only when the endpoint's EFFECTIVE visibility is `always` AND an envelope was retained, and the SERVER-scope global secrets (SCIM shared secret + global OAuth client id/secret) only when the SERVER visibility is `always`; anything `once` / missing / undecryptable resolves to `null` (never throws). **Per-endpoint (`GET /admin/endpoints/:id/connection-info` + the overview BFF the Connect tab reads):** the assembler now inlines `entraFields.secretToken` (shared_secret + bearer) and `entraFields.clientSecret` (oauth_client) and sets a new `secretRevealed` flag on each method ([connection-info.service.ts](api/src/modules/scim/services/connection-info.service.ts), [connection-info.types.ts](api/src/shared/types/connection-info.types.ts)); the ConnectTab / ConnectionPanel already render an inline `entraFields` value, so the secret now appears + folds into the Copy-JSON / .env / download payloads with no client change. **Server-level:** a new `GET /admin/settings/security/connection-secrets` ([admin-security-settings.controller.ts](api/src/modules/scim/controllers/admin-security-settings.controller.ts)) returns `{ revealed, visibility, sharedSecret, oauthClientId, oauthClientSecret }` when server visibility is `always`, and the admin Settings "Server connection info" card ([SettingsPage.tsx](web/src/pages/SettingsPage.tsx), via `useServerConnectionSecrets`) shows the Secret Token + global OAuth client id/secret + adds them to its JSON export. Every inline is an admin-only, `LogCategory.AUTH` audit-logged disclosure; flipping to `once` withholds them again (and the existing server-`once` purge still wipes retained ciphertext). Validation: resolver unit 10; connection-info assembler unit +3 (withhold default / inline-when-passed / no-credential-guard); security-settings controller unit +2 (revealed / withheld); connection-info E2E rewritten (`once` withholds + `always` inlines the real secret) + credential-reveal E2E +3 (server connection-secrets always/once/401); dashboard overview + connection-info controllers wired through the resolver; ConnectionPanel vitest +1 (inline entraFields secret) + SettingsPage vitest +2 (hidden on once / shown on always); full API unit 4232 -> 4246; web tsc baseline 96 held; live-test `9z-AW.T4/T6/T7`. Version 0.54.17 -> 0.54.18.

- **WI-D6: the Auth Diagnostics panel - the centerpiece expected-vs-received auth-failure UI.** [AuthDiagnosticsPanel.tsx](web/src/components/primitives/AuthDiagnosticsPanel.tsx) renders the recent Auth Decision Records (WI-D5) as an expandable list. Each row shows the outcome badge (accept/reject) + method + reason code + timestamp; expanding a rejected decision reveals the per-check **expected-vs-received diff** (which exact check failed and what arrived), the catalog reason code with a plain-English remediation hint, and an **R8 fix cross-link** that deep-links to the right surface (Settings > JWKS host allowlist for `jwks_host_not_allowlisted`, the endpoint Credentials tab for WIF/oauth_client reasons), plus the full non-secret record via the **R9** `CopyableJsonBlock` + `CopyableField` primitives. All values are non-secret (the WI-D5 store holds only sanitized trace fields). A new `useAuthDecisions` two-scope query hook ([queries.ts](web/src/api/queries.ts)) + a shared `auth-decision.types.ts` back it. The panel is embedded on THREE surfaces (design D6): the Connect tab + the endpoint Logs tab (per-endpoint scope) and the admin Logs page (global scope), so an operator sees the diagnostics wherever they are looking. Validation: panel vitest 10 (loading / empty / error / list / expand-diff / remediation + fix-link routing for WIF and JWKS reasons / copy-JSON / scope passthrough); the host-page specs (ConnectTab, LogsTab, LogsPage) updated to stub the embedded hook; Playwright `auth-diagnostics.spec.ts` (3, fully route-mocked: lists a reject, expands to the rendered diff + remediation + fix link, the fix link navigates to Credentials). Web tsc baseline held (96); all web vitest for the touched surfaces green (38). Version 0.54.16 -> 0.54.17.

- **WI-D5: the Auth Decision Record store - recent auth decisions queryable at two admin scopes.** A short-TTL, admin-only, in-memory store ([auth-decision-record.store.ts](api/src/oauth/auth-decision-record.store.ts)) captures every WI-D3 decision trace as a recent Auth Decision Record (the trace + a generated `id` + `recordedAt`), keeping a bounded (500 default, `AUTH_DECISION_STORE_MAX`) ring and evicting anything older than the TTL (30 min default, `AUTH_DECISION_STORE_TTL_MS`) on every access - a best-effort recent-diagnostics buffer, NOT an audit log (the persisted `RequestLog` + the WI-D4 `Auth decision` events are the durable record). Two admin query scopes ([auth-decisions.controller.ts](api/src/modules/scim/controllers/auth-decisions.controller.ts), admin-only - the default bearer guard applies, no `@Public`), mirroring the existing two-scope log API + the R4b endpoint-vs-server precedent: global `GET /scim/admin/auth-decisions` and per-endpoint `GET /scim/admin/endpoints/:endpointId/auth-decisions`, each supporting `?outcome=accept|reject`, `?reasonCode=`, and `?limit=` filters, newest-first. The WIF provider ([wif-assertion-token.provider.ts](api/src/modules/scim/controllers/wif-assertion-token.provider.ts) - via a `recordAndEmit` helper) and the oauth_client controller ([endpoint-oauth.controller.ts](api/src/modules/scim/controllers/endpoint-oauth.controller.ts)) now record the trace alongside the WI-D4 log event, so every decision surfaces in both. Only the WI-D3 non-secret trace fields are stored (decoded claim identifiers + jose header); the raw assertion / bearer token is never present. This is the data source the coming WI-D6 diagnostics UI renders. Validation: store unit 10 (record + newest-first + endpoint/outcome/reason filters + ring bound + TTL prune + no-raw-secret + clear); controller unit 7 (global + per-endpoint scope, outcome/reason/limit filters, combined filters, invalid-outcome ignore); WIF provider unit +2 (accept + reject captured, endpoint-scoped); E2E +2 (a rejected oauth_client attempt is queryable at BOTH scopes and scoped correctly + the scoped endpoint excludes other endpoints' records + no raw secret in the payload; both endpoints require admin auth -> 401); full API unit 4215 -> 4232; live-test `9z-AZ.T9/T10/T11`. **Injection note:** a constructor param typed `Foo | null` erases to `Object` in the emitted `design:paramtypes`, so `@Optional()` needs an explicit `@Inject(Foo)` (matching the existing `assertionProvider` param) or the dependency silently resolves to null. Version 0.54.15 -> 0.54.16.

- **WI-D4: one canonical `Auth decision` log event per auth attempt (through the existing ring-buffer + SSE + file, not a parallel mechanism).** Every token-mint auth attempt now emits exactly ONE structured, redacted, alert-friendly `LogCategory.AUTH` event with the fixed message `Auth decision` - `accept` at INFO, `reject` at WARN - derived from the WI-D3 decision trace, so an operator can alert on / count / filter auth outcomes without matching ad-hoc phrasings. `emitAuthDecisionEvent(logger, trace, LogCategory.AUTH)` ([auth-decision-trace.ts](api/src/oauth/auth-decision-trace.ts)) routes through the existing `ScimLogger` (the same ring buffer + SSE live-stream + per-endpoint file transport every other log uses - NOT a new pipeline), and the interceptor's existing `RequestLog` row remains the persisted HTTP record. The event payload carries `outcome` / `reasonCode` / `method` / `plane` / `endpointId` / `correlationId` / `failedChecks[]` (failed check ids only, never the raw values) / `subTraceCount` / non-secret `decodedClaims`; the raw assertion or bearer token is never present. Wired at the WIF provider ([wif-assertion-token.provider.ts](api/src/modules/scim/controllers/wif-assertion-token.provider.ts) - accept + single-trust reject + multi-trust aggregate reject) and the per-endpoint oauth_client controller ([endpoint-oauth.controller.ts](api/src/modules/scim/controllers/endpoint-oauth.controller.ts) - accept + reject). Query it via `GET /scim/admin/log-config/recent?category=auth` (ring buffer) or the SSE live stream. Validation: emitter unit +4 (one INFO on accept, one WARN on reject with reasonCode + failedChecks, no raw-value leak, undefined-key drop); WIF provider unit +2 (exactly one accept event / one reject event); oauth_client controller unit +2 (accept + merged-reject events); E2E +1 (a rejected oauth_client attempt surfaces the `Auth decision` reject event in the ring buffer); full API unit 4209 -> 4215; live-test `9z-AZ.T8`. Version 0.54.14 -> 0.54.15.

- **WI-D3: the Auth Decision Trace - one structured object powers the wire reason_code, the log, and the UI diff.** Instead of throwing an opaque error at the first failed check, the WIF validator now records each check as an ordered structured step in an `AuthDecisionTrace` ([auth-decision-trace.ts](api/src/oauth/auth-decision-trace.ts)), so the reject `reason_code`, the `error_description`, the (coming WI-D4) log event, and the (coming WI-D6) UI expected-vs-received diff all derive from the SAME object and can never drift. `AuthDecisionTraceBuilder` records `pass`/`fail`/`skip` checks (each with non-secret `expected`/`received`), sanitizes the decoded JWT claims to a non-secret identifier allowlist (`iss`/`sub`/`aud`/`tid`/`roles`/...) and the JOSE header to `alg`/`kid`/`typ` only, and `reject(code)` only records codes that exist in the WI-D2 catalog (an unknown code degrades to generic). `mapJwksErrorToReason()` classifies jose error codes (`ERR_JWT_EXPIRED` -> `assertion_expired`, `ERR_JOSE_ALG_NOT_ALLOWED` -> `assertion_alg_not_allowed`, `ERR_JWS_SIGNATURE_VERIFICATION_FAILED` -> `assertion_signature_invalid`) and the JWKS-validator plain-Error messages (allowlist/https/fetch) into catalog codes. The WIF validator ([wif-assertion-validator.service.ts](api/src/oauth/wif-assertion-validator.service.ts)) tags `WifAssertionInvalidError` with `.reasonCode` + `.trace`; the multi-trust provider ([wif-assertion-token.provider.ts](api/src/modules/scim/controllers/wif-assertion-token.provider.ts)) aggregates a per-trust sub-trace for each rejected trust under `wif_no_trust_accepted`; the per-endpoint token controller ([endpoint-oauth.controller.ts](api/src/modules/scim/controllers/endpoint-oauth.controller.ts)) surfaces the catalog `reason_code` on the wire for WIF (specific: `wif_issuer_mismatch`/`wif_tenant_mismatch`/...), `oauth_client` (merged T3 `oauth_client_auth_failed`, never distinguishing secret existence vs correctness), and request-shape errors (`grant_type_unsupported`/`missing_credentials`/`mutually_exclusive_credentials`/`unsupported_assertion_type`). The raw signature and any bearer token are NEVER placed in a trace. Validation: trace unit 20 (build accept/reject, unknown-code degrade, claim + jose sanitization, sub-traces, error mapping); WIF validator unit +7 (reason codes for issuer/audience/tenant/role mismatches + signature-plane + trace claim/header sanitization); token E2E +5 (`wif_issuer_mismatch` / `wif_tenant_mismatch` / `wif_no_trust_accepted` / merged `oauth_client_auth_failed` on the wire); full API unit 4209; live-test `9z-AZ.T7`. Pure refactor of WHAT is recorded, not the checks - no auth-behavior change. Version 0.54.13 -> 0.54.14.

- **WI-D2: auth-failure reason-code catalog module + public reference endpoint.** The single machine-readable source of truth for every auth-failure reason code an operator sees on the wire (`reason_code` in a token error), in the AUTH log events, and in the admin diagnostics UI - so the wire code, human description, remediation, and visibility tier can never drift. [auth-reason-catalog.ts](api/src/oauth/auth-reason-catalog.ts) defines 26 stable, additive reason codes across three planes (WIF `jwt-bearer`, `oauth_client`, resource `bearer`), each carrying its RFC-6749/6750 `wireError`, a visibility tier (T1 config-transparent / T2 protocol / T3 secret-opaque / T4 internal), a fixed actor-facing `actorDescription`, and an admin `remediation` hint. The `jwks_host_not_allowlisted` remediation references the R1 full-CRUD verbs (add / edit `PUT` / selective `PATCH` on `Settings > JWKS host allowlist`), not only the old `POST` (design delta D1). The T3 `oauth_client_auth_failed` reason is deliberately merged so the wire can never distinguish secret existence vs correctness (P2); `wireDescriptionFor()` enforces the tier (T4 collapses to a generic string). A new public endpoint `GET /scim/docs/auth-errors` ([auth-errors-catalog.controller.ts](api/src/oauth/auth-errors-catalog.controller.ts), `?plane=` filter, cache-friendly) publishes the catalog. The WI-D1 token-error filter now fills a token error's `error_description` from the catalog's tier-safe description when a known `reason_code` is present but no description was supplied ([scim-exception.filter.ts](api/src/modules/scim/filters/scim-exception.filter.ts)), connecting D1 <-> D2. Validation: catalog unit 13 (unique codes, valid wire/tier/plane, lookup, tier-safe merge, D1 remediation verbs); catalog E2E 6 (public, key-allowlist top-level + per-reason, plane filter, merged T3); filter unit 22 -> 23 (+1 catalog-fill); new live-test section `9z-AZ` (public catalog, valid wire/tier, plane coverage, plane filter, merged T3). API build clean. Version 0.54.12 -> 0.54.13.

- **WI-D1: OAuth token-endpoint errors return an RFC-6749-conformant `application/json` body (not a SCIM envelope).** The first R2 (auth error diagnostics + observability) work item. Token-endpoint failures (`POST /scim/oauth/token`, `POST /scim/endpoints/{id}/oauth/token`) previously flowed through the SCIM exception filter and came back wrapped in a SCIM error envelope (`schemas`, `detail`, `scimType`) with `application/scim+json` - which is wrong for an OAuth token endpoint and does not match what IdP clients (Entra, Okta, Ping) expect. The `ScimExceptionFilter` now detects the token endpoint + an OAuth-shaped error body and emits a flat RFC-6749 error instead ([scim-exception.filter.ts](api/src/modules/scim/filters/scim-exception.filter.ts)): `{ error, error_description, reason_code?, error_uri?, correlation_id, timestamp }` as `application/json`, before any SCIM wrapping. Every token error is now enriched with a `correlation_id` (sourced from the ALS correlation context, falling back to the `X-Request-Id` response header the request-logging interceptor already stamps) plus an ISO `timestamp`, so an operator can find the matching log line - mirroring Entra's token-error contract. `reason_code` / `error_uri` pass through untouched when a validator provides them (the seam WI-D2/WI-D3 populate). Validation: filter unit 18 -> 22 (+4: flat RFC-6749 body no SCIM wrapper as application/json; always-enriched correlation_id + timestamp; reason_code/error_uri passthrough; non-OAuth-error body on the token path still gets SCIM handling); token E2E updated ~9 assertions from SCIM `.detail` to RFC-6749 `.error` across [endpoint-oauth-client.e2e-spec.ts](api/test/e2e/endpoint-oauth-client.e2e-spec.ts) + [wif-assertion.e2e-spec.ts](api/test/e2e/wif-assertion.e2e-spec.ts) (36/36 inmemory; 89/89 across the authentication/oauth/token/contract E2E sweep); new live-test section `9z-AY` asserts the on-the-wire shape (status 4xx, string `error`, no `schemas` envelope, `error_description`, `correlation_id`, `timestamp`, `Content-Type: application/json`). API build clean. Version 0.54.11 -> 0.54.12.

### Changed

- **Version scheme: drop the `-alpha.N` pre-release suffix (operator request R9).** The project moves from `0.54.0-alpha.33` to plain semver `0.54.0`; subsequent changes increment the patch (`0.54.1`, `0.54.2`, ...) as usual. Only the two live version strings ([api/package.json](api/package.json), [web/package.json](web/package.json)) change; the runtime `/scim/admin/version` reads them dynamically. Historical `alpha.N` references in this changelog + feature docs are left as-is (they record what actually shipped at the time).

### Changed

- **Per-endpoint settings organized into categories + multi-option settings use typed controls.** The endpoint Settings tab previously rendered ~19 boolean flags in a single flat "Configuration Flags" card and showed the non-boolean settings (`PrimaryEnforcement`, `logLevel`) as read-only badges with "edit via the admin API". Now the boolean flags are grouped into related-category cards (Authentication methods, Validation & schema, PATCH semantics, Lifecycle & deletes, Concurrency & ETags, Discovery) and the multi-option settings render as editable Fluent Dropdowns with their full option set + descriptions ([SettingsTab.tsx](web/src/pages/SettingsTab.tsx)): `PrimaryEnforcement` (passthrough / normalize / reject) and `logLevel` (TRACE...OFF), each firing the same `{ profile: { settings: {...} } }` PATCH the toggles use. The settings-JSON export now includes the enum values too. Validation: SettingsTab vitest 20 -> 22 (category cards present; PrimaryEnforcement + logLevel render as Dropdowns not read-only); Playwright asserts the category cards + enum Dropdowns + export affordances. Web tsc baseline held; build clean. Version 0.54.5 -> 0.54.6.

### Changed

- **R6: Credentials tab organized into per-method sub-tabs (only enabled methods).** The endpoint Credentials tab now renders a Fluent `TabList` with an "All" overview tab plus one sub-tab per ENABLED authentication method - Shared secret, Per-endpoint bearer, OAuth2 client, WIF ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx)). Which tabs appear is driven by the endpoint config flags using the SAME precedence as the backend `getEffectiveAuthEnablement` (the two per-endpoint flags fall back to the legacy `PerEndpointCredentialsEnabled`; shared-secret defaults on), so the operator sees exactly the auth surface this endpoint accepts. Each method tab scopes the credential list to that type; the OAuth2 client tab defaults the create dialog to `oauth_client`; the Shared secret tab explains the global-secret model (no per-endpoint credential to create); the WIF tab shows the WIF trust section. The "All" tab preserves the prior full behavior (all 37 existing tests pass unchanged). Validation: CredentialsTab vitest 37 -> 44 (+7: tab-per-enabled-method, disabled-method omitted, All lists all, method filter, oauth default create-type, shared-secret info + no create button, WIF tab shows section); Playwright +1 (route-mocked: tabs render + filter by method + shared-secret + WIF). API/UI parity verified against `getEffectiveAuthEnablement` (UI-only change, no API change). Web tsc baseline held; build clean. Version 0.54.8 -> 0.54.9.

### Fixed

- **R1 follow-up: cross-backend parity + test-isolation fixes surfaced by the v0.54.10 dev deploy.** The dev-deployment pipeline's standalone prisma E2E gate caught three issues the inmemory-only runs had masked: **(1) a real cross-backend parity bug** - `PUT /scim/admin/settings/jwks-hosts/{id}` with a malformed/non-existent id returned `500` on the Prisma backend (the `id` column is a Postgres `@db.Uuid`, so a non-UUID string threw a cast error) while InMemory correctly returned `404`; the Prisma `update()` now guards the lookup and resolves any invalid-id / not-found to `null` -> `404`, matching InMemory ([prisma-jwks-host-allowlist.repository.ts](api/src/infrastructure/repositories/prisma/prisma-jwks-host-allowlist.repository.ts)). **(2) Test isolation** - the R1 E2E tests used fixed hostnames that collide on the unique-host constraint across runs against a persistent DB; they now use unique, self-cleaning hostnames ([wif-assertion.e2e-spec.ts](api/test/e2e/wif-assertion.e2e-spec.ts)). **(3) Seed race-resilience** - `ensureSeeded` wraps each per-host insert in try/catch so a concurrent app-init race (multiple instances, one DB) cannot abort the whole persisted-layer load; the compiled seed remains the safety floor ([jwks-host-allowlist.service.ts](api/src/oauth/jwks-host-allowlist.service.ts)). Also regenerated the **Settings-page visual-regression baseline** (intended: the page gained the R4b server-connection-info card + R1 JWKS full-CRUD + settings JSON export), verified against dev. Prisma + InMemory E2E both 25/25; service unit 14/14. Version 0.54.10 -> 0.54.11.

### Added

- **R1: JWKS host allowlist persisted with full admin CRUD (seed prepopulated) + selective PATCH.** The SSRF JWKS-host allowlist is now a fully persisted, CRUD-managed table instead of add/remove-only. **Seed prepopulation:** the compiled well-known IdP seed (Entra commercial/usgov/china, Google) is written into the `JwksHostAllowlistEntry` table on startup as editable rows ([jwks-host-allowlist.service.ts](api/src/oauth/jwks-host-allowlist.service.ts) `ensureSeeded`, idempotent), while the compiled seed also remains a permanent effective-union safety floor so an accidental removal cannot brick Entra/Google auth. **Full CRUD:** the repo interface + InMemory + Prisma impls gained `update(id, host, label)` ([jwks-host-allowlist.repository.interface.ts](api/src/domain/repositories/jwks-host-allowlist.repository.interface.ts)); the service gained `updateHost` + `patchHosts`; the admin controller ([admin-jwks-host.controller.ts](api/src/modules/scim/controllers/admin-jwks-host.controller.ts)) gained `PUT /scim/admin/settings/jwks-hosts/{id}` (edit a specific entry, 404 on unknown id) and `PATCH /scim/admin/settings/jwks-hosts` (**selectively add AND/OR remove** hosts in one call: `{ add?: string[], remove?: string[] }`, 400 on empty body / non-bare add host). The `view()` now returns `persistedEntries[]` (id + host + label) so the UI can edit/remove by id. **UI:** the Settings-page JWKS card ([SettingsPage.tsx](web/src/pages/SettingsPage.tsx)) is now full CRUD - every host is an editable row (Edit-in-place / Remove), plus a selective add/remove PATCH box, plus Copy/Download-as-JSON. Validation: service unit 9 -> 14 (+5: seed prepopulated as editable rows, seed row removable but host stays via floor, updateHost by id + unknown-id, patch selective add/remove + idempotent add); API E2E +4 (seed prepopulation, PUT edit + 404, PATCH add/remove, PATCH validation) and fixed a stale R7 assertion (oauth_client default is `client-id-<endpointId>`); SettingsPage vitest +3 (edit-in-place, PATCH box, updated fixture); Playwright +1 (route-mocked full-CRUD card); live-test 9z-AT7 T6 updated + T7/T8/T9 added (PUT, PATCH, PATCH-empty-400). API unit 4158 -> 4163; web tsc baseline held; build clean. Version 0.54.9 -> 0.54.10.

- **R8: contextual cross-links between related UI surfaces.** Added clickable cross-links so an operator can jump directly to the relevant next step: the Connect tab links to the same endpoint's Credentials tab ("Manage credentials") and Settings tab ("Enable / disable auth methods") ([ConnectTab.tsx](web/src/pages/ConnectTab.tsx)); the Credentials tab links to the same endpoint's Connect tab ("view the connection details") ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx)); the admin Settings page's server-connection-info card links to the endpoints list for per-endpoint details ([SettingsPage.tsx](web/src/pages/SettingsPage.tsx)). All use TanStack Router `useNavigate` with the same typed route params the rest of the app uses. Validation: ConnectTab vitest +1 (links navigate to credentials + settings routes); CredentialsTab vitest +1 (link navigates to the connect route); SettingsPage card covered by the R4b test. Web tsc baseline held; build clean. Version 0.54.7 -> 0.54.8.

- **R4b: SCIMServer-level (global) connection info card on the admin Settings page.** The per-endpoint Connect tab shows the per-endpoint values a client needs; this adds the server-scope equivalent for admins. A new "Server connection info (SCIMServer level)" card on the Settings page ([SettingsPage.tsx](web/src/pages/SettingsPage.tsx)) surfaces the base URL, the GLOBAL OAuth token endpoint (`/scim/oauth/token`), the global JWKS URI (`/scim/oauth/jwks`), the RFC 8414 OAuth AS metadata URL, and the SCIM ServiceProviderConfig - each a copyable field, plus a Copy/Download-as-JSON export. URLs derive from the request origin (same approach as the ConnectionPanel), labels follow Entra with a note that other IdPs use the equivalent field, and no secret is ever shown. Validation: SettingsPage vitest +1 (card renders the global token/JWKS/metadata URLs + export); Playwright asserts the card + global token endpoint. Web tsc baseline held; build clean. Version 0.54.6 -> 0.54.7.

- **Settings JSON copy/download on the per-endpoint + admin settings surfaces.** A new reusable [SettingsJsonExport](web/src/components/primitives/SettingsJsonExport.tsx) primitive (Copy as JSON via the shared `CopyJsonButton` + Download .json) is wired into: the per-endpoint Settings tab ([SettingsTab.tsx](web/src/pages/SettingsTab.tsx)) exporting the effective settings in the exact `{ profile: { settings: {...} } }` PATCH-body shape; the admin Settings page ([SettingsPage.tsx](web/src/pages/SettingsPage.tsx)) security section (the `{ credentialSecretVisibility }` PUT-body shape) and log-config section (`{ globalLevel, categoryLevels }`). The exported blob can be pasted straight back into an API request, saved as a backup, or diffed against an earlier capture. Validation: new primitive unit test (3: derived testids, pretty-printed round-trip copy, download-with-filename); SettingsTab vitest +1; SettingsPage vitest +2; Playwright asserts the security-settings export affordances. Web tsc baseline held; build clean. Version 0.54.4 -> 0.54.5.

- **R4: Connect tab uses Entra-exact labels + sequence with generic-IdP helper text.** The Connect tab's per-method field labels now match Microsoft Entra ID's provisioning form wording exactly (Tenant URL, OAuth token endpoint, Client ID, Client Secret, Secret Token, Audience) and keep Entra's field order, so an operator pasting into Entra recognizes each field 1:1. Because SCIMServer targets ANY SCIM 2.0 IdP (Okta, OneLogin, Ping, custom clients) - not only Entra - each field also carries a generic helper description naming the equivalent value in other products (e.g. Tenant URL -> "SCIM base URL / Base URL"), plus a per-method intro line ([ConnectionPanel.tsx](web/src/components/primitives/ConnectionPanel.tsx)). Validation: ConnectionPanel vitest 8 -> 10 (+2: intro mentions Entra + other IdPs; per-field descriptions render); Playwright connect-tab spec asserts the intro + Client ID description. Web tsc baseline held. Version 0.54.3 -> 0.54.4.

- **R7: per-endpoint OAuth2 client-credentials completed (readable id/secret + UI create + always-copyable).** The per-endpoint `oauth_client` flow existed at the token endpoint but had no first-class create UI and used opaque id/secret formats. **API:** the first `oauth_client` on an endpoint now defaults its public `client_id` to the readable `client-id-<endpointId>` form (an additional one gets `client-id-<uuid>`), and its secret uses the readable `client-secret-<uuid>` form ([admin-credential.controller.ts](api/src/modules/scim/controllers/admin-credential.controller.ts)); an explicit `clientId` still wins. Both are opaque to the token endpoint (it matches by `metadata.clientId` + bcrypt), so the change is transparent to auth. **Web:** the CredentialsTab create dialog gained a credential-type selector (Bearer token / OAuth2 client credentials); creating an `oauth_client` renders the Client Identifier + Client Secret as individually copyable `CopyableField`s plus a "Copy all as JSON" blob (`client_id` + `client_secret` + `token_endpoint` + `grant_type`) via the R9 `CopyJsonButton` ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx)). Validation: admin-credential.controller unit 50 -> 52 (+2: first uses `client-id-<endpointId>` + `client-secret-<uuid>`; additional gets `client-id-<uuid>`); connection-info + credential-reveal E2E updated for the new default id form (12/12); CredentialsTab vitest 35 -> 36 (+1: oauth_client create shows clientId + secret + copy-as-JSON); Playwright +1 (route-mocked oauth create surfaces the id/secret/JSON); live-test section 9z-AX (4 assertions: id format, secret format, the pair mints a token, wrong secret -> 401). Version 0.54.2 -> 0.54.3.

- **R3: the Connect tab always displays the secret when the credential secret visibility is Always.** Previously the Connect tab never showed a secret (secrets were one-time-at-create only). Now, for any enabled per-endpoint method (`bearer` / `oauth_client`) whose credential kept an encrypted copy (effective `CredentialSecretVisibility` = `always`), the Connect tab reveals and displays the secret persistently. **API:** `ConnectionEnabledMethod` gained `credentialId` + `secretRetained` ([connection-info.types.ts](api/src/shared/types/connection-info.types.ts)); the assembler populates them from the credential's `secretEnvelope` presence ([connection-info.service.ts](api/src/modules/scim/services/connection-info.service.ts)). The connection-info response still NEVER carries a secret value - the existing WI-8 reveal endpoint stays the sole authority that returns the plaintext. **Web:** a new `useConnectionRetainedSecrets` hook ([queries.ts](web/src/api/queries.ts)) reveals every retained method's secret in parallel (30 s cache to avoid audit-log spam); `ConnectionPanel` ([ConnectionPanel.tsx](web/src/components/primitives/ConnectionPanel.tsx)) renders the retained secret with a "re-viewable because visibility is Always" info note (NOT the one-time "copy now" warning) and folds it into the Copy-JSON / .env / download payloads; `ConnectTab` ([ConnectTab.tsx](web/src/pages/ConnectTab.tsx)) wires the reveal via an extracted child so the hook is called unconditionally (rules-of-hooks). Validation: connection-info service unit 15 -> 19 (+4); connection-info E2E 5 -> 6 (+1: credentialId + secretRetained surfaced after create, secret still never leaked, ENABLED_KEYS allowlist +2 documented keys); ConnectionPanel vitest +2; ConnectTab vitest +1; Playwright +1 (route-mocked Connect tab shows the retained secret value + note, no one-time warning); live-test section 9z-AW (5 assertions). Version 0.54.1 -> 0.54.2.

- **WIF trust management closure (self-audit gaps A-G).** A deep self-audit after the 2026-07 WIF overhaul found the features shipped without the repo-mandated Playwright/live coverage and with the create/edit-time reachability check only half-implemented. Closed all of it. **(C) Verify-on-save at BOTH ui + api** - `POST/PUT .../credentials` accept `verify:true`, which runs the SSRF-gated reachability + JWKS-serves-keys check BEFORE persisting and rejects with `422` + the `checks[]` (nothing persisted); the UI Save runs it first and offers a "Save anyway" override on failure ([admin-credential.controller.ts](api/src/modules/scim/controllers/admin-credential.controller.ts), [CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx)). **(F) Editable label** on the WIF `PUT` (new `IEndpointCredentialRepository.updateLabel`). **(D) Inline URL validation** on the issuer + JWKS fields ([EditableField.tsx](web/src/components/primitives/EditableField.tsx) gained `validationMessage`). **(E) Role-enforcement dropdown** (advisory/shadow/enforce) surfaced in the form + trust display + `EndpointOverviewWifTrust.roleEnforcement`. **(A) Playwright** - new [wif-trust-management.spec.ts](web/e2e/wif-trust-management.spec.ts) (7 tests: trust-grid values, edit mode, JWKS notice, verify checklist, verify-on-save override, inline validation, role-enforcement). **(B) Live tests** - new [live-test.ps1](scripts/live-test.ps1) section 9z-AV (10 assertions: PUT edit + label, verify pass/fail/disallowed, verify:true 422 gate). **(G) Docs** - new [docs/auth/WIF_TRUST_MANAGEMENT_OVERHAUL.md](docs/auth/WIF_TRUST_MANAGEMENT_OVERHAUL.md) + INDEX registration. Tests: admin-credential.controller 45->50, dashboard.controller 23/23, CredentialsTab vitest 32->35, wif-discovery-resolver 17/17, Playwright +7. Version 0.54.0-alpha.32 -> 0.54.0-alpha.33.

- **WIF trust UX overhaul + all-table scroll/resize (operator request, 7 parts).** A batch of WIF-credential and table improvements. **(1) Tables** - the last four `table-layout:auto` tables ([LogsPage.tsx](web/src/pages/LogsPage.tsx), [LogsTab.tsx](web/src/pages/LogsTab.tsx), [WorkbenchPage.tsx](web/src/pages/WorkbenchPage.tsx) history, [DiscoveryExplorerPage.tsx](web/src/pages/DiscoveryExplorerPage.tsx) diff) migrated to `table-layout:fixed` + percentage/colgroup widths (columns expand on resize) inside `overflowX:auto` wrappers with a sane `minWidth` (a narrow window scrolls instead of clipping); the `audit-table-layout.ps1` R5.3 allowlist is now empty. **(3) WIF form layout** - the scattered `minmax(280px)` field grid is now a single-column full-width vertical stack that expands with the window. **(5) Trust display** - the overview BFF projects each WIF trust's public fields via a closed-allowlist `projectWifTrust` ([dashboard.controller.ts](api/src/modules/dashboard/dashboard.controller.ts), new `EndpointOverviewCredential.wif`), and each configured trust row renders issuer/subject/audience/JWKS/tenant/roles/scope as copyable values. **(8) Roles advisory** - a WIF assertion missing a required role is now logged-and-allowed by default so the provisioning flow continues; rejection only when the trust sets `roleEnforcement:'enforce'` ([wif-assertion-validator.service.ts](api/src/oauth/wif-assertion-validator.service.ts)). **(7) JWKS allowlist awareness** - the WIF form shows the effective JWKS host allowlist and, when the entered JWKS host is not allowed, a warning with a one-click inline "Add to allowlist". **(4) Editable trusts** - a saved WIF trust is now editable in place via a new `PUT /admin/endpoints/:id/credentials/:cid` (WIF-only; `updateMetadata` added to the credential repository interface + inmemory + prisma impls) and an Edit button + edit-mode form. **(6) Reachability verification** - a new SSRF-gated `POST /admin/endpoints/:id/wif/verify` ([wif-discovery-resolver.service.ts](api/src/oauth/wif-discovery-resolver.service.ts) `verifyTrust`) fetches the issuer's OIDC discovery + the JWKS URI at config time and returns a per-check checklist (valid https, host allowed, reachable, serves a non-empty key set), surfaced as a "Verify issuer + JWKS reachability" button + checklist so there are no runtime surprises. Tests (all outcome-level per R10): dashboard.controller 21->23, wif-assertion-validator 11->14, wif-assertion E2E 19->21, admin-credential.controller 40->45, wif-discovery-resolver 11->17, CredentialsTab vitest 25->32. Version 0.54.0-alpha.31 -> 0.54.0-alpha.32.

### Fixed

- **R5: WIF trust form fields now auto-resize with the window (primitive-level fix).** The WIF trust inputs looked fixed-width and did not grow/shrink when the browser was resized. Root cause was in the shared [EditableField.tsx](web/src/components/primitives/EditableField.tsx) primitive: the Fluent `<Input>`/`<Textarea>` have no intrinsic full width, so they rendered at Fluent's ~default width and never stretched to fill their `flex: 1 1 auto` cell - no amount of parent flex-column stacking helped. Added a `width: 100%` class to the field element so every `EditableField` (not just WIF) fills its container and tracks the viewport. Verified by a measured-bounds Playwright test (R1/R5): the issuer field's `getBoundingClientRect().width` is > 500px at a 1440px viewport and shrinks by > 40px at an 820px viewport ([wif-trust-management.spec.ts](web/e2e/wif-trust-management.spec.ts) item R5). Per R2 this is Playwright-only (JSDOM cannot measure layout). Version 0.54.0 -> 0.54.1.
- **Visual-regression baseline refresh: endpoint-detail Users-tab (intended UI drift).** The Stage 5 Playwright visual gate flagged `endpoint-detail-users.png` after the alpha.33 dev deploy. Diff inspection (R3) confirmed the sole change is the endpoint tab bar gaining the `Connect` tab (`...Credentials -> Connect -> Logs -> Settings`), a legitimate feature tab, plus the expected masked dynamic regions and the version-badge bump - not a regression and unrelated to the table-layout work. Classified INTENDED and regenerated the single baseline against dev ([web/e2e/visual-regression.spec.ts-snapshots/endpoint-detail-users-chromium-win32.png](web/e2e/visual-regression.spec.ts-snapshots/endpoint-detail-users-chromium-win32.png)); the test now passes clean against dev.
- **WI-9: one-click credential rotate (lost-secret recovery)** - a new `POST /admin/endpoints/{id}/credentials/{cid}/rotate` ([admin-credential.controller.ts](api/src/modules/scim/controllers/admin-credential.controller.ts)) mints a fresh secret (shown once, retained encrypted if the effective `CredentialSecretVisibility` is `always`), then deactivates the old credential. An `oauth_client` keeps its public `client_id` so the IdP only updates the secret; a `bearer` returns a new `token`; a `wif` credential (no secret) is rejected with 400. The response carries `rotatedFrom` + the one-time secret and never exposes the stored envelope. This is the recovery path when a secret was shown once and lost, or the endpoint is `once` and the operator needs a fresh viewable secret. **UI:** a Rotate button + one-time result dialog on the CredentialsTab (new secret via R9 `CopyableField`, client_id shown when unchanged). Validation: 5 controller unit + 5 E2E (oauth rotate+client_id preserved+old-deactivated, rotated-secret-revealable, once-not-retained, wif-reject, 404) + 2 CredentialsTab vitest + 1 Playwright + live-test 9z-AT11. API unit 4124 -> 4129 (123 suites); API build clean; web build + tsc 0-new; CredentialsTab vitest 25/25. Version 0.54.0-alpha.27 -> 0.54.0-alpha.28.
- **WI-8: retained-secret reveal + server security settings (audit-logged)** - the admin can now re-view a retained credential secret and manage the server-scope ceiling. New `POST /admin/endpoints/{id}/credentials/{cid}/reveal` ([admin-credential.controller.ts](api/src/modules/scim/controllers/admin-credential.controller.ts)) decrypts the WI-7 `secretEnvelope` via `CredentialEncryptionService` when the effective `CredentialSecretVisibility` is `always` and an envelope exists, returning `{clientSecret|token, retained:true}`; when the effective setting is `once`, the credential predates retention, or the envelope is undecryptable (KEK changed), it returns a non-error `{retained:false, reason}` telling the operator to rotate. **Every reveal attempt writes a `LogCategory.AUTH` audit line** (GRANTED/DENIED/FAILED) and the response NEVER exposes the stored envelope. New `AdminSecuritySettingsController` serves `GET/PUT /admin/settings/security` - the server-scope `credentialSecretVisibility` (the most-restrictive-wins ceiling) plus the KEK status (`{configured, isDefault}`, never the value); flipping the server to `once` purges every retained ciphertext (`clearAllSecretEnvelopes`). **UI:** a Reveal button + one-time result dialog on the CredentialsTab (retained secret via an R9 `CopyableField`, or the not-retained explanation), and a "Credential secret security (server)" card on the global Settings page (server always/once radio + KEK status line). Validation: 6 controller unit + 6 E2E (reveal always/once/server-ceiling+purge/404, settings get/validate) + 5 CredentialsTab + SettingsPage vitest + 1 Playwright + live-test 9z-AT10. API unit 4118 -> 4124 (123 suites); API build clean (18 controllers); web build + tsc 0-new; CredentialsTab + SettingsPage vitest 41/41. Version 0.54.0-alpha.26 -> 0.54.0-alpha.27.
- **WI-7: `CredentialSecretVisibility` setting + retain-on-create wiring** - the operator-facing control for the re-viewable-secret feature (design 6A.2/6A.3), built on the WI-6 envelope foundation. A new two-value enum endpoint config flag `CredentialSecretVisibility` (`always` default | `once`) joins the registry ([endpoint-config.interface.ts](api/src/modules/endpoint/endpoint-config.interface.ts), 20 -> 21 flags) with a `credentialVisibility` validator + the `getEffectiveCredentialSecretVisibility` precedence helper (most-restrictive-wins, **server is the ceiling**: server `once` forces `once` everywhere). The server-scope value is persisted in a new `ServerSetting` key/value table (seeded `credentialSecretVisibility=always` by migration + the InMemory backend) behind a new `IServerSettingRepository`. A new [credential-security.service.ts](api/src/security/credential-security.service.ts) reads/writes the server value, computes the effective value, and purges retained ciphertext. Retain-on-create is wired into [admin-credential.controller.ts](api/src/modules/scim/controllers/admin-credential.controller.ts): when the effective visibility is `always`, a freshly-minted `bearer`/`oauth_client` secret is encrypted via `CredentialEncryptionService` and stored in a new `EndpointCredential.secretEnvelope` column (migration `20260707110000`); it degrades safely to show-once if encryption is unavailable, and the envelope is NEVER exposed on any response (reveal is WI-8). **UI:** an `always`/`once` segmented control on the endpoint SettingsTab. Validation: 8 config-helper/validator unit + 6 CredentialSecurityService unit + 4 E2E (flag accept always/once, reject invalid, no-envelope-leak) + 3 SettingsTab vitest + 1 Playwright + live-test 9z-AT9. API unit 4104 -> 4118 (122 suites); API build clean; web build + tsc 0-new; SettingsTab vitest 19/19. Flag doc 20 -> 21. Version 0.54.0-alpha.25 -> 0.54.0-alpha.26.
- **WI-6: retained-secret envelope-encryption foundation (KEK -> DEK)** - the crypto foundation for the re-viewable-credential-secret feature (design section 6A). A pure, DB-free crypto core ([credential-envelope.ts](api/src/security/credential-envelope.ts)) implements the standard two-level envelope scheme: a KEK (key-encryption-key) derived from the `CREDENTIAL_KEK` passphrase via scrypt wraps a per-install random DEK (data-encryption-key); the DEK AES-256-GCM encrypts each retained secret. Wrapped keys + secret envelopes are self-describing `v1.<iv>.<ct>.<tag>` base64url strings. The KEK loader ([credential-kek.ts](api/src/security/credential-kek.ts)) reads `CREDENTIAL_KEK` with the known public default `changeme-credential-kek` (like `changeme-oauth`). The runtime owner ([credential-encryption.service.ts](api/src/security/credential-encryption.service.ts)) provisions + persists a wrapped DEK on first boot (new `CredentialDek` table + InMemory/Prisma repos + migration `20260707100000_add_credential_dek`), unwraps it into memory, and exposes `encrypt`/`decrypt` + a KEK status (`configured`/`isDefault`). **The KEK is never on the authentication hot path** (auth compares the bcrypt hash); it is touched only by the retain/reveal/rotate paths (WI-7/8/9). Init never blocks startup - a KEK mismatch logs and leaves reveal unavailable rather than failing boot. The known-default KEK is now wired into [docker-compose.yml](docker-compose.yml), [DEPLOYMENT.md](DEPLOYMENT.md), and the [README.md](README.md) env table with the rotate-in-prod + identical-across-instances warnings. Validation: 15 envelope-core unit (derive determinism, wrap/unwrap independence, GCM tamper-detection, full KEK->DEK->secret round-trip) + 4 KEK-loader unit + 7 service unit (provision-once, no-re-provision, KEK-change blast-radius, KEK status, not-ready guard) = 26 new unit; API unit 4080 -> 4104 (121 suites); API build clean; app boots with the service in-graph. Version 0.54.0-alpha.24 -> 0.54.0-alpha.25.
- **WI-5: Connect tab + ConnectionPanel wiring** - a new per-endpoint **Connect** tab ([ConnectTab.tsx](web/src/pages/ConnectTab.tsx), route [endpoints.$endpointId.connect.tsx](web/src/routes/endpoints.$endpointId.connect.tsx)) renders the WI-4 `ConnectionPanel` from the WI-3 `overview.connectionInfo` (zero extra round trips on a warm cache), so an operator sees exactly what to paste into Microsoft Entra ID for THIS endpoint: the method selector, the per-method Entra field mapping (each value a copy button), the export affordances (Copy all JSON / Copy as .env / Download .json), and a list of the not-enabled methods with their enable hint. Wired into [EndpointDetailPage.tsx](web/src/pages/EndpointDetailPage.tsx) as a new `Connect` tab (path-based child route `/endpoints/{id}/connect`, testid `endpoint-tab-connect`) and registered in [router.ts](web/src/router.ts). The new lazy route gets its own size-limit budget (`ConnectTab`, 0.84 kB gzip). Validation: 5 ConnectTab vitest (loading / error / panel-from-overview / disabled-methods list) + 2 EndpointDetailPage vitest (Connect tab present + selected-on-URL) + a `connect-tab.spec.ts` Playwright spec (tab present, panel + method selector + copy-JSON/.env/Download, Tenant URL copy button carries the `/scim/v2` form, method-switch swaps fields) - this is where WI-4's browser coverage lands. Web build + size-limit (incl. the new ConnectTab budget) pass; router tests 20/20; web tsc 0-new. Version 0.54.0-alpha.23 -> 0.54.0-alpha.24.
- **WI-4: `<ConnectionPanel>` primitive + Entra-field mapping** - one reusable component ([ConnectionPanel.tsx](web/src/components/primitives/ConnectionPanel.tsx)) that renders an endpoint's connection properties for an identity-provider operator from the WI-2 `ConnectionInfo` shape (no URL hand-building). It shows a method selector (one radio per enabled method), the Entra `Authentication Method` label, each Entra field as a read-only R9 `CopyableField` (Tenant URL, Token Endpoint, Client Identifier, ...), and block-level exports: `Copy all as JSON` (R9 `CopyJsonButton`), `Copy as .env` (SCIM_* variable lines), and `Download .json`. When the selected method is `wif` it surfaces the expected audience; when the caller passes a create-moment `oneTimeSecret` for the selected method it renders the secret with a "copy it now, it will not be shown again" warning and folds it into the JSON / .env / download payloads (otherwise the secret row shows a `Shown once at creation` or `Create a credential` placeholder). Empty-state message when no method is enabled. All values flow through the existing R9 primitives (no hand-rolled clipboard state); stable `connection-panel-*` testids. Also added a `useConnectionInfo(endpointId)` query hook ([queries.ts](web/src/api/queries.ts)) for surfaces that need only the connection block (the Overview BFF already embeds it via WI-3). The panel is wired into the Connect surfaces in WI-5. Validation: 7 vitest (method radios + copy-JSON button, default-method fields, method switch, one-time-secret + warning, copy-as-.env SCIM_* lines, empty state, create-required placeholder); web build + size-limit (shared-primitives budget) pass; web tsc 0-new. Version 0.54.0-alpha.22 -> 0.54.0-alpha.23.
- **WI-3: surface connection-info on the per-endpoint Overview BFF** - the existing `GET /scim/admin/endpoints/{id}/overview` BFF now embeds the WI-2 `connectionInfo` (the assembled absolute URLs + per-method Entra field set) as a first-class response field, so the Overview UI + the coming `ConnectionPanel` never hand-build URLs and never need a second round trip. The `ConnectionInfo` type set moved to a shared [connection-info.types.ts](api/src/shared/types/connection-info.types.ts) (consumed by the API assembler, the dashboard BFF, and the web UI via the `@scim/types` alias); [connection-info.service.ts](api/src/modules/scim/services/connection-info.service.ts) re-exports them for back-compat. [dashboard.controller.ts](api/src/modules/dashboard/dashboard.controller.ts) gained the pure `ConnectionInfoService` (provided directly in [dashboard.module.ts](api/src/modules/dashboard/dashboard.module.ts) to avoid a module cycle) and derives the host from `X-Forwarded-Proto`/`X-Forwarded-Host`. The overview key-allowlist grew by exactly one documented key (`connectionInfo`); no secret ever leaks. Validation: dashboard controller spec key-allowlist + reqStub host derivation (36/36 with connection-info.service) + dashboard-overview E2E asserts the embedded `connectionInfo` shape + no bcrypt leak + web MSW fixture/`mutations.test.ts` updated (70/70) + live-test 9z-AT8.T6. API unit 4080 (118 suites); API build clean; web tsc 0-new. Version 0.54.0-alpha.21 -> 0.54.0-alpha.22.
- **WI-2: connection-info API (single server-side assembler)** - a new read-only admin endpoint `GET /scim/admin/endpoints/{id}/connection-info` assembles every absolute URL + the per-auth-method Entra field set once, server-side, so no UI hand-builds URLs. New [connection-info.service.ts](api/src/modules/scim/services/connection-info.service.ts) is a PURE assembler (takes the loaded endpoint + credentials + a base URL) that returns the [Part 6](docs/auth/CONNECTION_INFO_AND_ENTRA_SETUP.md) shape: `urls` (the `scimBaseUrl` LEADING `/scim/v2/endpoints/{id}` form from WI-1, `scimBaseUrlBare`, the bare `tokenEndpoint`, `serviceProviderConfig`, and the WI-12 per-endpoint `oauthMetadata` append form), `enabledMethods[]` (one per active auth method with its `entraAuthenticationMethod` + `entraFields` + `clientSecretState`), and `disabledMethods[]` (each with a `reason` + `enableHint`). Method enable/disable reuses `getEffectiveAuthEnablement` + `getConfigBoolean`, so it agrees byte-for-byte with the create-gate and the resource-plane guard. New [admin-connection-info.controller.ts](api/src/modules/scim/controllers/admin-connection-info.controller.ts) derives the host from `X-Forwarded-Proto`/`X-Forwarded-Host` (falling back to the request host) exactly as the OAuth metadata controllers do, so every surface agrees on the origin. **Hard contract:** NO secret value is ever present (`secretToken`/`clientSecret` are always null; secrets stay one-time-at-create) and NO internal `_`-prefixed key leaks - enforced by a key-allowlist E2E assertion. The UI consumer (BFF proxy + `ConnectionPanel` primitive + Connect surfaces) lands in WI-3/WI-4/WI-5. Validation: 15 service unit (URL permutations + API_PREFIX + per-method selection + clientSecretState transitions + WIF audience default/override + no-secret invariant) + 5 E2E (documented-key allowlist, X-Forwarded host derivation, config-driven enable/disable, secret-never-returned + set-shown-once flip, 404) + live-test section 9z-AT8. API unit 4065 -> 4080 (118 suites); API build clean. Version 0.54.0-alpha.20 -> 0.54.0-alpha.21.
- **WI-15: admin-editable JWKS host allowlist (hot-reload SSRF guard)** - the SSRF allowlist that gates every outbound WIF JWKS + discovery fetch is now editable at runtime instead of being env-only. A new [jwks-host-allowlist.service.ts](api/src/oauth/jwks-host-allowlist.service.ts) computes the EFFECTIVE allowlist as the UNION of three layers: a compiled well-known IdP seed (`WELL_KNOWN_JWKS_HOST_SEED` - Entra commercial/usgov/china, Google), the existing `JWKS_HOST_ALLOWLIST` env var, and a new persisted admin-editable layer (`JwksHostAllowlistEntry` table + InMemory backend). `isAllowed()` stays O(1) against a rebuilt Set; `addHost()`/`removeHost()` mutate the persisted layer and hot-reload the Set with NO redeploy. Both [external-jwks-validator.service.ts](api/src/oauth/external-jwks-validator.service.ts) and [wif-discovery-resolver.service.ts](api/src/oauth/wif-discovery-resolver.service.ts) now consult the service when injected, falling back to the local env Set for standalone unit tests. New admin API [admin-jwks-host.controller.ts](api/src/modules/scim/controllers/admin-jwks-host.controller.ts) at `GET/POST /scim/admin/settings/jwks-hosts` + `DELETE /scim/admin/settings/jwks-hosts/{host}`; POST rejects a non-bare hostname (scheme/port/path/spaces) with 400. This is a convenience/flexibility feature - it only ADDS trustable hosts; there is no deny-list and no lock flag (operator choice). **UI:** a new "JWKS host allowlist (WIF SSRF guard)" card on the global Settings page lists the built-in (seed + env) hosts, shows one removable row per persisted host, and adds a host via a bare-hostname input (lowercased client-side); the whole view is copyable as JSON. Validation: 9 service unit + 3 E2E (view exposes seed/env/effective union; runtime add makes discovery against the host succeed; non-bare-hostname reject) + 6 vitest (card render, per-host rows, built-in line, empty state, add-lowercased, remove) + 3 Playwright (card render, seed-host named, add-then-remove self-cleaning round-trip) + live-test section 9z-AT7 (6 assertions incl. hot-reload add/remove + seed-not-persisted). API unit 4042 -> 4065 (117 suites); API build clean; web tsc 0-new; web build + size-limit pass; SettingsPage vitest 15/15. Prisma migration `20260707000000_add_jwks_host_allowlist`. Version 0.54.0-alpha.19 -> 0.54.0-alpha.20.
- **WI-14: WIF trust discovery resolver + smart defaults** - a config-time resolver ([wif-discovery-resolver.service.ts](api/src/oauth/wif-discovery-resolver.service.ts)) turns one or two inputs into the WIF signing-trust fields by reading the SOURCE IdP's OIDC discovery document. `POST /admin/endpoints/{id}/wif/resolve` accepts Mode A (`discoveryUrl` - any OIDC IdP) or Mode B (`preset` + `tenantId` - entra-commercial/usgov/china, google), fetches `.well-known/openid-configuration` ONCE (operator-approved), and returns `{expectedIssuer, jwksUri, expectedAudience}` where the audience defaults to the endpointId. The fetch is gated by the same `JWKS_HOST_ALLOWLIST` (SSRF) as the runtime JWKS fetch - a disallowed host is rejected BEFORE any network call - and both the discovery host AND the advertised `jwks_uri` host are checked. Smart default for `oauth_client` ([admin-credential.controller.ts](api/src/modules/scim/controllers/admin-credential.controller.ts)): the FIRST `oauth_client` on an endpoint defaults its public `client_id` to the endpointId (no lookup); any additional one gets a generated `epc_` id; an explicit `clientId` always wins. **UI:** the CredentialsTab WIF section gained a "Resolve from Entra tenant id" row that fills Issuer + JWKS URI + audience from a single tenant id. Validation: 12 resolver unit + 3 create-gate/default unit + 5 E2E (resolve Mode A/B, SSRF reject, missing-inputs, oauth_client default) + 1 vitest (resolver row) + 1 Playwright (resolver row) + live-test section 9z-AT6. API unit; API build clean; web tsc 0-new; CredentialsTab vitest 21/21. Version 0.54.0-alpha.18 -> 0.54.0-alpha.19.
- **WI-11: per-method auth-enablement flag family (splits `PerEndpointCredentialsEnabled`)** - the single double-duty flag is split into three per-method flags, each gating one auth method independently at credential-create AND on the resource-plane validation path ([endpoint-config.interface.ts](api/src/modules/endpoint/endpoint-config.interface.ts)): `SecretTokenBearerAuthEnabled` (per-endpoint `bearer`), `OAuthClientCredentialsAuthEnabled` (per-endpoint `oauth_client`), and the NEW `SharedSecretBearerAuthEnabled` (whether the endpoint accepts the global `SCIM_SHARED_SECRET` - turn OFF to make an endpoint accept only its own credentials). A new `getEffectiveAuthEnablement()` helper computes each effective value with a **value-preserving fallback**: the two per-endpoint flags fall back to the legacy `PerEndpointCredentialsEnabled` when unset, and the shared-secret flag defaults to `true`, so every existing endpoint behaves byte-for-byte as before. The legacy flag is read as a one-release fallback. Wired into [admin-credential.controller.ts](api/src/modules/scim/controllers/admin-credential.controller.ts) (create gate, per credentialType) + [shared-secret.guard.ts](api/src/modules/auth/shared-secret.guard.ts) (per-method credential matching + a new endpoint-scoped shared-secret refusal that fails open on lookup error). **UI:** three new Fluent Switches on the SettingsTab. Validation: 9 config-helper unit + 4 guard unit + 4 create-gate unit + 6 E2E + 1 SettingsTab vitest + 1 Playwright (switch presence) + live-test section 9z-AT5. API unit 4026 -> 4042 (115 suites); web tsc 0-new; SettingsTab vitest 16/16. Version 0.54.0-alpha.17 -> 0.54.0-alpha.18.
- **WI-12: per-endpoint OAuth AS metadata (RFC 8414, append form)** - a standards-based OAuth client can now discover an endpoint's token endpoint + JWKS without prior configuration via `GET /scim/endpoints/{id}/.well-known/oauth-authorization-server` (the OIDC-style append form, matching Entra/Okta/Keycloak/Cognito). New [endpoint-oauth-metadata.controller.ts](api/src/modules/scim/controllers/endpoint-oauth-metadata.controller.ts) publishes `issuer` = the bare per-endpoint identifier (`<base>/scim/endpoints/{id}`, RFC 8414 section 3.3 mix-up defense), `token_endpoint` = the per-endpoint one (appended to the issuer, so trivially self-consistent), and `jwks_uri` = the shared global key set. Public (no bearer); honors `x-forwarded-proto`/`x-forwarded-host`. Registered before the generic `:resourceType` controller so its 2-segment route wins. **UI:** the CredentialsTab WIF return box surfaces the metadata URL as a new copyable "OAuth metadata URL (RFC 8414)" row. Validation: 5 controller unit + 2 E2E (append-form document shape; self-consistency + public) + 1 vitest (return-box row) + 1 Playwright (metadata row) + live-test section 9z-AT4 (incl. a no-route-shadowing check that `/Users` still resolves). API unit 4018 -> 4026 (115 suites); API build clean; web tsc 0-new; web vitest 20/20. Version 0.54.0-alpha.16 -> 0.54.0-alpha.17.
- **WI-13: WIF trust claim-name input aliases + `expectedTenantId`** - a power user can now paste a decoded token's bare claim names when creating a `wif` trust: `iss`/`sub`/`aud`/`tid`/`roles` are accepted as INPUT aliases for `expectedIssuer`/`expectedSubject`/`expectedAudience`/`allowedTenantId`/`requiredRoles`, and the clearer `expectedTenantId` is accepted as an alias of `allowedTenantId` ([admin-credential.controller.ts](api/src/modules/scim/controllers/admin-credential.controller.ts) `normalizeWifTrustAliases`). Aliases are normalized to the canonical keys BEFORE validation; an explicitly-supplied canonical key always wins over its alias; alias keys are never persisted (only canonical keys are stored). The runtime validation + storage contract is unchanged - this is a config-time input convenience filling the SAME fields. **UI:** the CredentialsTab WIF form ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx)) shows a claim-name alias hint and the tenant field is relabeled `Allowed tenant id (tid / expectedTenantId)`. Validation: 3 controller unit (alias normalization; `expectedTenantId`; canonical-wins) + 1 E2E (alias-shaped create mints + echoes canonical keys) + 1 vitest (alias hint) + 1 Playwright (alias hint visible) + live-test section 9z-AT3. API unit; API build clean; web tsc 0-new; web vitest 20/20. Version 0.54.0-alpha.15 -> 0.54.0-alpha.16.
- **WI-17: issuer-first WIF trust selection + source-stamped token (token level)** - the token-level half of the multi-IdP design ([docs/auth/CONNECTION_INFO_AND_ENTRA_SETUP.md](docs/auth/CONNECTION_INFO_AND_ENTRA_SETUP.md) section 5F.2). When an endpoint holds several WIF trusts, [wif-assertion-token.provider.ts](api/src/modules/scim/controllers/wif-assertion-token.provider.ts) now decodes the assertion's `iss` claim WITHOUT verifying it and tries the trust whose `expectedIssuer` matches FIRST, so the common case does exactly one JWKS verification (O(1)) instead of N. The decoded `iss` only SELECTS the order - the signature is still verified against the chosen trust's JWKS, so a spoofed claim gains nothing. If the `iss` is undecodable (non-JWT) or matches no trust, every trust is tried in turn (WI-16 fallback). The minted token is now stamped with a `src_iss` claim carrying the winning trust's issuer ([oauth.service.ts](api/src/oauth/oauth.service.ts) `generateEndpointAccessToken` gained an optional `sourceIssuer`), for downstream source attribution; plain `oauth_client` mints omit `src_iss`. **UI:** no distinct admin surface - `src_iss` lives inside the ISV's minted token, and multi-IdP trust management is the WI-16 "Configured federated trusts" list. Validation: 3 provider unit (issuer-first O(1) selection; `sourceIssuer` passed to mint; undecodable-iss fallback to try-all) + 2 oauth.service unit (`src_iss` present when supplied; omitted otherwise) + 1 E2E (minted token carries `src_iss`). API unit 4013 -> 4018; API build clean. Version 0.54.0-alpha.14 -> 0.54.0-alpha.15.
- **WI-16: multi-trust WIF (several identity providers into one endpoint, config level)** - an endpoint may now hold SEVERAL `wif` trusts (one per IdP), and every active trust is tried until one validates. [wif-assertion-token.provider.ts](api/src/modules/scim/controllers/wif-assertion-token.provider.ts) changed from `find` (first-only) to a `filter` + iterate: a rejecting or misconfigured trust is a non-match and the next is tried; if NONE accepts, the assertion is mine-but-invalid and fails closed (throws, never falls through). A single-trust endpoint is byte-for-byte unchanged. This is the config-level half of the multi-IdP design ([docs/auth/CONNECTION_INFO_AND_ENTRA_SETUP.md](docs/auth/CONNECTION_INFO_AND_ENTRA_SETUP.md) section 5F); the resource level is unchanged (all IdPs share one common pool; isolation is a separate endpoint). The winning credential id is stamped on the mint log line. **UI:** the CredentialsTab WIF section ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx)) now shows a "Configured federated trusts (N)" header with guidance that every listed trust authenticates simultaneously and that isolation means a separate endpoint. Validation: 2 provider unit + 2 E2E (`wif-assertion.e2e-spec.ts`: mints against the second of two trusts; all-reject -> invalid_client) + 1 vitest (multi-trust header) + 1 Playwright (route-mocked two-trust overview) + live-test section 9z-AT2 (two trusts both list active). API unit 4013/4013; full web vitest; web tsc 0-new; API build clean. Version 0.54.0-alpha.13 -> 0.54.0-alpha.14.

### Fixed

- **Operations "All users" / "All groups" tables read as a single-column list (R5 table-layout regression)** - the cross-endpoint [OperationsPage.tsx](web/src/pages/OperationsPage.tsx) tables declared 4 columns (userName/active/endpoint/created and displayName/members/endpoint/created) but rendered with `table-layout:auto` (the browser default) and an UNTRUNCATED name cell (`<Text>{userName}</Text>`). A long email-shaped userName then sized the first column to its natural width - measured **743px of a 1103px table (67%)** on dev - shoving the active/endpoint/created columns off the visible area so the operator saw only the userName column and reported "no table". This is the exact class the repo's **R5 rule** exists to prevent (tables with truncating cells MUST use `table-layout:fixed` + explicit column widths), which the sibling per-endpoint [UsersTab.tsx](web/src/pages/UsersTab.tsx) already followed. Fix: `rowTable` now sets `tableLayout:fixed`, every column carries a PERCENTAGE width (name 40%, active/members 14%, endpoint 20%, created 26% - percentages so the columns scale proportionally at ANY viewport; an early cut used px metadata columns + a `width:auto` name column, which then STARVED the name column to 1px on a narrow window), `rowCell` gains `overflow:hidden`, the name cell renders through the `TruncatedText` primitive (R4 self-contained ellipsis) instead of a raw `<Text>`, and the `created` cell now shows a short local date instead of a raw ISO string (the full value stays in the CSV export). **Regression coverage:** a new R1 bounded-column Playwright assertion in [operations-page.spec.ts](web/e2e/operations-page.spec.ts) measures real header bounds and fails if any column overflows the table right edge, if the userName column exceeds 60% of the table width (confirmed RED at 67% on the unfixed build), OR if the name column collapses below 30% at a narrow 900px viewport (confirmed RED at 0% on the px+auto cut). Validation: OperationsPage vitest 9/9; web tsc 0-new; RED->GREEN Playwright regression. Version 0.54.0-alpha.29 -> 0.54.0-alpha.31.
- **SPA deep-link 404 on 5 top-level routes (hard-refresh / bookmark break)** - a dev-deploy UI audit found that a hard refresh or deep link to `/discovery`, `/operations`, `/workbench`, `/me`, or `/manual-provision` returned a NestJS JSON 404 instead of the SPA shell, because the Express SPA-fallback allowlist `SPA_PATH_PREFIXES` in [spa-fallback.ts](api/src/bootstrap/spa-fallback.ts) had only ever been extended to `/admin`, `/endpoints`, `/logs`, `/settings` and was never updated when Phases L2/L5/L6/M1 shipped those five routes. In-app sidebar navigation worked (TanStack Router pushState never hits the server), so the gap only bit on hard-navigation - exactly the class the SPA-fallback contract exists to cover. Confirmed on dev via HTTP probes (`/`,`/endpoints`,`/logs`,`/settings` -> 200; the five above -> 404). Fix: expanded the allowlist to all 9 top-level SPA route prefixes (alphabetically sorted). **Regression coverage added at three layers so this cannot silently drift again:** (1) a route-coverage **drift guard** in [spa-fallback.spec.ts](api/src/bootstrap/spa-fallback.spec.ts) that reads the ACTUAL `web/src/routes/*.tsx` files, extracts every `getParentRoute: () => rootRoute` route's top-level URL prefix, and fails if any is absent from `SPA_PATH_PREFIXES`; (2) the exact-list + sorted-order unit assertions; (3) the [spa-fallback.e2e-spec.ts](api/test/e2e/spa-fallback.e2e-spec.ts) `SPA_PATHS` matrix now hard-navigates all five previously-missing routes (+ `/endpoints/{id}/connect`, `/operations?active=true`) asserting `200 text/html <!doctype html`. Validation: spa-fallback unit 9 -> 10; spa-fallback E2E 13 -> 22 deep-link paths; API build clean. Version 0.54.0-alpha.28 -> 0.54.0-alpha.29.
- **Polluted visual-regression baseline unmasked (Manual Provision) + load guard** - regenerating dev after the SPA fix surfaced that the committed `manual-provision-chromium-win32.png` visual baseline was itself a screenshot of the `{"message":"Cannot GET /manual-provision","error":"Not Found","statusCode":404}` JSON error page. It had been captured while the SPA-fallback bug was live, so the visual gate was comparing a 404 against a 404 and stayed GREEN - it never caught the routing bug (the R3 "a gate can green-light the very bug it exists to catch" failure mode). Per R3/R8 the diff PNGs were inspected and the change classified INTENDED; the baseline was regenerated against the now-correct page. To stop an error page ever becoming a baseline again, [visual-regression.spec.ts](web/e2e/visual-regression.spec.ts) now asserts `getByTestId('manual-provision-page')` is visible BEFORE screenshotting (a load guard every visual baseline should carry). Separately, [copy-and-truncate.spec.ts](web/e2e/copy-and-truncate.spec.ts) was producing a data-dependent false-RED (dev's longest userName - a 42-char, 267px value - fits under the 280px TruncatedText cap, so the browser correctly does not clip and `scrollWidth === clientWidth`); it now `test.skip`s the ellipsis-fired assertion when the probed userName does not actually overflow, keeping the R4/R5 bounded-width assertions always-on. Full Playwright vs dev after both fixes: 0 failures.
...
- **Per-endpoint OAuth token endpoint now accepts `application/x-www-form-urlencoded` (RFC 6749 §3.2) - fixes the Entra `CredentialValidationUnavailable` recurrence.** After the `client_secret_basic` fix (0.54.0-alpha.9) shipped, the operator still hit `SystemForCrossDomainIdentityManagementCredentialValidationFailure` / "Supported CredentialLocationInRequest is required" on the customer-facing prod. Root cause: Entra's tenant URL points at the **per-endpoint** token URL (`/scim/endpoints/{id}/oauth/token`), which lives under `endpoints/*` and was therefore caught by the SCIM content-type middleware ([scim-content-type-validation.middleware.ts](api/src/modules/scim/middleware/scim-content-type-validation.middleware.ts)) and rejected with `415 Unsupported Media Type` before the credentials were ever read - Entra's client-credentials grant sends the request as `application/x-www-form-urlencoded`, not SCIM JSON. The global `/scim/oauth/token` sits outside `endpoints/*` and was already exempt, which is why the earlier fix verified green on the global path. Fix: the middleware now exempts ANY `*/oauth/token` path (regex `/\/oauth\/token\/?$/`) from the SCIM media-type rule, since a token endpoint is an OAuth endpoint, not a SCIM resource endpoint. This matches the `A3` exemption already present on the feat/wif branch. Validation: E2E endpoint-oauth-client +2 (form-urlencoded body; form-urlencoded + `Authorization: Basic` = exact Entra flow) -> 10/10; live `live-test.ps1` 9z-AP T13-T14. API build clean. RCA: [docs/auth/EXECUTION_ISSUES_AND_RCA.md](docs/auth/EXECUTION_ISSUES_AND_RCA.md) Issue 2. Version 0.54.0-alpha.9 -> 0.54.0-alpha.10.
- **OAuth token endpoints now accept client credentials in the `Authorization: Basic` header (`client_secret_basic`), not only in the request body (`client_secret_post`).** Microsoft Entra's newer "OAuth2 client credentials grant" provisioning experience sends `client_id`/`client_secret` in the Basic header and pre-validates the supported credential location; a body-only endpoint failed the Entra connection test with `CredentialValidationUnavailable` / "Supported CredentialLocationInRequest is required". Root cause: both token endpoints (global `POST /scim/oauth/token` and per-endpoint `POST /scim/endpoints/{id}/oauth/token`) had only ever implemented `client_secret_post` (RFC 6749 section 2.3.1 defines two valid locations; the Basic branch was never present - `git log -S 'Basic'` shows no history). The gap was dormant because earlier Entra setups used the bearer/secret-token method (which never calls the token endpoint); switching an app to the OAuth2 client-credentials grant exercised the path for the first time. New helper [client-credential-location.ts](api/src/oauth/client-credential-location.ts) parses the Basic header (form-urlencoded halves, first-colon split) and resolves credentials with body precedence; both controllers call it before validating; the RFC 8414 metadata ([oauth-metadata.controller.ts](api/src/oauth/oauth-metadata.controller.ts)) now advertises `["client_secret_basic", "client_secret_post"]`. Validation: helper unit +8 ([client-credential-location.spec.ts](api/src/oauth/client-credential-location.spec.ts)); E2E - endpoint-oauth-client +1 (mint via Basic) and oauth-discovery +1 (advertise==enforce: metadata advertises `client_secret_basic` AND the endpoint accepts it) -> 14/14 across the two suites; live `live-test.ps1` 9z-AP T10-T12 (Basic mint + endpoint_id claim + metadata advertises the method). RCA: [docs/auth/EXECUTION_ISSUES_AND_RCA.md](docs/auth/EXECUTION_ISSUES_AND_RCA.md) Issue 1. API build clean; ESLint 0 errors. Version 0.54.0-alpha.8 -> 0.54.0-alpha.9.
...
- **WI-10: connection-info epic finalization + docs rollup.** Marked all 17 work items of the connection-info / secret-visibility / auth-method-hygiene / multi-IdP epic DONE in [docs/auth/CONNECTION_INFO_EXECUTION_LEDGER.md](docs/auth/CONNECTION_INFO_EXECUTION_LEDGER.md) and flipped the [CONNECTION_INFO_AND_ENTRA_SETUP.md](docs/auth/CONNECTION_INFO_AND_ENTRA_SETUP.md) top status from "Analysis + design" to "IMPLEMENTED (0.54.0-alpha.28)". Added [docs/auth/CONNECTION_INFO_EXECUTION_RCA.md](docs/auth/CONNECTION_INFO_EXECUTION_RCA.md) - the epic's issue/RCA ledger (8 issues; symptom / root-cause / fix / why-the-fix-works / prevention + detection-stage escape analysis), reconciled against the session transcript, promoting two recurring patterns (**PA shared-shape ripple**, **PB constructor-arity drift**). Registered the ledger + RCA in [docs/INDEX.md](docs/INDEX.md); bumped [docs/CONTEXT_INSTRUCTIONS.md](docs/CONTEXT_INSTRUCTIONS.md) + [Session_starter.md](Session_starter.md) to 0.54.0-alpha.28 with the epic achievement row. The KEK operator guide (WI-6) is live in [DEPLOYMENT.md](DEPLOYMENT.md) + [README.md](README.md) + [docker-compose.yml](docker-compose.yml); the flag reference ([docs/ENDPOINT_CONFIG_FLAGS_REFERENCE.md](docs/ENDPOINT_CONFIG_FLAGS_REFERENCE.md)) went 20 -> 21 flags for `CredentialSecretVisibility` (WI-7).
...
- Added [docs/strategy/ENGINEERING_LESSONS_AND_PATTERNS.md](docs/strategy/ENGINEERING_LESSONS_AND_PATTERNS.md) - the **central, continually-updated self-improving patterns ledger** (the `issue -> pattern -> rule` knowledge base between per-build RCA ledgers and hard rules). MUST be consulted at the start of planning / design / architecture / execution. Codifies three maintenance disciplines: **D1** capture each issue's RCA at fix-confirmation time (compaction-resilience - context compaction summarizes away early detail, so retroactive writing loses fidelity), **D2** reconcile the per-build ledger against the full session transcript at build end, **D3** promote a pattern to a hard rule after >= 2 escapes or 1 high-severity escape. Seeds 12 patterns (PA-1..PE-3). The same two disciplines were added to the "Execution Issue RCA Ledger" standing rule in [.github/copilot-instructions.md](.github/copilot-instructions.md), and R7 now routes generalizable learnings to the central doc.
- Added [docs/auth/EXECUTION_ISSUES_AND_RCA.md](docs/auth/EXECUTION_ISSUES_AND_RCA.md) - a comprehensive introspection ledger of every issue (17 across 7 types) encountered while executing the 11-step authentication build, each with symptom / root-cause / fix / why-the-fix-works / prevention, plus a detection-stage escape analysis. Reconciled against the full 4,666-line session transcript (provenance note records the method). Registered in [docs/INDEX.md](docs/INDEX.md). Establishes the standing rule (also added to [.github/copilot-instructions.md](.github/copilot-instructions.md)) that every multi-step build produces such a doc, because these issues come from unforeseen framework / environment / tooling / harness combinations the design stage cannot anticipate.
...
## [0.54.0-alpha.12] - 2026-06-19 - Auth A4: authZ seams (inert) + shadow authorization telemetry

Eleventh and final step of the reconciled authentication build. Wires the per-endpoint authorization seams (`identityModel`, `roleScopeMap`, `grantedScopes`, `roleEnforcement`) with enforcement deliberately OFF, plus the "would-have-rejected" shadow telemetry. Like A0, A4 is INERT - it stores and computes without enforcing, so no request is granted anything different today. Feature doc: [docs/auth/WIF_A4_AUTHZ_SEAMS_SHADOW_TELEMETRY.md](docs/auth/WIF_A4_AUTHZ_SEAMS_SHADOW_TELEMETRY.md).

### Added

- **`computeShadowDecision`** ([wif-shadow-telemetry.ts](api/src/oauth/wif-shadow-telemetry.ts)): a pure function that computes the future role -> scope authorization gate WITHOUT enforcing it. Given the validated assertion roles + the trust's `roleScopeMap` / `grantedScopes`, it returns `{ wouldReject, reason, wouldGrantScopes, narrows, identityModel, enforced }` where `enforced` is hard-coded `false`. This is the architecture section 11 "would-have-rejected shadow counter" - the safe-rollout signal that tells an operator whether flipping enforcement on would reject/narrow a live customer.
- **WIF trust seams** ([wif-assertion-validator.service.ts](api/src/oauth/wif-assertion-validator.service.ts) + [admin-credential.controller.ts](api/src/modules/scim/controllers/admin-credential.controller.ts)): `identityModel` (`per-app`/`first-party`), `roleScopeMap`, `grantedScopes`, and `roleEnforcement` (`off`/`shadow`/`enforce`, A4 ships `off`) ride the existing `wif` `EndpointCredential.metadata` (no new column, no secret) and are added to the `WIF_TRUST_KEYS` allowlist.
- **Shadow emission** ([wif-assertion-token.provider.ts](api/src/modules/scim/controllers/wif-assertion-token.provider.ts)): after the token is minted (Q6.4), the provider computes + logs the shadow decision (`LogCategory.AUTH`, "WIF shadow authorization decision (not enforced)") - it can never affect issuance. The `roleScopeMap` read path is guarded against prototype pollution via `isUnsafeObjectKey` (CWE-1321).

### Validation

- TDD: 9 shadow unit ([wif-shadow-telemetry.spec.ts](api/src/oauth/wif-shadow-telemetry.spec.ts)) + 1 provider unit (the headline A4 assertion: the token is STILL minted when the shadow would reject - enforcement off) + 5 live (`scripts/live-test.ps1` section 9z-AU: A4 seams persist + round-trip, still no secret). The Q6 WIF E2E (8) stays green with the shadow wiring in place.
- API unit: **3,952 -> 3,962** (111 suites). Build 0 err; ESLint 0 err / 464 warnings (no new). Backend-agnostic. Docker + dev-Azure batched to the post-A4 final checkpoint (covers Q1+Q2+A1+A2+A3+Q6+A4). Version 0.54.0-alpha.11 -> 0.54.0-alpha.12.

## [0.54.0-alpha.11] - 2026-06-19 - Auth Q6: WIF assertion validation, issuance, and reciprocal UI

Tenth step of the reconciled authentication build (the WIF-critical milestone). Binds the A3 token-endpoint seam to a real federated-identity validator + issuer and ships the reciprocal admin UI. An ISV can now present a signed Entra-style `client_assertion` (RFC 7523 jwt-bearer) at the per-endpoint token endpoint and receive its own short-lived, endpoint-scoped token. No secret is stored. Feature doc: [docs/auth/WIF_Q6_VALIDATE_ISSUE_UI.md](docs/auth/WIF_Q6_VALIDATE_ISSUE_UI.md).

### Added

- **`WifAssertionValidatorService`** ([wif-assertion-validator.service.ts](api/src/oauth/wif-assertion-validator.service.ts), Q6.3): the WIF security core. Layers `iss` / `sub` / `aud` (array-aware) / `tid` (cross-tenant isolation) / `requiredRoles` checks on top of the Q2 `ExternalJwksValidatorService.verify` signature primitive (which already enforces RS256/ES256 alg-pinning, the SSRF JWKS-host allowlist, cache-by-`kid`, and fail-closed-on-outage). Any failure throws `WifAssertionInvalidError`.
- **Own-token issuance** ([oauth.service.ts](api/src/oauth/oauth.service.ts), Q6.4): `generateEndpointAccessToken` gained an `options` arg - `trustedScope` (the admin-configured WIF scope, used verbatim, bypassing the caller-scope filter) and `ttlSec` (clamped to the Entra 1-6h window).
- **`WifAssertionTokenProvider`** ([wif-assertion-token.provider.ts](api/src/modules/scim/controllers/wif-assertion-token.provider.ts), Q6.4): implements the A3 `IAssertionTokenProvider` seam and is bound to `ASSERTION_TOKEN_PROVIDER` in [scim.module.ts](api/src/modules/scim/scim.module.ts). Three-outcome contract: no `wif` trust -> `null` (not-mine -> `invalid_client`); valid assertion -> mints the `endpoint_id`-scoped token; invalid assertion or misconfigured trust -> throws (mine-but-invalid-stop -> `invalid_client`).
- **WIF discovery advertisement** ([authentication-schemes.ts](api/src/modules/scim/discovery/authentication-schemes.ts) + [scim-discovery.service.ts](api/src/modules/scim/discovery/scim-discovery.service.ts), Q6.6): `/ServiceProviderConfig` advertises a `Workload Identity Federation` scheme (oauth2, RFC 7523) when the endpoint's `WifCredentialsEnabled` flag is on and no enabled `wif-*` method already advertises one.
- **Reciprocal CredentialsTab UI** ([CredentialsTab.tsx](web/src/pages/CredentialsTab.tsx), Q6.5): a gated "Federated Identity (WIF)" section mirroring the three-step setup - enter the 4 Entra trust values via `EditableField`, Save -> create the `wif` credential and show the 3 ISV return values via `CopyableField`, Test Connection -> a client-side readiness dry-run with a per-step result. The whole trust is grabbable via `CopyJsonButton`. Adds the `WifCredentialsEnabled` Switch to the SettingsTab ([SettingsTab.tsx](web/src/pages/SettingsTab.tsx), the 10-cell flag matrix UI cells).
- **`createTestApp` customize hook** ([app.helper.ts](api/test/e2e/helpers/app.helper.ts)) + a default `JWKS_FETCH` provider in [oauth.module.ts](api/src/oauth/oauth.module.ts) so E2E specs can override the JWKS fetch deterministically.

### Validation

- TDD: 11 validator + 6 issuance + 4 provider + 3 discovery unit (RED-first) + 8 E2E ([wif-assertion.e2e-spec.ts](api/test/e2e/wif-assertion.e2e-spec.ts), real RS256 assertion + mocked JWKS fetch: accept mints + authorizes a SCIM call; wrong iss/tid/role -> `invalid_client`; no-secret contract; discovery advertise on/off) + 6 live (`scripts/live-test.ps1` section 9z-AT) + 5 web vitest WIF describe + 1 SettingsTab + 3 Playwright ([web/e2e/wif-credentials.spec.ts](web/e2e/wif-credentials.spec.ts)).
- API unit: **3,928 -> 3,952** (110 suites). Web vitest CredentialsTab **13 -> 18**, SettingsTab **13 -> 14**. Build 0 err; ESLint 0 err / 464 warnings (no new); web tsc no new prod errors; web production build + size-limit budgets pass (CredentialsTab 3.84 kB / 110 kB). Backend-agnostic. Docker + dev-Azure batched to the post-Q6 critical-path checkpoint (covers Q1+Q2+A1+A2+A3+Q6). Version 0.54.0-alpha.10 -> 0.54.0-alpha.11.

...
- **v0.55.34 - global history and intent-based workflow context.** The app header now owns Back and Forward controls for every route and URL-owned page, tab, subtab, filter, comparison, and drawer transition. Forward availability tracks TanStack's in-app history index and resets when a new navigation branch replaces forward entries. At narrow widths the sidebar becomes an icon rail, preserving a measured 342px main area at a 390px viewport.
  ...
  - **Validation so far.** Full web **1,566/1,566** across **118** files at **85.40% lines / 75.37% branches / 75.22% functions / 82.68% statements**; focused review closure **78/78**; Chromium workflow regression **6/6**; dev live HTTP **1,532/1,532** with zero leaked endpoints; API/web builds, API lint, all route budgets, docs, and Mermaid **716/716** pass. Browser coverage includes stale Forward-branch truncation, endpoint Logs drawer Back/Forward, zero `/Me` calls under shared-secret, stale, or inactive endpoint scope, rendered Errors-only results, and measured 320px/390px shell plus Manual Provision bounds. PR, merge, and dev deployment remain pending. See [docs/GLOBAL_NAVIGATION_AND_WORKFLOW_CONTEXT.md](docs/GLOBAL_NAVIGATION_AND_WORKFLOW_CONTEXT.md) and [docs/UX_NAVIGATION_AND_WORKFLOW_EXECUTION_RCA.md](docs/UX_NAVIGATION_AND_WORKFLOW_EXECUTION_RCA.md).

...
### Added
- **Per-endpoint TLS version policy: the research and the falsification probe are now on `master`, even though the feature is not.** [docs/tls/PER_ENDPOINT_TLS_POLICY_OPTIONS.md](docs/tls/PER_ENDPOINT_TLS_POLICY_OPTIONS.md) + [scripts/tls-sni-policy-probe.mjs](scripts/tls-sni-policy-probe.mjs), cherry-picked from `feat/per-endpoint-tls-policy`. The durable value is the **"why not"**, not the plan: TLS version is negotiated before any HTTP exists, so the URL path that identifies an endpoint today is invisible at handshake time - an answer that holds permanently, for any future asker.
  ...
  - The branch's TLS 1.3 standalone stack (`docker/tls13/`) was deliberately **not** brought along - it is a form-factor harness, not a design record, and stays on the branch until a recurring need justifies it.

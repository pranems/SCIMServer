...
- **v0.55.33 - API outputs now reflect runtime truth across casing, discovery, Settings, and authenticated secret exports.** Endpoint UUID/name segments, built-in Bulk paths, custom ResourceType identifiers and endpoints, Schema URNs, and SCIM PATCH attribute/valuePath segments resolve case-insensitively while responses preserve canonical casing. Endpoint-name case-only duplicates are rejected by an additive unique index on `lower(name)`. Mixed-case extension URNs cannot bypass `returned:never`, and User/Group/custom responses recursively suppress internal persistence fields.
  ...
  - **Validation.** API unit **5,192/5,192** across **174** suites; API E2E **1,533/1,533** across **97** suites; web Vitest **1,544/1,544** across **116** files; local live HTTP **1,532/1,532** with zero leaked endpoints; available six-mode lanes **4/4** including coverage; focused Playwright **5/5** across automatic secret display/export and authoritative Settings values; API lint **0 errors**; API/web builds and all route budgets pass; migration lint **0 violations across 22 migrations**. Prisma execution lanes were skipped locally because `DATABASE_URL` is not configured. See [docs/API_OUTPUT_FIDELITY_AND_SECRET_SURFACE_CONTRACT.md](docs/API_OUTPUT_FIDELITY_AND_SECRET_SURFACE_CONTRACT.md) and [docs/API_OUTPUT_FIDELITY_EXECUTION_RCA.md](docs/API_OUTPUT_FIDELITY_EXECUTION_RCA.md). PR, merge, and dev deployment evidence remain pending; canary and customer prod are unchanged.

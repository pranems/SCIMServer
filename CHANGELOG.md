...
- **v0.55.30 - credential and WIF trust lifecycle is explicit and durable.** Connect now uses an explicit deactivate route while preserving legacy DELETE soft-deactivation, offers Activate for inactive rows, and exposes confirmed permanent deletion only after deactivation. Bearer, OAuth, and WIF cards share the same lifecycle language; Rotate remains active-only and secretless WIF trusts never offer it.
  ...
  - **Local consolidation.** API unit **5,159/5,159** across **174** suites; API E2E **1,528/1,528** across **96** suites; focused repository/controller **98/98**; web coverage **1,525/1,525** across **114** files at **85.38% lines / 74.87% branches / 75.27% functions / 82.54% statements**; focused web **163/163**; Playwright **2/2**; full local live **1,516/1,516**, including lifecycle T1-T13; all six persistence/web modes pass. API/web builds, API lint with zero errors, route budgets, docs freshness/content, and Mermaid **712/712** in both themes pass. PR, merge, and dev deployment remain pending. Canary and customer prod are unchanged.

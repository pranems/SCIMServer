...
- **v0.55.27 - Workbench ships executable server, admin, endpoint, and profile-aware SCIM examples.** A new Examples panel applies complete request drafts without sending: static health/admin requests work immediately; endpoint selection adds endpoint detail/overview/settings/discovery requests; ResourceType selection derives a profile-valid create POST and an existing-resource PATCH with concrete id and returned `If-Match`.
  - **One request contract.** Applying an example updates method, path, body, and template-specific headers. Enabled header rows now reach the network; all case variants of editable Authorization are removed before the stored admin bearer is applied. Endpoint names and unique string schema fields receive run-specific values so create examples remain runnable on shared estates.
  ...
  - **Validation.** Web Vitest + coverage **1,504/1,504** across 112 files; coverage **85.32% lines / 74.54% branches / 74.9% functions / 82.47% statements**; focused Workbench/profile/history/snippet **37 targeted tests** plus serializer and shared auth-header coverage; local Playwright **1/1** executed health, admin POST, endpoint PATCH, custom Device PATCH and stale-ETag rejection with cleanup; exact local live SCIM **1,500/1,500** on served version 0.55.27; fixture PowerShell AST parse **0 errors**, pure identity/idempotency self-test **4/4**, and disposable first-apply plus idempotent rerun **34/34 discovery/preservation/convergence assertions**, cleanup passed; API/web production builds and all 24 size budgets pass, with Workbench at **9.74 kB gzipped / 110 kB**; touched-file TypeScript diagnostics **0**. PR, merge, and final dev deployment remain pending. Canary and customer prod are unchanged.

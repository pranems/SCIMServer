...
- **v0.55.15 - per-type credential caps, observable pruning, and a gate for the direction nothing was checking.**
  ...
  - **Design/architecture gate disposition: (b) SCHEDULED.** `admin-credential.controller.ts` is now **1,221 lines** carrying credential CRUD, rotate, reveal, activate, the WIF resolve/verify/debug trio and the caps - past the ~400-line threshold. Not split here: refactoring inside a deploy-bound change, in the exact file that just produced the orphaned-decorator defect, is the wrong trade. Tracked as **D1** with the natural seam identified (the three `wif/*` diagnostic routes, ~40% of the file, sharing no state with credential CRUD).
